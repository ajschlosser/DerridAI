...
def test_release_version_0355_is_consistent():
    ...
    assert package["version"]=="0.43.0"
    assert 'version="0.43.0"' in MAIN
    assert "Corpus Viewer 0.43.0" in (ROOT/"web/index.html").read_text(encoding="utf-8")
    assert "DerridAI 0.43.0" in (ROOT/"web/src/App.vue").read_text(encoding="utf-8")
...
def test_toasts_and_operation_notifications_stack_above_sidebar():
    assert '.v030-sidebar{z-index:500!important' in STYLE
