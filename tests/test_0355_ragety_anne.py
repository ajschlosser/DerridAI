...
def test_release_version_0355_is_consistent():
    ...
    assert 'version="0.41.0"' in MAIN
    assert "Corpus Viewer 0.41.0" in (ROOT/"web/index.html").read_text(encoding="utf-8")
    assert "DerridAI 0.41.0" in (ROOT/"web/src/App.vue").read_text(encoding="utf-8")
