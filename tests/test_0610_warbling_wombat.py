...
def test_illegibility_slider_precedes_choose_source_pdf():
    component = text("web/src/components/CorpusSourceIngest.vue")
    assert component.index('id="source-illegibility"') < component.index("pdf_corpus.choose_pdf")
    assert "CorpusSourceIngest" in text("web/src/components/PdfCorpusBuilder.vue")
    assert "source_illegibility" in text("web/src/api/pdfCorpus.ts")
...
def test_dialogue_speakers_and_record_inheritance():
    ...
    records = cb.PdfCorpusBuildManager._construct_records(
    ...
    cb.PdfCorpusBuildManager._apply_manifest_metadata(records[0], {"speaker": "Not Ada", "title": "Seminar"})
    assert records[0]["speaker"] == "ADA"
    assert records[0]["document_title"] == "Seminar"


def test_whisper_spans_use_whisperx_speakers_after_full_transcript():
    source = text("api/app/source_audio.py")
    body = source[source.index("def extract_audio"):]
    assert body.index("transcribe_entire_file") < body.index("diarize_with_whisperx") < body.index("spans_from_transcript")
    transcript = {
