...
def test_0590_release_identity_and_product_name():
    ...
    assert 'FastAPI(title="DerridAI API", version="0.59.0")' in text('api/app/main.py')
    assert '<title>DerridAI 0.59.0</title>' in text('web/index.html')
    assert '# DerridAI 0.59.0' in text('README.md')
    assert '0.59.0 — Serious Sandpipers' in text('README.md')
    assert 'DerridAI Corpus Viewer 0.59.0' not in text('README.md')
