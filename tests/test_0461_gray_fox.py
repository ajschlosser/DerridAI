...
def test_gray_fox_release_identity():
    ...
    assert package["version"] == "0.52.0"
    assert 'APP_VERSION = "0.52.0"' in (ROOT / "api/app/config.py").read_text(encoding="utf-8")
    assert 'version="0.52.0"' in (ROOT / "api/app/main.py").read_text(encoding="utf-8")
    assert "0.52.0 — Lazy Lizard" in (ROOT / "README.md").read_text(encoding="utf-8")
