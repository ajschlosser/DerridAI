...
def test_gray_fox_release_identity():
    ...
    assert 'APP_VERSION = "0.57.6"' in (ROOT / "api/app/config.py").read_text(encoding="utf-8")
    assert 'version="0.57.6"' in (ROOT / "api/app/main.py").read_text(encoding="utf-8")
    assert "0.57.6 — Quippy Quokka" in (ROOT / "README.md").read_text(encoding="utf-8")
