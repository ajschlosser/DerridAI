...
def test_gray_fox_release_identity():
    ...
    assert package["version"] == "0.48.1"
    assert 'APP_VERSION = "0.48.1"' in (ROOT / "api/app/config.py").read_text(encoding="utf-8")
    assert 'version="0.48.1"' in (ROOT / "api/app/main.py").read_text(encoding="utf-8")
    assert "0.48.1 — Gifted Grungus" in (ROOT / "README.md").read_text(encoding="utf-8")
