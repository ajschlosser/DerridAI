...
def docx_to_text(data: bytes) -> tuple[str, dict[str, str]]:
    ...
    if "docProps/core.xml" in package.namelist():
        ...
        embedded["title"] = _xml_text(root, "title")
        embedded["author"] = _xml_text(root, "creator")
        embedded["subject"] = _xml_text(root, "subject")
        embedded["language"] = _xml_text(root, "language")
...
def _xml_text(root: ElementTree.Element, local: str) -> str:
    for node in root.iter():
        if node.tag.rsplit("}", 1)[-1] == local and node.text and node.text.strip():
