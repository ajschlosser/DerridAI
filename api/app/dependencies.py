...
def get_store(request: Request) -> ChromaStore:
    """Resolve the application-owned vector store without importing main.py.

    Routers depend on application state rather than module globals so they remain
    independently importable and testable. main.py owns service construction and
    places the configured store on app.state during application setup.
    """
