...
"""Authentication, user-management, and role-management API routes."""
...
def auth_update_user(
    ...
    if current.id == user_id and body.active is False:
        raise HTTPException(
            status_code=400,
            detail="You cannot deactivate your current session account.",
        )
    if current.id == user_id and body.role is not None and body.role != current.role:
        raise HTTPException(
            status_code=400,
            detail="You cannot change the role of your current session account.",
        )
    try:
...
def auth_delete_user(user_id: int, request: Request) -> dict[str, Any]:
    ...
    if current.id == user_id:
        raise HTTPException(
            status_code=400,
            detail="You cannot delete your current session account.",
        )
    try:
...
def auth_update_role_permissions(
    role: str,
