...
"""Language dictionaries and researcher content-policy API routes."""
...
def i18n_languages() -> dict[str, Any]:
    # Language metadata and dictionaries are public because authentication UI
    # itself is localized. Mutation and policy-management routes remain private.
...
def i18n_update_language(
    code: str,
    body: LanguageDictionaryUpdate,
    request: Request,
) -> dict[str, Any]:
...
@router.post("/languages/install")
def i18n_install_language(
    body: LanguageInstallRequest,
    request: Request,
) -> dict[str, Any]:
    ...
    if system_store.get_language(code) is not None:
        raise HTTPException(
            status_code=409,
            detail=(
                f"Locale {code} is already installed. Edit the existing dictionary "
                "or remove it before reinstalling."
            ),
        )
    ...
    if not model:
        raise HTTPException(
            status_code=400,
            detail="Select a model to translate the language dictionary.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Language translation failed: {exc}",
        ) from exc
    ...
    saved = system_store.put_language(
        ...
        translation_report={
            "status": (
                ...
                if int(translation_stats.get("fallback_count") or 0)
                else "complete"
            ),
            "source_locale": "en-US",
            "provider": body.provider,
            ...
            "translated_count": int(
                translation_stats.get("translated_count") or 0
            ),
            "key_count": int(
                translation_stats.get("key_count") or len(dictionary)
            ),
        },
    )

    # Policy generation is best-effort: a translated dictionary is valuable on
    # its own, and policy generation can be retried independently if a provider
...
def i18n_active_content_policy(request: Request) -> dict[str, Any]:
    """Return the hashed policy union used by the researcher client mirror."""
    request_user(request)
    return public_content_policy_mirror(system_store.list_ready_content_policies())


@router.get("/languages/{code}/content-policy")
def i18n_language_content_policy(code: str, request: Request) -> dict[str, Any]:
    ...
    if system_store.get_language(normalized) is None:
        raise HTTPException(status_code=404, detail="Language dictionary not found.")
    return admin_content_policy_view(
        system_store.get_content_policy(normalized),
        code=normalized,
    )


@router.put("/languages/{code}/content-policy")
def i18n_update_content_policy(
    code: str,
    body: LanguageContentPolicyUpdate,
    request: Request,
) -> dict[str, Any]:
    require_admin(request)
    try:
