...
def _apply_manifest_metadata(record: dict[str, Any], manifest: dict[str, Any]) -> None:
    """Apply reviewed manifest metadata without overwriting human authority."""
    ...
    if pdf_pages and isinstance(start_page, int):
        ...
        primary_structure_owned = bool(primary_assertion and primary_assertion.method in STRONG_STRUCTURAL_METHODS)
        region_structure_owned = bool(region_assertion and region_assertion.method in STRONG_STRUCTURAL_METHODS)
        ...
        if (
            not any(isinstance(item, dict) and item.get("code") == "main_text_start_straddle" for item in issues)
            and str(record.get("review_reason") or "").endswith("Choose main text, front matter, or split it.")
        ):
