...
- **Deciding metadata.** The **Metadata** tab lists the fields still to decide first, each open with its proposed value, its source, and the model's reason. Fields the record cannot be accepted without come first and say **Required to accept**. Every field has the same two buttons in the same place: **Confirm** (or **Save value** once you change it) and **No value**. `Ctrl+Enter` (`⌘Enter` on Apple platforms) confirms from inside the field. After a decision, focus moves to the next field's **Confirm**, so `Enter` confirms each proposal in turn; when none is left, focus moves to **Accept & next**. A decided field folds to one line where it was, so nothing below it moves. Fields that already have values, fields you can add, and inherited document metadata are one-line rows under their own headings; **Edit** opens a row, **Cancel** closes it unchanged. **Select from text**, **Save with selection** (saves the value with the selected record text as its evidence) and **Evidence** are the links under each open field. Cache, document-metadata and raw-JSON tools are under **Advanced metadata**.
