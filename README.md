...
**0.41.0 — Aardvark** completes a major Corpus Builder workflow pass: a dedicated record-first Focus Review, field-aware hybrid metadata inference, explicit enrichment/review/publication handoffs, metadata issue queues, auditable field provenance, and end-to-end workflow regression coverage.
# DerridAI Corpus Viewer 0.41.0
...
Aardvark restructures Corpus Builder around four explicit responsibilities: **source extraction → corpus construction → scholarly enrichment → human validation/publication**. The main lifecycle card now exposes the current stage, work remaining, and next action instead of asking users to infer state from low-level progress counters. Publication remains a separate immutable snapshot, while edits reopen the build as a draft. A build timeline records stage/status transitions for troubleshooting and audit.

**Focus Review is rebuilt as a true full-screen, record-first workspace.** The proposed record is the dominant reading surface, with record position, page range, size, disposition, review reason, and decision controls kept visible. Metadata, evidence, and extracted source context live in a secondary tabbed panel; the normal record list and PDF workspace no longer compete for space inside Focus Review. Accept/Reject are explicit **Accept & next / Reject & next** actions, structural controls remain available, keyboard focus is trapped/restored correctly, background scrolling is disabled, and responsive/reduced-motion behavior is included.

Metadata enrichment is now field-aware. The current corpus profile is `derrida-scholarly-v8` and metadata prompt contract is `derridai-record-metadata-v4`. The profile defines controlled enums for `region_type` and `discourse_role` and requires `region_type`, `primary_text`, and `discourse_role` as publication-critical hybrid fields. Deterministic evidence such as reviewed document page ranges wins when available; ambiguous values are classified by the configured LLM using the allowed enum, then schema/evidence validation decides whether the value is accepted. Per-field provenance records whether a value is deterministic, LLM-inferred, human-confirmed, unresolved, or invalid.

Incomplete metadata is no longer summarized as a vague retry state. Builds expose a structured metadata issue summary with affected records, unresolved fields, counts by field, provider/model context, and a dedicated **Metadata** queue. Retrying enrichment resumes only incomplete work from checkpoints. The UI explains what will run, what remains afterward, and links directly to the affected records. Completing record review now produces an explicit completion state: rejected records, unresolved metadata, validation readiness, and publication eligibility are separate concerns rather than overloaded into one status.

The persistent review-hydration bug receives another lifecycle-level fix: whenever a refreshed or running build advertises persisted records, the client hydrates the record/source workspace after paint without depending on checkbox, form, or other user interaction. Evidence selection also synchronizes the source PDF page to the first bound source block. Bulk and individual review decisions continue to use server-side disposition state and optimistic revisions.

Source normalization remains Unicode-safe end to end: native PDF/OCR strings are NFC-normalized without ASCII transliteration or stripping, and JSON/JSONL serialization uses UTF-8 with `ensure_ascii=False`. Regression coverage includes Latin diacritics, Polish, Greek, Japanese, and decomposed Unicode forms.

Storybook coverage is expanded with full-screen Focus Review states, metadata issue summaries, review-completion states, metadata queue navigation, lifecycle states, and the build event timeline. New UI text is localized in English and Canadian French, including hybrid metadata labels and field-status terminology. Controls use semantic buttons/tabs/disclosures, text state indicators in addition to color, visible focus treatment, keyboard-compatible flow, and responsive layouts in support of the project's WCAG 2.2 AA target.

Release validation includes the full Python regression suite, Python compilation, JavaScript syntax checks, TypeScript/Vue script transpilation, Git whitespace checks, and explicit attempts to run the production Vue and Storybook builds. The release artifact records any environment limitation rather than treating source-level checks as equivalent to a completed production build.
