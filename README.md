...
**0.40.25 — Sensible Chuckles** focuses the Corpus Builder on a record-first review queue, explicit publication lifecycle, reliable refresh hydration, Unicode-safe source text, and tested Accept/Reject actions.
# DerridAI Corpus Viewer 0.40.25
...
## 0.40.25 — Sensible Chuckles

This pass turns Corpus Builder into a record-first review workflow with explicit, separate **build** and **publication** lifecycles. The main path is **Source → Analyze → Build records → Review → Publish**. Build state no longer becomes `published`; a completed build remains `ready`, while an immutable publication snapshot is tracked separately through `publication` / `publication_status`. Publication is the only lifecycle state shown at 100%, and any subsequent structural or metadata edit archives the old publication and reopens an unpublished draft revision.

The persistent intermediate-results bug is fixed at its source. Deterministic records are saved before metadata enrichment, and the client now hydrates the review workspace as soon as `record_count` becomes non-zero—even while the build is still running. Refresh hydration is independent of checkbox/form interaction, includes bounded retries when build metadata becomes visible slightly before `records.jsonl`, and re-runs automatically when the advertised topology count advances. This makes the PDF/source pane and generated records appear without requiring a click or filter toggle.

Record review is now the visual center of the workspace. A compact queue navigator sits beside a large readable proposed-record pane, while PDF/source context is secondary. The old **Needs review only** checkbox is replaced by accessible queue tabs for **All / Pending / Needs attention / Accepted / Rejected**. Accept/Reject update immediately, preserve selection logic, and advance to the next proposal. Bulk Accept/Reject operate against the active queue/filter on the server, rather than ambiguously applying to an unrelated set. Focus view remains record-first and now uses **Accept & next / Reject & next** decision language.

The top of the build screen now emphasizes one question: **what happens next?** A lifecycle card reports reviewed/remaining records and incomplete metadata, while segmentation/LLM counters move behind a **Technical build details** disclosure. The document manifest remains progressively disclosed after confirmation. A build with all records accepted but incomplete metadata enters an explicit `awaiting_metadata` / `metadata_review` state; publication is blocked until metadata is complete and validation passes. This prevents combinations such as “published” with incomplete metadata or a misleading partial-progress state.

PDF rendering now uses `GlobalWorkerOptions.workerSrc` instead of reusing a shared `workerPort`, and teardown performs a single awaited loading-task/document destruction path after cancelling the active render. This removes the worker-lifecycle race that produced `PDFWorker.fromPort - the worker is being destroyed` during rapid record/page changes.

Source-text normalization is Unicode-safe. Native PDF and OCR text are normalized to Unicode NFC while preserving non-ASCII letters, diacritics, punctuation, and scripts; JSON/JSONL output continues to use UTF-8 with `ensure_ascii=False`. Regression coverage includes names/terms such as **Édouard Glissant**, **différance**, **Łódź**, and **東京**.

The Storybook library adds reusable **CorpusReviewQueueTabs** and **CorpusBuildLifecycleCard** components, with reviewing/ready/published and active-queue states. New and updated UI strings are provided in English and Canadian French, and the review controls continue to use semantic buttons/tabs, text state labels in addition to color, visible focus styles, keyboard-friendly action order, responsive layouts, and reduced-motion-safe behavior.

Release validation: **284 Python tests pass**, Python compilation passes, `runtime.js` passes Node syntax checking, `git diff --check` passes, and **155 Vue/TypeScript script blocks transpile with zero syntax diagnostics**. The actual `npm install` gate was attempted repeatedly; this isolated environment cannot resolve `registry.npmjs.org` (`EAI_AGAIN`) and the repository intentionally does not vendor `node_modules`. Final `npm run build` and `npm run build-storybook` attempts therefore stop at missing local `vue-tsc` / Storybook executables. This package records that environmental limitation rather than claiming a production frontend build that did not run.
