...
const selected=ref<Set<TextCleanupRule>>(new Set(['page_numbers','repeated_short_lines','line_hyphenation','whitespace']));
...
onMounted(()=>{window.addEventListener('keydown',keydown);title.value?.focus({preventScroll:true})});
onBeforeUnmount(()=>window.removeEventListener('keydown',keydown));
</script>
<template><Teleport to="body"><div class="cleanup-backdrop" @mousedown.self="emit('close')"><section class="cleanup-dialog" role="dialog" aria-modal="true" aria-labelledby="cleanup-title" aria-describedby="cleanup-help">
