...
const total = computed(() =>
  Number(
    props.build.metadata_tasks_total ||
      Math.max(
        0,
        Number(props.build.metadata_enrichment_total || props.build.record_count || 0) * 3,
      ),
  ),
);
...
const queued = computed(() =>
  Number(
    props.build.metadata_tasks_queued ||
      Math.max(0, total.value - complete.value - failed.value - skipped.value - running.value),
  ),
);
...
const lastProgress = computed(() =>
  props.build.metadata_last_progress_at
    ? new Date(props.build.metadata_last_progress_at).toLocaleTimeString()
...
function activeElapsed(startedAt?: string | null) {
  return startedAt ? formatDuration(elapsedSince(startedAt)) : "";
}
const stalled = computed(() => {
  if (!props.build.metadata_last_progress_at || String(props.build.stage) !== "enriching")
    return false;
  const configured = (props.build.request?.stage_timeouts || {}) as Record<string, unknown>;
  const metadataDeadlines = ["discourse", "quotation", "indexing"]
    .map((key) => Number(configured[key] || 240))
...
function taskLabel(task: string) {
  return i18n.t(`pdf_corpus.metadata_task.${task}`, task);
}
</script>

<template>
  <section
    class="metadata-live-status"
  ...
  >
    <div class="status-main">
      <div class="status-title-row">
        <div>
          ...
          <h2>
            {{
              stalled
                ? i18n.t("pdf_corpus.metadata_stalled_title")
                : i18n.t("pdf_corpus.metadata_live_title")
            }}
          </h2>
        </div>
        <strong class="status-count"
          >{{ settled }} / {{ total }}
          <small>{{ i18n.t("pdf_corpus.metadata_tasks_unit") }}</small></strong
        >
      </div>
      <p>
        {{
          i18n.tf("pdf_corpus.metadata_live_summary", {
            complete,
            running,
            queued,
      ...
      <div
        ...
        :aria-label="i18n.t('pdf_corpus.metadata_task_progress')"
        :aria-valuenow="settled"
        :aria-valuemin="0"
        :aria-valuemax="Math.max(1, total)"
        role="progressbar"
      >
        <span :style="{ width: `${total ? Math.min(100, (settled / total) * 100) : 0}%` }"></span>
      </div>
    </div>
    ...
    <div
      v-if="active.length"
      class="active-tasks"
      :aria-label="i18n.t('pdf_corpus.active_metadata_tasks')"
    >
    ...
    <div class="status-actions">
      <button
        type="button"
        class="btn"
        :disabled="disabled || Boolean(build.metadata_settle_requested)"
        @click="emit('settle')"
      >
        {{
          build.metadata_settle_requested
            ? i18n.t("pdf_corpus.settle_requested")
            : i18n.t("pdf_corpus.continue_unresolved")
        }}
      </button>
      <button type="button" class="btn" :disabled="disabled" @click="emit('cancel')">
        {{ i18n.t("pdf_corpus.cancel_build") }}
      </button>
    </div>
  </section>
</template>
...
.metadata-live-status[data-stalled="true"] {
