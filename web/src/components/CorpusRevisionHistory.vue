...
const items = computed(() => {
  ...
  for (const entry of props.record.text_revision_history || []) {
    rows.push({
      ...
      kind: "text",
      label:
        entry.source === "automatic_cleanup"
          ? i18n.t("pdf_corpus.history_auto_cleanup")
          : i18n.t("pdf_corpus.history_text_edit"),
      detail: entry.diff ? i18n.t("pdf_corpus.history_diff_available") : undefined,
    });
  }
  for (const entry of props.record.metadata_decisions || []) {
    ...
    rows.push({
      at: entry.at,
      kind: "metadata",
      label: i18n.tf("pdf_corpus.history_metadata_change", {
        field: i18n.t(`record.${field}`, field.replace(/_/g, " ")),
      }),
      detail: entry.source
        ? i18n.t(
            `pdf_corpus.history_source.${entry.source}`,
  ...
  for (const entry of props.record.metadata_enrichment_history || []) {
    for (const event of entry.informational || []) {
      ...
      const pass = event.pass
        ? i18n.tf("pdf_corpus.enrichment_pass_number", { pass: event.pass })
        : "";
      rows.push({
  ...
  for (const entry of props.record.review_events || []) {
    ...
    rows.push({
      at: entry.at,
      kind: "review",
      label: i18n.t(`pdf_corpus.history_event.${event}`, event.replace(/_/g, " ")),
      detail: entry.transaction_id ? String(entry.transaction_id) : undefined,
    });
  }
...
<template>
  <section class="revision-history" aria-labelledby="revision-history-title">
    <header>
      <div>
        <span class="eyebrow">{{ i18n.t("pdf_corpus.audit_trail") }}</span>
        <h3 id="revision-history-title">{{ i18n.t("pdf_corpus.revision_history") }}</h3>
      </div>
      <span class="count">{{ items.length }}</span>
    </header>
    <ol v-if="items.length">
      <li v-for="(item, index) in items" :key="`${item.at}-${item.kind}-${index}`">
        <div>
          <b>{{ item.label }}</b
...
.revision-history li:last-child {
  border-bottom: 0;
}
.revision-history li > div {
  display: grid;
  gap: 2px;
}
.revision-history b {
