...
let loadingTask:pdfjsLib.PDFDocumentLoadingTask|null=null; let pdfDocument:pdfjsLib.PDFDocumentProxy|null=null; let renderTask:pdfjsLib.RenderTask|null=null; let documentGeneration=0; let renderGeneration=0; let resizeObserver:ResizeObserver|null=null; let lastRenderedPage=0; let lastRenderedWidth=0;
...
async function destroyDocument(){documentGeneration+=1;renderGeneration+=1;lastRenderedPage=0;lastRenderedWidth=0;await cancelRender().catch(()=>{});const task=loadingTask;const doc=pdfDocument;loadingTask=null;pdfDocument=null;if(task){try{await task.destroy?.()}catch{}}else if(doc){try{await doc.destroy?.()}catch{}}}
