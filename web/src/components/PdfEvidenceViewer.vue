...
async function cancelRender() {
  ...
  if (active) {
    try {
      ...
      if (!(exc instanceof Error && exc.name === "RenderingCancelledException")) throw exc;
    }
  }
}
...
async function loadDocument() {
  ...
  try {
    ...
    if (token === documentGeneration) loading.value = false;
  }
}
async function renderPage() {
  ...
  try {
    ...
    if (
      !(exc instanceof Error && exc.name === "RenderingCancelledException") &&
      token === renderGeneration
    )
      error.value = exc instanceof Error ? exc.message : String(exc);
  } finally {
    if (token === renderGeneration) loading.value = false;
  }
}
function setZoom(value: number) {
  if (!props.zoomable) return;
  zoom.value = Math.max(1, Math.min(2.5, Number(value.toFixed(2))));
...
onMounted(() => {
  if (typeof ResizeObserver !== "undefined" && shell.value) {
    ...
    resizeObserver = new ResizeObserver(() => {
      ...
      timer = window.setTimeout(() => void renderPage(), 80);
    });
    ...
    <div class="page-stage" :class="{ zoomed: zoom > 1 }" :aria-busy="loading ? 'true' : 'false'">
      ...
      <div class="block-overlay" aria-hidden="true">
        <span
          ...
          :key="block.block_id"
          class="source-box"
          :class="{ evidence: evidenceSet.has(block.block_id) }"
...
.viewer-controls :is(button):focus-visible {
