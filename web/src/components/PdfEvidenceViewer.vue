...
let loadingTask:any=null; let pdfDocument:any=null; let renderTask:any=null; let documentGeneration=0; let renderGeneration=0; let resizeObserver:ResizeObserver|null=null;
...
<style scoped>
.pdf-evidence-viewer{display:grid;gap:6px;padding:10px;background:var(--soft);border-bottom:1px solid var(--line)}.page-stage{position:relative;justify-self:center;max-width:100%;line-height:0;background:#fff;box-shadow:0 1px 8px rgb(0 0 0 / .12)}canvas{display:block;max-width:100%;height:auto}.block-overlay{position:absolute;inset:0;pointer-events:none}.source-box{position:absolute;border:1px solid color-mix(in srgb,var(--accent) 38%,transparent);background:color-mix(in srgb,var(--accent) 7%,transparent);border-radius:2px}.source-box.evidence{border:2px solid var(--accent);background:color-mix(in srgb,var(--accent) 20%,transparent);box-shadow:0 0 0 1px rgb(255 255 255 / .8)}.viewer-state{padding:7px 9px;border:1px solid var(--line);border-radius:7px;background:var(--card);font-size:.8125rem}.viewer-state.error{border-color:#a13f3f;color:#7d2222}.viewer-caption{margin:0;font-size:.8125rem;color:var(--muted);line-height:1.4}@media(prefers-reduced-motion:reduce){.source-box{transition:none}}
</style>
