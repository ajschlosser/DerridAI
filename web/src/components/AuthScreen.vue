...
<template>
  <main class="auth-page">
    <section class="auth-card">
      <div class="auth-card-topline"><div class="auth-brand-lockup"><BrandMark :size="78"/><div><strong>DerridAI</strong><span>{{i18n.t('ui.corpus_viewer','Corpus Viewer')}} 0.40.20</span></div></div><div class="auth-language-switcher"><LanguageFlag :code="i18n.locale" :symbol="currentLocaleInfo?.flag" :label="currentLocaleInfo?.name" size="small"/><select :value="i18n.locale" @change="i18n.setLocale(($event.target as HTMLSelectElement).value)"><option v-for="language in i18n.languages" :key="language.code" :value="language.code">{{language.name}}</option></select></div></div>
