...
<template>
  <div class="hands-free">
    ...
    <label v-if="showEnable" class="hf-check"><input type="checkbox" :checked="modelValue.enabled" :disabled="disabled" @change="set('enabled', ($event.target as HTMLInputElement).checked)"><span><b>{{ i18n.t("pdf_corpus.hands_free_enable") }}</b></span></label>
    <fieldset class="hf-grid" :disabled="disabled || (showEnable && !modelValue.enabled)">
      <label class="hf-field" :for="`${id}-passes`"><span>{{ i18n.t("pdf_corpus.hands_free_passes") }}</span>
      ...
      <label class="hf-field" :for="`${id}-conf`"><span>{{ i18n.tf("pdf_corpus.hands_free_confidence", { percent: pct(modelValue.min_confidence) }) }}</span>
      ...
      <label class="hf-field" :for="`${id}-unresolved`"><span>{{ i18n.t("pdf_corpus.hands_free_unresolved") }}</span>
      ...
      <label class="hf-check"><input type="checkbox" :checked="modelValue.accept_records" @change="set('accept_records', ($event.target as HTMLInputElement).checked)"><span><b>{{ i18n.t("pdf_corpus.hands_free_accept") }}</b></span></label>
      <label class="hf-check"><input type="checkbox" :checked="modelValue.publish" @change="set('publish', ($event.target as HTMLInputElement).checked)"><span><b>{{ i18n.t("pdf_corpus.hands_free_publish") }}</b><small>{{ i18n.t("pdf_corpus.hands_free_publish_help") }}</small></span></label>
    </fieldset>
  </div>
</template>
