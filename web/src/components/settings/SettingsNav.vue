...
const emit = defineEmits<{"update:modelValue": [SettingsSectionId]; select: [SettingsSectionId]}>();
const root = ref<HTMLElement | null>(null);
const activeId = computed(() => props.items.some(item => item.id === props.modelValue) ? props.modelValue : props.items[0]?.id || "workspace");
...
@media (forced-colors: active){
  .settings-nav-item[aria-selected="true"]{outline:2px solid CanvasText}
}
</style>
