...
<template>
  <UiCard :as="'section'" class="settings-section" :padded="false" :heading-id="`settings-heading-${sectionId}`">
    <header class="settings-section-head">
      <div>
        <p class="settings-kicker">{{ persistence }}</p>
        <component :is="`h${headingLevel}`" :id="`settings-heading-${sectionId}`">{{ title }}</component>
...
.settings-section-actions{display:flex;flex-wrap:wrap;gap:8px;align-items:center;padding-top:4px}
@media (max-width:640px){
  .settings-section{padding:16px}
  .settings-section-head :is(h2,h3){font-size:1.125rem}
}
</style>
