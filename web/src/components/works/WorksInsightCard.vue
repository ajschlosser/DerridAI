...
const total = computed(() =>
  props.insight.values.reduce((sum, item) => sum + Number(item.value || 0), 0),
);
...
<template>
  <article
    class="work-insight-card"
    :class="{ 'work-insight-card-pie': props.insight.type === 'pie' }"
  >
        ...
        <ol class="work-insight-pie-legend">
          <li v-for="(item, index) in props.insight.values" :key="item.key">
            ...
            <button
              v-else
              type="button"
              :data-work-insight-field="props.insight.field"
              :data-work-insight-value="item.key"
              @click="emit('search', props.insight.field, item.key)"
            >
              <i :style="{ background: colors[index % colors.length] }"></i>
              <span>{{ item.key }}</span>
            </button>
...
li.work-insight-empty {
  list-style: none;
}

.work-insight-empty b {
  color: var(--text);
  font-size: 0.86rem;
}

.work-insight-empty span {
  font-size: 0.78rem;
  line-height: 1.4;
}
</style>