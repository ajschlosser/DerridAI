...
const provider = computed(() =>
  String(
    (props.build.request as Record<string, unknown> | undefined)?.provider_profile_id ||
      props.build.provider ||
      "—",
  ),
);
const operation = computed(() => props.build.metadata_operation || {});
const operationRunning = computed(() =>
  ["queued", "running"].includes(String(operation.value.state || "")),
);
const progress = computed(() => {
  const total = Math.max(1, Number(operation.value.fields_total || 0));
  return Math.max(
    0,
    Math.min(100, Math.round((Number(operation.value.fields_resolved || 0) / total) * 100)),
  );
});
const firstHumanRecord = computed(() =>
  String(human.value.find((issue) => issue.record_id)?.record_id || ""),
);
const totalIssues = computed(() => Number(summary.value.fields_unresolved ?? issues.value.length));
function reasonLabel(code?: string) {
  return i18n.t(
    `pdf_corpus.metadata_reason.${code || "unresolved"}`,
    String(code || "unresolved").replace(/_/g, " "),
  );
}
</script>
...
<template>
  <section
    v-if="totalIssues > 0"
    class="metadata-issues"
    aria-labelledby="metadata-resolution-title"
  >
    <header class="resolution-head">
      <div>
        ...
        <p>
          {{
            i18n.tf("pdf_corpus.metadata_resolution_intro", {
      ...
      <div class="resolution-count" aria-label="Metadata issue count">
        <strong>{{ summary.fields_unresolved || 0 }}</strong
        ><span>{{ i18n.t("pdf_corpus.fields_to_resolve") }}</span>
      </div>
    </header>

    <section
      v-if="operationRunning || operation.state === 'completed' || operation.state === 'failed'"
      class="operation"
    ...
    >
      <div class="operation-copy">
        <b>{{
          operationRunning
            ? i18n.t("pdf_corpus.metadata_retry_running")
            : operation.state === "completed"
              ? i18n.t("pdf_corpus.metadata_retry_complete")
              : i18n.t("pdf_corpus.metadata_retry_failed")
        }}</b>
        <span v-if="operationRunning">{{
          i18n.tf("pdf_corpus.metadata_retry_progress", {
            processed: Number(operation.records_processed || 0),
            total: Number(operation.records_total || 0),
            resolved: Number(operation.fields_resolved || 0),
          })
        }}</span>
        <span v-else-if="operation.state === 'completed'">{{
          i18n.tf("pdf_corpus.metadata_retry_result", {
            resolved: Number(operation.fields_resolved || 0),
            remaining: Number(operation.fields_remaining || 0),
          })
        }}</span>
    ...
    <div class="resolution-paths">
      <article class="resolution-path" :data-empty="retryable.length === 0">
        ...
        <h4>
          {{
            i18n.tf("pdf_corpus.retryable_metadata_fields", {
              count: Number(summary.auto_retry_fields ?? retryable.length),
            })
          }}
        </h4>
        ...
        <div class="execution">
          <span
            >{{ i18n.t("pdf_corpus.provider_profile") }} <code>{{ provider }}</code></span
          ><span
        ...
        >
          {{
            operationRunning
              ? i18n.t("pdf_corpus.retrying_metadata")
              : i18n.tf("pdf_corpus.retry_fields_with_model", {
                  count: Number(summary.auto_retry_fields ?? retryable.length),
                  model,
                })
          }}
      ...
      <article class="resolution-path" :data-empty="human.length === 0">
        ...
        <h4>
          {{
            i18n.tf("pdf_corpus.human_metadata_fields", {
              count: Number(summary.human_review_fields ?? human.length),
            })
          }}
        </h4>
    ...
    <details v-if="issues.length" class="issue-details">
      <summary>
        {{ i18n.tf("pdf_corpus.view_metadata_issue_details", { count: issues.length }) }}
      </summary>
      <div class="issue-table" role="table" :aria-label="i18n.t('pdf_corpus.metadata_issue_table')">
        <div class="issue-row issue-header" role="row">
          <span role="columnheader">{{ i18n.t("pdf_corpus.record") }}</span
          ><span role="columnheader">{{ i18n.t("pdf_corpus.field") }}</span
          ><span role="columnheader">{{ i18n.t("pdf_corpus.reason") }}</span
          ><span role="columnheader">{{ i18n.t("pdf_corpus.next_step") }}</span>
        </div>
        <button
          v-for="issue in issues"
          :key="`${issue.record_id}-${issue.field}`"
          type="button"
          class="issue-row"
          role="row"
          @click="issue.record_id && emit('review', String(issue.record_id))"
        >
          <span role="cell"
            ><b>{{ issue.record_id }}</b
            ><small
              >{{ i18n.t("pdf_corpus.pages") }} {{ issue.page_start ?? "—" }}–{{
                issue.page_end ?? "—"
              }}</small
            ></span
          >
          <span role="cell">{{
            i18n.t(`record.${issue.field}`, String(issue.field || "").replace(/_/g, " "))
          }}</span>
          <span role="cell"
            >{{ reasonLabel(issue.issue_type)
            }}<small v-if="issue.reason">{{ issue.reason }}</small></span
          >
          <span role="cell">{{
            issue.retryable
              ? i18n.t("pdf_corpus.retry_automatically")
              : i18n.t("pdf_corpus.review_manually")
          }}</span>
        </button>
      </div>
    </details>
...
.issue-row {
  display: grid;
  grid-template-columns: minmax(130px, 0.8fr) minmax(120px, 0.7fr) minmax(220px, 1.5fr) minmax(
      130px,
      0.7fr
    );
  gap: 10px;
