...
<section class="metadata-review" aria-labelledby="metadata-review-title">
  ...
  <div v-if="activeFields.length" class="metadata-grid" role="list">
    <CorpusMetadataFieldEditor v-for="field in activeFields" :key="field" :field="field" :value="record[field]" :status="status(field)" :options="options(field)" :control="spec(field).control" :allow-custom="Boolean(spec(field).allowCustom)" :required="requiredFields.has(field)" :busy="busy" :saving="savingField===field" :saved="savedField===field" :constraint="constraint(field)" :calibrated-acceptance="calibrated(field)" :open="unresolved.has(field)" @save="value=>emit('resolve',field,value)" @no-value="emit('noValue',field)" @source="emit('source',field)" @dirty="value=>emit('dirty',value)"/>
  </div>
