...
const props=defineProps<{record:CorpusRecord;regionTypes:string[];discourseRoles:string[];busy?:boolean;savingField?:string;savedField?:string;confidenceCalibration?:Record<string,Record<string,Record<string,number>>>;knownValues?:Record<string,string[]>;schema?:MetadataSchema|null}>();
...
const legacyOrder=["region_type","primary_text","discourse_role","region_author","speaker","position_holder","target","stance","proposition_status","claim_scope","semantic_function","is_direct_quote","quoted_speaker","quoted_author","quoted_work","quoted_position_holder","quoted_addressee","quoted_referent","quotation_chain","topics","concepts","persons","works_referenced","work","document_title","short_title","original_title","document_author","translator","edition","publication_year","publisher","publication_place","isbn","document_language","original_language","document_is_translation"];
const documentFields=legacyOrder.slice(legacyOrder.indexOf("work"));
const schemaFields=computed<Record<string,SchemaField>>(()=>Object.fromEntries((props.schema?.fields||[]).map(field=>[field.name,field])));
const fieldOrder=computed<string[]>(()=>props.schema?["region_type","primary_text","discourse_role",...props.schema.fields.map(field=>field.name),...documentFields]:legacyOrder);
...
const fields=computed(()=>fieldOrder.value.filter(field=>props.record.metadata_field_status?.[field]||props.record[field]!==undefined||unresolved.value.has(field)));
...
const addableFields=computed(()=>fieldOrder.value.filter(field=>!inheritedFieldSet.has(field)&&!fields.value.includes(field)));
...
function spec(field:string){return metadataFieldSpec(field,props.regionTypes,props.discourseRoles,schemaFields.value[field])}
...
<section class="metadata-review" aria-labelledby="metadata-review-title">
  ...
  <details v-if="addableFields.length" class="settled-metadata add-metadata"><summary>{{i18n.t('pdf_corpus.add_metadata_section','Add more details')}} <span>{{addableFields.length}}</span></summary><p class="add-metadata-help">{{i18n.t('pdf_corpus.add_metadata_help','These fields are empty on this record. Fill in any that apply, such as a quoted speaker.')}}</p><div class="metadata-grid" role="list"><div v-for="field in addableFields" :key="field" class="metadata-list-item" role="listitem"><CorpusMetadataFieldEditor :label="fieldLabel(field)" :field="field" :value="record[field]" :status="status(field)" :options="options(field)" :control="spec(field).control" :allow-custom="Boolean(spec(field).allowCustom)" :busy="busy" :saving="savingField===field" :saved="savedField===field" @save="value=>emit('resolve',field,value)" @no-value="emit('noValue',field)" @source="emit('source',field)" @dirty="value=>emit('dirty',value)" /></div></div></details>
...
@media (max-height:860px){.metadata-head p{display:none}.metadata-head h3{font-size:1rem}.metadata-review{gap:10px}}
</style>
