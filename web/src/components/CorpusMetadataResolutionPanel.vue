...
function options(field:string){if(field==='region_type')return props.regionTypes;if(field==='discourse_role')return props.discourseRoles;const known=new Set<string>();const value=props.record[field];if(Array.isArray(value))value.forEach(v=>{if(typeof v==='string'&&v.trim())known.add(v.trim())});else if(typeof value==='string'&&value.trim())known.add(value.trim());for(const candidate of ['region_author','speaker','position_holder','quoted_speaker','quoted_author','quoted_position_holder','document_author','translator']){const v=props.record[candidate];if(typeof v==='string'&&v.trim())known.add(v.trim());if(Array.isArray(v))v.forEach(x=>{if(typeof x==='string'&&x.trim())known.add(x.trim())})}return Array.from(known);}
...
<section class="metadata-review" aria-labelledby="metadata-review-title">
  ...
  <div v-if="activeFields.length" class="metadata-grid" role="list">
    <CorpusMetadataFieldEditor v-for="field in activeFields" :key="field" :field="field" :value="record[field]" :status="status(field)" :options="options(field)" :required="requiredFields.has(field)" :boolean="booleanFields.has(field)" :number="numberFields.has(field)" :array="arrayFields.has(field)" :busy="busy" :saving="savingField===field" :saved="savedField===field" :constraint="constraint(field)" :calibrated-acceptance="calibrated(field)" :open="unresolved.has(field)" @save="value=>emit('resolve',field,value)" @no-value="emit('noValue',field)" @source="emit('source',field)" @dirty="value=>emit('dirty',value)"/>
  </div>
