...
<template>
<UiDialog :open="open" size="xlarge" :title="i18n.t('pdf_corpus.jsonl_preview_title','JSONL record preview')" :description="i18n.t('pdf_corpus.jsonl_preview_help','This is generated by the same backend serializer used for publication. Preview provenance uses a temporary publication identifier.')" :close-label="i18n.t('ui.close','Close')" @close="emit('close')">
  <div class="preview-status" :data-ready="wouldPublish?'true':'false'" role="status">
    <b>{{wouldPublish?i18n.t('pdf_corpus.jsonl_ready','Ready for publication'):i18n.t('pdf_corpus.jsonl_not_ready','Not yet publication-ready')}}</b>
    <span v-if="unresolvedFields?.length">{{i18n.tf('pdf_corpus.jsonl_unresolved','{count} unresolved field(s)',{count:unresolvedFields.length})}}: {{unresolvedFields.join(', ')}}</span>
    <span v-if="validationErrors?.length">{{i18n.tf('pdf_corpus.jsonl_validation_errors','{count} validation issue(s)',{count:validationErrors.length})}}</span>
  </div>
  <ul v-if="validationErrors?.length" class="validation-list"><li v-for="item in validationErrors" :key="item">{{item}}</li></ul>
  <pre class="json-preview" tabindex="0" :aria-label="i18n.t('pdf_corpus.jsonl_preview_title','JSONL record preview')">{{jsonl}}</pre>
  <template #footer><span class="footer-note">{{i18n.t('pdf_corpus.jsonl_preview_exact','The final published record uses this schema and serializer.')}}</span><div class="actions"><UiButton :label="i18n.t('pdf_corpus.copy_jsonl','Copy JSONL')" @click="copy"/><UiButton variant="primary" :label="i18n.t('pdf_corpus.download_jsonl_preview','Download preview')" @click="download"/></div></template>
</UiDialog>
</template>
<style scoped>
.preview-status{display:grid;gap:4px;padding:12px;border:1px solid var(--line);border-radius:10px;background:var(--soft);font-size:.875rem;line-height:1.45}.preview-status[data-ready="true"]{border-color:#9ec4a8;background:#eef8f1}.validation-list{margin:0;padding-inline-start:22px;color:var(--warning,#7c5700);font-size:.875rem;line-height:1.45}.json-preview{margin:14px 0 0;max-height:58vh;overflow:auto;padding:14px;border:1px solid var(--line);border-radius:10px;background:var(--soft);white-space:pre-wrap;overflow-wrap:anywhere;font:13px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace}.footer-note{font-size:.8125rem;color:var(--muted)}.actions{display:flex;gap:8px;flex-wrap:wrap}@media(max-width:700px){.actions{width:100%;flex-direction:column}}
</style>
