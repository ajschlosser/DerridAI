...
interface HistoryEntry { run_id?: string; pass?: number; model?: string; outcome?: string; added_fields?: string[]; replaced?: Replaced[]; disputes?: Dispute[] }
...
const last=computed<HistoryEntry|null>(()=>{const history=(props.record.metadata_enrichment_history as HistoryEntry[]|undefined)??[];return [...history].reverse().find(item=>item.outcome==="enriched"||item.outcome==="disputed")??null});
