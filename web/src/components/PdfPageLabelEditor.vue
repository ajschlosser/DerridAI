...
type PageInfo={pdf_page:number;printed_page_label?:string|null;printed_page_label_source?:string|null;width?:number;height?:number};
const props=withDefaults(defineProps<{pages?:PageInfo[];disabled?:boolean}>(),{pages:()=>[],disabled:false});
...
watch(()=>props.pages,()=>{draft.value=Object.fromEntries(props.pages.map(page=>[page.pdf_page,String(page.printed_page_label??"")]));dirty.value=new Set();offset.value=0},{immediate:true,deep:true});
function update(page:number,value:string){draft.value={...draft.value,[page]:value};const next=new Set(dirty.value);next.add(page);dirty.value=next}
function save(){const labels:Record<number,string|null>={};for(const page of dirty.value)labels[page]=draft.value[page]?.trim()||null;emit("save",labels)}
function previous(){offset.value=Math.max(0,offset.value-pageSize)}
function next(){if(offset.value+pageSize<props.pages.length)offset.value+=pageSize}
</script>
