...
.response-selection-bar {
  ...
  border-radius: 14px;
  background: linear-gradient(135deg, var(--card), var(--card) 64%);
  box-shadow: 0 4px 16px rgba(15, 23, 42, 0.035);
}
.response-selection-main {
  min-width: 0;
}
...
.response-selection-label svg {
  width: 12px;
  height: 12px;
  color: var(--accent-fg);
}
.response-selection-main h2 {
  margin: 4px 0 7px;
  color: var(--text-2);
  font:
    600 clamp(18px, 1.6vw, 22px) / 1.28 Georgia,
    "Times New Roman",
    serif;
  display: -webkit-box;
...
.response-selection-meta svg {
  width: 12px;
  height: 12px;
}
.response-selection-meta .grade {
  border-color: var(--line);
  background: var(--card);
  color: var(--tone-ok-fg);
  font-weight: 780;
}
.response-selection-browse {
  min-height: 42px;
  display: flex;
  align-items: center;
...
.response-selection-browse svg {
  width: 16px;
  height: 16px;
}
.response-selection-browse:hover {
  border-color: var(--tone-ok-edge);
  background: var(--tone-ok-bg);
}
.response-selection-browse:focus-visible {
