...
const props = withDefaults(
  defineProps<{ records?: ResponseFaqRecord[]; selectedId?: string; search?: string }>(),
  { records: () => [], selectedId: "", search: "" },
);
const emit = defineEmits<{ select: [record: ResponseFaqRecord] }>();
const i18n = useI18nStore();

type HighlightPart = { text: string; match: boolean };
function recordKey(record: ResponseFaqRecord, index: number) {
  return String(
    record.record_id || `${record.question || "response"}-${record.created_at || index}`,
  );
}
...
function gradeValue(record: ResponseFaqRecord) {
  ...
  const result = (grade?.result && typeof grade.result === "object" ? grade.result : grade) as
    | Record<string, unknown>
    | undefined;
  const raw = result?.overall ?? result?.score;
  return raw == null ? "" : String(raw);
}
function evidenceCount(record: ResponseFaqRecord) {
  return Number(record.evidence_count ?? record.evidence?.length ?? 0);
}
    ...
    >
      ...
      <span class="response-library-list-main">
        <span class="response-library-list-question">
          <template
            v-for="(part, partIndex) in highlightParts(record.question || '')"
            :key="partIndex"
...
.response-library-list > button.active .response-library-question-icon {
  background: var(--card);
  color: var(--accent-fg);
}
.response-library-list-main {
  min-width: 0;
  display: grid;
  gap: 7px;
}
.response-library-list-question {
  color: var(--text);
  font-size: 0.9375rem;
  font-weight: 790;
  line-height: 1.38;
...
.response-library-list-question mark {
  border-radius: 3px;
  background: #fff0a8;
  color: inherit;
  padding: 0 1px;
}
.response-library-list-meta {
  display: flex;
  align-items: center;
  gap: 5px 9px;
  flex-wrap: wrap;
...
.response-library-list-meta > span {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  min-width: 0;
...
.response-library-list-meta .model {
  max-width: 220px;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
.response-library-list-meta svg {
  width: 11px;
  height: 11px;
}
.response-library-list-meta .grade {
  color: var(--tone-ok-fg);
  font-weight: 800;
}
.response-library-open {
  align-self: center;
  color: #9aa7b5;
  font-size: 1.375rem;
  line-height: 1;
}
.response-library-list > button:hover .response-library-open,
.response-library-list > button.active .response-library-open {
  color: var(--accent-fg);
}
.response-library-list > button:focus-visible {
