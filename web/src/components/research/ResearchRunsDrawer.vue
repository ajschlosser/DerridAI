...
const props = withDefaults(
  ...
  { jobs: () => [], selectedJobId: "", selectedJob: null, canManage: false },
);
const emit = defineEmits<{
  open: [job: ResearchJob];
  cancel: [job: ResearchJob];
  remove: [job: ResearchJob];
  refresh: [];
}>();
const i18n = useI18nStore();
const dialog = ref<HTMLDialogElement | null>(null);
const active = computed(
  () => props.jobs.filter((job) => ["queued", "running", "cancelling"].includes(job.status)).length,
);
function open() {
...
function when(value?: string | null) {
  ...
  return Number.isFinite(date.getTime()) ? date.toLocaleString(i18n.locale) : String(value);
}
...
<template>
  <dialog
    ref="dialog"
    class="research-drawer research-runs-dialog"
    aria-labelledby="research-runs-title"
    @cancel.prevent="close"
  >
    <div class="research-drawer-shell">
      <header class="research-drawer-head">
        <div>
          <span class="section-label">{{ i18n.t("research.runs") }}</span>
          <h2 id="research-runs-title">{{ i18n.t("research.research_history") }}</h2>
          <p>
            {{ active }} {{ i18n.t("research.active_runs") }} · {{ jobs.length }}
            {{ i18n.t("research.retained_runs") }}
          </p>
        </div>
        <div class="tools">
          <button class="btn" type="button" @click="emit('refresh')">
            <AppIcon name="refresh" />{{ i18n.t("ui.refresh") }}</button
          ><button
            class="btn icon-only"
            type="button"
      ...
      <div class="research-runs-content">
        <section
          v-if="selectedJob"
          class="research-run-inspector"
          aria-labelledby="research-run-inspector-title"
        >
          <div class="research-run-inspector-head">
            <div>
              <span class="section-label">{{ i18n.t("research.current_run") }}</span>
              <h3 id="research-run-inspector-title">
                {{ selectedJob.prompt || i18n.t("research.untitled_run") }}
              </h3>
              <p>
            ...
            <span :class="['research-run-status', selectedJob.status]">{{
              statusLabel(selectedJob)
            }}</span>
          </div>
          <div v-if="selectedJob.result?.stages?.length" class="research-stage-timeline">
            <div
              v-for="(stage, index) in selectedJob.result.stages"
              :key="`${stage.name || 'stage'}-${index}`"
            >
              <span>{{ index + 1 }}</span
              ><b>{{ stageLabel(stage.name) }}</b
          ...
          >
            ...
            <div>
              <pre v-if="selectedJob.result?.query_metadata">{{
                JSON.stringify(selectedJob.result.query_metadata, null, 2)
              }}</pre>
              <pre v-if="selectedJob.result?.retrieval">{{
                JSON.stringify(selectedJob.result.retrieval, null, 2)
              }}</pre>
            </div>
          </details>
        </section>
        <div class="research-runs-list">
          <article
            v-for="job in jobs"
            :key="job.id"
            :class="['research-run-card', { selected: job.id === selectedJobId }]"
          >
            <button class="research-run-card-main" type="button" @click="emit('open', job)">
              <span :class="['research-run-status-dot', job.status]"></span>
              <span class="research-run-card-copy"
                ><b>{{ job.prompt || i18n.t("research.untitled_run") }}</b
                ><small
                  >{{ job.source_collection || i18n.t("research.evidence_only") }} ·
                  {{ job.model || job.provider || "" }}</small
                ><span
                  >{{ detail(job)
                  }}<template v-if="job.finished_at || job.created_at">
                    · {{ when(job.finished_at || job.created_at) }}</template
                  ></span
                ></span
              >
            ...
            <div v-if="canManage" class="research-run-card-actions">
              <button
                ...
                class="research-text-action danger"
                type="button"
                :disabled="job.status === 'cancelling' || job.cancel_requested"
                @click="emit('cancel', job)"
              >
                {{ job.cancel_requested ? i18n.t("research.cancelling") : i18n.t("ui.cancel") }}
              </button>
              <button
                v-else
                class="research-text-action danger"
          ...
          <div v-if="!jobs.length" class="research-evidence-empty">
            <span aria-hidden="true">∴</span><b>{{ i18n.t("research.no_runs") }}</b>
            <p>{{ i18n.t("research.no_runs_help") }}</p>
          </div>
        </div>
      </div>
    </div>
