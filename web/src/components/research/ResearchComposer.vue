...
function historyLabel(item:Record<string,unknown>){
  const value=String(item.prompt||item.instructions||i18n.t("research.saved_prompt")).replace(/\s+/g," ").trim();
  ...
  <section class="research-composer-v031 card" aria-labelledby="research-compose-title">
    <header class="research-composer-heading">
      ...
      <div class="research-heading-actions">
        <button v-if="canManageRuns" class="btn" type="button" @click="emit('runs')"><AppIcon name="history"/>{{i18n.t('research.runs')}}</button>
        <button v-if="canConfigure" class="btn" type="button" @click="emit('settings')"><AppIcon name="gear"/>{{i18n.t('research.expert_settings')}}</button>
      </div>
    </header>

    <div class="research-question-shell">
      <label class="sr-only" for="researchQuestion">{{i18n.t('research.question')}}</label>
      <textarea id="researchQuestion" class="research-question-input" :value="prompt" :disabled="!canConfigure" :placeholder="i18n.t('research.question_placeholder')" @input="emit('update:prompt',($event.target as HTMLTextAreaElement).value)"></textarea>
      <div class="research-question-tools">
        <div class="research-history-popover">
          ...
          <div v-if="historyOpen" id="researchHistoryMenu" class="research-history-menu" role="dialog" :aria-label="i18n.t('research.recent_questions')">
            <label class="sr-only" for="researchHistorySearch">{{i18n.t('research.search_history')}}</label>
            <input id="researchHistorySearch" v-model="historyQuery" class="control" type="search" :placeholder="i18n.t('research.search_history')" autofocus>
            <div class="research-history-list">
              ...
              <p v-if="!filteredHistory.length" class="note">{{i18n.t('research.no_recent_questions')}}</p>
            </div>
          </div>
        </div>
        <span>{{prompt.length.toLocaleString(i18n.locale)}} {{i18n.t('research.characters')}}</span>
      </div>
    </div>

    <details class="research-instructions-disclosure">
      <summary>{{i18n.t('research.instructions')}}<span>{{instructions?i18n.t('research.added'):i18n.t('research.optional')}}</span></summary>
      <label class="sr-only" for="researchInstructions">{{i18n.t('research.instructions')}}</label>
      <textarea id="researchInstructions" :value="instructions" :disabled="!canConfigure" :placeholder="i18n.t('research.instructions_placeholder')" @input="emit('update:instructions',($event.target as HTMLTextAreaElement).value)"></textarea>
    </details>
