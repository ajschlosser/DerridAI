...
const status = computed(() =>
  String(
    props.collection.status || (props.collection.count || 0 ? "ready" : "empty"),
  ).toLowerCase(),
);
const statusTone = computed(() =>
  status.value === "ready"
    ? "success"
    : status.value.includes("fail") || status.value === "stale"
      ? "danger"
      : status.value === "empty"
        ? "neutral"
        : "info",
...
const syncLabel = computed(() =>
  props.pendingCount
    ? i18n.tf("vector.unsynced_changes_count", {
        count: Number(props.pendingCount).toLocaleString(i18n.locale),
      })
    : i18n.t("vector.sync"),
);
</script>
  ...
  <section class="card vector-collection-hero vector-collection-hero-v037">
    <div class="vector-hero-copy">
      <div class="vector-hero-kicker">
        ...
        <UiStatusBadge
          v-if="collection.protected"
          :label="i18n.t('vector.protected')"
          tone="warning"
        />
      </div>
      <h2>{{ collection.name }}</h2>
      <p v-if="collection.description" class="vector-collection-description">
        {{ collection.description }}
      </p>
      <div class="vector-collection-contract">
        <span
          ><b>{{ Number(collection.count || 0).toLocaleString(i18n.locale) }}</b>
          {{ i18n.t("dynamic.records") }}</span
        >
        <span>{{ collection.retrieval_mode || "semantic" }}</span>
        <span>{{ providerLabel }}</span>
        <span v-if="collection.embedding_dimension"
          >{{ Number(collection.embedding_dimension).toLocaleString(i18n.locale) }}d</span
        >
        <span>{{ collection.distance_metric || "l2" }}</span>
        <span v-if="collection.language_codes?.length">{{
          collection.language_codes.join(", ")
        }}</span>
      </div>
      <div class="vector-collection-provenance">
        <span v-if="collection.source_label || collection.source_kind"
          >{{ i18n.t("vector.source") }}:
          <b>{{ collection.source_label || collection.source_kind }}</b></span
        >
        ...
        <span v-if="collection.build_id" :title="collection.build_id"
          >{{ i18n.t("vector.build") }}: <code>{{ collection.build_id.slice(-12) }}</code></span
        >
      </div>
    </div>
          ...
          <UiButton
            :label="
              collection.protected
                ? i18n.t('vector.disable_protection')
                : i18n.t('vector.enable_protection')
            "
            :icon="collection.protected ? 'unlock' : 'lock'"
            @click="emit('protection')"
          />
          <UiButton
