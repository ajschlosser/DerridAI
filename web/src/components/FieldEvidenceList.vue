...
<template>
  <ul v-if="fieldNames.length" class="evidence-list" :aria-label="i18n.t('pdf_corpus.field_evidence')">
    <li v-for="field in fieldNames" :key="field">
      <button type="button" :aria-pressed="props.selectedField===field" :class="{active:props.selectedField===field}" @click="select(String(field))">
        <b>{{field}}</b>
        <span v-if="props.evidence?.[field]">{{Math.round(Number(props.evidence[field]?.confidence||0)*100)}}% · {{(props.evidence[field]?.block_ids||[]).join(', ')||i18n.t('pdf_corpus.no_source_bound')}}</span>
        <span v-else>{{i18n.t('pdf_corpus.no_source_bound')}}</span>
