...
<style scoped>
.provider-profile-select-component{container-type:inline-size;display:grid;gap:11px;padding:14px;border:1px solid var(--line,#e2e8f0);border-radius:12px;background:linear-gradient(180deg,var(--card) 0%,var(--card) 100%);box-shadow:0 1px 2px rgba(15,23,42,.03)}
...
@container (max-width:460px){.provider-summary-card{grid-template-columns:40px minmax(0,1fr)}.provider-stat-group{grid-column:1/-1}.provider-stat{flex:1;text-align:left}}
@media(max-width:720px){.provider-picker-head{align-items:start;flex-direction:column}.provider-manage-button{padding:0}}
</style>
