...
<template>
  <section class="corpus-build-progress" :aria-label="i18n.t('pdf_corpus.build_progress','Corpus build progress')" aria-live="polite">
    ...
    <div v-if="Number(props.unresolvedCount||0)>0" class="unresolved-line" role="status"><b>{{props.unresolvedCount}}</b> {{i18n.t('pdf_corpus.unresolved_regions','unresolved segmentation region(s)')}}</div>
