...
const stageExplanation=computed(()=>{
  ...
  const map:Record<string,string>={structure:"Reading the PDF and identifying document-level structure and bibliography.",document_review:"Finalizing document-level metadata automatically before segmentation.",segmenting:"Generating plausible boundary candidates deterministically and asking the LLM only small local split/keep questions.",reconciling:"Finalizing topology. Failed or weak classifications default to KEEP; only explicit ambiguity or hard-size safety splits become boundary review items.",enriching:"Enriching each constructed record with discourse, quotation, and indexing metadata. Source text and boundaries are already preserved.",review:"Automated processing is complete. Review only records that need human attention.",ready:"The record set passed automated validation and is ready for acceptance/publication."};
  return i18n.t(`pdf_corpus.stage_help.${stage}`,map[stage]||"Processing the current corpus-build stage.");
});
const nextStage=computed(()=>{const order=["structure","segmenting","reconciling","enriching","review","ready"];const i=order.indexOf(String(props.stage||""));return i>=0&&i<order.length-1?i18n.t(`pdf_corpus.stage.${order[i+1]}`,order[i+1].replace(/_/g," ")):""});
</script>
