...
<template>
  <section class="corpus-build-progress" :aria-label="i18n.t('pdf_corpus.build_progress','Corpus build progress')" aria-live="polite">
    ...
    <div v-if="props.metadataOperation&&['queued','running'].includes(String(props.metadataOperation.state||''))" class="unresolved-line" role="status"><b>{{i18n.t('pdf_corpus.metadata_enrichment_in_progress','Metadata enrichment in progress')}}</b><span>{{props.metadataOperation.records_processed||0}} / {{props.metadataOperation.records_total||0}} {{i18n.t('pdf_corpus.records','records')}} · {{props.metadataOperation.provider||'ollama'}} · {{props.metadataOperation.model||i18n.t('pdf_corpus.default_model','default model')}}</span><span v-if="props.metadataOperation.current_pass">{{i18n.tf('pdf_corpus.enrichment_pass_number','Pass {pass}',{pass:props.metadataOperation.current_pass})}}</span></div>
