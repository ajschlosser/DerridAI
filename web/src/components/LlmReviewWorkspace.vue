...
const proposedCount = computed(() =>
  Object.values(results.value).reduce(
    (count, result) => count + Object.keys(result.proposal?.changes || {}).length,
    0,
  ),
);
const approvedCount = computed(() =>
  Object.values(approvals.value).reduce((count, fields) => count + fields.length, 0),
);
const title = computed(() =>
  mode.value === "auto" ? i18n.t("operations.job.auto") : i18n.t("llm.review_workspace"),
);
const canRun = computed(
  () =>
    selectedCount.value > 0 &&
    Boolean(model.value.trim() || profile.value?.model || status.value?.configured_model),
);
...
function reset(next: { items: TouchupItem[]; initialMode: string }) {
  ...
  model.value = String(
    info.value.profiles.find((item) => item.id === profileId.value)?.model || "",
  );
  instructions.value = "";
  status.value = null;
  running.value = false;
...
async function refreshStatus() {
  ...
  if (!model.value)
    model.value = String(status.value?.configured_model || profile.value?.model || "");
}
...
function setProfile(value: string) {
  ...
  model.value = String(info.value?.profiles.find((item) => item.id === value)?.model || "");
  status.value = null;
  void refreshStatus();
}
function toggleField(field: string, checked: boolean) {
  if (checked && field === "text") selection.value = ["text"];
    ...
    selection.value = [
      ...selection.value.filter((item) => item !== "text" && item !== field),
      field,
    ];
...
function choosePreset(preset: string) {
  selection.value =
    preset === "text"
      ? info.value?.availableFields.includes("text")
        ? ["text"]
        : []
      : [
          ...(preset === "semantic"
            ? info.value?.semanticPreset || []
            : info.value?.attributionPreset || []),
        ];
}
...
function errorMessage(key: string) {
  const value = results.value[key]?.error;
  return value instanceof Error ? value.message : String(value);
}
function fieldsFor(result: TouchupResult) {
  return Object.keys(result.proposal?.changes || {});
}
...
async function run() {
  ...
  if (selection.value.includes("text") && selection.value.length > 1) {
    error.value = i18n.t("llm.text_review_separate");
    return;
  }
  const config = touchupRuntime.touchupRequestConfig(
    profileId.value,
    model.value.trim(),
    selection.value,
  );
  if (!config?.model) {
    error.value = i18n.t("llm.model_required");
    return;
  }
  error.value = "";
  if (mode.value !== "foreground") {
    running.value = true;
    try {
      await touchupRuntime.touchupSubmitBackground(
        items.value,
        config,
        selection.value,
        instructions.value,
  ...
  for (let index = 0; index < items.value.length; index += 1) {
    ...
    try {
      const proposal = await touchupRuntime.touchupRequest(
        item,
        selection.value,
        config,
        instructions.value,
      ...
      approvals.value[item.key] = Object.keys(proposal.changes || {}).filter(
        (field) => field !== "text",
      );
    } catch (exc) {
...
function selectAllChanges() {
  for (const result of Object.values(results.value))
    if (result.proposal) approvals.value[result.item.key] = fieldsFor(result);
}
function clearChanges() {
  for (const result of Object.values(results.value)) approvals.value[result.item.key] = [];
}
...
<template>
  <UiDialog
    ...
    size="xlarge"
    :title="title"
    :description="i18n.tf('llm.review_workspace_summary', { count: items.length })"
  ...
  >
      ...
      <aside class="workspace-config">
        ...
        <section class="workspace-section">
          ...
          <select
            class="control"
            :value="profileId"
            @change="setProfile(($event.target as HTMLSelectElement).value)"
          >
            <option v-for="item in info.profiles" :key="item.id" :value="item.id">
              {{ item.name || item.id }}
            </option>
          </select>
          <p
            class="workspace-status"
            :data-state="status?.available ? 'ready' : status ? 'error' : 'pending'"
          >
            {{ statusLabel }}
          </p>
        </section>
        <section class="workspace-section">
          ...
          <div class="preset-row">
            <UiButton
              size="small"
              :label="i18n.t('llm.attribution')"
              @click="choosePreset('attribution')"
            /><UiButton
              size="small"
              :label="i18n.t('llm.semantics')"
              @click="choosePreset('semantic')"
            /><UiButton
              size="small"
              :label="i18n.t('llm.ocr_text')"
              @click="choosePreset('text')"
            />
          </div>
        </section>
        <section class="workspace-section">
          ...
          <div class="field-actions">
            <UiButton
              size="small"
              :label="i18n.t('llm.select_metadata')"
              @click="
                selection = info.availableFields.filter((field) => field !== 'text').slice(0, 12)
              "
            /><UiButton size="small" :label="i18n.t('ui.clear')" @click="selection = []" /><span>{{
              selectedCount
            }}</span>
          </div>
          <div
            ...
            :key="group.name"
            class="field-group"
            v-show="group.fields.some((field) => info.availableFields.includes(field))"
          >
            <h4>{{ group.name }}</h4>
            <label
              v-for="field in group.fields"
              v-show="info.availableFields.includes(field)"
              :key="field"
              class="check-item"
              ><input
                type="checkbox"
              ...
              /><span>{{ info.fieldLabels[field] || field }}</span
              ><small v-if="info.highRiskFields.includes(field)">{{
                i18n.t("llm.verify")
              }}</small></label
            >
          </div>
        ...
        <section class="workspace-section">
          <h3>{{ i18n.t("llm.model") }}</h3>
          <input v-model="model" class="control" :placeholder="i18n.t('llm.model_name')" /><label
            class="instruction-field"
            ><span>{{ i18n.t("llm.additional_instructions") }}</span
            ><textarea
              v-model="instructions"
              :placeholder="i18n.t('llm.instructions_optional')"
            ></textarea>
          </label>
        </section>
      </aside>
      <section class="workspace-results" aria-live="polite">
        <div v-if="running && mode === 'auto'" class="auto-improve-running">
          <div class="spinner"></div>
          <h3>{{ i18n.t("llm.auto_progress") }}</h3>
          <p>
            {{ Object.keys(results).length }} / {{ items.length }}
            {{ i18n.t("llm.records_reviewed") }}
          </p>
          <div class="batch-progress-bar">
            <i
              :style="{
                width: `${Math.round((Object.keys(results).length / Math.max(items.length, 1)) * 100)}%`,
              }"
            />
          </div>
        </div>
        <template v-else>
          <header class="queue-header">
            <div>
              <b>{{
                running
                  ? i18n.t("llm.review_in_progress")
                  : mode === "auto"
                    ? i18n.t("llm.auto_results")
                    : i18n.t("llm.review_queue")
              }}</b
              ><span
                >{{ Object.keys(results).length }} / {{ items.length }} · {{ proposedCount }}
                {{ i18n.t("llm.proposed_changes") }}</span
              >
            </div>
            <div class="queue-tools">
              <UiButton
                size="small"
                :label="i18n.t('llm.expand_all')"
                @click="expanded = items.map((item) => item.key)"
              /><UiButton
                size="small"
                :label="i18n.t('llm.collapse_all')"
                @click="expanded = []"
              /><UiButton
                v-if="proposedCount"
                size="small"
                :label="i18n.t('llm.select_all_changes')"
                @click="selectAllChanges"
              /><UiButton
                v-if="proposedCount"
                size="small"
                :label="i18n.t('llm.select_none')"
                @click="clearChanges"
              />
            </div>
          </header>
          <section
            ...
            :key="item.key"
            class="queue-card"
            :class="{ active: running && activeIndex === index, error: results[item.key]?.error }"
          >
            <button type="button" class="queue-summary" @click="toggleExpanded(item.key)">
              <span class="queue-index">{{ index + 1 }}</span
              ><span
                ><b>{{ displayValue(item.record.record_id) || `Record ${index + 1}` }}</b
                ><small
                  >{{ displayValue(item.record.work) || item.file.name }} ·
                  {{ displayValue(item.record.page_start) || "?" }}</small
                ></span
              ><span class="queue-status">{{
                results[item.key]?.error
                  ? i18n.t("llm.failed")
                  : results[item.key]
                    ? `${Object.keys(proposalFor(item.key).changes || {}).length} ${i18n.t("llm.changes")}`
                    : running && activeIndex === index
                      ? i18n.t("llm.reviewing")
                      : i18n.t("llm.queued")
              }}</span
              ><span aria-hidden="true">{{ isExpanded(item.key) ? "▾" : "▸" }}</span>
            </button>
            <div v-if="isExpanded(item.key)" class="queue-body">
              <div v-if="!results[item.key]" class="empty-result">
                {{
                  running && activeIndex === index
                    ? i18n.t("llm.waiting_model")
                    : i18n.t("llm.waiting_queue")
                }}
              </div>
              <div v-else-if="results[item.key].error" class="llm-error" role="alert">
              ...
              <template v-else
                ><div class="record-actions">
                  <UiButton
                    size="small"
                    :label="i18n.t('llm.select_record_changes')"
                    @click="approvals[item.key] = fieldsFor(results[item.key])"
                  /><UiButton
                    size="small"
                    :label="i18n.t('llm.clear_record')"
                    @click="approvals[item.key] = []"
                  />
                </div>
                <article
                  v-for="field in fieldsFor(results[item.key])"
                  :key="field"
                  class="proposal"
                >
                  <header>
                    <label
                      ><input
                        ...
                        @change="
                          setChecked(item, field, ($event.target as HTMLInputElement).checked)
                        "
                      />
                      {{ info.fieldLabels[field] || field }}</label
                    ><small v-if="info.highRiskFields.includes(field)">{{
                      i18n.t("llm.verify")
                    }}</small>
                  </header>
                  <div class="proposal-grid">
                    <div>
                      <b>{{ i18n.t("llm.current") }}</b>
                      <pre>{{ pretty(item.record[field]) }}</pre>
                    </div>
                    <div>
                      <b>{{ i18n.t("llm.proposed") }}</b>
                      <pre>{{ pretty(proposalFor(item.key).changes?.[field]) }}</pre>
                    </div>
                  </div>
                  <p>
                    {{ proposalFor(item.key).rationale?.[field] || i18n.t("llm.no_rationale") }}
                  </p>
                </article></template
              >
            </div>
    ...
    <template #footer
      ><span class="footer-note">{{
        error ||
        (running
          ? i18n.t("llm.review_running")
          : `${approvedCount} ${i18n.t("llm.selected_changes")}`)
      }}</span>
      <div class="footer-actions">
        <UiButton :label="i18n.t('ui.close')" @click="close" /><UiButton
          v-if="!running && Object.keys(results).length"
          :label="i18n.t('llm.review_again')"
          @click="reviewAgain"
        /><UiButton
          v-if="!running && proposedCount"
          variant="soft"
          :label="i18n.t('llm.accept_all')"
          @click="apply(true)"
        /><UiButton
          v-if="!running && Object.keys(results).length"
          variant="primary"
          :disabled="!canRun"
          :label="i18n.t('llm.apply_selected')"
          @click="apply(false)"
        /><UiButton
          v-if="!running && !Object.keys(results).length"
          variant="primary"
          :disabled="!canRun"
          :label="mode === 'auto' ? i18n.t('operations.job.auto') : i18n.t('llm.run_review')"
          @click="run"
        /></div
    ></template>
  </UiDialog>
...
.workspace-status[data-state="ready"] {
...
.queue-card.active {
  ...
  box-shadow: 0 0 0 3px color-mix(in srgb, var(--tone-ok-border) 12%, transparent);
}
.queue-card.error {
...
.auto-improve-running h3,
.auto-improve-running p {
  margin: 0;
}
.batch-progress-bar {
  width: min(560px, 90%);
