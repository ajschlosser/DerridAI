...
<article class="metadata-field" :data-attention="status?.status==='unresolved'||status?.status==='invalid'?'true':'false'">
  ...
  <div v-else class="field-editor">
    ...
    <div class="field-meta">
      <span v-if="isLlm" class="proposal">{{status?.auto_populated?i18n.t('pdf_corpus.llm_suggestion_autofilled','LLM value auto-filled from a >65% confidence suggestion'):i18n.t('pdf_corpus.llm_suggestion_prefilled','LLM suggestion prefilled — verify before saving')}}</span>
