...
<template>
  <section class="quality" :aria-labelledby="`quality-${build.build_id||'current'}`">
    ...
    <p v-if="build.trash_quality?.exceeds_threshold" class="quality-alert" role="alert">{{i18n.tf('pdf_corpus.quality.garbage_alert','Deterministic source-quality check flagged {records}% of Records or {pages}% of PDF pages as potentially unusable. Review the SourceUnits and extraction warnings before enrichment.',{records:Math.round(Number(build.trash_quality.trash_ratio||0)*100),pages:Math.round(Number(build.trash_quality.unusable_page_ratio||0)*100)})}}</p>
    <p v-if="build.trash_quality?.median_noise!=null" class="detail">{{i18n.tf('pdf_corpus.text_noise.summary','Median record noise {median}% · unusable at {threshold}% or higher · {count} record(s) flagged',{median:Math.round(Number(build.trash_quality.median_noise||0)),threshold:Math.round(Number(build.trash_quality.noise_unusable_threshold||45)),count:Number(build.trash_quality.trash_record_count||0)})}}</p>
