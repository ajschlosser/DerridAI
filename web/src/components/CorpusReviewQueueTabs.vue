...
const primaryModel=computed<ReviewQueue>(()=>issueQueues.has(props.modelValue)?"issues":props.modelValue);
...
const issueFilter=computed(()=>issueQueues.has(props.modelValue)?props.modelValue:"issues");
function selectPrimary(value:ReviewQueue){emit("update:modelValue",value)}
function selectIssue(event:Event){emit("update:modelValue",(event.target as HTMLSelectElement).value as ReviewQueue)}
...
<style scoped>
.queue-controls{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.queue-tabs{display:flex;align-items:center;gap:6px;flex-wrap:wrap}.queue-tab{min-height:40px;border:1px solid transparent;border-radius:999px;background:transparent;color:var(--muted);padding:7px 11px;display:flex;align-items:center;gap:7px;cursor:pointer;font-size:0.8125rem;font-weight:700}.queue-tab strong{min-width:22px;padding:2px 6px;border-radius:999px;background:var(--soft);color:var(--text);font-size:.8125rem;text-align:center}.queue-tab[aria-pressed="true"]{background:var(--card);border-color:var(--line);color:var(--text);box-shadow:0 1px 2px rgb(0 0 0/.04)}.queue-tab:focus-visible,.issue-filter select:focus-visible{outline:3px solid var(--accent);outline-offset:2px}.queue-tab:disabled{opacity:.55;cursor:not-allowed}.issue-filter{display:flex;align-items:center;gap:7px;font-size:.8125rem;font-weight:700;color:var(--muted)}.issue-filter .control{min-height:40px;font-size:0.8125rem;min-width:150px}.queue-count-help{flex-basis:100%;margin:0;color:var(--muted);font-size:.75rem}@media(max-width:700px){.queue-controls,.issue-filter{align-items:stretch}.issue-filter{width:100%;display:grid}.issue-filter .control{width:100%}}
</style>
