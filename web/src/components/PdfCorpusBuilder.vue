...
const sourcePdfUrl=computed(()=>{if(!selectedAssetId.value)return "";const kind=selectedAsset.value?.media_kind;if(kind&&kind!=="pdf")return "";return pdfCorpusApi.assetContentUrl(selectedAssetId.value)});
...
function ingestSummary(asset:PdfAsset){const meta=asset.initial_metadata||{};const speakers=Array.isArray(meta.speakers)?meta.speakers.map(String).filter(Boolean):[];const quality=asset.source_quality?.valid_for_enrichment===false?i18n.t("pdf_corpus.needs_attention","Needs attention"):i18n.t("pdf_corpus.passed","Passed");return i18n.tf("pdf_corpus.source_ingested_checked","Source ingested: {pages} pages · {blocks} source blocks. Deterministic check: title {title}; author {author}; speaker {speaker}; source quality {quality}.",{pages:asset.page_count,blocks:asset.block_count,title:String(meta.title||"—"),author:String(meta.document_author||"—"),speaker:String(meta.speaker||speakers.join(", ")||"—"),quality})}
async function rememberIngested(asset:PdfAsset){await refreshAssets();selectedAssetId.value=asset.asset_id;setMessage(ingestSummary(asset))}
async function upload(file?:File|null){if(!file)return;busy.value="upload";setMessage("");try{const asset=await pdfCorpusApi.uploadAsset(file,"auto",sourceIllegibility.value);await rememberIngested(asset)}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
async function loadSourceUrl(){const url=sourceUrl.value.trim();if(!url)return;busy.value="upload";setMessage("");try{const asset=await pdfCorpusApi.importUrl(url,sourceIllegibility.value);await rememberIngested(asset)}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
async function searchGutenberg(){const query=gutenbergQuery.value.trim();if(!query){gutenbergHits.value=[];return}busy.value="gutenberg";setMessage("");try{const result=await pdfCorpusApi.searchGutenberg(query);gutenbergHits.value=result.items||[];if(!gutenbergHits.value.length)setMessage(i18n.t("pdf_corpus.gutenberg_empty","No Project Gutenberg texts matched that search."))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
async function importGutenberg(etextId:number){busy.value="upload";setMessage("");try{const asset=await pdfCorpusApi.importGutenberg(etextId,sourceIllegibility.value);await rememberIngested(asset)}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
...
async function runMetadataEnrichment(payload:{providerProfileId:string;model:string;families:string[];scope:string}){if(!currentBuild.value)return;busy.value='metadata-enrichment';try{const actionPayload=directProfilePayloadWithModel(payload.providerProfileId,payload.model)||{provider_profile_id:payload.providerProfileId,model:payload.model||undefined};currentBuild.value=await pdfCorpusApi.rerunMetadataEnrichment(currentBuild.value.build_id,{...actionPayload,families:payload.families,scope:payload.scope});metadataEnrichmentOpen.value=false;syncBuildInRail(currentBuild.value);registerBuildOperation(currentBuild.value);startPolling();setMessage(i18n.t('pdf_corpus.metadata_enrichment_started','Metadata enrichment pass started. Accepted records will reopen only when new or disputed metadata needs review.'))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),'error')}finally{busy.value=''}}
  ...
  <section class="corpus-builder" :aria-labelledby="'pdf-corpus-builder-title'">
    ...
    <section v-if="showBuildConfiguration" class="builder-setup" :aria-labelledby="'pdf-corpus-config-title'">
      ...
      <section class="setup-section setup-source-section" aria-labelledby="pdf-corpus-source-title">
        <div class="setup-section-head"><div><span class="setup-step">1</span><div><h3 id="pdf-corpus-source-title">{{i18n.t('pdf_corpus.source_setup_title','Source document')}}</h3><p>{{i18n.t('pdf_corpus.source_setup_help','Choose a PDF, text, rich text, Word, image, or audio file, or add the PDF currently open in Explorer. URLs and Project Gutenberg texts load into the same source spans.')}}</p></div></div></div>
        <CorpusSourceIngest v-model:asset-id="selectedAssetId" v-model:illegibility="sourceIllegibility" v-model:source-url="sourceUrl" v-model:gutenberg-query="gutenbergQuery" :assets="assets" :hits="gutenbergHits" :selected-asset="selectedAsset" :disabled="busy!==''||buildRunning" :busy="busy" @use-current="useCurrentPdf" @file="upload" @load-url="loadSourceUrl" @search-gutenberg="searchGutenberg" @import-gutenberg="importGutenberg" />
      </section>
      ...
      <main class="build-main">
        ...
        <template v-if="showReviewWorkspace">
          ...
          <section class="review-toolbar" :aria-label="i18n.t('pdf_corpus.review_controls','Record review controls')">
            ...
            <div class="workspace-switcher" role="group" :aria-label="i18n.t('pdf_corpus.review_workspace','Review workspace')"><button type="button" class="btn small" :aria-pressed="reviewWorkspaceMode==='record'" @click="setReviewWorkspaceMode('record')">{{i18n.t('pdf_corpus.workspace.record','Record')}}</button><button type="button" class="btn small" :aria-pressed="reviewWorkspaceMode==='metadata'" :disabled="!selectedRecord" @click="setReviewWorkspaceMode('metadata')">{{i18n.t('pdf_corpus.workspace.metadata','Metadata workspace')}}</button><button type="button" class="btn small" :aria-pressed="reviewWorkspaceMode==='source'" :disabled="!selectedRecord" @click="setReviewWorkspaceMode('source')">{{i18n.t('pdf_corpus.workspace.source','Source workspace')}}</button></div>
            <div class="review-bulk"><button type="button" class="btn small primary" @click="acceptCleanRecords" :disabled="busy!==''||readyCount===0">{{i18n.tf('pdf_corpus.accept_clean','Accept clean ({count})',{count:readyCount})}}</button><button type="button" class="btn small" @click="bulkMetadataOpen=!bulkMetadataOpen" :disabled="busy!==''">{{i18n.t('pdf_corpus.bulk_edit_metadata','Bulk edit metadata')}}</button><button type="button" class="btn small" @click="bulkDisposition('rejected')" :disabled="busy!==''||reviewLocked||selectedReviewCount===0">{{i18n.tf('pdf_corpus.reject_selected_count','Reject selected ({count})',{count:selectedReviewCount})}}</button></div>
          ...
          <section class="review-grid record-first-review" :class="{'queue-collapsed':reviewQueueCollapsed,'detail-mode':reviewWorkspaceMode!=='record','metadata-workspace':reviewWorkspaceMode==='metadata','source-workspace':reviewWorkspaceMode==='source'}" :aria-busy="recordsLoading">
            <nav ref="recordListEl" class="records-pane" tabindex="-1" :aria-labelledby="'pdf-corpus-records-pane'">
              ...
              <div v-for="record in records" :key="record.record_id" class="record-row-wrap">
                ...
                <button type="button" class="record-row" :class="{active:record.record_id===selectedRecordId}" :aria-current="record.record_id===selectedRecordId?'true':undefined" @click="selectRecord(record)">
                  <span class="record-state" :data-state="recordState(record)" aria-hidden="true"></span>
                  <span><b>{{record.record_id}}</b><small>{{i18n.t('pdf_corpus.pages','pp.')}} {{record.page_start}}–{{record.page_end}} · {{record.text_length.toLocaleString()}} {{i18n.t('pdf_corpus.characters','chars')}}</small><span class="record-row-status" :data-state="recordState(record)">{{recordStateLabel(record)}}</span><small v-if="recordIssueKinds(record).length" class="record-issue-summary">{{recordIssueKinds(record).map(kind=>i18n.t(`pdf_corpus.record_state.${kind}`,kind)).join(' · ')}}</small></span>
                </button>
              </div>
...
.record-first-review{grid-template-columns:minmax(205px,.48fr) minmax(520px,1.5fr) minmax(360px,.95fr)}.record-first-review>.records-pane{border-inline-end:1px solid var(--line)}.record-review-pane,.review-inspector,.records-pane{min-width:0;max-height:76vh;overflow:auto}.review-inspector{border-inline-start:1px solid var(--line);background:var(--soft)}.review-inspector-tabs{position:sticky;top:0;z-index:6;display:grid;grid-template-columns:repeat(3,1fr);background:var(--card);border-bottom:1px solid var(--line)}.review-inspector-tabs button{min-height:42px;border:0;border-inline-end:1px solid var(--line);background:transparent;color:var(--muted);font-size:.8125rem;font-weight:800;cursor:pointer;display:flex;justify-content:center;align-items:center;gap:6px}.review-inspector-tabs button[aria-selected="true"]{background:var(--soft);color:var(--text);box-shadow:inset 0 -3px 0 var(--accent)}.review-inspector-tabs button span{min-width:18px;border-radius:999px;background:#fff3cf;color:#604300;padding:1px 5px;font-size:.8125rem}.review-inspector-panel{min-width:0}.review-inspector-panel>.resolution-panel{border-top:0;background:transparent}.review-inspector-panel .record-data{border-top:1px solid var(--line);padding:10px 12px;background:var(--card)}.inspector-help{margin:0;padding:12px 14px 0;font-size:.8125rem;line-height:1.45;color:var(--muted)}.compact-source-blocks{padding-top:6px}.source-review-panel{background:var(--card);min-height:100%}.source-inspector-head{position:static;z-index:auto}.metadata-row-warning{color:#765700!important;font-weight:800}.metadata-accept-blocker{display:block}.metadata-accept-blocker b{margin-inline-end:4px}.record-primary-text{max-width:76ch}.decision-bar{min-height:56px}.review-toolbar{grid-template-columns:minmax(0,1fr) minmax(210px,.7fr) auto auto}.review-bulk .btn{min-height:34px}
...
.review-readonly-banner{display:flex;align-items:center;padding:10px 14px;border:1px solid #d9bf76;border-radius:10px;background:#fff8e9;color:#604300}.review-readonly-banner>div{display:grid;gap:3px}.review-readonly-banner b{font-size:13px}.review-readonly-banner span{font-size:.8125rem;line-height:1.45}
...
.record-row{min-height:76px;padding:10px 11px;gap:9px}.record-row span> b{font-size:.8125rem}.record-row small{font-size:.8125rem;line-height:1.35}.record-row-status{display:inline-flex;width:max-content;margin-top:5px;padding:2px 7px;border-radius:999px;background:#eef4ef;color:#315843;font-size:.8125rem;font-weight:800}.record-row-status[data-state="metadata"],.record-row-status[data-state="topology"],.record-row-status[data-state="source"]{background:#fff2cf;color:#684d00}.record-row-status[data-state="rejected"]{background:#fdeaea;color:#7b2424}.record-row-status[data-state="accepted"]{background:#e9f7ee;color:#28623a}.record-issue-summary{margin-top:3px;color:var(--muted)}
.record-state[data-state="metadata"],.record-state[data-state="topology"],.record-state[data-state="source"]{background:#b58400}.record-state[data-state="ready"]{background:#46765b}
.record-review-pane{display:flex;flex-direction:column;background:var(--card)}.record-review-head{padding:14px 20px}.record-review-head h3{font-size:17px}.record-review-head p{font-size:.8125rem}.record-primary-text{flex:1;max-width:76ch;width:100%;padding:30px 38px;font:17px/1.72 Georgia,serif}.review-reason{margin:14px 20px 0;font-size:.8125rem;line-height:1.45}.metadata-accept-blocker{margin:0 20px 14px;font-size:.8125rem;line-height:1.45}
...
.review-inspector{background:var(--card);border-inline-start:1px solid var(--line)}.review-inspector-tabs button{min-height:48px;font-size:13px}.review-inspector-tabs button span{font-size:.8125rem}.inspector-help{font-size:.8125rem}.source-inspector-head{font-size:.8125rem}.source-block p{font-size:.8125rem;line-height:1.5}.source-block header{font-size:.8125rem}
...
.builder-message{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;font-size:13px;line-height:1.45}.builder-message.warning{border-color:#d6b45b;background:#fff9e9;color:#604b12}.message-dismiss{flex:0 0 auto;min-width:36px;min-height:36px;border:0;border-radius:8px;background:transparent;color:inherit;font-size:20px;cursor:pointer}.message-dismiss:focus-visible{outline:3px solid var(--accent);outline-offset:2px}
...
.review-action-feedback{grid-column:1/-1;margin:0;padding:9px 11px;border:1px solid #a9c8b2;border-radius:9px;background:#eef8f1;color:#285c39;font-size:.8125rem;line-height:1.5;font-weight:650}
...
.setup-source-section{display:grid;gap:13px;padding:16px 18px}.setup-section-head>div,.phase-label{display:flex;gap:12px;align-items:flex-start}.setup-section-head h3,.phase-label h3{margin:0;font-size:1rem}.setup-section-head p,.phase-label p{margin:3px 0 0;color:var(--muted);font-size:.875rem;line-height:1.45;max-width:86ch}.setup-step{display:grid;place-items:center;flex:0 0 30px;inline-size:30px;block-size:30px;border-radius:50%;background:var(--soft);border:1px solid var(--line);font-size:.8125rem;font-weight:850}.source-setup-grid{display:grid;grid-template-columns:minmax(280px,1fr) minmax(280px,1fr);gap:12px;align-items:end}.source-setup-grid label,.source-ingest{display:grid;gap:5px}.source-ingest{gap:10px;align-content:end}.illegibility-field{display:grid;gap:4px}.illegibility-field small{color:var(--muted);line-height:1.4}.illegibility-field input[type=range]{inline-size:100%}.source-actions{display:flex;gap:8px;flex-wrap:wrap}.alternate-sources{display:grid;gap:10px}.url-source,.gutenberg-source{display:grid;grid-template-columns:minmax(220px,1fr) auto;gap:8px;align-items:end}.url-source label,.gutenberg-source label{display:grid;gap:5px}.gutenberg-hits{display:flex;flex-wrap:wrap;gap:8px}.gutenberg-hits .btn{text-align:left}.source-facts{display:flex;gap:8px 14px;flex-wrap:wrap;color:var(--muted);font-size:.8125rem}.source-facts span{overflow-wrap:anywhere}.setup-phase{display:grid;gap:12px;padding:16px}.phase-label{padding:0 2px}.document-structure-config{border:0!important;box-shadow:none!important;padding:0!important}
