...
const semanticIndexing=ref(false);
...
const canStartConcurrentBuild=computed(()=>Boolean(selectedAssetId.value&&contextSafe.value&&(selectedProviderId.value||!activeBuildCount.value)));
...
const selectedRecordEnrichmentPending=computed(()=>{
  ...
  if(state)return !["complete","failed","skipped"].includes(state);
...
async function acceptCleanRecords(){
  ...
  try{
    ...
    await restoreReviewViewport(viewport,{record:true,inspector:true});
    setMessage(i18n.tf("pdf_corpus.accept_clean_done","Accepted {count} clean record(s). Only exceptions remain.",{count:result.changed}));
  }catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}
}
async function bulkDisposition(disposition:"accepted"|"rejected"){
  ...
  try{
    ...
    await restoreReviewViewport(viewport,{record:true,inspector:true});
    setMessage(result.blocked_metadata?i18n.tf("pdf_corpus.bulk_done_metadata_blocked","Accepted {count} eligible record(s); {blocked} record(s) still require metadata decisions and are now shown in the metadata queue.",{count:result.changed,blocked:result.blocked_metadata}):i18n.tf("pdf_corpus.bulk_done","Updated {count} record(s).",{count:result.changed}));
  }catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}
}

async function undoReview(){if(!currentBuild.value)return;const viewport=captureReviewViewport();busy.value="record";try{const result=await pdfCorpusApi.undoReview(currentBuild.value.build_id);await refreshBuild();await refreshRecords(true,result.selected_record_id||"");await restoreReviewViewport(viewport,{record:true});setMessage(i18n.t("pdf_corpus.undo_done","The last merge or split was undone."))}catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
...
async function rerunMetadata(){if(!currentBuild.value||!selectedRecord.value)return;const viewport=captureReviewViewport();busy.value="record";try{const payload={...providerPayload.value} as Record<string,unknown>;if(metadataRerunFamily.value!=="all")payload.families=[metadataRerunFamily.value];const row=await pdfCorpusApi.rerunMetadata(currentBuild.value.build_id,selectedRecord.value.record_id,payload);selectedRecord.value=row;metadataDraft.value=JSON.stringify(recordMetadata(row),null,2);await refreshBuild();await refreshRecords(false,row.record_id);await restoreReviewViewport(viewport);reviewInspectorTab.value="metadata";setMessage(metadataRerunFamily.value==="all"?i18n.t("pdf_corpus.metadata_rerun","Interpretive metadata rerun against the current reviewed record text."):i18n.tf("pdf_corpus.metadata_family_rerun","{family} metadata rerun against the current reviewed record text.",{family:i18n.t(`pdf_corpus.metadata_family.${metadataRerunFamily.value}`,metadataRerunFamily.value)}))}catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
    ...
    <div class="builder-workspace" :class="{'review-mode':showReviewWorkspace&&!finishPhase}">
      ...
      <main class="build-main">
        ...
        <template v-if="showReviewWorkspace">
          ...
          <section v-if="buildRunning&&currentBuild?.stage==='enriching'" class="review-readonly-banner collaboration-banner" role="status" aria-live="polite"><div><b>{{i18n.t('pdf_corpus.collaborative_review_title','Human review and enrichment are working together')}}</b><span>{{selectedRecordEnrichmentPending?i18n.t('pdf_corpus.collaborative_review_active','You can edit this record now. Human-owned fields take precedence; background enrichment will skip or preserve your decisions.'):i18n.t('pdf_corpus.collaborative_review_settled','This record has settled while enrichment continues on other records.')}}</span></div></section>

          <section v-if="currentBuild" class="review-session-bar" role="status" aria-live="polite">
            <div class="review-session-title"><span class="eyebrow">{{i18n.t('pdf_corpus.review_mode','Record review')}}</span><b>{{currentBuild.source_filename}}</b><small>{{currentBuild.model||i18n.t('pdf_corpus.provider_default','Provider default')}} · {{currentBuild.build_id}}</small></div>
            <div class="review-session-stats"><strong>{{currentBuild.accepted_count||0}}</strong><span>{{i18n.t('pdf_corpus.accepted_label','accepted')}}</span><strong>{{readyCount}}</strong><span>{{i18n.t('pdf_corpus.queue_ready','Reviewable')}}</span><strong>{{pendingCount}}</strong><span>{{i18n.t('pdf_corpus.remaining','remaining')}}</span><strong>{{issueCount}}</strong><span>{{i18n.t('pdf_corpus.need_attention','need attention')}}</span></div>
            <button type="button" class="btn small" @click="focusView=true" :disabled="!selectedRecord">{{i18n.t('pdf_corpus.focus_view','Focus view')}}</button>
          </section>
          ...
          <section class="review-toolbar" :aria-label="i18n.t('pdf_corpus.review_controls','Record review controls')">
            ...
            <CorpusBulkMetadataEditor v-if="bulkMetadataOpen" class="bulk-metadata-workspace" :selected-count="selectedReviewCount" :total-count="Number(currentBuild?.record_count||recordTotal)" :disabled="busy!==''||reviewLocked" @apply="applyBulkMetadata" @close="bulkMetadataOpen=false" />
          ...
          <section class="review-grid record-first-review" :aria-busy="recordsLoading">
            ...
            <article ref="reviewPaneEl" class="record-review-pane" :aria-labelledby="selectedRecord?'review-record-title':undefined">
              <template v-if="selectedRecord">
                ...
                <CorpusSourceIssuePanel v-if="selectedRecord.source_quality_issues?.length" :issues="selectedRecord.source_quality_issues" />
                ...
                <section class="record-text-review" aria-labelledby="reviewed-record-text-title"><header><div><b id="reviewed-record-text-title">{{i18n.t('pdf_corpus.reviewed_record_text','Reviewed record text')}}</b><span v-if="selectedRecord.text_review_status==='human_corrected'" class="human-corrected">{{i18n.t('pdf_corpus.human_corrected','Human corrected')}}</span></div><button type="button" class="btn small" @click="editingText?cancelTextEdit():beginTextEdit()" :disabled="busy!==''||reviewLocked">{{editingText?i18n.t('ui.cancel','Cancel'):i18n.t('pdf_corpus.edit_text','Edit text')}}</button></header><textarea v-if="editingText" v-model="textDraft" class="record-text-editor" :aria-label="i18n.t('pdf_corpus.reviewed_record_text','Reviewed record text')"></textarea><div v-else class="record-primary-text">{{selectedRecord.text}}</div><div v-if="editingText" class="text-review-actions"><label v-if="selectedRecord.source_quality_issues?.length" class="resolve-source-check"><input v-model="resolveSourceOnTextSave" type="checkbox"><span>{{i18n.t('pdf_corpus.resolve_source_with_correction','Mark this record-level source issue resolved by the reviewed correction')}}</span></label><div><button type="button" class="btn" @click="cancelTextEdit">{{i18n.t('ui.cancel','Cancel')}}</button><button type="button" class="btn primary" @click="saveReviewedText()" :disabled="busy!==''||!textDraft.trim()">{{busy==='text'?i18n.t('ui.saving','Saving…'):i18n.t('pdf_corpus.save_reviewed_text','Save reviewed text')}}</button></div></div></section>
            ...
            <aside ref="reviewInspectorEl" class="review-inspector" :aria-label="i18n.t('pdf_corpus.review_details','Review details')">
              ...
              <section v-if="selectedRecord&&reviewInspectorTab==='metadata'" id="review-panel-metadata" class="review-inspector-panel" role="tabpanel" aria-labelledby="review-tab-metadata" tabindex="0">
                <CorpusMetadataResolutionPanel :record="selectedRecord" :region-types="regionTypes" :discourse-roles="discourseRoles" :busy="busy!==''||reviewLocked" :saving-field="metadataSavingField" :saved-field="metadataSavedField" @resolve="resolveMetadataField" @source="showMetadataSource" />
                <details v-if="currentBuild?.manifest" class="document-metadata-editor"><summary>{{i18n.t('pdf_corpus.edit_document_metadata','Edit document metadata')}}</summary><p>{{buildRunning&&['enriching','metadata_retry'].includes(String(currentBuild?.stage||''))?i18n.t('pdf_corpus.document_metadata_live_help','Bibliographic document metadata can be corrected while enrichment continues. Human changes become authoritative for inherited record fields; structural page-boundary changes wait until background work stops.'):i18n.t('pdf_corpus.document_metadata_edit_help','Changes here update inherited metadata and citations across the build. Explicit human record overrides are preserved.')}}</p><DocumentManifestEditor :manifest="currentBuild.manifest||{}" :disabled="busy!==''||(buildRunning&&!['enriching','metadata_retry'].includes(String(currentBuild?.stage||'')))" @save="saveManifest" /></details>
              ...
              <section v-else-if="selectedRecord" id="review-panel-source" class="review-inspector-panel source-review-panel" role="tabpanel" aria-labelledby="review-tab-source" tabindex="0">
                <CorpusSourceIssuePanel v-if="selectedRecord.source_quality_issues?.length" :issues="selectedRecord.source_quality_issues" />
                <CorpusSourceIssuePanel v-else-if="selectedRecord.resolved_source_quality_issues?.length" :issues="selectedRecord.resolved_source_quality_issues" :resolved="true" />
...
/* 0.47.1 Gregarious Guinea Pig — editable reviewed text, selective enrichment, explicit ownership, and concurrent-build clarity. */
...
.review-session-bar{position:sticky;top:0;z-index:12;display:grid;grid-template-columns:minmax(220px,1fr) auto auto;gap:18px;align-items:center;padding:12px 16px;border:1px solid var(--line);border-radius:12px;background:color-mix(in srgb,var(--card) 96%,transparent);backdrop-filter:blur(8px);box-shadow:0 4px 16px rgb(0 0 0/.04)}
.review-session-title{display:grid;gap:2px}.review-session-title .eyebrow{font-size:.8125rem}.review-session-title b{font-size:15px}.review-session-stats{display:flex;align-items:baseline;gap:6px 10px;color:var(--muted);font-size:.8125rem}.review-session-stats strong{font-size:16px;color:var(--text)}
...
@media(max-width:1100px){.review-session-bar{grid-template-columns:1fr auto}.review-session-stats{grid-column:1/-1}.review-toolbar{top:92px;grid-template-columns:1fr 1fr}.review-bulk{justify-content:flex-start}.record-first-review{height:auto;min-height:0;grid-template-columns:220px minmax(0,1fr)}.review-inspector{grid-column:1/-1;min-height:420px;height:auto;max-height:70vh}.record-review-pane,.records-pane{height:68vh}}
@media(max-width:760px){.review-session-bar,.review-toolbar{position:static;grid-template-columns:1fr}.review-session-stats{flex-wrap:wrap}.record-first-review{grid-template-columns:1fr}.records-pane{height:260px}.record-review-pane{height:auto}.record-primary-text{padding:22px 18px;font-size:16px}.review-inspector{max-height:none}.decision-bar{position:sticky;bottom:0;flex-direction:column;align-items:stretch}.structural-actions,.decision-actions{justify-content:stretch}.decision-bar .btn{flex:1}}
...
.llm-contribution{display:grid;grid-template-columns:minmax(240px,1fr) minmax(360px,1.2fr);gap:18px;padding:14px 16px;border:1px solid var(--line);border-radius:12px;background:var(--soft)}.llm-contribution h3{margin:2px 0 4px;font-size:16px}.llm-contribution p{margin:0;font-size:.8125rem;line-height:1.5;color:var(--muted)}.llm-contribution dl{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;margin:0}.llm-contribution dl>div{display:grid;align-content:start;gap:3px;padding:9px;border-radius:8px;background:var(--card);border:1px solid var(--line)}.llm-contribution dt{font-size:.8125rem;line-height:1.35;color:var(--muted)}.llm-contribution dd{margin:0;font-size:18px;font-weight:800}
.record-text-review{display:grid;gap:10px;margin:14px 20px 0;border:1px solid var(--line);border-radius:10px;overflow:hidden;background:var(--card)}.record-text-review>header{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;border-bottom:1px solid var(--line);background:var(--soft)}.record-text-review>header>div{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.record-text-review>header b{font-size:14px}.human-corrected{display:inline-flex;padding:2px 7px;border-radius:999px;background:#e9f7ee;color:#28623a;font-size:.8125rem;font-weight:800}.record-text-review .record-primary-text{padding:20px 24px;min-height:220px;max-height:62vh;overflow-y:auto;overscroll-behavior:contain}.record-text-editor{width:calc(100% - 24px);min-height:330px;height:min(62vh,720px);max-height:72vh;overflow-y:auto;overscroll-behavior:contain;margin:0 12px;padding:14px;border:1px solid var(--line);border-radius:8px;background:var(--card);color:var(--text);font:16px/1.65 Georgia,serif;resize:vertical}.record-text-editor:focus-visible{outline:3px solid var(--accent);outline-offset:2px}.text-review-actions{display:grid;gap:10px;padding:0 12px 12px}.text-review-actions>div{display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap}.resolve-source-check{display:flex;align-items:flex-start;gap:8px;padding:10px;border:1px solid #d9bf76;border-radius:8px;background:#fff8e9;color:#604300;font-size:.8125rem;line-height:1.45}.original-extraction-snapshot{max-height:320px;overflow:auto;white-space:pre-wrap;padding:12px;margin:10px 12px;border:1px solid var(--line);border-radius:8px;background:var(--soft);font:12px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace}
@media(max-width:1000px){.llm-contribution{grid-template-columns:1fr}.llm-contribution dl{grid-template-columns:repeat(3,minmax(0,1fr))}}
@media(max-width:760px){.mode-options{grid-template-columns:1fr}.llm-contribution dl{grid-template-columns:repeat(2,minmax(0,1fr))}.record-text-review{margin-inline:10px}.record-text-review>header{align-items:flex-start;flex-direction:column}.record-text-editor{min-height:260px}.text-review-actions>div{justify-content:stretch}.text-review-actions .btn{flex:1}}
...
.document-metadata-editor{margin:12px 14px;border:1px solid var(--line);border-radius:9px;background:var(--card);overflow:hidden}.document-metadata-editor>summary{min-height:40px;padding:10px 12px;cursor:pointer;font-size:13px!important;font-weight:800}.document-metadata-editor>p{margin:0;padding:0 12px 10px;font-size:.8125rem;line-height:1.5;color:var(--muted)}.document-metadata-editor>section,.document-metadata-editor>div{border-top:1px solid var(--line)}
