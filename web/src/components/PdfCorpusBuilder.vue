...
const llmActionConcurrentLoad=computed(()=>llmActionProviderId.value?activeBuilds.value.filter(build=>String((build.request as any)?.provider_profile_id||"")===llmActionProviderId.value).reduce((sum,build)=>sum+Math.max(1,Number((build.request as any)?.max_concurrent_requests||1)),0):0);
