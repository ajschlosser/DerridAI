...
async function saveDocumentLayout(plan:any){if(!selectedAssetId.value)return;busy.value="document-layout";try{const asset=await pdfCorpusApi.updateDocumentLayout(selectedAssetId.value,plan);assets.value=assets.value.map(item=>item.asset_id===asset.asset_id?asset:item);setMessage(i18n.t("pdf_corpus.document_structure_saved","Document structure saved. Deterministic page, region, and thread metadata will be used by new builds."))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
  ...
  <section class="corpus-builder" :aria-labelledby="'pdf-corpus-builder-title'">
    <header class="builder-header">
      ...
      <div class="header-actions">
        ...
        <button type="button" class="btn" @click="useCurrentPdf" :disabled="busy!==''||buildRunning">{{i18n.t('pdf_corpus.use_current_pdf','Use current Explorer PDF')}}</button>
        <button type="button" class="btn primary" @click="uploadInput?.click()" :disabled="busy!==''||buildRunning">{{busy==='upload'?i18n.t('pdf_corpus.extracting','Extracting…'):i18n.t('pdf_corpus.choose_pdf','Choose source PDF')}}</button>
    ...
    <section v-if="showBuildConfiguration" class="builder-setup" :aria-labelledby="'pdf-corpus-config-title'">
      ...
      <section class="setup-card enrichment-strategy" aria-labelledby="pdf-corpus-enrichment-mode-title">
        <div class="setup-card-heading"><b id="pdf-corpus-enrichment-mode-title">{{i18n.t('pdf_corpus.enrichment_strategy','Enrichment strategy')}}</b><small>{{i18n.t('pdf_corpus.enrichment_strategy_help','Fast mode routes only genuinely useful metadata questions to the LLM. Deep mode runs broader attribution, quotation, and indexing analysis.')}}</small></div>
        <div class="mode-options" role="radiogroup" :aria-label="i18n.t('pdf_corpus.enrichment_strategy','Enrichment strategy')">
          <label><input v-model="enrichmentMode" type="radio" value="fast"><span><b>{{i18n.t('pdf_corpus.enrichment_fast','Fast corpus build')}}</b><small>{{i18n.t('pdf_corpus.enrichment_fast_help','Deterministic metadata first; quotation analysis only when signaled; semantic indexing off unless enabled below.')}}</small></span></label>
      ...
      <CorpusExecutionSettings class="execution-config" :generation="effectiveGeneration" :stage-limits="stageLimits" :stage-timeouts="stageTimeouts" :max-concurrent-requests="maxConcurrentRequests" :use-profile-defaults="useProfileDefaults" :disabled="busy!==''" @update:generation="generationOverrides=$event" @update:stage-limits="stageLimits=$event" @update:stage-timeouts="stageTimeouts=$event" @update:max-concurrent-requests="maxConcurrentRequests=$event" @update:use-profile-defaults="useProfileDefaults=$event" />

      <div class="build-launch-row">
        <div class="launch-copy"><b>{{i18n.t('pdf_corpus.ready_to_build','Ready to build')}}</b><span>{{selectedAsset?selectedAsset.filename:i18n.t('pdf_corpus.choose_source_prompt','Choose a source PDF to continue.')}}</span><span class="capacity-note">{{activeBuildCount?i18n.tf('pdf_corpus.active_build_capacity','{count} active build(s). Starting another build is independent and uses the selected provider profile capacity.',{count:activeBuildCount}):i18n.t('pdf_corpus.no_active_builds','No other corpus builds are active.')}}<template v-if="selectedProviderId&&selectedProfileActiveBuildCount"> · {{i18n.tf('pdf_corpus.profile_active_builds','{count} active on this profile',{count:selectedProfileActiveBuildCount})}}</template></span></div>
        ...
        <button type="button" class="btn primary build-button" @click="startBuild" :disabled="!canStartConcurrentBuild||busy!==''">{{busy==='build'?i18n.t('pdf_corpus.starting','Starting…'):i18n.t(activeBuildCount?'pdf_corpus.start_another_build':'pdf_corpus.build_records',activeBuildCount?'Start another build':'Build record set')}}</button>
      </div>

      <DocumentStructureConfigurator v-if="selectedAsset?.pages?.length" class="document-structure-config" :asset="selectedAsset" :pdf-url="sourcePdfUrl" :disabled="busy!==''||Boolean(buildRunning&&currentBuild?.asset_id===selectedAssetId)" :saving="busy==='document-layout'" @save="saveDocumentLayout" @save-page-labels="savePageLabels" />
    ...
    <div class="builder-workspace" :class="{'review-mode':showReviewWorkspace&&!finishPhase}">
      ...
      <main class="build-main">
        ...
        <template v-if="showReviewWorkspace">
          ...
          <section class="review-grid record-first-review" :class="{'queue-collapsed':reviewQueueCollapsed}" :aria-busy="recordsLoading">
            ...
            <aside ref="reviewInspectorEl" class="review-inspector" :aria-label="i18n.t('pdf_corpus.review_details','Review details')">
              <div class="review-inspector-tabs" role="tablist" :aria-label="i18n.t('pdf_corpus.review_detail_views','Review detail views')">
              ...
              <section v-else-if="selectedRecord" id="review-panel-source" class="review-inspector-panel source-review-panel" role="tabpanel" aria-labelledby="review-tab-source" tabindex="0">
