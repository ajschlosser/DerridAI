...
async function adjudicateBoundary(direction:"previous"|"next"){if(!currentBuild.value||!selectedRecord.value)return;const id=selectedRecord.value.record_id;const viewport=captureReviewViewport();busy.value="boundary";try{const result=await pdfCorpusApi.adjudicateBoundary(currentBuild.value.build_id,id,direction);await refreshBuild();await refreshRecords(false,id);await restoreReviewViewport(viewport,{record:true});const decision=String(result.decision?.decision||"uncertain");setMessage(i18n.tf("pdf_corpus.boundary_check_done","Boundary second-reader result: {decision}.",{decision:i18n.t(`pdf_corpus.boundary_llm.${decision}`,decision)}))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
          ...
          <section class="review-grid record-first-review" :aria-busy="recordsLoading">
            ...
            <aside ref="reviewInspectorEl" class="review-inspector" :aria-label="i18n.t('pdf_corpus.review_details','Review details')">
              ...
              <section v-else-if="selectedRecord" id="review-panel-source" class="review-inspector-panel source-review-panel" role="tabpanel" aria-labelledby="review-tab-source" tabindex="0">
                ...
                <CorpusBoundaryAdjudication :record="selectedRecord" :can-previous="canMergePrevious" :can-next="canMergeNext" :busy="busy!==''" @adjudicate="adjudicateBoundary" />
