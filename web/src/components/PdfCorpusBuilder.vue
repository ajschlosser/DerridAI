...
async function adjudicateBoundary(direction:"previous"|"next",profileId=llmActionProviderId.value||selectedProviderId.value){if(!currentBuild.value||!selectedRecord.value)return;const id=selectedRecord.value.record_id;const viewport=captureReviewViewport();busy.value="boundary";try{const actionPayload=directProfilePayload(profileId)||{provider_profile_id:profileId};const result=await pdfCorpusApi.adjudicateBoundary(currentBuild.value.build_id,id,direction,actionPayload);await refreshBuild();await refreshRecords(false,id);await restoreReviewViewport(viewport,{record:true});const decision=String(result.decision?.decision||"uncertain");setMessage(i18n.tf("pdf_corpus.boundary_check_done","Boundary second-reader result: {decision}.",{decision:i18n.t(`pdf_corpus.boundary_llm.${decision}`,decision)}))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
...
async function runMetadataEnrichment(payload:{providerProfileId:string;families:string[];scope:string}){if(!currentBuild.value)return;busy.value='metadata-enrichment';try{const actionPayload=directProfilePayload(payload.providerProfileId)||{provider_profile_id:payload.providerProfileId};currentBuild.value=await pdfCorpusApi.rerunMetadataEnrichment(currentBuild.value.build_id,{...actionPayload,families:payload.families,scope:payload.scope});metadataEnrichmentOpen.value=false;syncBuildInRail(currentBuild.value);registerBuildOperation(currentBuild.value);startPolling();setMessage(i18n.t('pdf_corpus.metadata_enrichment_started','Metadata enrichment pass started. Accepted records will reopen only when new or disputed metadata needs review.'))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),'error')}finally{busy.value=''}}
...
function openLlmTouchup(text:string){textDraft.value=text;editingText.value=true;llmTouchupResult.value={source_text:"",proposed_text:"",changes:[],warnings:[],model:""};llmActionProviderId.value=llmActionProviderId.value||selectedProviderId.value||providerProfiles.value[0]?.id||"";llmTouchupOpen.value=true}
async function runLlmTouchup(instructions="",profileId=llmActionProviderId.value){
  if(!currentBuild.value||!selectedRecord.value)return;
  busy.value="text-touchup";llmTouchupOpen.value=true;
  try{const actionPayload=directProfilePayload(profileId)||{provider_profile_id:profileId};const result=await pdfCorpusApi.touchupText(currentBuild.value.build_id,selectedRecord.value.record_id,{...actionPayload,instructions,text:textDraft.value||selectedRecord.value.text});llmTouchupResult.value={source_text:result.source_text,proposed_text:result.proposed_text,changes:result.changes||[],warnings:result.warnings||[],model:result.model||""}}
  catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}
}
...
async function rerunMetadata(){if(!currentBuild.value||!selectedRecord.value)return;const viewport=captureReviewViewport();busy.value="record";try{const profileId=llmActionProviderId.value||selectedProviderId.value;const payload={...(directProfilePayload(profileId)||{provider_profile_id:profileId})} as Record<string,unknown>;if(metadataRerunFamily.value!=="all")payload.families=[metadataRerunFamily.value];const row=await pdfCorpusApi.rerunMetadata(currentBuild.value.build_id,selectedRecord.value.record_id,payload);selectedRecord.value=row;metadataDraft.value=JSON.stringify(recordMetadata(row),null,2);await refreshBuild();await refreshRecords(false,row.record_id);await restoreReviewViewport(viewport);reviewInspectorTab.value="metadata";setMessage(metadataRerunFamily.value==="all"?i18n.t("pdf_corpus.metadata_rerun","Interpretive metadata rerun against the current reviewed record text."):i18n.tf("pdf_corpus.metadata_family_rerun","{family} metadata rerun against the current reviewed record text.",{family:i18n.t(`pdf_corpus.metadata_family.${metadataRerunFamily.value}`,metadataRerunFamily.value)}))}catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
...
<template>
  <section class="corpus-builder" :aria-labelledby="'pdf-corpus-builder-title'">
          ...
          <section class="review-grid record-first-review" :aria-busy="recordsLoading">
            ...
            <aside ref="reviewInspectorEl" class="review-inspector" :aria-label="i18n.t('pdf_corpus.review_details','Review details')">
              ...
              <section v-if="selectedRecord&&reviewInspectorTab==='metadata'" id="review-panel-metadata" class="review-inspector-panel" role="tabpanel" aria-labelledby="review-tab-metadata" tabindex="0">
                <CorpusMetadataResolutionPanel :record="selectedRecord" :region-types="regionTypes" :discourse-roles="discourseRoles" :busy="busy!==''||reviewLocked" :saving-field="metadataSavingField" :saved-field="metadataSavedField" :confidence-calibration="currentBuild?.llm_confidence_calibration||{}" @resolve="resolveMetadataField" @no-value="resolveMetadataNoValue" @resolve-many="resolveMetadataSuggestions" @source="showMetadataSource" @dirty="handleMetadataDirty" />
              ...
              <section v-else-if="selectedRecord" id="review-panel-source" class="review-inspector-panel source-review-panel" role="tabpanel" aria-labelledby="review-tab-source" tabindex="0">
                ...
                <CorpusBoundaryAdjudication :record="selectedRecord" :can-previous="canMergePrevious" :can-next="canMergeNext" :busy="busy!==''" :profiles="providerProfiles" :provider-profile-id="llmActionProviderId||selectedProviderId" @update:provider-profile-id="value=>llmActionProviderId=value" @adjudicate="adjudicateBoundary" />
    ...
    <CorpusLlmTextTouchupDialog :open="llmTouchupOpen" :source-text="textDraft||selectedRecord?.text||''" :proposed-text="llmTouchupResult.proposed_text" :changes="llmTouchupResult.changes" :warnings="llmTouchupResult.warnings" :model="llmTouchupResult.model" :busy="busy==='text-touchup'" :profiles="providerProfiles" :provider-profile-id="llmActionProviderId" :record-id="selectedRecord?.record_id" @update:provider-profile-id="value=>llmActionProviderId=value" @close="llmTouchupOpen=false" @run="runLlmTouchup" @apply="applyLlmTouchup" />
    <MetadataEnrichmentDialog :open="metadataEnrichmentOpen" :profiles="providerProfiles" :provider-profile-id="llmActionProviderId" :busy="busy==='metadata-enrichment'" :record-count="currentBuild?.record_count||0" :accepted-count="currentBuild?.accepted_count||0" @update:provider-profile-id="value=>llmActionProviderId=value" @close="metadataEnrichmentOpen=false" @run="runMetadataEnrichment" />
    <Teleport to="body"><CorpusRecordFocusReview v-if="focusView&&selectedRecord" :record="selectedRecord" :source-blocks="visibleBlocks" :busy="busy!==''||reviewLocked" :can-merge-previous="canMergePrevious" :can-merge-next="canMergeNext" :can-accept="!selectedMetadataBlocked&&!Boolean(selectedRecord.source_quality_issues?.length)" :region-types="regionTypes" :discourse-roles="discourseRoles" :recurring-lines="recurringCleanupLines" :document-terms="cleanupDocumentTerms" :confidence-calibration="currentBuild?.llm_confidence_calibration||{}" :source-pdf-url="selectedAsset?`${pdfCorpusApi.assetContentUrl(selectedAsset.asset_id)}#page=${selectedPdfPage||1}`:''" :source-pdf-page="selectedPdfPage" :can-history-back="focusHistoryIndex>0" :can-history-forward="focusHistoryIndex>=0&&focusHistoryIndex<focusHistory.length-1" :can-previous-record="selectedRecordIndex>0||recordOffset>0" :can-next-record="selectedRecordIndex>=0&&(selectedRecordIndex<records.length-1||recordOffset+pageSize<recordTotal)" :provider-profiles="providerProfiles" :llm-provider-profile-id="llmActionProviderId||selectedProviderId" @close="focusView=false" @history-back="focusHistoryMove(-1)" @history-forward="focusHistoryMove(1)" @previous-record="focusQueueMove(-1)" @next-record="focusQueueMove(1)" @save-text="saveTextFromFocus" @resolve-metadata="resolveMetadataField" @confirm-no-metadata-value="resolveMetadataNoValue" @metadata-dirty="handleMetadataDirty" @preview-jsonl="openJsonlPreview" @llm-touchup="openLlmTouchup" @accept="acceptFromFocus" @reject="setDisposition('rejected')" @skip="skipRecord" @undo="undoReview" @redo="redoReview" @slice="sliceRecord" @merge="merge" @update-llm-provider-profile="value=>llmActionProviderId=value" @adjudicate-boundary="adjudicateBoundary" /></Teleport>
  </section>
</template>
