...
const canResume=computed(()=>Boolean(currentBuild.value?.resumable && ["failed","interrupted","cancelled","blocked"].includes(String(currentBuild.value.status))));
const canRetryMetadata=computed(()=>Boolean(currentBuild.value && !buildRunning.value && !currentBuild.value.segmentation_blocked && Number(currentBuild.value.record_count||0)>0 && Number(currentBuild.value.metadata_completed||0)<Number(currentBuild.value.record_count||0)));
const awaitingManifestReview=computed(()=>currentBuild.value?.status==="awaiting_manifest_review");
const hasRecordTopology=computed(()=>Boolean(currentBuild.value && !currentBuild.value.segmentation_blocked && !awaitingManifestReview.value && Number(currentBuild.value.record_count||0)>0));
    ...
    <div class="builder-workspace">
      ...
      <main class="build-main">
        <section v-if="currentBuild" class="build-summary" :aria-labelledby="'pdf-corpus-current-build'">
          ...
          <CorpusBuildProgress :status="currentBuild.status" :stage="currentBuild.stage" :progress="currentBuild.progress||0" :record-count="currentBuild.record_count||0" :review-count="currentBuild.needs_review_count||0" :accepted-count="currentBuild.accepted_count||0" :error="currentBuild.error" :warnings="currentBuild.warnings||[]" :validation="currentBuild.validation||null" :llm-metrics="currentBuild.llm_metrics||null" :unresolved-count="currentBuild.segmentation_unresolved_regions?.length||0" />
          ...
          <section v-if="currentBuild.segmentation_blocked" class="segmentation-blocked" role="alert" aria-labelledby="segmentation-blocked-title">
            <div><h3 id="segmentation-blocked-title">{{i18n.t('pdf_corpus.segmentation_blocked','Segmentation blocked')}}</h3><p>{{i18n.t('pdf_corpus.segmentation_blocked_help','DerridAI could not validate every required semantic transition, so it stopped before constructing records. No giant fallback record was fabricated. Adjust the provider/settings or retry the unresolved regions.')}}</p></div>
            <button type="button" class="btn primary" @click="resumeBuild" :disabled="busy!==''">{{i18n.t('pdf_corpus.retry_segmentation','Retry unresolved segmentation')}}</button>
            <details v-if="currentBuild.segmentation_unresolved_regions?.length"><summary>{{i18n.tf('pdf_corpus.unresolved_count','{count} unresolved region(s)',{count:currentBuild.segmentation_unresolved_regions.length})}}</summary><ul><li v-for="(region,index) in currentBuild.segmentation_unresolved_regions.slice(0,20)" :key="index"><code>{{region.after_block_id||region.left_block_id||region.start_block_id||'?'}}</code> → <code>{{region.next_block_id||region.right_block_id||region.end_block_id||'?'}}</code><span v-if="region.reason"> · {{region.reason}}</span></li></ul></details>
          </section>
