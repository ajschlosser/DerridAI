...
async function sliceRecord(direction:"previous"|"next",offset:number){if(!currentBuild.value||!selectedRecord.value)return;const id=selectedRecord.value.record_id;const viewport=captureReviewViewport();busy.value="record";try{const result=await pdfCorpusApi.sliceRecord(currentBuild.value.build_id,id,direction,offset,Number(selectedRecord.value.record_revision||1));boundarySliceOpen.value=false;await refreshBuild();await refreshRecords(false,result.record.record_id);await restoreReviewViewport(viewport,{record:true});setMessage(i18n.t("pdf_corpus.slice_done","Boundary adjusted. Both neighboring records were reopened for review and affected metadata was marked stale."))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
  ...
  <section class="corpus-builder" :aria-labelledby="'pdf-corpus-builder-title'">
    ...
    <section v-if="showBuildConfiguration" class="builder-setup" :aria-labelledby="'pdf-corpus-config-title'">
      ...
      <section class="setup-section setup-source-section" aria-labelledby="pdf-corpus-source-title">
        ...
        <div v-if="selectedAsset" class="source-facts"><span>{{selectedAsset.page_count}} {{i18n.t('pdf_corpus.pages','pages')}}</span><span>{{selectedAsset.block_count}} {{i18n.t('pdf_corpus.blocks','blocks')}}</span><span>{{selectedAsset.ocr_pages}} {{i18n.t('pdf_corpus.ocr_pages','OCR page(s)')}}</span><span>SHA-256 {{selectedAsset.sha256.slice(0,16)}}…</span><span>{{formatDate(selectedAsset.created_at)}}</span></div>
      </section>
          ...
          <section ref="reviewGridEl" class="review-grid record-first-review" :style="{'--rw-queue':`${queueSplitter.size.value}px`,'--rw-inspector':`${inspectorSplitter.size.value}px`}" :class="{'queue-collapsed':reviewQueueCollapsed,'detail-mode':reviewWorkspaceMode!=='record','metadata-workspace':reviewWorkspaceMode==='metadata','source-workspace':reviewWorkspaceMode==='source'}" :aria-busy="recordsLoading">
            ...
            <article v-show="reviewWorkspaceMode==='record'" ref="reviewPaneEl" class="record-review-pane" :aria-labelledby="selectedRecord?'review-record-title':undefined">
              <template v-if="selectedRecord">
                <header class="record-review-head"><div><button v-if="reviewQueueCollapsed" type="button" class="link-button queue-toggle" @click="reviewQueueCollapsed=false">{{i18n.t('pdf_corpus.show_queue','Show queue')}}</button><span class="eyebrow">{{i18n.t('pdf_corpus.proposed_record','Proposed record')}}</span><h3 id="review-record-title">{{selectedRecord.record_id}}</h3><p>{{i18n.t('pdf_corpus.pages','pp.')}} {{selectedRecord.page_start}}–{{selectedRecord.page_end}} · {{selectedRecord.text_length.toLocaleString()}} {{i18n.t('pdf_corpus.characters','chars')}}<template v-if="Number(selectedRecord.activity?.human_view_count||selectedRecord.human_view_count||0)+Number(selectedRecord.activity?.human_review_count||0)+Number(selectedRecord.activity?.llm_review_count||0)+Number(selectedRecord.activity?.enrichment_pass_count||0)>0"> · {{i18n.tf('pdf_corpus.record_activity_summary','Viewed {views} · human reviews {human} · LLM reviews {llm} · enrichment passes {passes}',{views:Number(selectedRecord.activity?.human_view_count||selectedRecord.human_view_count||0),human:Number(selectedRecord.activity?.human_review_count||0),llm:Number(selectedRecord.activity?.llm_review_count||0),passes:Number(selectedRecord.activity?.enrichment_pass_count||0)})}}</template></p></div><div class="record-head-actions"><div class="decision-history" role="group" :aria-label="i18n.t('pdf_corpus.record_history','Review history')"><button type="button" class="btn small" @click="undoReview" :disabled="busy!==''"><AppIcon name="history"/><span class="btn-text">{{i18n.t('pdf_corpus.undo','Undo')}}</span></button><button type="button" class="btn small" @click="redoReview" :disabled="busy!==''"><AppIcon name="history" class="flip-inline"/><span class="btn-text">{{i18n.t('pdf_corpus.redo','Redo')}}</span></button></div><button type="button" class="btn small" @click="openFocusView">{{i18n.t('pdf_corpus.focus_view','Focus view')}}</button></div></header>
