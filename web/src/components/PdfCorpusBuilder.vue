...
const selectedMetadataBlocked=computed(()=>Boolean((selectedRecord.value?.metadata_review_fields||[]).length));
...
async function bulkDisposition(disposition:"accepted"|"rejected"){
  ...
  try{
    ...
    setMessage(result.blocked_metadata?i18n.tf("pdf_corpus.bulk_done_metadata_blocked","Updated {count} record(s); {blocked} still need metadata review before acceptance.",{count:result.changed,blocked:result.blocked_metadata}):i18n.tf("pdf_corpus.bulk_done","Updated {count} record(s).",{count:result.changed}));
  }catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}
}
...
<template>
  <section class="corpus-builder" :aria-labelledby="'pdf-corpus-builder-title'">
      ...
      <main class="build-main">
        ...
        <template v-if="showReviewWorkspace">
          ...
          <section class="review-toolbar" :aria-label="i18n.t('pdf_corpus.review_controls','Record review controls')">
            ...
            <div class="review-bulk"><button type="button" class="btn small" @click="bulkDisposition('accepted')" :disabled="busy!==''||recordTotal===0||metadataIncompleteOnly">{{i18n.t('pdf_corpus.accept_all_queue','Accept queue')}}</button><button type="button" class="btn small" @click="bulkDisposition('rejected')" :disabled="busy!==''||recordTotal===0||metadataIncompleteOnly">{{i18n.t('pdf_corpus.reject_all_queue','Reject queue')}}</button><button type="button" class="btn small" @click="focusView=true" :disabled="!selectedRecord">{{i18n.t('pdf_corpus.focus_view','Focus view')}}</button></div>
          ...
          <section class="review-grid record-first-review" :aria-busy="recordsLoading">
            ...
            <article class="record-review-pane" :aria-labelledby="selectedRecord?'review-record-title':undefined">
              <template v-if="selectedRecord">
                ...
                <p v-if="selectedMetadataBlocked" id="record-metadata-blocker" class="metadata-accept-blocker" role="status">{{i18n.t('pdf_corpus.resolve_metadata_before_accept','Resolve the highlighted metadata fields below before accepting this record.')}}</p><div v-if="reviewQueue!=='metadata'" class="decision-bar" :aria-label="i18n.t('pdf_corpus.record_decision','Record decision')"><div class="structural-actions"><button type="button" class="btn small" @click="merge('previous')" :disabled="busy!==''||!canMergePrevious">{{i18n.t('pdf_corpus.merge_previous','Merge previous')}}</button><button type="button" class="btn small" @click="merge('next')" :disabled="busy!==''||!canMergeNext">{{i18n.t('pdf_corpus.merge_next','Merge next')}}</button><button type="button" class="btn small" @click="undoReview" :disabled="busy!==''">{{i18n.t('pdf_corpus.undo','Undo')}}</button></div><div class="decision-actions"><button type="button" class="btn small" @click="skipRecord" :disabled="busy!==''">{{i18n.t('pdf_corpus.skip','Skip')}}</button><button type="button" class="btn small danger" @click="rejectRecord" :disabled="busy!==''">{{i18n.t('pdf_corpus.reject','Reject')}}</button><button type="button" class="btn small primary" @click="toggleAccept" :disabled="busy!==''||(!selectedRecord.accepted&&selectedMetadataBlocked)" :aria-describedby="selectedMetadataBlocked?'record-metadata-blocker':undefined">{{selectedRecord.accepted?i18n.t('pdf_corpus.reopen','Reopen'):i18n.t('pdf_corpus.accept_next','Accept & next')}}</button></div></div>
                <CorpusMetadataResolutionPanel v-if="reviewQueue==='metadata'||Object.keys(selectedRecord.metadata_field_status||{}).length>0" :record="selectedRecord" :region-types="regionTypes" :discourse-roles="discourseRoles" :busy="busy!==''" @resolve="resolveMetadataField" @source="showMetadataSource" />
    ...
    <Teleport to="body"><CorpusRecordFocusReview v-if="focusView&&selectedRecord" :record="selectedRecord" :source-blocks="visibleBlocks" :busy="busy!==''" :can-merge-previous="canMergePrevious" :can-merge-next="canMergeNext" :can-accept="!selectedMetadataBlocked" @close="focusView=false" @accept="setDisposition('accepted')" @reject="setDisposition('rejected')" @skip="skipRecord" @undo="undoReview" @merge="merge" /></Teleport>
  </section>
</template>
