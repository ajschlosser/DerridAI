...
const showSegmentationBlocked=computed(()=>Boolean(currentBuild.value?.segmentation_blocked && !buildRunning.value));
const retryingSegmentation=computed(()=>Boolean(buildRunning.value && (currentBuild.value?.retrying_segmentation||currentBuild.value?.segmentation_blocked)));
...
async function startBuild(){if(!selectedAssetId.value)return;busy.value="build";setMessage("");try{const payload={asset_id:selectedAssetId.value,profile_id:"derrida-scholarly-v3",...providerPayload.value};const build=await pdfCorpusApi.createBuild(payload);selectedBuildId.value=build.build_id;currentBuild.value=build;registerBuildOperation(build);await refreshBuilds();startPolling();setMessage(i18n.t("pdf_corpus.build_started","Corpus build started. Progress and completed checkpoints are persisted server-side."))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
    ...
    <div class="builder-workspace">
      ...
      <main class="build-main">
        <section v-if="currentBuild" class="build-summary" :aria-labelledby="'pdf-corpus-current-build'">
          ...
          <section v-if="showSegmentationBlocked" class="segmentation-blocked" role="alert" aria-labelledby="segmentation-blocked-title">
            ...
            <button type="button" class="btn primary" @click="resumeBuild" :disabled="busy!==''||buildRunning">{{i18n.t('pdf_corpus.retry_segmentation','Retry unresolved segmentation')}}</button>
