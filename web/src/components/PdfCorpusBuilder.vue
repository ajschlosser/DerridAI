...
function mergeUnavailable(direction:"previous"|"next"):string|undefined{
  ...
  if(structuralReviewLocked.value)return i18n.t("pdf_corpus.reason.structure_locked","Merging and slicing wait until background enrichment stops.");
...
const providerPayload=computed<Record<string,unknown>>(()=>{
  ...
  if(selectedProviderId.value){
    ...
    // A published profile can fill in a missing key. The browser key still has to
    // travel with the request, or a profile published without one calls OpenAI unauthenticated.
    ...
    if(selectedReviewProviderId.value&&selectedReviewProviderId.value!==selectedProviderId.value){
      ...
      if(review){
        ...
        if(!serverProviderIds.value.has(selectedReviewProviderId.value)){
          const {provider_profile_id: _profileId,max_concurrent_requests:_max,base_url:_url,api_key:_key,...rest}=review;
          Object.assign(reviewConfig,rest);
        }
          ...
          <section ref="reviewGridEl" class="review-grid record-first-review" :style="{'--rw-queue':`${queueSplitter.size.value}px`,'--rw-inspector':`${inspectorSplitter.size.value}px`}" :class="{'queue-collapsed':reviewQueueCollapsed,'detail-mode':reviewWorkspaceMode!=='record','metadata-workspace':reviewWorkspaceMode==='metadata','source-workspace':reviewWorkspaceMode==='source'}" :aria-busy="recordsLoading">
            ...
            <aside ref="reviewInspectorEl" class="review-inspector" :aria-label="i18n.t('pdf_corpus.review_details','Review details')">
              ...
              <section v-if="selectedRecord&&(reviewWorkspaceMode==='metadata'||(reviewWorkspaceMode==='record'&&reviewInspectorTab==='metadata'))" id="review-panel-metadata" class="review-inspector-panel" role="tabpanel" aria-labelledby="review-tab-metadata" tabindex="0">
                ...
                <details class="record-data"><summary>{{i18n.t('pdf_corpus.advanced_metadata','Advanced metadata')}}</summary><p class="help">{{i18n.t('pdf_corpus.metadata_help','Typed controls above are preferred. This advanced editor is for supported interpretive metadata only; source-bound fields remain protected.')}}</p><LlmExecutionControl :model-value="llmActionProviderId||selectedProviderId" :model-override="llmActionModel" :profiles="providerProfiles" :disabled="busy!==''" :task="i18n.t('pdf_corpus.metadata_rerun_provider_help','Rerun metadata with the selected LLM profile. Existing human-confirmed values remain authoritative.')" @update:model-value="value=>llmActionProviderId=value" @update:model-override="value=>llmActionModel=value" /><label class="sr-only" for="pdf-corpus-metadata">{{i18n.t('pdf_corpus.interpretive_metadata','Interpretive metadata')}}</label><textarea id="pdf-corpus-metadata" v-model="metadataDraft" class="metadata-json" spellcheck="false" @input="metadataEditorDirty=true"></textarea><div class="data-actions"><button type="button" class="btn small" @click="saveMetadata" :disabled="busy!==''">{{i18n.t('pdf_corpus.save_metadata','Save metadata')}}</button><label class="rerun-family"><span>{{i18n.t('pdf_corpus.rerun_family','Rerun')}}</span><select v-model="metadataRerunFamily" class="control small"><option value="all">{{i18n.t('pdf_corpus.metadata_family.all','All metadata families')}}</option><option value="discourse">{{i18n.t('pdf_corpus.metadata_family.discourse','Discourse / attribution')}}</option><option value="quotation">{{i18n.t('pdf_corpus.metadata_family.quotation','Quotation relations')}}</option><option value="indexing">{{i18n.t('pdf_corpus.metadata_family.indexing','Semantic indexing')}}</option></select></label><button type="button" class="btn small" @click="rerunMetadata()" :disabled="busy!==''">{{i18n.t('pdf_corpus.rerun_metadata','Rerun metadata')}}</button><button type="button" class="btn small" @click="llmActionProviderId=selectedProviderId||providerProfiles[0]?.id||'';metadataEnrichmentOpen=true" :disabled="busy!==''">{{i18n.t('pdf_corpus.metadata_enrichment_again','Run metadata enrichment again')}}</button></div></details>
              </section>
