...
async function runMetadataEnrichment(payload:{providerProfileId:string;model:string;families:string[];scope:string;passes:number}){if(!currentBuild.value)return;busy.value='metadata-enrichment';try{const actionPayload=directProfilePayloadWithModel(payload.providerProfileId,payload.model)||{provider_profile_id:payload.providerProfileId,model:payload.model||undefined};currentBuild.value=await pdfCorpusApi.rerunMetadataEnrichment(currentBuild.value.build_id,{...actionPayload,families:payload.families,scope:payload.scope,passes:payload.passes});metadataEnrichmentOpen.value=false;syncBuildInRail(currentBuild.value);registerBuildOperation(currentBuild.value);startPolling();setMessage(i18n.t('pdf_corpus.metadata_enrichment_started','Metadata enrichment pass started. Accepted records will reopen only when new or disputed metadata needs review.'))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),'error')}finally{busy.value=''}}
...
<template>
  <section class="corpus-builder" :aria-labelledby="'pdf-corpus-builder-title'">
    ...
    <MetadataEnrichmentDialog :groups="currentBuild?.schema?.groups?.map(g=>({key:g.key,label:g.label}))" :open="metadataEnrichmentOpen" :profiles="providerProfiles" :provider-profile-id="llmActionProviderId" :model-override="llmActionModel" :busy="busy==='metadata-enrichment'" :record-count="currentBuild?.record_count||0" :accepted-count="currentBuild?.accepted_count||0" @update:provider-profile-id="value=>llmActionProviderId=value" @update:model-override="value=>llmActionModel=value" @close="metadataEnrichmentOpen=false" @run="runMetadataEnrichment" />
