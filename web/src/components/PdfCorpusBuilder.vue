      ...
      <main class="build-main">
        ...
        <template v-if="showReviewWorkspace">
          ...
          <section class="review-toolbar" :aria-label="i18n.t('pdf_corpus.review_controls','Record review controls')">
            ...
            <CorpusBulkMetadataEditor v-if="bulkMetadataOpen" :records="records" :region-types="regionTypes" :discourse-roles="discourseRoles" :selected-count="selectedReviewCount" :total-count="Number(currentBuild?.record_count||recordTotal)" :disabled="busy!==''||reviewLocked" @apply="applyBulkMetadata" @close="bulkMetadataOpen=false" />
          ...
          <section ref="reviewGridEl" class="review-grid record-first-review" :style="{'--rw-queue':`${queueSplitter.size.value}px`,'--rw-inspector':`${inspectorSplitter.size.value}px`}" :class="{'queue-collapsed':reviewQueueCollapsed,'detail-mode':reviewWorkspaceMode!=='record','metadata-workspace':reviewWorkspaceMode==='metadata','source-workspace':reviewWorkspaceMode==='source'}" :aria-busy="recordsLoading">
            ...
            <aside ref="reviewInspectorEl" class="review-inspector" :aria-label="i18n.t('pdf_corpus.review_details','Review details')">
              ...
              <section v-if="selectedRecord&&(reviewWorkspaceMode==='metadata'||(reviewWorkspaceMode==='record'&&reviewInspectorTab==='metadata'))" id="review-panel-metadata" class="review-inspector-panel" role="tabpanel" aria-labelledby="review-tab-metadata" tabindex="0">
                <CorpusMetadataResolutionPanel :record="selectedRecord" :region-types="regionTypes" :discourse-roles="discourseRoles" :busy="busy!==''||reviewLocked" :saving-field="metadataSavingField" :saved-field="metadataSavedField" :confidence-calibration="currentBuild?.llm_confidence_calibration||{}" :known-values="metadataKnownValues" @resolve="resolveMetadataField" @no-value="resolveMetadataNoValue" @resolve-many="resolveMetadataSuggestions" @source="showMetadataSource" @dirty="handleMetadataDirty" />
