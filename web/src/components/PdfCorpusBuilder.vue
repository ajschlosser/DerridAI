...
async function loadSchemaChoices(){try{schemaChoices.value=(await metadataSchemasApi.list()).items;if(!schemaChoices.value.some(item=>item.id===schemaId.value))schemaId.value="default"}catch{/* the built-in schema still works without the list */}}
...
const stageTimeouts=ref<Record<string,number>>({manifest:300,segmentation:300,reconciliation:240,discourse:240,quotation:240,indexing:180});
...
const metadataFamilyOptions=computed(()=>currentBuild.value?.schema?.groups?.map(group=>({key:group.key,label:group.label}))||[
  {key:"discourse",label:i18n.t("pdf_corpus.metadata_family.discourse","Discourse / attribution")},
  {key:"quotation",label:i18n.t("pdf_corpus.metadata_family.quotation","Quotation relations")},
  {key:"indexing",label:i18n.t("pdf_corpus.metadata_family.indexing","Semantic indexing")},
]);
...
function restoreBuilderDraft(){
  try{
    ...
    if(draft.stageTimeouts&&typeof draft.stageTimeouts==="object")stageTimeouts.value={...stageTimeouts.value,...draft.stageTimeouts};
...
function recordStateIcon(record:CorpusRecord){const state=recordState(record);return state==="accepted"?"check":state==="rejected"?"close":(state==="metadata"||state==="topology"||state==="source")?"warning":state==="preparing"?"refresh":""}
// The status already says the primary state; list only the other issues, so a row never says "Metadata" twice.
function extraIssueKinds(record:CorpusRecord){const state=recordState(record);return recordIssueKinds(record).filter((kind:string)=>kind!==state)}
...
watch(()=>[showReviewWorkspace.value,reviewHydrated.value,currentBuild.value?.build_id] as const,async([show,hydrated,id])=>{
...
const queueSplitter=useSplitter({key:"derridai.review.queueWidth",min:224,max:420,initial:256,edge:"start",container:()=>reviewGridEl.value});
const inspectorSplitter=useSplitter({key:"derridai.review.inspectorWidth",min:320,max:640,initial:368,edge:"end",container:()=>reviewGridEl.value});
...
function mergeUnavailable(direction:"previous"|"next"):string|undefined{
  if(busy.value!=="")return i18n.t("pdf_corpus.reason.busy","Wait for the current action to finish.");
  if(structuralReviewLocked.value)return i18n.t("pdf_corpus.reason.structure_locked","Merging and slicing wait until record topology is ready.");
  if(direction==="previous"&&!canMergePrevious.value)return i18n.t("pdf_corpus.reason.no_previous","There is no previous record to combine with.");
  if(direction==="next"&&!canMergeNext.value)return i18n.t("pdf_corpus.reason.no_next","There is no next record to combine with.");
  return undefined;
}
function sliceUnavailable():string|undefined{
  if(busy.value!=="")return i18n.t("pdf_corpus.reason.busy","Wait for the current action to finish.");
  if(editingText.value)return i18n.t("pdf_corpus.reason.finish_text_edit","Finish or cancel the text edit first.");
  if(metadataEditorDirty.value)return i18n.t("pdf_corpus.reason.finish_metadata_edit","Save or discard the metadata changes first.");
  if(!canMergePrevious.value&&!canMergeNext.value)return i18n.t("pdf_corpus.reason.no_neighbour","Slicing needs a neighbouring record to move text into.");
  return undefined;
}
const recordActionItems=computed<CorpusActionMenuItem[]>(()=>[
  {id:"previous",label:i18n.t("pdf_corpus.combine_previous","Combine with previous record"),reason:mergeUnavailable("previous")},
  {id:"next",label:i18n.t("pdf_corpus.combine_next","Combine with next record"),reason:mergeUnavailable("next")},
  {id:"slice",label:i18n.t("pdf_corpus.slice_record","Slice record"),reason:sliceUnavailable()},
  {id:"preview",label:i18n.t("pdf_corpus.preview_jsonl","Preview JSONL")},
]);
function runRecordAction(id:string){if(id==="previous")merge("previous");else if(id==="next")merge("next");else if(id==="slice")boundarySliceOpen.value=true;else if(id==="preview")openJsonlPreview()}
...
const selectedRecordActivitySummary=computed(()=>{
  ...
  return views+human+llm+passes>0?` · ${i18n.tf("pdf_corpus.record_activity_summary","Viewed {views} · human reviews {human} · LLM reviews {llm} · enrichment passes {passes}",{views,human,llm,passes})}`:"";
});
...
const selectedProviderLabel=computed(()=>selectedProfile.value?.name||selectedProviderId.value||(manualProvider.value==="openai"?"OpenAI-compatible":"Ollama"));
const selectedStructureSummary=computed(()=>{
  const plan=selectedAsset.value?.document_layout;
  if(!plan)return i18n.t("pdf_corpus.readiness.structure_unset","Automatic defaults; review recommended");
  const parts:string[]=[];
  parts.push(plan.page_layout==="two_up"?i18n.t("pdf_corpus.two_up_layout","2 printed pages per PDF page"):i18n.t("pdf_corpus.single_page_layout","1 printed page per PDF page"));
  if(plan.main_text_pdf_start){
    const printed=plan.main_text_printed_start?i18n.tf("pdf_corpus.readiness.printed_anchor"," → printed p. {page}",{page:plan.main_text_printed_start}):"";
    parts.push(i18n.tf("pdf_corpus.readiness.main_start","Main text PDF {page}{printed}",{page:plan.main_text_pdf_start,printed}));
  }else parts.push(i18n.t("pdf_corpus.readiness.main_start_unset","Main-text start not set"));
  if(plan.bibliography_pdf_start)parts.push(i18n.tf("pdf_corpus.readiness.bibliography_start","Bibliography PDF {page}",{page:plan.bibliography_pdf_start}));
  return parts.join(" · ");
});
const setupWarnings=computed(()=>{
  const warnings:string[]=[];
  if(selectedAsset.value?.pages?.length&&!selectedAsset.value.document_layout?.main_text_pdf_start)warnings.push(i18n.t("pdf_corpus.readiness.review_structure","Review document structure before building so printed-page and region defaults are explicit."));
  if(selectedProviderId.value&&selectedProfileActiveBuildCount.value)warnings.push(i18n.tf("pdf_corpus.profile_active_builds","{count} active on this profile",{count:selectedProfileActiveBuildCount.value}));
  return warnings;
});
...
function directProfilePayloadWithModel(profileId:string,modelOverride=""): Record<string,unknown>|null{
...
const providerPayload=computed<Record<string,unknown>>(()=>{
  ...
  const payload:Record<string,unknown>={provider:manualProvider.value,use_profile_defaults:false,max_concurrent_requests:maxConcurrentRequests.value,stage_limits:{...stageLimits.value},stage_timeouts:{...stageTimeouts.value},record_sizing:{...recordSizing.value},enrichment_mode:enrichmentMode.value,semantic_indexing:semanticIndexing.value,auto_clean_text:autoCleanText.value,llm_touchup_during_enrichment:llmTouchupDuringEnrichment.value,text_cleanup_rules:["page_numbers","repeated_short_lines","line_hyphenation","paragraph_lines","empty_lines","ocr_artifacts","whitespace"]};
...
type ReviewViewport={windowY:number;queueTop:number;recordTop:number;inspectorTop:number};
function captureReviewViewport():ReviewViewport{return {windowY:window.scrollY,queueTop:recordListEl.value?.scrollTop||0,recordTop:reviewPaneEl.value?.scrollTop||0,inspectorTop:reviewInspectorEl.value?.scrollTop||0}}
async function restoreReviewViewport(snapshot:ReviewViewport,{record=false,inspector=false}:{record?:boolean;inspector?:boolean}={}){await nextTick();window.scrollTo({top:snapshot.windowY,left:window.scrollX,behavior:"auto"});if(recordListEl.value)recordListEl.value.scrollTop=snapshot.queueTop;if(reviewPaneEl.value&&!record)reviewPaneEl.value.scrollTop=snapshot.recordTop;if(reviewInspectorEl.value&&!inspector)reviewInspectorEl.value.scrollTop=snapshot.inspectorTop}
function focusFirstMetadataBlocker(){reviewInspectorTab.value="metadata";void nextTick(()=>{const root=reviewInspectorEl.value;const field=root?.querySelector<HTMLElement>('[data-unresolved-field="true"]');const control=field?.querySelector<HTMLElement>('input:not([disabled]),select:not([disabled]),button:not([disabled]),textarea:not([disabled])');if(field&&root){const top=Math.max(0,field.offsetTop-root.offsetTop-56);root.scrollTo({top,behavior:"smooth"})}control?.focus({preventScroll:true})})}
function setReviewWorkspaceMode(mode:"record"|"metadata"|"source"){reviewWorkspaceMode.value=mode;if(mode!=="record")reviewInspectorTab.value=mode;void nextTick(()=>{if(mode==="record")reviewPaneEl.value?.focus?.({preventScroll:true});else reviewInspectorEl.value?.focus?.({preventScroll:true})})}
function reviewInspectorKeydown(event:KeyboardEvent){const tabs=["metadata","evidence","source"] as const;const current=tabs.indexOf(reviewInspectorTab.value);let next=current;if(event.key==="ArrowRight")next=(current+1)%tabs.length;else if(event.key==="ArrowLeft")next=(current-1+tabs.length)%tabs.length;else if(event.key==="Home")next=0;else if(event.key==="End")next=tabs.length-1;else return;event.preventDefault();reviewInspectorTab.value=tabs[next];void nextTick(()=>reviewInspectorEl.value?.querySelector<HTMLElement>(`[data-review-tab="${tabs[next]}"]`)?.focus({preventScroll:true}))}
function recordMetadata(record:CorpusRecord){
  ...
  const editableFields=[
    ...
    "language","region_type","region_author","primary_text","speaker","position_holder","target","discourse_role","proposition_status","semantic_function","stance","claim_scope","is_direct_quote","quoted_speaker","quoted_author","quoted_work","quoted_position_holder","quoted_addressee","quoted_referent","quotation_chain","topics","concepts","persons","works_referenced","needs_review","review_reason"
  ];
...
async function switchBuildProvider(profileId:string,modelOverride=""){
  ...
  try{
    ...
    setMessage(i18n.tf('pdf_corpus.profile_model_switched','New metadata tasks will use {profile} / {model}. In-flight requests continue unchanged.',{profile:profile?.name||profileId,model:modelOverride||profile?.model||'—'}));
  }catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),'error')}finally{busy.value=""}
}
...
async function refreshRecords(reset=false, preferredId=""){
  ...
  try{
    ...
    let result:{items:CorpusRecord[];total:number;offset:number;limit:number;queue_counts?:CorpusBuild["review_queue_counts"]}={items:[],total:0,offset:recordOffset.value,limit:pageSize};
    ...
    for(let attempt=0;attempt<5;attempt++){
      result=await pdfCorpusApi.records(selectedBuildId.value,recordOffset.value,pageSize,reviewQueue.value,recordQuery.value);
      if(result.total>0||expected===0||reviewQueue.value!=="all"||Boolean(recordQuery.value))break;
      await new Promise(resolve=>window.setTimeout(resolve,120*(attempt+1)));
      if(requestId!==recordRequestSerial)return;
    }
    if(requestId!==recordRequestSerial)return;
    if(result.queue_counts&&currentBuild.value)currentBuild.value={...currentBuild.value,review_queue_counts:result.queue_counts};
...
async function refreshAll(){
  ...
  if(["all","ready","issues","metadata","topology","source","accepted","rejected"].includes(requestedQueue))reviewQueue.value=requestedQueue;
  await refreshBuild();
...
function selectRecord(record:CorpusRecord){
  ...
  resolveSourceOnTextSave.value=Boolean(record.source_quality_issues?.length);const fallback=JSON.stringify(recordMetadata(record),null,2);try{metadataDraft.value=localStorage.getItem(metadataDraftKey(selectedBuildId.value,record.record_id))||fallback}catch{metadataDraft.value=fallback}void refreshBlocks();if(selectedBuildId.value&&!sameRecord)void pdfCorpusApi.markViewed(selectedBuildId.value,record.record_id).then(result=>{if(selectedRecord.value?.record_id===record.record_id)selectedRecord.value.activity=result.activity}).catch(()=>undefined);void restoreReviewViewport(viewport,{record:!sameRecord})
}
function beginTextEdit(useTouchupProposal=false){if(!selectedRecord.value)return;editingText.value=true;if(useTouchupProposal&&selectedRecord.value.text_touchup_proposal?.proposed_text)textDraft.value=selectedRecord.value.text_touchup_proposal.proposed_text;else if(!textDraft.value)textDraft.value=String(selectedRecord.value.text||"")}
// eslint-disable-next-line no-empty -- SA-12: legacy best-effort fallback; audit user-visible failure handling separately.
...
const allVisibleSelected=computed(()=>records.value.length>0&&records.value.every(record=>selectedReviewIds.value.has(record.record_id)));
...
async function reviewMetadataRecord(recordId:string){
  ...
  await nextTick();await refreshRecords(true,recordId);
...
async function openPdfExplorer(){await router.replace({query:{...route.query,mode:"explorer"}})}
...
async function setDisposition(disposition:"pending"|"accepted"|"rejected"){
  ...
  try{
    if(disposition==="pending"){
      ...
      const result=await pdfCorpusApi.reviewDecision(currentBuild.value.build_id,id,disposition,"",Number(selectedRecord.value.record_revision||1),reviewQueue.value);
      currentBuild.value=result.build;syncBuildInRail(result.build);
      if(result.blocked){
        selectedRecord.value=result.record;
        const idx=records.value.findIndex(row=>row.record_id===id);if(idx>=0)records.value.splice(idx,1,result.record);
        if(result.blocker==="source_problem"){
          reviewInspectorTab.value="source";reviewQueue.value="source";
          setMessage(i18n.t("pdf_corpus.accept_blocked_source","This record has a source-extraction problem. Inspect the source, repair/rebuild, or reject the proposal."),"error");
        }else{
        ...
        if(reviewQueue.value==="all"||reviewQueue.value===disposition){
          ...
          records.value.splice(idx,1);recordTotal.value=Math.max(0,recordTotal.value-1);
        }
        if(result.next_record){
          const existing=records.value.find(row=>row.record_id===result.next_record?.record_id);
          if(existing){selectRecord(existing);await nextTick();if(reviewPaneEl.value)reviewPaneEl.value.scrollTop=0}
          else if(reviewQueue.value!=="all"){
            reviewQueue.value="all";await nextTick();await refreshRecords(true,result.next_record.record_id);
...
async function toggleAccept(){await attemptAccept()}
async function acceptFromFocus(){if(selectedMetadataBlocked.value){focusView.value=false;await nextTick()}await attemptAccept()}
...
async function acceptCleanRecords(){
  ...
  const viewport=captureReviewViewport();
  if(!window.confirm(i18n.tf("pdf_corpus.accept_clean_confirm","Accept {count} clean record(s)? Records with metadata, topology, or source problems will remain for review.",{count:clean})))return;
  busy.value="bulk";
  try{
    const result=await pdfCorpusApi.bulkDisposition(currentBuild.value.build_id,"accepted","ready","");
    await refreshBuild();reviewQueue.value=issueCount.value>0?"issues":"all";await nextTick();await refreshRecords(true);
    await restoreReviewViewport(viewport,{record:true,inspector:true});
    bulkActionFeedback.value=result.changed>0?i18n.tf("pdf_corpus.accept_clean_done","Accepted {count} clean record(s). Only exceptions remain.",{count:result.changed}):i18n.t("pdf_corpus.accept_clean_none_changed","No records changed. The remaining records need review or were already dispositioned.");
    setMessage(bulkActionFeedback.value);
  }catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}
}
async function bulkDisposition(disposition:"accepted"|"rejected"){
  if(!currentBuild.value)return;const recordIds=Array.from(selectedReviewIds.value);const count=recordIds.length;if(!count){setMessage(i18n.t("pdf_corpus.select_records_first","Select one or more records first."));return}const viewport=captureReviewViewport();const verb=disposition==="accepted"?i18n.t("pdf_corpus.accept_selected","Accept selected"):i18n.t("pdf_corpus.reject_selected","Reject selected");
  ...
  try{
    const result=await pdfCorpusApi.bulkDisposition(currentBuild.value.build_id,disposition,reviewQueue.value,recordQuery.value,"",recordIds);
    selectedReviewIds.value=new Set();
    await refreshBuild();
    if(disposition==="accepted"&&Number(result.blocked_metadata||0)>0){
      reviewQueue.value="metadata";recordQuery.value="";await nextTick();await refreshRecords(true,result.blocked_record_ids?.[0]||"");reviewInspectorTab.value="metadata";
    }else{await refreshRecords(true)}
    await restoreReviewViewport(viewport,{record:true,inspector:true});
    bulkActionFeedback.value=result.blocked_metadata?i18n.tf("pdf_corpus.bulk_done_metadata_blocked","Accepted {count} eligible record(s); {blocked} record(s) still require metadata decisions and are now shown in the metadata queue.",{count:result.changed,blocked:result.blocked_metadata}):i18n.tf("pdf_corpus.bulk_done","Updated {count} record(s).",{count:result.changed});
    setMessage(bulkActionFeedback.value);
  }catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}
}
...
async function saveReviewedText(resolveIssues=resolveSourceOnTextSave.value){
  ...
  try{
    ...
    setMessage(resolveIssues?i18n.t("pdf_corpus.text_saved_resolved","Reviewed text saved and the record-level source issue was marked resolved. Rerun metadata if the correction affects interpretation."):i18n.t("pdf_corpus.text_saved","Reviewed text saved. The immutable extracted text remains available in Source."));
  }catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}
}

async function markTextReviewed(){if(!selectedRecord.value)return;textDraft.value=String(selectedRecord.value.text||"");await saveReviewedText(false)}
...
async function toggleEvidenceBlock(blockId:string){
  ...
  try{
    ...
    await refreshBuild();await refreshRecords(false,row.record_id);await restoreReviewViewport(viewport);
    setMessage(i18n.t("pdf_corpus.evidence_saved","Evidence binding saved. The record remains open for review."));
  }catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}
}
async function merge(direction:"previous"|"next"){if(!currentBuild.value||!selectedRecord.value)return;const id=selectedRecord.value.record_id;const viewport=captureReviewViewport();busy.value="record";try{const row=await pdfCorpusApi.merge(currentBuild.value.build_id,id,direction,Number(selectedRecord.value.record_revision||1));await refreshBuild();await refreshRecords(false,row.record_id);await restoreReviewViewport(viewport,{record:true});setMessage(i18n.tf("pdf_corpus.merged","Merged with {direction} record; the merged boundary now requires review.",{direction:i18n.t(`pdf_corpus.${direction}`,direction)}))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
async function split(afterBlockId:string){if(!currentBuild.value||!selectedRecord.value)return;const id=selectedRecord.value.record_id;const viewport=captureReviewViewport();busy.value="record";try{const result=await pdfCorpusApi.split(currentBuild.value.build_id,id,afterBlockId,Number(selectedRecord.value.record_revision||1));await refreshBuild();await refreshRecords(false,result.records[0]?.record_id||id);await restoreReviewViewport(viewport,{record:true});setMessage(i18n.t("pdf_corpus.split_done","Record split at the selected semantic source boundary. Both records require review."))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
async function sliceRecord(direction:"previous"|"next"|"keep",offset:number,keepEnd?:number){if(!currentBuild.value||!selectedRecord.value)return;const id=selectedRecord.value.record_id;const viewport=captureReviewViewport();busy.value="record";try{const result=await pdfCorpusApi.sliceRecord(currentBuild.value.build_id,id,direction,offset,Number(selectedRecord.value.record_revision||1),keepEnd);boundarySliceOpen.value=false;await refreshBuild();await refreshRecords(false,result.record.record_id);await restoreReviewViewport(viewport,{record:true});setMessage(i18n.t("pdf_corpus.slice_done","Boundary adjusted. Both neighboring records were reopened for review and affected metadata was marked stale."))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
...
async function resolveMetadataField(field:string,value:unknown){
  ...
  try{
    const result=await pdfCorpusApi.metadataDecision(currentBuild.value.build_id,recordId,field,value,Number(selectedRecord.value.record_revision||1));
    selectedRecord.value=result.record;currentBuild.value=result.build;syncBuildInRail(result.build);
    ...
    if(remaining.length===0){
      ...
      setMessage(i18n.t("pdf_corpus.metadata_resolved_ready","Metadata confirmed. Review the record, then choose Accept & next or Reject & next."));
      await nextTick();
      acceptButtonEl.value?.focus({preventScroll:true});
    }else{
      setMessage(i18n.tf("pdf_corpus.metadata_field_confirmed","Confirmed {field}. {count} metadata decision(s) remain for this record.",{field:i18n.t(`record.${field}`,field.replace(/_/g,' ')),count:remaining.length}));
    }
    await restoreReviewViewport(viewport,{inspector:true});
  }catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value="";metadataSavingField.value=""}
}
...
async function runMetadataEnrichment(payload:{providerProfileId:string;model:string;families:string[];scope:string;passes:number;recordIds:string[]}){if(!currentBuild.value)return;busy.value='metadata-enrichment';try{const actionPayload=directProfilePayloadWithModel(payload.providerProfileId,payload.model)||{provider_profile_id:payload.providerProfileId,model:payload.model||undefined};currentBuild.value=await pdfCorpusApi.rerunMetadataEnrichment(currentBuild.value.build_id,{...actionPayload,families:payload.families,scope:payload.scope,passes:payload.passes,record_ids:payload.recordIds});metadataEnrichmentOpen.value=false;syncBuildInRail(currentBuild.value);registerBuildOperation(currentBuild.value);startPolling();setMessage(i18n.t('pdf_corpus.metadata_enrichment_started','Metadata enrichment pass started. Accepted records will reopen only when new or disputed metadata needs review.'))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),'error')}finally{busy.value=''}}
...
function openLlmTouchup(text:string){textDraft.value=text;editingText.value=true;llmTouchupError.value="";llmTouchupResult.value={source_text:"",proposed_text:"",changes:[],warnings:[],model:""};llmActionProviderId.value=llmActionProviderId.value||selectedProviderId.value||providerProfiles.value[0]?.id||"";llmActionModel.value=String(providerProfiles.value.find(p=>p.id===llmActionProviderId.value)?.model||"");llmTouchupOpen.value=true}
async function runLlmTouchup(instructions="",profileId=llmActionProviderId.value,model=llmActionModel.value){
  if(!currentBuild.value||!selectedRecord.value)return;
  busy.value="text-touchup";llmTouchupError.value="";llmTouchupOpen.value=true;
  try{const actionPayload=directProfilePayloadWithModel(profileId,model)||{provider_profile_id:profileId,model:model||undefined};const result=await pdfCorpusApi.touchupText(currentBuild.value.build_id,selectedRecord.value.record_id,{...actionPayload,instructions,text:textDraft.value||selectedRecord.value.text});llmTouchupResult.value={source_text:result.source_text,proposed_text:result.proposed_text,changes:result.changes||[],warnings:result.warnings||[],model:result.model||""}}
  catch(exc){llmTouchupError.value=exc instanceof Error?exc.message:String(exc);setMessage(llmTouchupError.value,"error")}finally{busy.value=""}
}
...
async function rerunMetadata(){if(!currentBuild.value||!selectedRecord.value)return;const viewport=captureReviewViewport();busy.value="record";try{const profileId=llmActionProviderId.value||selectedProviderId.value;const payload={...(directProfilePayloadWithModel(profileId,llmActionModel.value)||{provider_profile_id:profileId,model:llmActionModel.value||undefined})} as Record<string,unknown>;if(metadataRerunFamily.value!=="all")payload.families=[metadataRerunFamily.value];const row=await pdfCorpusApi.rerunMetadata(currentBuild.value.build_id,selectedRecord.value.record_id,payload);selectedRecord.value=row;metadataDraft.value=JSON.stringify(recordMetadata(row),null,2);await refreshBuild();await refreshRecords(false,row.record_id);await restoreReviewViewport(viewport);reviewInspectorTab.value="metadata";setMessage(metadataRerunFamily.value==="all"?i18n.t("pdf_corpus.metadata_rerun","Interpretive metadata rerun against the current reviewed record text."):i18n.tf("pdf_corpus.metadata_family_rerun","{family} metadata rerun against the current reviewed record text.",{family:i18n.t(`pdf_corpus.metadata_family.${metadataRerunFamily.value}`,metadataRerunFamily.value)}))}catch(exc){await restoreReviewViewport(viewport);setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
...
async function applyBulkMetadata(payload:{changes:Record<string,unknown>;applyToAll:boolean}){
  if(!currentBuild.value)return;busy.value="bulk-metadata";try{
    const result=await pdfCorpusApi.bulkMetadata(currentBuild.value.build_id,payload.changes,{recordIds:payload.applyToAll?[]:Array.from(selectedReviewIds.value),applyToAll:payload.applyToAll,reviewQueue:reviewQueue.value,query:recordQuery.value});
    bulkMetadataOpen.value=false;await refreshBuild();await refreshRecords(false,selectedRecordId.value);setMessage(i18n.tf("pdf_corpus.bulk_metadata_applied","Updated metadata on {count} record(s).",{count:result.changed}));
  }catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}
}
function startNewBuildSetup(){stopPolling();selectedBuildId.value="";currentBuild.value=null;records.value=[];recordTotal.value=0;selectedRecordId.value="";selectedRecord.value=null;sourceBlocks.value=[];reviewHydrated.value=false;bulkMetadataOpen.value=false;void router.replace({query:{...route.query,build:undefined,record:undefined,queue:undefined}})}
async function cancelBuild(){if(!currentBuild.value)return;await pdfCorpusApi.cancel(currentBuild.value.build_id);setMessage(i18n.t("pdf_corpus.cancel_requested","Cancellation requested. The segmented workspace remains editable, and you can start another build after cancellation settles."));startPolling()}
async function settleMetadata(){if(!currentBuild.value)return;busy.value="settle";try{currentBuild.value=await pdfCorpusApi.settleMetadata(currentBuild.value.build_id);syncBuildInRail(currentBuild.value);setMessage(i18n.t("pdf_corpus.settle_requested_notice","Remaining automatic metadata tasks will settle as review exceptions after any active request reaches its deadline."));startPolling()}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
...
watch(()=>[currentBuild.value?.build_id,currentBuild.value?.record_count,currentBuild.value?.metadata_total,currentBuild.value?.metadata_enriched_count,currentBuild.value?.status,currentBuild.value?.stage] as const,async ([buildId,count,metadataTotal,metadataEnriched,status,stage])=>{
  ...
  if(!reviewHydrated.value||expected>hydratedTopologyCount.value||!selectedRecord.value){await ensureReviewHydrated(selectedRecordId.value)}
},{flush:"post"});
...
watch(()=>route.query.queue,value=>{const queue=String(value||"") as ReviewQueue;if(["all","ready","issues","metadata","topology","source","accepted","rejected"].includes(queue)&&queue!==reviewQueue.value)reviewQueue.value=queue});
...
watch([selectedProviderId,selectedReviewProviderId,selectedAssetId,manualProvider,manualModel,manualBaseUrl,useProfileDefaults,generationOverrides,stageLimits,stageTimeouts,recordSizing,maxConcurrentRequests,enrichmentMode,semanticIndexing,autoCleanText,llmTouchupDuringEnrichment],persistBuilderDraft,{deep:true});
...
onMounted(()=>{window.addEventListener("keydown",reviewShortcut);restoreBuilderDraft();void loadSchemaChoices();void refreshAll().then(()=>{if(buildRunning.value)startPolling()}).catch(exc=>setMessage(exc instanceof Error?exc.message:String(exc),"error"))});
  ...
  <section class="corpus-builder" :aria-labelledby="'pdf-corpus-builder-title'">
    <header class="builder-header">
      <div>
        ...
        <p>{{i18n.t('pdf_corpus.subtitle','Deterministic extraction and provenance come first. Targeted LLM enrichment is optional, measurable, and used only where it can add scholarly value.')}}</p>
      </div>
      <div class="header-actions">
        <CorpusBuildHistoryMenu :builds="builds" :total="buildsTotal" :selected-build-id="selectedBuildId" @select="chooseBuild" @refresh="refreshBuilds" />
        <button v-if="currentBuild&&(buildRunning?canStartConcurrentBuild:['cancelled','failed','interrupted','ready','awaiting_review'].includes(String(currentBuild.status||'')))" type="button" class="btn" @click="startNewBuildSetup" :disabled="busy!==''">{{buildRunning?i18n.t('pdf_corpus.start_concurrent_build','Start another build'):i18n.t('pdf_corpus.start_new_build','Start new build')}}</button>
      </div>
    </header>

    <div ref="statusRegion" tabindex="-1" class="status-region" aria-live="polite" aria-atomic="true">
      <div v-if="error" class="builder-message" :class="transientNetworkError?'warning':'error'" :role="transientNetworkError?'status':'alert'"><span>{{displayError}}</span><button type="button" class="message-dismiss" :aria-label="i18n.t('ui.dismiss','Dismiss')" @click="error=''">×</button></div>
    ...
    <section v-if="showBuildConfiguration" class="builder-setup" :aria-labelledby="'pdf-corpus-config-title'">
      ...
      <section class="setup-section setup-source-section" aria-labelledby="pdf-corpus-source-title">
        <div class="setup-section-head"><div><span class="setup-step">1</span><div><h3 id="pdf-corpus-source-title">{{i18n.t('pdf_corpus.source_setup_title','Source PDF')}}</h3><p>{{i18n.t('pdf_corpus.source_setup_help','Choose an extracted PDF, or add the PDF currently open in Explorer.')}}</p></div></div></div>
        <div class="source-setup-grid">
          <label for="pdf-corpus-source"><span>{{i18n.t('pdf_corpus.source_asset','Source asset')}}</span><select id="pdf-corpus-source" v-model="selectedAssetId" class="control"><option value="">{{i18n.t('pdf_corpus.choose_persisted_pdf','Choose a persisted PDF…')}}</option><option v-for="asset in assets" :key="asset.asset_id" :value="asset.asset_id">{{asset.filename}} · {{asset.page_count}} pp · {{asset.block_count}} {{i18n.t('pdf_corpus.source_units','source units')}}</option></select></label>
          <div class="source-actions"><button type="button" class="btn" @click="useCurrentPdf" :disabled="busy!==''||buildRunning">{{i18n.t('pdf_corpus.use_current_pdf','Use current Explorer PDF')}}</button><button type="button" class="btn" @click="uploadInput?.click()" :disabled="busy!==''||buildRunning">{{busy==='upload'?i18n.t('pdf_corpus.extracting','Extracting…'):i18n.t('pdf_corpus.choose_pdf','Choose source PDF')}}</button><input ref="uploadInput" class="sr-only" type="file" accept="application/pdf,.pdf" :aria-label="i18n.t('pdf_corpus.choose_pdf','Choose source PDF')" @change="upload(($event.target as HTMLInputElement).files?.[0])"></div>
        </div>
        <div v-if="selectedAsset" class="source-facts"><span>{{selectedAsset.page_count}} {{i18n.t('pdf_corpus.pages','pages')}}</span><span>{{selectedAsset.block_count}} {{i18n.t('pdf_corpus.source_units','source units')}}</span><span>{{selectedAsset.ocr_pages}} {{i18n.t('pdf_corpus.ocr_pages','OCR page(s)')}}</span><span v-if="selectedAsset.metadata?.author"><b>{{i18n.t('pdf_corpus.pdf_author','PDF author')}}</b>: {{String(selectedAsset.metadata.author)}}</span><span>SHA-256 {{selectedAsset.sha256.slice(0,16)}}…</span><span>{{formatDate(selectedAsset.created_at)}}</span></div>
        <p v-if="selectedAsset" class="source-facts-help">{{i18n.t('pdf_corpus.source_facts_help','Blocks are extracted text layout units, not records. OCR page(s) counts pages where OCR was used; zero means the PDF text layer was used directly. An embedded PDF author is a deterministic candidate and remains reviewable in the document manifest.')}}</p>
      </section>

      <section v-if="selectedAsset?.pages?.length" class="setup-phase" aria-labelledby="pdf-corpus-structure-phase-title">
        <div class="phase-label"><span class="setup-step">2</span><div><h3 id="pdf-corpus-structure-phase-title">{{i18n.t('pdf_corpus.document_structure','Document structure & pagination')}}</h3><p>{{i18n.t('pdf_corpus.structure_before_build_help','Set physical-to-printed page rules before record construction so deterministic pagination, regions, and threads have an explicit source of truth.')}}</p></div></div>
        <DocumentStructureConfigurator class="document-structure-config" :asset="selectedAsset" :pdf-url="sourcePdfUrl" :disabled="busy!==''||Boolean(buildRunning&&currentBuild?.asset_id===selectedAssetId)" :saving="busy==='document-layout'" @save="saveDocumentLayout" @save-page-labels="savePageLabels" />
      </section>

      <details class="setup-section setup-disclosure" open>
        <summary><span class="setup-step">3</span><span><b>{{i18n.t('pdf_corpus.llm_enrichment_title','LLM & enrichment')}}</b><small>{{selectedProviderLabel}}<template v-if="selectedProfileModel"> · {{selectedProfileModel}}</template> · {{enrichmentMode==='deep'?i18n.t('pdf_corpus.enrichment_deep','Deep scholarly enrichment'):i18n.t('pdf_corpus.enrichment_fast','Fast corpus build')}}</small></span></summary>
        <div class="setup-disclosure-body">
          ...
          <section class="enrichment-strategy" aria-labelledby="pdf-corpus-enrichment-mode-title"><div class="setup-card-heading"><b id="pdf-corpus-enrichment-mode-title">{{i18n.t('pdf_corpus.enrichment_strategy','Enrichment strategy')}}</b><small>{{i18n.t('pdf_corpus.enrichment_strategy_help','Fast mode routes only genuinely useful metadata questions to the LLM. Deep mode runs broader attribution, quotation, and indexing analysis.')}}</small></div><div class="mode-options" role="radiogroup" :aria-label="i18n.t('pdf_corpus.enrichment_strategy','Enrichment strategy')"><label><input v-model="enrichmentMode" type="radio" value="fast"><span><b>{{i18n.t('pdf_corpus.enrichment_fast','Fast corpus build')}}</b><small>{{i18n.t('pdf_corpus.enrichment_fast_help','Deterministic metadata first; quotation analysis only when signaled; semantic indexing off unless enabled below.')}}</small></span></label><label><input v-model="enrichmentMode" type="radio" value="deep"><span><b>{{i18n.t('pdf_corpus.enrichment_deep','Deep scholarly enrichment')}}</b><small>{{i18n.t('pdf_corpus.enrichment_deep_help','Runs discourse, quotation, and semantic indexing more broadly. Slower and intended for deliberate enrichment work.')}}</small></span></label></div><div v-if="enrichmentMode==='deep'" class="included-feature"><b>{{i18n.t('pdf_corpus.semantic_indexing','Semantic indexing')}}</b><span>{{i18n.t('pdf_corpus.semantic_indexing_included','Included in Deep enrichment')}}</span></div><label v-else class="semantic-index-toggle"><input v-model="semanticIndexing" type="checkbox"><span><b>{{i18n.t('pdf_corpus.semantic_indexing','Semantic indexing')}}</b><small>{{i18n.t('pdf_corpus.semantic_indexing_help','Generate topics, concepts, persons, and referenced works. Deep mode always includes indexing.')}}</small></span></label>          <label class="semantic-index-toggle"><input v-model="autoCleanText" type="checkbox"><span><b>{{i18n.t('pdf_corpus.auto_clean_all_records','Clean all record text before enrichment')}}</b><small>{{i18n.t('pdf_corpus.auto_clean_all_records_help','Recommended. Removes repeated headers and page numbers, repairs prose line wrapping, trims blank-line noise and obvious OCR artifacts while preserving immutable extracted text for audit.')}}</small></span></label><label class="semantic-index-toggle"><input v-model="llmTouchupDuringEnrichment" type="checkbox"><span><b>{{i18n.t('pdf_corpus.llm_touchup_during_enrichment','Use LLM to touch-up text as part of enrichment')}}</b><small>{{i18n.t('pdf_corpus.llm_touchup_during_enrichment_help','Proposes conservative corrections for errata, diacritics, formatting, quotations, and line breaks. Nothing becomes authoritative until you review and save the proposal.')}}</small></span></label></section>
          <details v-if="!selectedProviderId" :open="advancedOpen" class="advanced-config" @toggle="advancedOpen=($event.currentTarget as HTMLDetailsElement).open"><summary>{{i18n.t('pdf_corpus.manual_provider','Manual provider compatibility settings')}}</summary><p class="help">{{i18n.t('pdf_corpus.manual_provider_help','Used only when no provider profile is selected. Provider profiles are recommended because credentials remain server-owned and resumable builds can reuse the same configuration.')}}</p><div class="advanced-grid"><label for="pdf-corpus-provider">{{i18n.t('pdf_corpus.provider','Provider')}}</label><select id="pdf-corpus-provider" v-model="manualProvider" class="control"><option value="ollama">Ollama</option><option value="openai">{{i18n.t('pdf_corpus.openai_compatible','OpenAI-compatible')}}</option></select><label for="pdf-corpus-model">{{i18n.t('pdf_corpus.model','Model')}}</label><input id="pdf-corpus-model" v-model="manualModel" class="control" :placeholder="i18n.t('pdf_corpus.provider_default','Provider default')"><label for="pdf-corpus-url">{{i18n.t('pdf_corpus.base_url','Base URL')}}</label><input id="pdf-corpus-url" v-model="manualBaseUrl" class="control" :placeholder="i18n.t('pdf_corpus.provider_default','Provider default')"><label for="pdf-corpus-key">{{i18n.t('pdf_corpus.api_key','API key')}}</label><input id="pdf-corpus-key" v-model="manualApiKey" class="control" type="password" autocomplete="off" :placeholder="i18n.t('pdf_corpus.not_persisted','Not persisted in build manifest')"></div></details>
        </div>
      </details>
      ...
      <details class="setup-section setup-disclosure">
        <summary><span class="setup-step">5</span><span><b>{{i18n.t('schemas.title','Metadata schema')}}</b><small>{{chosenSchema?.name||i18n.t('schemas.builtin','Built in')}}<template v-if="chosenSchema"> · {{i18n.tf('schemas.field_count','{count} fields',{count:chosenSchema.field_count})}}</template></small></span></summary>
        <div class="setup-disclosure-body schema-choice">
          <label class="schema-choice-field"><span>{{i18n.t('schemas.choose','Metadata schema')}}</span>
            <select v-model="schemaId" class="control" :disabled="busy!==''"><option v-for="item in schemaChoices" :key="item.id" :value="item.id">{{item.name}}{{item.builtin?` (${i18n.t('schemas.builtin','Built in')})`:''}}</option></select>
            <small>{{i18n.t('schemas.choose_help','The metadata fields on every record and what the model looks for in each. The build keeps its own copy.')}}</small></label>
          <button type="button" class="btn small" @click="schemaEditorOpen=true">{{i18n.t('schemas.manage','Manage schemas…')}}</button>
        </div>
      </details>

      <details class="setup-section setup-disclosure">
        <summary><span class="setup-step">6</span><span><b>{{i18n.t('pdf_corpus.hands_free_title','Hands-free mode')}}</b><small>{{handsFree.enabled?i18n.t('pdf_corpus.hands_free_on','On: the build settles itself, then reports what is left'):i18n.t('pdf_corpus.hands_free_off','Off: you review every record')}}</small></span></summary>
      ...
      <CorpusBuildReadiness :source-filename="selectedAsset?.filename||''" :page-count="selectedAsset?.page_count||0" :block-count="selectedAsset?.block_count||0" :structure-summary="selectedStructureSummary" :provider-label="selectedProviderLabel" :model-label="selectedProfileModel||manualModel" :enrichment-mode="enrichmentMode" :target-chars="recordSizing.preferred_record_chars" :tolerance-chars="recordSizing.record_length_tolerance" :context-safe="contextSafe" :active-build-count="activeBuildCount" :can-start="canStartConcurrentBuild" :busy="busy==='build'" :warnings="setupWarnings" @build="startBuild" />
    </section>
    ...
    <div class="builder-workspace" :class="{'review-mode':showReviewWorkspace&&!finishPhase}">
      ...
      <main class="build-main">
        <section v-if="currentBuild" class="build-summary" :aria-labelledby="'pdf-corpus-current-build'">
          ...
          <CorpusFinishWorkspace v-if="!awaitingManifestReview && finishPhase" :build="currentBuild" :busy="busy!==''" @retry-metadata="retryIncompleteMetadata" @review-metadata="openMetadataIssueQueue" @review-rejected="openRejectedQueue" @review-records="openAllReviewQueue" @review-source="openSourceIssueQueue" @restore-rejected="restoreAllRejected" @start-new="startNewBuildSetup" @edit-document-metadata="documentMetadataOpen=true" @rerun-enrichment="llmActionProviderId=selectedProviderId||providerProfiles[0]?.id||'';metadataEnrichmentOpen=true" @publish="publish({download:false})" />
          ...
          <section v-if="segmentationNeedsReview&&(!showReviewWorkspace||finishPhase)" class="segmentation-blocked segmentation-review-localized" role="status" aria-labelledby="segmentation-review-title">
        ...
        <template v-if="showReviewWorkspace">
          ...
          <section class="review-toolbar" :aria-label="i18n.t('pdf_corpus.review_controls','Record review controls')">
            ...
            <div class="workspace-switcher" role="group" :aria-label="i18n.t('pdf_corpus.review_workspace','Review workspace')"><button type="button" class="btn small" :aria-pressed="reviewWorkspaceMode==='record'" @click="setReviewWorkspaceMode('record')">{{i18n.t('pdf_corpus.workspace.record','Record')}}</button><button type="button" class="btn small" :aria-pressed="reviewWorkspaceMode==='metadata'" :aria-label="i18n.t('pdf_corpus.workspace.metadata','Metadata workspace')" :disabled="!selectedRecord" @click="setReviewWorkspaceMode('metadata')">{{i18n.t('pdf_corpus.workspace.metadata_short','Metadata')}}</button><button type="button" class="btn small" :aria-pressed="reviewWorkspaceMode==='source'" :aria-label="i18n.t('pdf_corpus.workspace.source','Source workspace')" :disabled="!selectedRecord" @click="setReviewWorkspaceMode('source')">{{i18n.t('pdf_corpus.workspace.source_short','Source')}}</button></div>
            <div class="review-bulk"><button type="button" class="btn small primary" @click="acceptCleanRecords" :disabled="busy!==''||readyCount===0">{{i18n.tf('pdf_corpus.accept_clean','Accept clean ({count})',{count:readyCount})}}</button><CorpusActionMenu :label="i18n.t('pdf_corpus.bulk_actions','Bulk actions')" :items="bulkActionItems" :disabled="busy!==''" placement="bottom" @select="runBulkAction"/></div>
            <p v-if="bulkActionFeedback" class="review-action-feedback" role="status" aria-live="polite">{{bulkActionFeedback}}</p>
            <CorpusBulkMetadataEditor v-if="bulkMetadataOpen" :schema="currentBuild?.schema" :records="records" :region-types="regionTypes" :discourse-roles="discourseRoles" :selected-count="selectedReviewCount" :total-count="Number(currentBuild?.record_count||recordTotal)" :disabled="busy!==''||reviewLocked" @apply="applyBulkMetadata" @close="bulkMetadataOpen=false" />
          ...
          <section ref="reviewGridEl" class="review-grid record-first-review" :style="{'--rw-queue':`${queueSplitter.size.value}px`,'--rw-inspector':`${inspectorSplitter.size.value}px`}" :class="{'queue-collapsed':reviewQueueCollapsed,'detail-mode':reviewWorkspaceMode!=='record','metadata-workspace':reviewWorkspaceMode==='metadata','source-workspace':reviewWorkspaceMode==='source'}" :aria-busy="recordsLoading">
            <nav ref="recordListEl" class="records-pane" tabindex="-1" :aria-labelledby="'pdf-corpus-records-pane'">
              <div class="pane-head"><b id="pdf-corpus-records-pane">{{i18n.t('pdf_corpus.review_queue','Review queue')}}</b><button type="button" class="link-button queue-toggle" @click="reviewQueueCollapsed=true">{{i18n.t('pdf_corpus.hide_queue','Hide queue')}}</button><label class="select-visible"><input type="checkbox" :checked="allVisibleSelected" :disabled="!records.length||busy!==''" @change="toggleVisibleSelection(($event.target as HTMLInputElement).checked)"><span>{{i18n.t('pdf_corpus.select_visible','Select visible')}}</span></label><span>{{recordTotal}}</span></div>
              <div v-for="record in records" :key="record.record_id" class="record-row-wrap">
                <label class="record-select"><input type="checkbox" :checked="selectedReviewIds.has(record.record_id)" :aria-label="i18n.tf('pdf_corpus.select_record_id','Select {record}',{record:record.record_id})" @change="toggleReviewSelection(record.record_id,($event.target as HTMLInputElement).checked)"><span class="sr-only">{{i18n.tf('pdf_corpus.select_record_id','Select {record}',{record:record.record_id})}}</span></label>
                <button type="button" class="record-row" :class="{active:record.record_id===selectedRecordId}" :aria-current="record.record_id===selectedRecordId?'true':undefined" @click="selectRecord(record)">
                  <span class="record-state-icon" :data-state="recordState(record)" aria-hidden="true"><AppIcon v-if="recordStateIcon(record)" :name="recordStateIcon(record)"/></span>
                  <span class="record-row-main"><b>{{record.record_id}}</b><small>{{i18n.t('pdf_corpus.pages','pp.')}} {{record.page_start}}–{{record.page_end}} · {{record.text_length.toLocaleString()}} {{i18n.t('pdf_corpus.characters','chars')}}</small><span class="record-row-status" :data-state="recordState(record)">{{recordStateLabel(record)}}</span><small v-if="extraIssueKinds(record).length" class="record-issue-summary">{{extraIssueKinds(record).map(kind=>i18n.t(`pdf_corpus.record_state.${kind}`,kind)).join(' · ')}}</small></span>
                </button>
              </div>
            ...
            <div v-if="!reviewQueueCollapsed" class="review-splitter" data-splitter="queue" role="separator" tabindex="0" aria-orientation="vertical" :aria-label="i18n.t('pdf_corpus.resize_queue','Resize review queue')" v-bind="queueSplitter.aria()" @pointerdown="queueSplitter.onPointerDown" @keydown="queueSplitter.onKeydown" @dblclick="queueSplitter.reset"></div>

            <article v-show="reviewWorkspaceMode==='record'" ref="reviewPaneEl" class="record-review-pane" :aria-labelledby="selectedRecord?'review-record-title':undefined">
              <template v-if="selectedRecord">
                <header class="record-review-head"><div><button v-if="reviewQueueCollapsed" type="button" class="link-button queue-toggle" @click="reviewQueueCollapsed=false">{{i18n.t('pdf_corpus.show_queue','Show queue')}}</button><span class="eyebrow">{{i18n.t('pdf_corpus.proposed_record','Proposed record')}}</span><h3 id="review-record-title">{{selectedRecord.record_id}}</h3><p>{{i18n.t('pdf_corpus.pages','pp.')}} {{selectedRecord.page_start}}–{{selectedRecord.page_end}} · {{selectedRecord.text_length.toLocaleString()}} {{i18n.t('pdf_corpus.characters','chars')}}{{selectedRecordActivitySummary}}</p></div><div class="record-head-actions"><div class="decision-history" role="group" :aria-label="i18n.t('pdf_corpus.record_history','Review history')"><button type="button" class="btn small" @click="undoReview" :disabled="busy!==''"><AppIcon name="history"/><span class="btn-text">{{i18n.t('pdf_corpus.undo','Undo')}}</span></button><button type="button" class="btn small" @click="redoReview" :disabled="busy!==''"><AppIcon name="history" class="flip-inline"/><span class="btn-text">{{i18n.t('pdf_corpus.redo','Redo')}}</span></button></div><button type="button" class="btn small" @click="openFocusView">{{i18n.t('pdf_corpus.focus_view','Focus view')}}</button></div></header>
                ...
                <aside v-if="selectedRecord.text_touchup_proposal?.status==='pending_review'" class="review-reason touchup-review-notice" role="status"><b>{{i18n.t('pdf_corpus.llm_touchup_proposal_available','LLM text touch-up proposal available')}}</b><span>{{i18n.t('pdf_corpus.llm_touchup_proposal_help','Review the proposed corrections before saving; the current reviewed text is unchanged.')}}</span><button type="button" class="btn small" @click="beginTextEdit(true)" :disabled="busy!==''||reviewLocked">{{i18n.t('pdf_corpus.review_touchup_proposal','Review proposal')}}</button></aside>
            ...
            <div v-if="reviewWorkspaceMode==='record'" class="review-splitter" data-splitter="inspector" role="separator" tabindex="0" aria-orientation="vertical" :aria-label="i18n.t('pdf_corpus.resize_inspector','Resize review details')" v-bind="inspectorSplitter.aria()" @pointerdown="inspectorSplitter.onPointerDown" @keydown="inspectorSplitter.onKeydown" @dblclick="inspectorSplitter.reset"></div>
            <aside ref="reviewInspectorEl" class="review-inspector" :aria-label="i18n.t('pdf_corpus.review_details','Review details')">
              <div v-if="reviewWorkspaceMode!=='record'" class="detail-workspace-head"><div><span class="eyebrow">{{i18n.t('pdf_corpus.review_workspace','Review workspace')}}</span><h3>{{reviewWorkspaceMode==='metadata'?i18n.t('pdf_corpus.workspace.metadata','Metadata workspace'):i18n.t('pdf_corpus.workspace.source','Source workspace')}}</h3><p>{{reviewWorkspaceMode==='metadata'?i18n.t('pdf_corpus.workspace.metadata_help','Resolve attribution, discourse, and indexing metadata with evidence and provenance visible.'):i18n.t('pdf_corpus.workspace.source_help','Inspect the source page, immutable extraction, boundary decisions, and transcription tools at full working width.')}}</p></div><button type="button" class="btn small" @click="setReviewWorkspaceMode('record')">{{i18n.t('pdf_corpus.workspace.back_record','Back to record')}}</button></div>
              <div v-show="reviewWorkspaceMode==='record'" class="review-inspector-tabs" role="tablist" :aria-label="i18n.t('pdf_corpus.review_detail_views','Review detail views')">
                <button id="review-tab-metadata" data-review-tab="metadata" type="button" role="tab" aria-controls="review-panel-metadata" :aria-selected="reviewInspectorTab==='metadata'" :tabindex="reviewInspectorTab==='metadata'?0:-1" @keydown="reviewInspectorKeydown" @click="reviewInspectorTab='metadata'">{{i18n.t('pdf_corpus.metadata_tab','Metadata')}}<span v-if="selectedMetadataBlockingFields.length">{{selectedMetadataBlockingFields.length}}</span></button>
                <button id="review-tab-evidence" data-review-tab="evidence" type="button" role="tab" aria-controls="review-panel-evidence" :aria-selected="reviewInspectorTab==='evidence'" :tabindex="reviewInspectorTab==='evidence'?0:-1" @keydown="reviewInspectorKeydown" @click="reviewInspectorTab='evidence'">{{i18n.t('pdf_corpus.evidence_tab','Evidence')}}</button>
              ...
              <section v-if="selectedRecord&&(reviewWorkspaceMode==='metadata'||(reviewWorkspaceMode==='record'&&reviewInspectorTab==='metadata'))" id="review-panel-metadata" class="review-inspector-panel" role="tabpanel" aria-labelledby="review-tab-metadata" tabindex="0">
                <CorpusMetadataResolutionPanel :schema="currentBuild?.schema" :record="selectedRecord" :region-types="regionTypes" :discourse-roles="discourseRoles" :busy="busy!==''" :saving-field="metadataSavingField" :saved-field="metadataSavedField" :confidence-calibration="currentBuild?.llm_confidence_calibration||{}" :known-values="metadataKnownValues" @resolve="resolveMetadataField" @no-value="resolveMetadataNoValue" @resolve-many="resolveMetadataSuggestions" @source="showMetadataSource" @dirty="handleMetadataDirty" />
                ...
                <details class="record-data"><summary>{{i18n.t('pdf_corpus.advanced_metadata','Advanced metadata')}}</summary><p class="help">{{i18n.t('pdf_corpus.metadata_help','Typed controls above are preferred. This advanced editor is for supported interpretive metadata only; source-bound fields remain protected.')}}</p><LlmExecutionControl :model-value="llmActionProviderId||selectedProviderId" :model-override="llmActionModel" :profiles="providerProfiles" :disabled="busy!==''" :task="i18n.t('pdf_corpus.metadata_rerun_provider_help','Rerun only this Record with the selected LLM profile. Existing human-confirmed values remain authoritative.')" @update:model-value="value=>llmActionProviderId=value" @update:model-override="value=>llmActionModel=value" /><label class="sr-only" for="pdf-corpus-metadata">{{i18n.t('pdf_corpus.interpretive_metadata','Interpretive metadata')}}</label><textarea id="pdf-corpus-metadata" v-model="metadataDraft" class="metadata-json" spellcheck="false" @input="metadataEditorDirty=true"></textarea><div class="data-actions"><button type="button" class="btn small" @click="saveMetadata" :disabled="busy!==''">{{i18n.t('pdf_corpus.save_metadata','Save metadata')}}</button><label class="rerun-family"><span>{{i18n.t('pdf_corpus.rerun_family','Rerun family')}}</span><select v-model="metadataRerunFamily" class="control small"><option value="all">{{i18n.t('pdf_corpus.metadata_family.all','All metadata families')}}</option><option v-for="family in metadataFamilyOptions" :key="family.key" :value="family.key">{{family.label}}</option></select></label><button type="button" class="btn small" :title="i18n.t('pdf_corpus.rerun_metadata_help','Re-evaluate only this Record using the selected metadata family.')" @click="rerunMetadata()" :disabled="busy!==''">{{i18n.t('pdf_corpus.rerun_metadata','Rerun this Record')}}</button><button type="button" class="btn small" :title="i18n.t('pdf_corpus.metadata_enrichment_again_help','Run a multi-Record enrichment pass with scope, family, and pass options.')" @click="llmActionProviderId=selectedProviderId||providerProfiles[0]?.id||'';metadataEnrichmentOpen=true" :disabled="busy!==''">{{i18n.t('pdf_corpus.metadata_enrichment_again','Run enrichment across Records')}}</button></div></details>
              </section>
              <section v-else-if="selectedRecord&&reviewWorkspaceMode==='record'&&reviewInspectorTab==='evidence'" id="review-panel-evidence" class="review-inspector-panel" role="tabpanel" aria-labelledby="review-tab-evidence" tabindex="0">
                ...
                <FieldEvidenceList :evidence="selectedRecord.metadata_evidence||{}" :fields="evidenceCandidateFields" :selected-field="selectedEvidenceField" @select="selectedEvidenceField=$event" />
                <div v-if="selectedEvidenceField" class="source-blocks compact-source-blocks"><article v-for="block in visibleBlocks" :key="block.block_id" class="source-block" :class="{'evidence-block':evidenceBlockIds.has(block.block_id)}"><header><span>{{block.block_id}}</span><span>PDF {{block.page}} · {{block.type}}</span></header><p>{{block.text}}</p><button type="button" class="evidence-toggle" :aria-pressed="evidenceBlockIds.has(block.block_id)" @click="toggleEvidenceBlock(block.block_id)" :disabled="busy!==''">{{evidenceBlockIds.has(block.block_id)?i18n.t('pdf_corpus.remove_evidence','Remove as evidence'):i18n.t('pdf_corpus.add_evidence','Add as evidence')}} · {{selectedEvidenceField}}</button></article></div>
              </section>
              <section v-else-if="selectedRecord&&(reviewWorkspaceMode==='record'||reviewWorkspaceMode==='source')" id="review-panel-source" class="review-inspector-panel source-review-panel" role="tabpanel" aria-labelledby="review-tab-source" tabindex="0">
                <CorpusSourceSummary :pdf-url="sourcePdfUrl" :page="selectedPdfPage" :page-count="selectedAsset?.page_count||0" :page-width="selectedPageMeta?.width||0" :page-height="selectedPageMeta?.height||0" :blocks="selectedPageBlocks" :evidence-block-ids="evidenceIdsArray" :can-previous="selectedPdfPageIndex>0" :can-next="selectedPdfPageIndex<recordPdfPages.length-1" @previous="previousSourcePage" @next="nextSourcePage" @open-viewer="sourceTranscriptionOpen=true" @open-pdf-explorer="openPdfExplorer" />
                <details class="source-tool-section"><summary>{{i18n.t('pdf_corpus.boundary_second_reader','Boundary second reader')}}</summary><CorpusBoundaryAdjudication :record="selectedRecord" :can-previous="canMergePrevious" :can-next="canMergeNext" :busy="busy!==''" :profiles="providerProfiles" :provider-profile-id="llmActionProviderId||selectedProviderId" :model-override="llmActionModel" :concurrency-risk="Boolean(providerProfiles.find(p=>p.id===(llmActionProviderId||selectedProviderId))?.type==='ollama'&&llmActionConcurrentLoad+1>Number(providerProfiles.find(p=>p.id===(llmActionProviderId||selectedProviderId))?.max_concurrent_requests||1))" :active-requests="llmActionConcurrentLoad" :concurrency-limit="Number(providerProfiles.find(p=>p.id===(llmActionProviderId||selectedProviderId))?.max_concurrent_requests||1)" @update:provider-profile-id="value=>llmActionProviderId=value" @update:model-override="value=>llmActionModel=value" @adjudicate="adjudicateBoundary" /></details>
                <details class="source-tool-section"><summary>{{i18n.t('pdf_corpus.extracted_source_text','Immutable extracted source')}}</summary><p class="inspector-help">{{i18n.t('pdf_corpus.extracted_source_text_help','This is the audit reference produced by PDF extraction. Human corrections change the reviewed record text, never these source blocks.')}}</p><pre v-if="selectedRecord.source_extracted_text" class="original-extraction-snapshot">{{selectedRecord.source_extracted_text}}</pre><div class="source-blocks"><article v-for="(block,index) in visibleBlocks" :key="block.block_id" class="source-block" :class="{'evidence-block':evidenceBlockIds.has(block.block_id)}"><header><span>{{block.block_id}}</span><span>PDF {{block.page}} · {{block.type}}</span></header><p>{{block.text}}</p><button v-if="selectedEvidenceField" type="button" class="evidence-toggle" :aria-pressed="evidenceBlockIds.has(block.block_id)" @click="toggleEvidenceBlock(block.block_id)" :disabled="busy!==''">{{evidenceBlockIds.has(block.block_id)?i18n.t('pdf_corpus.remove_evidence','Remove as evidence'):i18n.t('pdf_corpus.add_evidence','Add as evidence')}} · {{selectedEvidenceField}}</button><button v-if="index<visibleBlocks.length-1" type="button" class="split-button" @click="split(block.block_id)" :disabled="busy!==''">{{i18n.t('pdf_corpus.split_after','Split after this block')}}</button></article></div></details><details class="source-tool-section"><summary>{{i18n.t('pdf_corpus.revision_history','Revision history')}}</summary><CorpusRevisionHistory :record="selectedRecord" /></details>
              </section>
              <div v-else class="inspector-empty">{{i18n.t('pdf_corpus.select_record','Select a generated record to inspect its source binding and metadata.')}}</div>
            </aside>
          </section>
    ...
    <MetadataEnrichmentDialog :groups="currentBuild?.schema?.groups?.map(g=>({key:g.key,label:g.label}))" :open="metadataEnrichmentOpen" :profiles="providerProfiles" :provider-profile-id="llmActionProviderId" :model-override="llmActionModel" :busy="busy==='metadata-enrichment'" :record-count="currentBuild?.record_count||0" :accepted-count="currentBuild?.accepted_count||0" :selected-count="selectedReviewIds.size" :selected-record-ids="[...selectedReviewIds]" @update:provider-profile-id="value=>llmActionProviderId=value" @update:model-override="value=>llmActionModel=value" @close="metadataEnrichmentOpen=false" @run="runMetadataEnrichment" />
...
<style scoped>
.corpus-builder{padding:20px 22px 32px;display:grid;gap:14px}.builder-header{display:flex;justify-content:space-between;gap:24px;align-items:flex-end}.builder-header h1{font-size:1.625rem;margin:3px 0 6px}.builder-header p{margin:0;max-width:850px;color:var(--muted);line-height:1.5}.eyebrow{font-size:.8125rem;text-transform:uppercase;letter-spacing:.09em;color:var(--muted);font-weight:700}.header-actions,.summary-actions,.inspector-actions,.pager{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.status-region:focus{outline:none}.builder-message{padding:10px 12px;border:1px solid var(--line);border-radius:10px;background:var(--soft);font-size:.8125rem}.builder-message.error,.build-warning{border-color:var(--tone-danger-border);background:var(--tone-danger-bg);color:var(--tone-danger-fg)}.builder-setup{display:grid;grid-template-columns:minmax(280px,.9fr) minmax(420px,1.35fr);gap:12px;align-items:start;border:1px solid var(--line);background:var(--card);border-radius:14px;padding:14px}.setup-card{min-width:0;height:100%;display:grid;align-content:start;gap:8px;padding:13px;border:1px solid var(--line);border-radius:12px;background:linear-gradient(180deg,var(--card),var(--card))}.setup-source,.provider-area{display:grid;gap:8px}.setup-card-heading{display:grid;gap:3px}.setup-card-heading b{font-size:.8125rem}.setup-card-heading small{font-size:.8125rem!important;line-height:1.45}.builder-setup label{font-size:.8125rem;font-weight:700;color:var(--muted)}.builder-setup small{font-size:.8125rem;color:var(--muted)}.execution-config,.advanced-config,.build-launch-row{grid-column:1/-1}.advanced-config{grid-column:1/-1}.advanced-config summary{cursor:pointer;font-size:.8125rem;font-weight:700}.escalation-field{display:grid;grid-template-columns:minmax(150px,.7fr) minmax(220px,1fr);gap:12px;align-items:center;padding:10px 11px;border:1px solid var(--line);border-radius:10px;background:var(--soft)}.escalation-field>span{display:grid;gap:2px}.escalation-field b{font-size:.8125rem}.escalation-field small{font-size:.8125rem;line-height:1.4;color:var(--muted)}.advanced-grid{display:grid;grid-template-columns:max-content 1fr max-content 1fr;gap:8px 10px;margin-top:8px;align-items:center}.build-launch-row{display:grid;grid-template-columns:minmax(180px,1fr) minmax(280px,1.2fr) auto;gap:12px;align-items:center;padding:11px 12px;border:1px solid var(--line);border-radius:11px;background:var(--soft)}.launch-copy{display:grid;gap:2px;min-width:0}.launch-copy b{font-size:.8125rem}.launch-copy span{font-size:.8125rem;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.build-button{min-height:38px;padding-inline:17px}.context-blocker{padding:8px 10px;border-radius:8px;background:var(--tone-danger-bg);color:var(--tone-danger-fg);font-size:.8125rem}.builder-workspace{display:grid;grid-template-columns:250px minmax(0,1fr);min-height:720px;border:1px solid var(--line);border-radius:14px;overflow:hidden;background:var(--card)}.build-rail{border-inline-end:1px solid var(--line);background:var(--soft);padding:10px;overflow:auto}.rail-title,.pane-head{display:flex;justify-content:space-between;align-items:center;gap:8px}.rail-title{padding:6px 4px 10px}.rail-title>div{display:grid}.rail-title b,.pane-head b{font-size:.8125rem}.rail-title span,.pane-head span{font-size:.8125rem;color:var(--muted)}.icon-button{border:0;background:transparent;cursor:pointer;font-size:1.125rem}.build-row{width:100%;border:1px solid transparent;background:transparent;border-radius:10px;padding:9px;display:grid;grid-template-columns:10px 1fr;gap:9px;text-align:start;cursor:pointer}.build-row:hover,.build-row.active{background:var(--card);border-color:var(--line)}.build-row span:last-child{display:grid;gap:2px;min-width:0}.build-row b{font-size:.8125rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.build-row small{font-size:.8125rem;color:var(--muted)}.status-dot{width:8px;height:8px;border-radius:50%;background:#777;margin-top:3px}
.status-dot[data-status="ready"],.status-dot[data-status="published"]{background:#287a4c}
.status-dot[data-status="running"],.status-dot[data-status="queued"]{background:#8e6815}.status-dot[data-status="failed"],.status-dot[data-status="interrupted"]{background:#a13f3f}
.build-main{min-width:0;display:grid;align-content:start}.build-summary{padding:15px 16px;border-bottom:1px solid var(--line);display:grid;gap:10px}.summary-top{display:flex;justify-content:space-between;gap:18px}.summary-top h2{margin:2px 0;font-size:1.125rem}.summary-top p{margin:0;font-size:.8125rem;color:var(--muted)}.build-status-line,.provenance-strip,.validation-strip{display:flex;gap:12px;align-items:center;flex-wrap:wrap;font-size:.8125rem;color:var(--muted)}.pill{border:1px solid var(--line);border-radius:999px;padding:3px 7px;text-transform:uppercase;font-weight:800;letter-spacing:.04em}.progress-track{height:7px;border-radius:999px;background:var(--soft);overflow:hidden}.progress-track span{display:block;height:100%;background:var(--accent);transition:width .25s}.provenance-strip code{font-size:.8125rem}.manifest-details{border:1px solid var(--line);border-radius:9px;padding:8px 10px}.manifest-details>summary{cursor:pointer;font-size:.8125rem;font-weight:800}.build-warning{display:grid;gap:3px;padding:9px;border:1px solid;border-radius:8px;font-size:.8125rem}.warnings{font-size:.8125rem}.warnings summary{cursor:pointer;font-weight:700}.validation-strip{padding:7px 9px;border-radius:8px;background:var(--tone-ok-bg)}.validation-strip.invalid{background:var(--tone-warn-bg);color:var(--tone-warn-fg)}.provider-model-summary{margin:6px 0 0;font-size:.8125rem;color:var(--muted)}.provider-model-summary code{font-size:.8125rem;color:var(--text)}.review-handoff{display:flex;justify-content:space-between;gap:18px;align-items:center;padding:11px 13px;border-bottom:1px solid var(--line);background:var(--soft)}.review-handoff>div{display:grid;gap:2px}.review-handoff b{font-size:.8125rem}.review-handoff span{font-size:.8125rem;color:var(--muted);line-height:1.4}.review-handoff>strong{white-space:nowrap;font-size:.8125rem}.review-handoff[data-state="published"]{background:var(--tone-ok-bg)}.review-toolbar{display:grid;grid-template-columns:auto minmax(180px,1fr) auto auto auto;gap:10px;align-items:center;padding:9px 12px;border-bottom:1px solid var(--line);font-size:.8125rem}.check{display:flex;gap:6px;align-items:center}.bulk-metadata-workspace{grid-column:1/-1;margin:0 10px 10px}.review-bulk{display:flex;gap:6px;flex-wrap:wrap}.danger{border-color:var(--tone-danger-border)!important;color:var(--tone-danger-fg)!important}.review-grid{display:grid;grid-template-columns:minmax(360px,1.2fr) minmax(240px,.65fr) minmax(300px,.85fr);min-height:650px}.source-pane,.records-pane,.inspector-pane{min-width:0;overflow:auto;max-height:76vh}.source-pane,.records-pane{border-inline-end:1px solid var(--line)}.pane-head{position:sticky;top:0;z-index:3;background:var(--card);padding:9px 11px;border-bottom:1px solid var(--line)}.source-page-nav{display:flex;align-items:center;justify-content:center;gap:8px;padding:6px 10px;border-bottom:1px solid var(--line);font-size:.8125rem;color:var(--muted)}.source-blocks{display:grid;gap:8px;padding:10px}.source-block{border:1px solid var(--line);border-radius:9px;padding:9px;position:relative}.source-block.evidence-block{box-shadow:inset 3px 0 0 var(--accent)}[dir="rtl"] .source-block.evidence-block{box-shadow:inset -3px 0 0 var(--accent)}.source-block header{display:flex;justify-content:space-between;gap:8px;font-size:.8125rem;color:var(--muted)}.source-block p{white-space:pre-wrap;font:13px/1.52 Georgia,serif;margin:7px 0}.split-button{display:block;width:100%;border:0;border-top:1px dashed var(--line);background:transparent;color:var(--muted);font-size:.8125rem;padding:5px;cursor:pointer}.evidence-toggle{display:block;width:100%;margin:5px 0;border:1px solid var(--line);border-radius:7px;background:var(--soft);color:var(--text);font-size:.8125rem;padding:6px;text-align:start;cursor:pointer}.evidence-toggle[aria-pressed="true"]{border-color:var(--accent);box-shadow:inset 3px 0 0 var(--accent)}.record-row{width:100%;border:0;border-bottom:1px solid var(--line);background:transparent;padding:10px;display:grid;grid-template-columns:10px 1fr;gap:8px;text-align:start;cursor:pointer}.record-row:hover,.record-row.active{background:var(--soft)}.record-row span:last-child{display:grid;gap:3px}.record-row b{font-size:.8125rem}.record-row small{font-size:.8125rem;color:var(--muted)}.inspector-pane details{border-bottom:1px solid var(--line);padding:10px 12px}.inspector-pane summary{font-size:.8125rem;font-weight:800;cursor:pointer}.inspector-actions{padding:9px 11px;border-bottom:1px solid var(--line)}.record-text{white-space:pre-wrap;font:13px/1.55 Georgia,serif;margin-top:9px}.metadata-json{width:100%;min-height:230px;resize:vertical;font:12px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace;border:1px solid var(--line);border-radius:8px;padding:8px;background:var(--bg);color:var(--text);margin:7px 0}.help{font-size:.8125rem;color:var(--muted);line-height:1.45}.evidence-list{display:grid;gap:8px;margin-top:8px}.evidence-list button{display:grid;gap:2px;padding:7px;background:var(--soft);border:1px solid transparent;border-radius:7px;text-align:start;color:inherit;cursor:pointer}.evidence-list button.active{border-color:var(--accent)}.evidence-list b{font-size:.8125rem}.evidence-list span,.evidence-list small{font-size:.8125rem;color:var(--muted)}.rail-empty,.inspector-empty,.builder-empty{padding:26px 14px;color:var(--muted);font-size:.8125rem}.builder-empty{min-height:420px;display:grid;place-content:center;text-align:center}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.btn:focus-visible,.control:focus-visible,.build-row:focus-visible,.record-row:focus-visible,.icon-button:focus-visible,.split-button:focus-visible,.evidence-toggle:focus-visible,.evidence-list button:focus-visible,summary:focus-visible{outline:3px solid var(--accent);outline-offset:2px}
.build-guidance{display:grid;grid-template-columns:34px minmax(0,1fr);gap:10px;align-items:start;padding:11px 12px;border:1px solid var(--line);border-radius:10px}.build-guidance .guidance-icon{width:30px;height:30px;display:grid;place-items:center;border-radius:8px;font-weight:850}.build-guidance h3{margin:0 0 3px;font-size:.8125rem}.build-guidance p{margin:0;font-size:.8125rem;line-height:1.45}.build-guidance small{display:block;margin-top:4px;font-size:.8125rem;color:inherit;opacity:.8}.running-guidance{background:var(--accent-soft,#eef6f2);border-color:var(--accent-soft-2,#dce9e3)}.running-guidance .guidance-icon{background:var(--card);color: var(--accent-fg)}.failure-guidance{background:var(--tone-danger-bg);border-color:var(--tone-danger-edge);color:var(--tone-danger-fg)}.failure-guidance .guidance-icon{background:var(--card);color:var(--tone-danger-fg)}.manifest-gate{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:start;padding:12px;border:1px solid var(--line);border-radius:10px;background:var(--soft)}.manifest-gate h3{margin:0 0 4px;font-size:0.8125rem}.manifest-gate p{margin:0;max-width:800px;font-size:.8125rem;line-height:1.45}.segmentation-blocked{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:start;padding:12px;border:1px solid var(--tone-danger-border);border-radius:10px;background:var(--tone-danger-bg)}.segmentation-blocked h3{margin:0 0 4px;font-size:0.8125rem}.segmentation-blocked p{margin:0;max-width:800px;font-size:.8125rem;line-height:1.45}.segmentation-blocked details{grid-column:1/-1;font-size:.8125rem}.segmentation-blocked ul{margin:6px 0 0;padding-inline-start:20px}.segmentation-blocked code{font-size:.8125rem}.segmentation-review-localized{border-color:var(--tone-warn-border);background:var(--tone-warn-bg);color:var(--tone-warn-fg)}@media(prefers-reduced-motion:reduce){.progress-track span{transition:none}}
...
.active-build-settings{margin:10px 0;border:1px solid var(--line);border-radius:10px;background:var(--card)}.active-build-settings>summary{padding:10px 12px;cursor:pointer;font-size:.8125rem;font-weight:800}.active-build-settings>div{display:flex;gap:14px;align-items:center;flex-wrap:wrap;padding:10px 12px;border-top:1px solid var(--line);font-size:.8125rem;color:var(--muted)}
.technical-details{border:1px solid var(--line);border-radius:10px;background:var(--card);overflow:hidden}.technical-details>summary{padding:9px 11px;cursor:pointer;font-size:.8125rem;font-weight:800}.technical-details>div,.technical-details>section{border-top:1px solid var(--line)}
.review-toolbar{grid-template-columns:minmax(0,1fr) minmax(220px,.7fr) auto auto}.record-first-review{grid-template-columns:minmax(220px,.52fr) minmax(500px,1.45fr) minmax(360px,.9fr)}.record-first-review>.records-pane{border-inline-end:1px solid var(--line)}.record-first-review>.source-pane{border-inline-start:1px solid var(--line);border-inline-end:0}.record-review-pane{min-width:0;max-height:76vh;overflow:auto;background:var(--card)}.record-review-head{position:sticky;top:0;z-index:4;background:var(--card);display:flex;justify-content:space-between;gap:16px;align-items:flex-start;padding:12px 16px;border-bottom:1px solid var(--line)}.record-review-head h3{margin:2px 0;font-size:0.9375rem}.record-review-head p{margin:0;font-size:.8125rem;color:var(--muted)}.record-primary-text{max-width:80ch;margin:0 auto;padding:28px 32px;white-space:pre-wrap;font:17px/1.72 Georgia,serif}.review-reason{display:grid;gap:3px;margin:12px 16px 0;padding:10px 12px;border-radius:9px;background:var(--tone-warn-bg);color:var(--tone-warn-fg);font-size:.8125rem}.metadata-accept-blocker{margin:0;padding:9px 12px;border:1px solid var(--tone-warn-edge);border-radius:8px;background:var(--tone-warn-bg);color:var(--tone-warn-fg);font-size:.8125rem}.decision-bar{position:sticky;bottom:0;z-index:4;display:flex;justify-content:space-between;align-items:center;gap:12px;padding:10px 12px;border-top:1px solid var(--line);background:color-mix(in srgb,var(--card) 96%,transparent);backdrop-filter:blur(6px)}.decision-actions,.data-actions{display:flex;gap:7px;flex-wrap:wrap}.record-data{border-top:1px solid var(--line);padding:10px 14px}.record-data>summary,.source-text-details>summary{cursor:pointer;font-size:.8125rem;font-weight:800}.source-text-details{border-top:1px solid var(--line);padding:8px}.source-pane,.records-pane{max-height:76vh;overflow:auto}.source-pane .pdf-evidence-viewer{max-width:100%}
@media(max-width:1400px){.record-first-review{grid-template-columns:minmax(210px,.5fr) minmax(460px,1.35fr) minmax(320px,.8fr)}.record-primary-text{font-size:1rem;padding:24px}}
...
@media(max-width:760px){.review-toolbar{grid-template-columns:1fr}.record-first-review{grid-template-columns:1fr}.record-first-review>.records-pane{max-height:260px;border-inline-end:0;border-bottom:1px solid var(--line)}.record-review-pane{max-height:none}.metadata-accept-blocker{margin:0;padding:9px 12px;border:1px solid var(--tone-warn-edge);border-radius:8px;background:var(--tone-warn-bg);color:var(--tone-warn-fg);font-size:.8125rem}.decision-bar{position:static;align-items:stretch;flex-direction:column}.record-primary-text{font-size:1rem;padding:20px 18px}}
.review-stage-summary{display:grid;grid-template-columns:minmax(0,1fr) minmax(260px,.9fr);gap:14px;align-items:center;padding:10px 12px;border:1px solid var(--line);border-radius:10px;background:var(--soft)}.review-stage-summary>div{display:grid;gap:3px}.review-stage-summary b{font-size:.8125rem}.review-stage-summary>span{font-size:.8125rem;line-height:1.45;color:var(--muted)}
.record-first-review{grid-template-columns:minmax(205px,.48fr) minmax(520px,1.5fr) minmax(360px,.95fr)}.record-first-review>.records-pane{border-inline-end:1px solid var(--line)}.record-review-pane,.review-inspector,.records-pane{min-width:0;max-height:76vh;overflow:auto}.review-inspector{border-inline-start:1px solid var(--line);background:var(--soft)}.review-inspector-tabs{position:sticky;top:0;z-index:6;display:grid;grid-template-columns:repeat(3,1fr);background:var(--card);border-bottom:1px solid var(--line)}.review-inspector-tabs button{min-height:42px;border:0;border-inline-end:1px solid var(--line);background:transparent;color:var(--muted);font-size:.8125rem;font-weight:800;cursor:pointer;display:flex;justify-content:center;align-items:center;gap:6px}.review-inspector-tabs button[aria-selected="true"]{background:var(--soft);color:var(--text);box-shadow:inset 0 -3px 0 var(--accent)}.review-inspector-tabs button span{min-width:18px;border-radius:999px;background:var(--tone-warn-bg);color:var(--tone-warn-fg);padding:1px 5px;font-size:.8125rem}.review-inspector-panel{min-width:0}.review-inspector-panel>.resolution-panel{border-top:0;background:transparent}.review-inspector-panel .record-data{border-top:1px solid var(--line);padding:10px 12px;background:var(--card)}.inspector-help{margin:0;padding:12px 14px 0;font-size:.8125rem;line-height:1.45;color:var(--muted)}.compact-source-blocks{padding-top:6px}.source-review-panel{background:var(--card);min-height:100%}.source-inspector-head{position:static;z-index:auto}.metadata-row-warning{color:var(--tone-warn-fg)!important;font-weight:800}.metadata-accept-blocker{display:block}.metadata-accept-blocker b{margin-inline-end:4px}.record-primary-text{max-width:76ch}.decision-bar{min-height:56px}.review-toolbar{grid-template-columns:minmax(0,1fr) minmax(210px,.7fr) auto auto}.review-bulk .btn{min-height:34px}
@media(max-width:1300px){.record-first-review{grid-template-columns:minmax(200px,.48fr) minmax(460px,1.4fr) minmax(330px,.9fr)}}
@media(max-width:1050px){.record-first-review{grid-template-columns:minmax(200px,.48fr) minmax(0,1.5fr)}.review-inspector{grid-column:1/-1;border-inline-start:0;border-top:1px solid var(--line);max-height:60vh}.review-stage-summary{grid-template-columns:1fr}.review-toolbar{grid-template-columns:1fr 1fr}}
@media(max-width:760px){.record-first-review{grid-template-columns:1fr}.review-inspector{max-height:none}.review-toolbar{grid-template-columns:1fr}.review-stage-summary{grid-template-columns:1fr}.review-inspector-tabs{top:0}}
...
.review-toolbar .control{min-height:40px;font-size:.8125rem}.review-bulk{display:flex;gap:8px;justify-content:flex-end}.pager{grid-column:1/-1;display:flex;justify-content:flex-end;gap:8px;font-size:.8125rem;color:var(--muted)}
.record-first-review{display:grid;grid-template-columns:minmax(220px,260px) minmax(560px,1.8fr) minmax(340px,.9fr);min-height:620px;height:calc(100vh - 235px);border:1px solid var(--line);border-radius:12px;overflow:hidden;background:var(--card)}
.records-pane,.record-review-pane,.review-inspector{max-height:none;height:100%;overflow:auto}.record-first-review.queue-collapsed{grid-template-columns:minmax(560px,1.8fr) minmax(360px,1fr)}.record-first-review.queue-collapsed .records-pane{display:none}.queue-toggle{font-size:.8125rem}
.records-pane{background:var(--soft);border-inline-end:1px solid var(--line)}.records-pane .pane-head{position:sticky;top:0;z-index:4;background:var(--soft);min-height:46px;padding:12px;font-size:.8125rem}
.record-row{min-height:76px;padding:10px 11px;gap:9px}.record-row span> b{font-size:.8125rem}.record-row small{font-size:.8125rem;line-height:1.35}.record-row-status{display:inline-flex;width:max-content;margin-top:5px;padding:2px 7px;border-radius:999px;background:var(--soft);color:var(--accent-fg);font-size:.8125rem;font-weight:800}.record-row-status[data-state="metadata"],.record-row-status[data-state="topology"],.record-row-status[data-state="source"]{background:var(--tone-warn-bg);color:var(--tone-warn-fg)}.record-row-status[data-state="rejected"]{background:var(--tone-danger-bg);color:var(--tone-danger-fg)}.record-row-status[data-state="accepted"]{background:var(--tone-ok-bg);color:var(--tone-ok-fg)}.record-issue-summary{margin-top:3px;color:var(--muted)}
.record-review-pane{display:flex;flex-direction:column;background:var(--card)}.record-review-head{padding:14px 20px}.record-review-head h3{font-size:1.0625rem}.record-review-head p{font-size:.8125rem}.record-primary-text{flex:1;max-width:76ch;width:100%;padding:30px 38px;font:17px/1.72 Georgia,serif}.review-reason{margin:14px 20px 0;font-size:.8125rem;line-height:1.45}.metadata-accept-blocker{margin:0 20px 14px;font-size:.8125rem;line-height:1.45}
.decision-bar{min-height:64px;padding:11px 14px}.decision-bar .btn{min-height:40px;font-size:.8125rem}.decision-actions .primary{min-width:132px;font-weight:800}
.review-inspector{background:var(--card);border-inline-start:1px solid var(--line)}.review-inspector-tabs button{min-height:48px;font-size:0.8125rem}.review-inspector-tabs button span{font-size:.8125rem}.inspector-help{font-size:.8125rem}.source-inspector-head{font-size:.8125rem}.source-block p{font-size:.8125rem;line-height:1.5}.source-block header{font-size:.8125rem}
.build-summary:has(+ .review-session-bar){padding-bottom:4px}
@media(max-width:1350px){.record-first-review{grid-template-columns:minmax(210px,240px) minmax(500px,1.55fr) minmax(320px,.85fr)}}
.source-tool-section{min-width:0;border-top:1px solid var(--line);background:var(--card)}.source-tool-section>summary{display:flex;align-items:center;min-height:44px;padding:10px 12px;font-weight:800;cursor:pointer}.source-tool-section :deep(.boundary-adjudication){border:0;border-radius:0}.source-block,.source-blocks,.source-text-details{min-width:0}.source-block p,.source-block header,.original-extraction-snapshot{overflow-wrap:anywhere;max-width:100%}.review-inspector{overflow-x:hidden}
.builder-message{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;font-size:0.8125rem;line-height:1.45}.builder-message.warning{border-color:var(--tone-warn-border);background:var(--tone-warn-bg);color:var(--tone-warn-fg)}.message-dismiss{flex:0 0 auto;min-width:36px;min-height:36px;border:0;border-radius:8px;background:transparent;color:inherit;font-size:1.25rem;cursor:pointer}.message-dismiss:focus-visible{outline:3px solid var(--accent);outline-offset:2px}

.record-row-wrap{display:grid;grid-template-columns:36px minmax(0,1fr);align-items:stretch;border-bottom:1px solid var(--line)}.record-row-wrap .record-row{border-bottom:0}.record-select{display:grid;place-items:center;min-width:36px;cursor:pointer}.record-select input,.select-visible input{inline-size:18px;block-size:18px;accent-color:var(--accent)}.select-visible{display:flex;align-items:center;gap:6px;font-size:.8125rem!important;font-weight:650;color:var(--muted);cursor:pointer}.review-session-title small{font-size:.8125rem!important;color:var(--muted);font-weight:500;overflow-wrap:anywhere}

/* Accessible type floor for dense Corpus Builder controls. Keep status/meta text readable at zoom and in both locales. */
.corpus-builder small,.corpus-builder label,.corpus-builder summary,.corpus-builder code{font-size:.8125rem!important;line-height:1.4}
.corpus-builder :where(.eyebrow,.builder-setup label,.builder-setup small,.setup-card-heading small,.advanced-config summary,.escalation-field b,.escalation-field small,.launch-copy b,.launch-copy span,.context-blocker,.rail-title b,.rail-title span,.pane-head b,.pane-head span,.build-row b,.build-row small,.summary-top p,.build-status-line,.provenance-strip,.validation-strip,.provider-model-summary,.review-handoff b,.review-handoff span,.review-handoff>strong,.review-toolbar,.source-page-nav,.source-block header,.split-button,.evidence-toggle,.record-row b,.record-row small,.inspector-pane summary,.help,.active-build-settings>summary,.active-build-settings>div,.technical-details>summary,.record-review-head p,.review-stage-summary b,.review-stage-summary>span,.review-inspector-tabs button,.review-inspector-tabs button span,.inspector-help,.review-readonly-banner span,.pager,.record-row-status,.source-inspector-head){font-size:.8125rem!important;line-height:1.4}
.source-head-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap}.source-head-actions .btn{min-height:36px;font-size:.8125rem}
@media(max-width:760px){.source-head-actions{justify-content:flex-start}.source-inspector-head{align-items:flex-start;flex-direction:column}}

/* Feral Fox review and enrichment controls. */
.enrichment-strategy{display:grid;gap:12px}.mode-options{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.mode-options>label,.semantic-index-toggle{display:flex;align-items:flex-start;gap:10px;padding:12px;border:1px solid var(--line);border-radius:10px;background:var(--card);cursor:pointer}.mode-options input,.semantic-index-toggle input,.resolve-source-check input{inline-size:18px;block-size:18px;flex:0 0 auto;margin-top:2px;accent-color:var(--accent)}.mode-options span,.semantic-index-toggle span{display:grid;gap:4px}.mode-options b,.semantic-index-toggle b{font-size:0.875rem}.mode-options small,.semantic-index-toggle small{font-size:.8125rem!important;color:var(--muted)}.mode-options>label:has(input:checked){border-color:var(--accent);box-shadow:0 0 0 2px color-mix(in srgb,var(--accent) 18%,transparent)}.mode-options>label:focus-within,.semantic-index-toggle:focus-within{outline:3px solid color-mix(in srgb,var(--accent) 35%,transparent);outline-offset:2px}.semantic-index-toggle:has(input:disabled){opacity:.7;cursor:not-allowed}.capacity-note{display:block;margin-top:4px;font-size:.8125rem!important;color:var(--muted)}
.record-text-review{display:grid;gap:10px;margin:14px 20px 0;border:1px solid var(--line);border-radius:10px;overflow:hidden;background:var(--card)}.record-text-review>header{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;border-bottom:1px solid var(--line);background:var(--soft)}.record-text-review>header>div{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.record-text-review>header b{font-size:0.875rem}.human-corrected{display:inline-flex;padding:2px 7px;border-radius:999px;background:var(--tone-ok-bg);color:var(--tone-ok-fg);font-size:.8125rem;font-weight:800}.record-text-review .record-primary-text{padding:20px 24px;min-height:220px;max-height:62vh;overflow-y:auto;overscroll-behavior:contain}.record-text-editor{width:calc(100% - 24px);min-height:330px;height:min(62vh,720px);max-height:72vh;overflow-y:auto;overscroll-behavior:contain;margin:0 12px;padding:14px;border:1px solid var(--line);border-radius:8px;background:var(--card);color:var(--text);font:16px/1.65 Georgia,serif;resize:vertical}.record-text-editor:focus-visible{outline:3px solid var(--accent);outline-offset:2px}.text-review-actions{position:sticky;bottom:0;z-index:5;display:grid;gap:10px;padding:10px 12px 12px;border-top:1px solid var(--line);background:var(--card);box-shadow:0 -8px 18px rgb(15 23 42 / .06)}.text-review-actions>div{display:flex;justify-content:flex-end;align-items:center;gap:8px;flex-wrap:wrap}.record-text-head-actions{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.text-save-hint{margin-inline-end:auto;color:var(--muted);font-size:.8125rem;line-height:1.4}.resolve-source-check{display:flex;align-items:flex-start;gap:8px;padding:10px;border:1px solid var(--tone-warn-edge);border-radius:8px;background:var(--tone-warn-bg);color:var(--tone-warn-fg);font-size:.8125rem;line-height:1.45}.original-extraction-snapshot{max-height:320px;overflow:auto;white-space:pre-wrap;padding:12px;margin:10px 12px;border:1px solid var(--line);border-radius:8px;background:var(--soft);font:12px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace}
.rerun-family{display:flex;align-items:center;gap:7px;font-size:.8125rem}.rerun-family .control{min-width:190px;min-height:36px;font-size:.8125rem}.data-actions{flex-wrap:wrap;align-items:center}
.corpus-builder :where(.build-guidance p,.build-guidance small,.manifest-gate p,.segmentation-blocked p,.segmentation-blocked details,.build-warning,.warnings,.metadata-json,.evidence-list b,.evidence-list span,.evidence-list small,.rail-empty,.inspector-empty,.builder-empty){font-size:.8125rem!important;line-height:1.45}
/* 0.48.1 Gifted Grungus — visible review feedback and dedicated metadata workspaces. */
.review-action-feedback{grid-column:1/-1;margin:0;padding:9px 11px;border:1px solid var(--tone-ok-edge);border-radius:9px;background:var(--tone-ok-bg);color:var(--tone-ok-fg);font-size:.8125rem;line-height:1.5;font-weight:650}
.document-metadata-launch{display:flex;justify-content:space-between;align-items:center;gap:14px;margin:12px 14px;padding:12px;border:1px solid var(--line);border-radius:10px;background:var(--soft)}.document-metadata-launch>div{display:grid;gap:3px}.document-metadata-launch b{font-size:.875rem}.document-metadata-launch span{font-size:.8125rem;line-height:1.45;color:var(--muted)}
@media(max-width:760px){.document-metadata-launch{align-items:stretch;flex-direction:column}.document-metadata-launch .btn{width:100%}}


/* 0.59.0 Serious Sandpipers — Corpus Builder workflow composition. */
.builder-setup{container-type:inline-size;display:grid;grid-template-columns:minmax(0,1fr);gap:14px;align-items:start;border:0;background:transparent;border-radius:0;padding:0;max-width:1440px;width:100%;margin-inline:auto}
.setup-section,.setup-phase{min-width:0;border:1px solid var(--line);border-radius:14px;background:var(--card);box-shadow:0 1px 2px rgb(15 23 42 / .035)}
.setup-source-section{display:grid;gap:13px;padding:16px 18px}.setup-section-head>div,.phase-label{display:flex;gap:12px;align-items:flex-start}.setup-section-head h3,.phase-label h3{margin:0;font-size:1rem}.setup-section-head p,.phase-label p{margin:3px 0 0;color:var(--muted);font-size:.875rem;line-height:1.45;max-width:86ch}.setup-step{display:grid;place-items:center;flex:0 0 30px;inline-size:30px;block-size:30px;border-radius:50%;background:var(--soft);border:1px solid var(--line);font-size:.8125rem;font-weight:850}.source-setup-grid{display:grid;grid-template-columns:minmax(320px,1fr) auto;gap:12px;align-items:end}.source-setup-grid label{display:grid;gap:5px}.source-actions{display:flex;gap:8px;flex-wrap:wrap}.source-facts{display:flex;gap:8px 14px;flex-wrap:wrap;color:var(--muted);font-size:.8125rem}.source-facts span{overflow-wrap:anywhere}.setup-phase{display:grid;gap:12px;padding:16px}.phase-label{padding:0 2px}.document-structure-config{border:0!important;box-shadow:none!important;padding:0!important}
.setup-disclosure{overflow:hidden}.setup-disclosure>summary{list-style:none;cursor:pointer;display:flex;align-items:center;gap:12px;min-height:66px;padding:12px 16px}.setup-disclosure>summary::-webkit-details-marker{display:none}.setup-disclosure>summary>span:last-child{display:grid;gap:2px}.setup-disclosure>summary b{font-size:.9375rem}.setup-disclosure>summary small{color:var(--muted);font-size:.8125rem!important;font-weight:500}.setup-disclosure[open]>summary{border-bottom:1px solid var(--line);background:var(--soft)}.setup-disclosure-body{display:grid;gap:16px;padding:16px}.provider-area{padding:0;border:0}.enrichment-strategy{padding-top:2px}.included-feature{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:11px 12px;border:1px solid var(--line);border-radius:10px;background:var(--soft)}.included-feature b{font-size:.875rem}.included-feature span{color:var(--muted);font-size:.8125rem}.execution-wrapper{padding:14px 16px}.setup-section-inline-head{display:flex;gap:12px;align-items:center;margin-bottom:10px}.setup-section-inline-head>span:last-child{display:grid;gap:2px}.setup-section-inline-head b{font-size:.9375rem}.setup-section-inline-head small{font-size:.8125rem!important;color:var(--muted)}.execution-wrapper>.execution-config{border:0}.execution-wrapper :deep(.execution-settings-shell){border:0}.build-launch-row{display:none}
.workspace-switcher{display:flex;gap:6px;align-items:center;flex-wrap:wrap}.workspace-switcher .btn[aria-pressed="true"]{border-color:var(--accent);background:var(--soft);box-shadow:inset 0 -2px 0 var(--accent)}
.record-first-review.detail-mode{grid-template-columns:minmax(220px,260px) minmax(0,1fr)}.record-first-review.detail-mode.queue-collapsed{grid-template-columns:minmax(0,1fr)}.record-first-review.detail-mode .record-review-pane{display:none}.record-first-review.detail-mode .review-inspector{grid-column:auto;border-inline-start:1px solid var(--line);background:var(--card)}.detail-workspace-head{position:sticky;top:0;z-index:7;display:flex;justify-content:space-between;align-items:flex-start;gap:16px;padding:16px 18px;border-bottom:1px solid var(--line);background:var(--card)}.detail-workspace-head h3{margin:2px 0 4px;font-size:1.05rem}.detail-workspace-head p{margin:0;max-width:78ch;color:var(--muted);font-size:.875rem;line-height:1.5}.detail-mode .review-inspector-tabs{top:94px}.detail-mode .metadata-review{padding:16px 18px}.detail-mode .source-review-panel{padding-bottom:18px}.detail-mode .source-review-panel>.source-summary{max-width:1100px;margin-inline:auto}.detail-mode .source-tool-section{max-width:1100px;margin-inline:auto}.detail-mode .review-inspector{overflow-x:hidden}
@container (max-width:760px){.source-setup-grid{grid-template-columns:1fr}.source-actions{display:grid}.source-actions .btn{width:100%}.mode-options{grid-template-columns:1fr}.setup-section-head>div,.phase-label{align-items:flex-start}}
@media(max-width:900px){.source-setup-grid{grid-template-columns:1fr}.source-actions .btn{flex:1 1 200px}.setup-section-head>div,.phase-label{align-items:flex-start}}
@media(max-width:620px){.setup-source-section,.setup-phase,.setup-disclosure-body,.execution-wrapper{padding:13px}.source-actions{display:grid}.source-actions .btn{width:100%}.setup-disclosure>summary{padding:11px 13px}}
@media(max-width:1050px){.workspace-switcher{grid-column:1/-1}.record-first-review.detail-mode{grid-template-columns:1fr}.record-first-review.detail-mode .records-pane{max-height:260px}.detail-mode .review-inspector-tabs{top:0}.detail-workspace-head{position:static}}
/* Review workspace: one frame that fills the screen under the top bar, one scroll per pane. */
.builder-workspace.review-mode{overflow:clip}
.review-frame{display:flex;flex-direction:column;min-width:0;scroll-margin-top:var(--ref-topbar,60px)}
.review-frame .review-toolbar{position:static;flex:0 0 auto;border-radius:12px 12px 0 0;border-bottom:0}
.review-frame .review-grid.record-first-review{position:relative;overflow:hidden;flex:1 1 auto;min-height:0;grid-template-rows:minmax(0,1fr);grid-template-columns:var(--rw-queue,18rem) .5rem minmax(0,1fr) .5rem var(--rw-inspector,25rem);border:1px solid var(--line);border-radius:0 0 12px 12px}
.review-frame .review-grid.queue-collapsed{grid-template-columns:minmax(0,1fr) .5rem var(--rw-inspector,25rem)}
.review-frame .review-grid.detail-mode{grid-template-columns:var(--rw-queue,18rem) .5rem minmax(0,1fr)}
...
.review-splitter::after{content:"";position:absolute;inset-block:calc(50% - 1.5rem);inset-inline:.1875rem;border-radius:2px;background:var(--line-strong)}
.review-splitter:hover::after,.review-splitter:focus-visible::after{background:var(--accent-fg)}
.review-splitter:focus-visible{outline:3px solid var(--focus-ring);outline-offset:-2px}
:global(body.splitter-dragging){cursor:col-resize;user-select:none}
/* Wide and medium screens: the frame fills the screen under the top bar. */
@media (min-width:800px) and (min-height:34rem){.review-frame{position:sticky;top:var(--ref-topbar,60px);height:calc(100dvh - var(--ref-topbar,60px) - .75rem);min-height:30rem}}
...
@media (min-width:800px) and (max-width:1279.98px) and (min-height:34rem){
  .review-frame .review-grid.record-first-review{grid-template-columns:var(--rw-queue,16rem) .5rem minmax(0,1fr);grid-template-rows:minmax(0,1.9fr) minmax(0,1fr)}
  .review-frame .review-grid.queue-collapsed{grid-template-columns:minmax(0,1fr)}
  .review-frame .review-grid .review-splitter[data-splitter="inspector"]{display:none}
  .review-frame .review-inspector{grid-column:1/-1;border-inline-start:0;border-top:1px solid var(--line)}
  .review-frame .review-grid.detail-mode{grid-template-columns:var(--rw-queue,16rem) .5rem minmax(0,1fr);grid-template-rows:minmax(0,1fr)}
  .review-frame .review-grid.detail-mode .records-pane{grid-row:1}
  .review-frame .review-grid.detail-mode .review-inspector{grid-column:3;grid-row:1;border-top:0}
  .review-frame .review-grid.detail-mode.queue-collapsed{grid-template-columns:minmax(0,1fr)}
  .review-frame .review-grid.detail-mode.queue-collapsed .review-inspector{grid-column:1}
}
...
@media (max-width:799.98px),(max-height:33.99rem){
  ...
  .review-frame .review-grid.record-first-review,.review-frame .review-grid.queue-collapsed,.review-frame .review-grid.detail-mode,.review-frame .review-grid.detail-mode.queue-collapsed{overflow:visible;grid-template-rows:auto;grid-template-columns:minmax(0,1fr)}
  .review-frame .records-pane{max-height:18rem;height:auto}
  .review-frame .record-review-pane,.review-frame .review-inspector{height:auto;overflow:visible}
}
/* Below the wide layout the toolbar wraps instead of overflowing. */
@media (max-width:1279.98px){
  .review-frame .review-toolbar{display:flex;flex-wrap:wrap;align-items:center}
  .review-frame .review-toolbar>:first-child{flex:1 1 100%}
  .review-frame .review-toolbar .pager{order:9;margin-inline-start:auto}
  .review-frame .review-toolbar #pdf-corpus-record-search{flex:1 1 12rem}
  .review-frame .review-toolbar .review-bulk{flex-wrap:wrap}
  .review-frame .review-toolbar .review-action-feedback,.review-frame .review-toolbar>:is(section,form,aside){flex:1 1 100%}
}
...
.decision-history .btn{display:inline-flex;align-items:center;gap:.375rem}
.decision-history .btn svg{inline-size:1rem;block-size:1rem}
.flip-inline{transform:scaleX(-1)}
...
@container record (max-width:30rem){
  .decision-history .btn-text{position:absolute;inline-size:1px;block-size:1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}
  .decision-history .btn{padding-inline:.625rem}
}
/* When the pane is tight, More actions shrinks to its dots so the dock stays on one row. */
@container record (max-width:33rem){
  .record-decision-dock :deep(.action-menu-label),.record-decision-dock :deep(.action-menu-caret){position:absolute;inline-size:1px;block-size:1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}
  .record-decision-dock :deep(.action-menu-dots){display:inline-block}
  .record-decision-dock :deep(.action-menu-trigger){min-inline-size:2.5rem;justify-content:center}
  .record-head-actions{flex-wrap:nowrap}
}
@container record (max-width:24rem){
  .record-decision-dock .decision-bar{flex-direction:column-reverse;align-items:stretch}
  .decision-actions{margin-inline-start:0}
  .decision-actions .btn{flex:1 1 0}
  .record-decision-dock .action-menu,.record-decision-dock :deep(.action-menu-trigger){inline-size:100%}
}

/* Queue rows: a state icon (a shape, not only a colour), the id, where it is, and the state once. */
.review-frame .record-row{display:grid;grid-template-columns:1.25rem minmax(0,1fr);gap:.625rem;align-items:start;min-height:0;padding:.625rem .75rem}
.record-state-icon{display:grid;place-items:center;inline-size:1.25rem;block-size:1.25rem;margin-block-start:.0625rem;box-sizing:border-box;border:2px solid var(--muted);border-radius:50%;color:var(--muted)}
.record-state-icon svg{inline-size:.75rem;block-size:.75rem}
.record-state-icon[data-state="accepted"]{border-color:var(--tone-ok-border);background:var(--tone-ok-bg);color:var(--tone-ok-fg)}
.record-state-icon[data-state="rejected"]{border-color:var(--tone-danger-border);background:var(--tone-danger-bg);color:var(--tone-danger-fg)}
.record-state-icon[data-state="metadata"],.record-state-icon[data-state="topology"],.record-state-icon[data-state="source"]{border-color:var(--tone-warn-border);background:var(--tone-warn-bg);color:var(--tone-warn-fg)}
.record-state-icon[data-state="ready"]{border-color:var(--tone-ok-border);color:var(--tone-ok-fg)}
.record-row-main{display:grid;gap:.125rem;min-width:0}
.record-row-main b{overflow-wrap:anywhere}
.record-head-actions{display:flex;flex:0 0 auto;flex-wrap:wrap;align-items:center;justify-content:flex-end;gap:.5rem}
.review-frame .record-review-head>div:first-child{min-width:0;flex:1 1 8rem}
.review-frame .record-review-head h3{overflow-wrap:anywhere}
/* The toolbar is two rows on a wide screen: the queues, then search, workspace and bulk actions. */
.review-frame .review-toolbar{gap:.5rem .75rem;align-items:center;padding:.5rem .75rem}
@media (min-width:1280px){
  .review-frame .review-toolbar{display:grid;grid-template-columns:minmax(10rem,1fr) auto auto}
  .review-frame .review-toolbar>:first-child{grid-column:1/3}
  .review-frame .review-toolbar .pager{grid-column:3;grid-row:1;justify-self:end}
  .review-frame .review-toolbar #pdf-corpus-record-search{grid-column:1}
  .review-frame .review-toolbar .workspace-switcher{grid-column:2}
  .review-frame .review-toolbar .review-bulk{grid-column:3;flex-wrap:nowrap;justify-content:flex-end}
  .review-frame .review-toolbar .review-action-feedback{grid-column:1/-1;margin:0}
  .review-frame .review-toolbar>:is(section,form,aside){grid-column:1/-1}
}
/* The queue header: the title and count, then the select-all control and the hide control. */
.review-frame .records-pane .pane-head{display:grid;grid-template-columns:minmax(0,1fr) auto;grid-template-areas:"title count" "select hide";align-items:center;gap:.25rem .5rem;min-height:0;padding:.5rem .75rem}
.review-frame .records-pane .pane-head>b{grid-area:title}
.review-frame .records-pane .pane-head>span{grid-area:count}
...
.schema-choice{display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end}.schema-choice-field{display:grid;gap:4px;flex:1 1 16rem;font-size:.8125rem;font-weight:700}.schema-choice-field small{color:var(--muted);font-weight:500}
</style>
