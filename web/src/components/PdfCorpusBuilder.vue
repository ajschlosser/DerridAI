...
<template>
  <section class="corpus-builder" :aria-labelledby="'pdf-corpus-builder-title'">
    ...
    <section v-if="showBuildConfiguration" class="builder-setup" :aria-labelledby="'pdf-corpus-config-title'">
      ...
      <DocumentStructureConfigurator v-if="selectedAsset?.pages?.length" class="document-structure-config" :asset="selectedAsset" :pdf-url="sourcePdfUrl" :disabled="busy!==''||Boolean(buildRunning&&currentBuild?.asset_id===selectedAssetId)" @save="saveDocumentLayout" @save-page-labels="savePageLabels" />
        ...
        <template v-if="showReviewWorkspace">
          ...
          <section class="review-grid record-first-review" :aria-busy="recordsLoading">
            <nav ref="recordListEl" class="records-pane" tabindex="-1" :aria-labelledby="'pdf-corpus-records-pane'">
              <div class="pane-head"><b id="pdf-corpus-records-pane">{{i18n.t('pdf_corpus.review_queue','Review queue')}}</b><label class="select-visible"><input type="checkbox" :checked="allVisibleSelected" :disabled="!records.length||busy!==''" @change="toggleVisibleSelection(($event.target as HTMLInputElement).checked)"><span>{{i18n.t('pdf_corpus.select_visible','Select visible')}}</span></label><span>{{recordTotal}}</span></div>
            ...
            <aside ref="reviewInspectorEl" class="review-inspector" :aria-label="i18n.t('pdf_corpus.review_details','Review details')">
              ...
              <section v-else-if="selectedRecord" id="review-panel-source" class="review-inspector-panel source-review-panel" role="tabpanel" aria-labelledby="review-tab-source" tabindex="0">
                ...
                <CorpusBoundaryAdjudication :record="selectedRecord" :can-previous="canMergePrevious" :can-next="canMergeNext" :busy="busy!==''" :profiles="providerProfiles" :provider-profile-id="llmActionProviderId||selectedProviderId" :model-override="llmActionModel" :concurrency-risk="Boolean(providerProfiles.find(p=>p.id===(llmActionProviderId||selectedProviderId))?.type==='ollama'&&llmActionConcurrentLoad+1>Number(providerProfiles.find(p=>p.id===(llmActionProviderId||selectedProviderId))?.max_concurrent_requests||1))" :active-requests="llmActionConcurrentLoad" :concurrency-limit="Number(providerProfiles.find(p=>p.id===(llmActionProviderId||selectedProviderId))?.max_concurrent_requests||1)" @update:provider-profile-id="value=>llmActionProviderId=value" @update:model-override="value=>llmActionModel=value" @adjudicate="adjudicateBoundary" />
...
.records-pane,.record-review-pane,.review-inspector{max-height:none;height:100%;overflow:auto}
