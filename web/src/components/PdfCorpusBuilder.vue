...
function restoreBuilderDraft(){
  try{
    ...
    if(typeof draft.selectedProviderId==="string")selectedProviderId.value=draft.selectedProviderId;
    if(typeof draft.selectedReviewProviderId==="string")selectedReviewProviderId.value=draft.selectedReviewProviderId;
...
const hasRecordTopology=computed(()=>Boolean(currentBuild.value && !awaitingManifestReview.value && Number(currentBuild.value.record_count||0)>0));
...
const providerPayload=computed<Record<string,unknown>>(()=>{
  ...
  if(selectedProviderId.value){
    ...
    if(serverProviderIds.value.has(selectedProviderId.value)){
      for(const key of ["provider","model","base_url","api_key"])delete payload[key];
...
async function refreshRecords(reset=false, preferredId=""){if(reset)recordOffset.value=0;if(!selectedBuildId.value){records.value=[];recordTotal.value=0;selectedRecord.value=null;return}const result=await pdfCorpusApi.records(selectedBuildId.value,recordOffset.value,pageSize,reviewOnly.value,recordQuery.value);records.value=result.items;recordTotal.value=result.total;const wanted=preferredId||selectedRecordId.value;const match=wanted?records.value.find(row=>row.record_id===wanted):undefined;if(match){selectRecord(match);return}if(records.value[0]){selectRecord(records.value[0]);return}selectedRecordId.value="";selectedRecord.value=null;sourceBlocks.value=[]}
...
async function setDisposition(disposition:"pending"|"accepted"|"rejected"){if(!currentBuild.value||!selectedRecord.value)return;const id=selectedRecord.value.record_id;busy.value="record";try{await pdfCorpusApi.disposition(currentBuild.value.build_id,id,disposition,"",Number(selectedRecord.value.record_revision||1));await refreshBuild();await refreshRecords(false);if(disposition!=="pending")await advanceFrom(id);setMessage(disposition==="accepted"?i18n.t("pdf_corpus.accepted_notice","Record accepted. Advanced to the next record."):disposition==="rejected"?i18n.t("pdf_corpus.rejected_notice","Record rejected. Advanced to the next record."):i18n.t("pdf_corpus.reopened_notice","Record reopened for review."))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
...
async function bulkDisposition(disposition:"accepted"|"rejected"){if(!currentBuild.value)return;const count=recordTotal.value;if(!count)return;const verb=disposition==="accepted"?i18n.t("pdf_corpus.accept_all","Accept all"):i18n.t("pdf_corpus.reject_all","Reject all");if(!window.confirm(i18n.tf("pdf_corpus.bulk_confirm","{action} {count} record(s) matching the current filter?",{action:verb,count})))return;busy.value="bulk";try{const result=await pdfCorpusApi.bulkDisposition(currentBuild.value.build_id,disposition,reviewOnly.value?true:null,recordQuery.value);await refreshBuild();await refreshRecords(true);setMessage(i18n.tf("pdf_corpus.bulk_done","Updated {count} record(s).",{count:result.changed}))}catch(exc){setMessage(exc instanceof Error?exc.message:String(exc),"error")}finally{busy.value=""}}
...
<template>
  <section class="corpus-builder" :aria-labelledby="'pdf-corpus-builder-title'">
    ...
    <CorpusWorkflowStepper :stage="currentBuild?.stage||''" :status="currentBuild?.status||''" :has-asset="Boolean(selectedAssetId)" :has-manifest="Boolean(currentBuild?.manifest&&Object.keys(currentBuild.manifest).length)" />
      ...
      <main class="build-main">
        ...
        <section v-if="hasRecordTopology" class="review-grid">
          ...
          <section ref="recordListEl" class="records-pane" :aria-labelledby="'pdf-corpus-records-pane'">
            ...
            <div v-if="!records.length" class="rail-empty">{{i18n.t('pdf_corpus.no_records_filter','No records match this review filter.')}} <button type="button" class="btn small" @click="reviewOnly=false;recordQuery=''">{{i18n.t('pdf_corpus.show_all_records','Show all records')}}</button></div>
          </section>
