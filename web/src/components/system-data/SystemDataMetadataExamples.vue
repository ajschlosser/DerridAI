  ...
  <div class="metadata-workspace">
        ...
        <p>
          {{
            t(
              ...
              "Evidence-bound examples derived from reviewed corpus metadata. This projection is derived and rebuildable; it is not an ordinary corpus collection.",
            )
          }}
        </p>
      </div>
    ...
    <form class="filters" @submit.prevent="load(0)">
      <label
        ...
        ><select v-model="filters.field" class="control">
          <option value="">{{ t("runtime.system_all", "All") }}</option>
          <option v-for="item in page.facets.fields" :key="item">{{ item }}</option>
        </select></label
      >
      <label
        ><span>{{ t("runtime.system_exemplar_kind", "Kind") }}</span
        ><select v-model="filters.kind" class="control">
          <option value="">{{ t("runtime.system_all", "All") }}</option>
          <option v-for="item in page.facets.kinds" :key="item">{{ item }}</option>
        </select></label
      >
      <label
        ><span>{{ t("runtime.system_exemplar_language", "Language") }}</span
        ><select v-model="filters.language" class="control">
          <option value="">{{ t("runtime.system_all", "All") }}</option>
          <option v-for="item in page.facets.languages" :key="item">{{ item }}</option>
        </select></label
      >
      <label
        ><span>{{ t("runtime.system_exemplar_build", "Build") }}</span
        ><select v-model="filters.scope_id" class="control">
          <option value="">{{ t("runtime.system_all", "All") }}</option>
          <option v-for="item in page.facets.scopes" :key="item">{{ item }}</option>
        </select></label
      >
      <label
        ><span>{{ t("runtime.system_exemplar_schema", "Schema") }}</span
        ><select v-model="filters.schema_id" class="control">
          <option value="">{{ t("runtime.system_all", "All") }}</option>
          <option v-for="item in page.facets.schemas" :key="item">{{ item }}</option>
        </select></label
      >
      <label
        ><span>{{ t("runtime.system_exemplar_record", "Record ID") }}</span
        ><input v-model="filters.record_id" class="control" type="search"
      /></label>
      <div class="filter-actions">
        <button class="btn" type="submit">{{ t("common.apply", "Apply") }}</button
        ><button class="btn" type="button" @click="clearAll">
          {{ t("runtime.system_clear_all", "Clear all") }}
        </button>
      </div>
    </form>
    ...
    <aside v-if="detail" class="detail-panel" aria-label="Metadata example details">
      <header>
        <div>
          ...
          <h3>{{ detail.field_name }} → {{ valueText(detail.field_value) }}</h3>
        </div>
        <button
          class="icon-btn"
          type="button"
      ...
      <dl>
        <div>
          <dt>{{ t("runtime.system_status", "Status") }}</dt>
          <dd>{{ detail.assertion_status || detail.kind }}</dd>
        </div>
        <div>
          <dt>{{ t("runtime.system_source_record", "Source record") }}</dt>
          <dd>{{ detail.record_id }}</dd>
        </div>
        <div>
          <dt>{{ t("runtime.system_source_revision", "Source revision") }}</dt>
          <dd>{{ detail.record_revision ?? "—" }}</dd>
        </div>
        <div>
          <dt>{{ t("runtime.system_build_scope", "Build / scope") }}</dt>
          <dd>{{ detail.scope_id || "—" }}</dd>
        </div>
        <div>
          <dt>{{ t("runtime.system_exemplar_schema", "Schema") }}</dt>
          <dd>{{ detail.schema_id || "—" }} {{ detail.schema_version || "" }}</dd>
        </div>
        <div>
          <dt>{{ t("runtime.system_exemplar_region", "Region") }}</dt>
          <dd>{{ detail.region_type || "—" }}</dd>
        </div>
        <div>
          <dt>{{ t("runtime.system_exemplar_evidence_blocks", "Evidence blocks") }}</dt>
          <dd>{{ (detail.evidence_block_ids || []).join(", ") || "—" }}</dd>
        </div>
        <div>
          <dt>{{ t("runtime.system_exemplar_evidence_hash", "Evidence hash") }}</dt>
          <dd class="mono">{{ detail.evidence_hash || "—" }}</dd>
        </div>
      </dl>
      <section>
        <h4>{{ t("runtime.system_exemplar_context", "Evidence context") }}</h4>
