...
const usable = computed(() =>
  props.models.filter((model) => model.name && modelMatchesKind(model.name, props.kind)),
);
const shown = computed(() => {
  const needle = query.value.trim().toLocaleLowerCase();
  return usable.value.filter((model) => String(model.name).toLocaleLowerCase().includes(needle));
});
const detail = (model: DiscoveredModel) =>
  [model.parameter_size, model.quantization_level].filter(Boolean).join(" · ") ||
  i18n.t("providers.model_no_details");
function openList() {
  query.value = "";
  open.value = true;
  ...
  <div class="provider-model-field">
    ...
    <div class="picker-row">
      ...
      <datalist :id="datalistId">
        <option v-for="model in usable" :key="model.name" :value="model.name"></option>
      </datalist>
      <UiButton
        :label="
          usable.length
            ? i18n.tf('providers.view_models_count', { count: usable.length })
            : i18n.t('providers.view_models')
        "
        :disabled="busy"
        @click="openList"
      />
    ...
    <UiDialog
      v-if="open"
      size="medium"
      :title="i18n.t('providers.available_models')"
      :description="
        i18n.tf('providers.models_discovered', { name: profileName, count: usable.length })
      "
    ...
    >
      <div class="model-search">
        ...
        <input
          ...
          autofocus
        />
        <p class="model-count" role="status" aria-live="polite">
          {{ i18n.tf("providers.models_match", { count: shown.length }) }}
        </p>
      </div>
      ...
      <ul
        ...
        class="model-list"
        :aria-label="i18n.t('providers.available_models')"
      >
        <li v-for="model in shown" :key="model.name">
          <button
            type="button"
            class="model-row"
            :aria-current="model.name === modelValue ? 'true' : undefined"
            @click="choose(String(model.name))"
          >
            <span class="model-row-main"
              ><b>{{ model.name }}</b
              ><small>{{ detail(model) }}</small></span
            >
            <span class="model-row-use">{{
              model.name === modelValue
                ? i18n.t("providers.model_in_use")
                : i18n.t("providers.use_model")
            }}</span>
          </button>
        </li>
      </ul>
