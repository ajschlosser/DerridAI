...
const overviewFields=['document_author','edition','year','publication_year','publisher','translator','document_language','original_language','region_type','region_author','primary_text','needs_review','review_reason'];
...
function present(value:unknown){if(value===null||value===undefined||value==='')return '—';if(Array.isArray(value))return value.length?value.join(', '):'—';if(typeof value==='boolean')return value?i18n.t('legacy.yes','Yes'):i18n.t('legacy.no','No');if(typeof value==='object')return JSON.stringify(value);return String(value)}
  ...
  <aside class="record-inspector" :aria-label="i18n.t('record.inspector','Record inspector')">
    ...
    <div class="record-inspector-body">
      <section v-if="tab==='overview'" :id="panelId('overview')" class="record-overview-panel" role="tabpanel" :aria-labelledby="tabId('overview')">
        ...
        <div class="record-page-context"><span>{{i18n.t('record.page','Page')}}</span><strong>{{pageSpan}}</strong></div>
        <dl class="record-meta-list">
          <div v-for="key in overviewFields" :key="key"><dt>{{fieldLabel(key)}}</dt><dd><button v-if="searchable(key,record[key])" type="button" @click="emit('search',key,String(record[key]))">{{present(record[key])}}</button><span v-else>{{present(record[key])}}</span></dd></div>
        </dl>
...
<style scoped>
.record-inspector{position:sticky;top:92px;max-height:calc(100dvh - 112px);min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr);border:1px solid #dfe6e2;border-radius:16px;background:#fff;box-shadow:0 8px 24px rgba(15,23,42,.045);overflow:hidden}.record-inspector-tabs{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:4px;padding:7px;border-bottom:1px solid #e4e9e6;background:#fafbfa}.record-inspector-tabs button{position:relative;min-width:0;min-height:38px;display:flex;align-items:center;justify-content:center;gap:5px;border:1px solid transparent;border-radius:8px;background:transparent;padding:5px 7px;color:#66758a;font-size:11px;font-weight:780;line-height:1.15;text-align:center;white-space:normal;cursor:pointer}.record-inspector-tabs button:hover{background:#fff;border-color:#e3e9e5}.record-inspector-tabs button.active{border-color:#cadbd1;background:#fff;color:#2e4a3b;box-shadow:0 1px 4px rgba(15,23,42,.05)}.record-inspector-tabs button>span{min-width:18px;height:18px;display:grid;place-items:center;border-radius:999px;background:#eaf0ec;font-size:9px}.record-inspector-body{min-height:0;overflow-x:hidden;overflow-y:auto;overscroll-behavior:auto;padding:15px 16px 18px;scrollbar-gutter:stable}.record-overview-panel,.record-indexing-panel{display:grid;gap:17px}.record-page-context{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 11px;border:1px solid #dde6e0;border-radius:10px;background:#f8faf9}.record-page-context span{color:#6a788a;font-size:11.5px;font-weight:750}.record-page-context strong{color:#304237;font-size:13px;font-variant-numeric:tabular-nums}.inspector-section-head p{margin:0;color:#64748b;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em}.inspector-section-head h2{margin:2px 0 0;color:#2c3e53;font-size:17px}.record-meta-list{display:grid;margin:0}.record-meta-list>div{display:grid;grid-template-columns:minmax(100px,.68fr) minmax(0,1fr);gap:10px;padding:9px 0;border-bottom:1px solid #edf1ef}.record-meta-list dt{color:#6a788a;font-size:11.5px}.record-meta-list dd{margin:0;min-width:0;color:#304237;font-size:12.5px;font-weight:700;overflow-wrap:anywhere}.record-meta-list dd button{max-width:100%;border:0;background:transparent;padding:0;color:#2d5942;font:inherit;text-align:left;cursor:pointer;text-decoration:underline;text-decoration-color:#bdd3c6;text-underline-offset:2px;overflow-wrap:anywhere}.quoted-meta,.citation-preview{display:grid;gap:9px;padding-top:2px}.quoted-meta h3,.citation-preview h3{margin:0;color:#33475d;font-size:13px}.citation-preview>div{padding:10px;border:1px solid #e5eae7;border-radius:10px;background:#fafcfb}.citation-preview small{color:#748195;font-size:10.5px;font-weight:800;text-transform:uppercase}.citation-preview p{margin:4px 0 0;color:#34453a;font:12.5px/1.5 Georgia,"Times New Roman",serif;overflow-wrap:anywhere}.record-indexing-panel :deep(.record-index-terms+.record-index-terms){padding-top:15px;border-top:1px solid #edf1ef}button:focus-visible{outline:3px solid color-mix(in srgb,var(--ui-accent,#3c8d62) 42%,#fff);outline-offset:2px}
