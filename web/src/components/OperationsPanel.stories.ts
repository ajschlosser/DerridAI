...
const mixed = [
  op({
    ...
    total: 307,
    completed: 92,
    progressLabel: "30% overall",
    facts: [
      { name: "records", value: "60" },
      { name: "need review", value: "21" },
    ],
  }),
  op({
    id: "u",
  ...
  op({
    ...
    status: "failed",
    result: null,
    error: "Provider unreachable: connection refused (http://host.docker.internal:11434)",
    startedAt: iso(-3690),
    finishedAt: iso(-3600),
  }),
  op({
  ...
  op({
    ...
    status: "cancelled",
    result: null,
    finishedAt: iso(-199000),
    startedAt: iso(-200000),
  }),
];
    ...
    bridge: bridgeFor(
      Array.from({ length: 5 }, (_, i) =>
        op({
          ...
          type: "llm",
          icon: "edit",
          label: `Auto-improve ${i + 1}`,
          status: i === 4 ? "queued" : "running",
          finishedAt: null,
          startedAt: i === 4 ? null : iso(-120 - i * 40),
          result: { kind: "review-partial" },
          total: 50,
          completed: 10 + i * 8,
          progressLabel: `${10 + i * 8} of 50`,
        }),
      ),
    ),
  },
...
export const FailuresOnly: Story = {
  args: {
    bridge: bridgeFor([
      op({ id: "f1", status: "failed", result: null, error: "Provider unreachable" }),
      op({
        id: "f2",
        type: "pdf_corpus",
        icon: "pdf",
        ...
        status: "blocked",
        result: { kind: "build" },
        subtitle: "Segmentation needs a decision",
      }),
    ]),
  },
};
  ...
  args: {
    bridge: bridgeFor([
      op({
        ...
        icon: "pdf",
        label:
          "PDF corpus build of a book with an extraordinarily long title that must wrap without clipping or overflowing the card",
        status: "running",
        finishedAt: null,
        result: null,
        subtitle:
  ...
  args: {
    bridge: bridgeFor(
      Array.from({ length: 14 }, (_, i) =>
        op({
          id: `h${i}`,
          label: `Languages · ${["Deutsch", "Español", "Italiano", "Nederlands"][i % 4]}`,
          finishedAt: iso(-3000 - i * 40000),
          startedAt: iso(-3100 - i * 40000),
        }),
      ),
    ),
  },
