...
const activeId = computed(() => props.tabs.some(tab => tab.id === props.modelValue) ? props.modelValue : props.tabs[0]?.id || "");
...
watch(activeId, id => { if (id && id !== props.modelValue) emit("update:modelValue", id); });
</script>
<template>
