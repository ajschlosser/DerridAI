...
<style scoped>
.ui-dialog-backdrop{position:fixed;inset:0;z-index:12000;display:grid;place-items:center;padding:24px;background:var(--scrim);backdrop-filter:blur(2px)}
.ui-dialog{width:min(760px,calc(100vw - 32px));max-height:min(92vh,960px);display:grid;grid-template-rows:auto minmax(0,1fr) auto;overflow:hidden;border:1px solid var(--border-strong);border-radius:var(--radius-overlay);background:var(--surface-overlay);color:var(--text-primary);box-shadow:var(--shadow-overlay)}
.ui-dialog[data-size="large"]{width:min(980px,calc(100vw - 32px))}.ui-dialog[data-size="xlarge"]{width:min(1180px,calc(100vw - 32px))}
.ui-dialog-header{display:flex;justify-content:space-between;align-items:flex-start;gap:20px;padding:20px 22px;border-bottom:1px solid var(--border-subtle);background:var(--surface-overlay)}
.ui-dialog-heading{min-width:0;display:grid;gap:6px}.ui-dialog-heading h2{margin:0;font-size:1.25rem;line-height:1.25;overflow-wrap:anywhere}.ui-dialog-heading p{max-width:72ch;margin:0;color:var(--text-tertiary);font-size:.875rem;line-height:1.5}
.ui-dialog-body{min-height:0;overflow:auto;padding:20px 22px;overscroll-behavior:contain;background:var(--surface-overlay)}
.ui-dialog-footer{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 22px;border-top:1px solid var(--border-subtle);background:var(--surface-overlay)}
.ui-dialog :deep(:focus-visible){outline:var(--focus-ring-width) solid var(--focus-ring,var(--accent));outline-offset:var(--focus-ring-offset)}
