...
const emit = defineEmits<{
  apply: [payload: { changes: Record<string, unknown>; applyToAll: boolean }];
  close: [];
}>();
...
const legacyGroups: BulkGroup[] = [
  {
    key: "discourse",
    label: "pdf_corpus.bulk_group_discourse",
    fallback: "Discourse & attribution",
    help: "pdf_corpus.bulk_group_discourse_help",
    helpFallback: "Who is speaking, what the passage does, and what it is about.",
    fields: [
      "region_type",
      "region_author",
      "speaker",
  ...
  {
    key: "indexing",
    label: "pdf_corpus.bulk_group_indexing",
    fallback: "Semantic indexing",
    help: "pdf_corpus.bulk_group_indexing_help",
    helpFallback: "Topics, concepts, people and works the passages mention.",
    fields: ["topics", "concepts", "persons", "works_referenced"],
  },
  {
    key: "identity",
    label: "pdf_corpus.bulk_group_identity",
    fallback: "Document details on these records",
    help: "pdf_corpus.bulk_group_identity_help",
    helpFallback:
      "These are inherited from Edit document metadata. Change them here only to override them on the chosen records.",
    fields: [
      "work",
      "document_title",
      "document_author",
...
const filteredGroups = computed(() => {
  ...
  return groups.value
    .map((group) => ({
      ...group,
      fields: group.fields.filter((field) => name(field).toLowerCase().includes(q)),
    }))
...
function valueFor(field: string): unknown {
  ...
  if (booleanFields.value.has(field))
    return raw === "true" ? true : raw === "false" ? false : undefined;
  if (!raw) return undefined;
  if (listFields.value.has(field))
    return raw
      .split(/[,\n]/)
      .map((v) => v.trim())
      .filter(Boolean);
...
function apply() {
  if (activeCount.value) emit("apply", { changes: changes.value, applyToAll: applyToAll.value });
}
function reset() {
  ...
  <Teleport to="body"
    ><div class="bulk-backdrop" @mousedown.self="emit('close')">
      <form
        ...
        aria-describedby="bulk-metadata-help"
      ...
      >
        ...
        <div class="bulk-scroll">
          ...
          <MetadataFormSection
            ...
            :key="group.key"
            :title="i18n.t(group.label, group.fallback)"
            :description="i18n.t(group.help, group.helpFallback)"
          >
            <UiField v-for="field in group.fields" :key="field" :label="name(field)">
              <select
                v-if="enumValues[field]?.length"
              ...
              <select
                v-else-if="booleanFields.has(field)"
                v-model="values[field]"
                class="control"
                :disabled="disabled || cleared[field]"
              >
              ...
              <template v-else
                ><input
                  ...
                  /></datalist
              ></template>
              <span class="bulk-clear"
                ><input
                  :id="`bulk-clear-${field}`"
                  v-model="cleared[field]"
                  type="checkbox"
                  :disabled="disabled"
...
.bulk-dialog {
  ...
  background: var(--bg);
  color: var(--text);
  box-shadow: 0 32px 100px rgb(0 0 0 / 0.38);
}
.bulk-head {
  display: flex;
