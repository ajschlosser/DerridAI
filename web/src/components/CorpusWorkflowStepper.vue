...
const props=defineProps<{stage?:string;status?:string;published?:boolean;hasAsset?:boolean;hasManifest?:boolean;acceptedCount?:number;recordCount?:number}>();
...
<style scoped>.workflow{border:1px solid var(--line);border-radius:12px;background:var(--card);padding:9px 12px}.workflow ol{list-style:none;margin:0;padding:0;display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px}.workflow li{min-width:0;display:flex;align-items:center;gap:8px;padding:6px 8px;border-radius:9px;color:var(--muted)}.workflow li[data-state="current"]{background:var(--soft);color:var(--text)}.workflow li[data-state="complete"]{color:var(--text)}.marker{display:grid;place-items:center;inline-size:24px;block-size:24px;flex:0 0 24px;border:1px solid var(--line);border-radius:50%;font-size:.8125rem;font-weight:800}.workflow li[data-state="current"] .marker{border-color:var(--accent);box-shadow:0 0 0 2px color-mix(in srgb,var(--accent) 15%,transparent)}.step-copy{display:grid;gap:1px;min-width:0}.step-copy b{font-size:.8125rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.step-copy small{font-size:.8125rem;color:var(--muted)}@media(max-width:820px){.workflow ol{grid-template-columns:1fr 1fr}}@media(max-width:480px){.workflow ol{grid-template-columns:1fr}}</style>
