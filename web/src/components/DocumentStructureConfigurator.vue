...
// field so the reviewer can check the clues and save it; a page the reviewer already confirmed is never replaced.
type StartInference = {
  page: number | null;
  confidence: number;
...
const extractionSummary = computed(() => {
  ...
  const regions = new Set(pages.map((page) => page.deterministic_region_type).filter(Boolean));
  const threads = new Set(pages.flatMap((page) => page.thread_ids || []));
  return { methods: [...methods], regions: [...regions], threads: threads.size };
});
        ...
        <div class="page-nav">
          ...
          ><label
            ...
            ><input
              ...
              :value="current"
              @change="setPage(Number(($event.target as HTMLInputElement).value))" /></label
      ...
      <section class="rules">
        <fieldset>
          ...
          <label class="radio-row"
            ...
            <span>{{ i18n.t("pdf_corpus.single_page_layout") }}</span></label
          ><label class="radio-row"
            ><input v-model="draft.page_layout" type="radio" value="two_up" />
            <span>{{ i18n.t("pdf_corpus.two_up_layout") }}</span></label
