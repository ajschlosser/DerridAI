...
const determinate = computed(
  () => active.value && props.view.total > 0 && props.view.status !== "queued",
);
...
const statusText = computed(() =>
  i18n.t(
    `operations.panel.status_${props.view.status}`,
    STATUS_FALLBACK[props.view.status] ?? props.view.status,
  ),
);
...
<template>
  <li
    class="ops-row"
    :class="[`tone-${tone}`, { 'is-active': active, 'is-fresh': fresh }]"
    :data-op-id="view.id"
  ...
  >
    ...
    <div class="ops-main">
      <div class="ops-heading">
        ...
        <span class="ops-status" :class="`tone-${tone}`">
          <svg
            v-if="view.status === 'completed'"
            class="ops-status-icon ops-check"
            viewBox="0 0 16 16"
            aria-hidden="true"
          >
          ...
          <span
            v-else-if="active"
            class="ops-status-icon ops-spinner"
            :class="{ 'is-queued': view.status === 'queued' }"
            aria-hidden="true"
          ></span>
          <svg
            v-else-if="view.status === 'failed' || view.status === 'blocked'"
            class="ops-status-icon"
            viewBox="0 0 16 16"
            aria-hidden="true"
          >
            <path d="M8 2.5l6 10.5H2z" />
            <path d="M8 7v3M8 11.6v.1" />
          </svg>
      ...
      <p class="ops-meta">
        <time
          ...
          :title="absoluteTime(whenIso, locale)"
          >{{ whenText }}</time
      ...
      <dl v-if="view.facts.length" class="ops-facts">
        <div v-for="fact in view.facts" :key="fact.name">
          <dt>{{ fact.name }}</dt>
          <dd>{{ fact.value }}</dd>
        </div>
      </dl>
    ...
    <div class="ops-actions">
      <button
        v-if="view.result"
        type="button"
        class="ops-btn is-primary"
        data-primary
        :aria-label="resultAria"
        @click="emit('result', view.id)"
      >
        {{ resultLabel }}
      </button>
      <button
