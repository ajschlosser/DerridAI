...
const steps=computed(()=>[
  {id:"extraction",label:i18n.t("pdf_corpus.pipeline.extract"),state:stateFor("extraction",Number(props.build.source_block_count||0)>0?"complete":"waiting"),detail:i18n.tf("pdf_corpus.pipeline.extract_detail_count", {count:Number(props.build.source_block_count||0)})},
  {id:"construction",label:i18n.t("pdf_corpus.pipeline.construct"),state:stateFor("construction",Number(props.build.record_count||0)>0?"complete":"waiting"),detail:i18n.tf("pdf_corpus.pipeline.records_created", {count:Number(props.build.record_count||0)})},
  {id:"enrichment",label:i18n.t("pdf_corpus.pipeline.enrich"),state:stateFor("enrichment",metadataRemaining.value===0?"complete":"attention"),detail:metadataRemaining.value?i18n.tf("pdf_corpus.pipeline.metadata_remaining", {count:metadataRemaining.value}):i18n.t("pdf_corpus.pipeline.metadata_complete")},
  {id:"review",label:i18n.t("pdf_corpus.pipeline.review"),state:stateFor("review",reviewed.value>=Number(props.build.record_count||0)&&Number(props.build.record_count||0)>0?"complete":"attention"),detail:i18n.tf("pdf_corpus.pipeline.review_progress", {done:reviewed.value,total:Number(props.build.record_count||0)})},
  {id:"validation",label:i18n.t("pdf_corpus.pipeline.validate"),state:stateFor("validation",props.build.validation?.valid?"complete":"blocked"),detail:props.build.validation?.valid?i18n.t("pdf_corpus.pipeline.validation_passed"):i18n.t("pdf_corpus.pipeline.validation_wait")},
  {id:"publication",label:i18n.t("pdf_corpus.pipeline.publish"),state:stateFor("publication",props.build.publication?"complete":readiness.value.can_publish?"active":"blocked"),detail:props.build.publication?i18n.t("pdf_corpus.pipeline.published"):readiness.value.can_publish?i18n.t("pdf_corpus.pipeline.ready_publish"):i18n.t("pdf_corpus.pipeline.publish_wait")},
]);
...
<template>
  <section class="lifecycle-card" aria-labelledby="corpus-lifecycle-title">
    <header><div><span class="eyebrow">{{i18n.t('pdf_corpus.workflow')}}</span><h3 id="corpus-lifecycle-title">{{i18n.t('pdf_corpus.pipeline_title')}}</h3></div><strong>{{Math.round(Number(build.progress||0)*100)}}%</strong></header>
    ...
    <div class="next-action" role="status"><b>{{i18n.t('pdf_corpus.next_step')}}</b><span>{{nextLabel}}</span></div>
  </section>
</template>
<style scoped>
