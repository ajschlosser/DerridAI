...
<style scoped>
.hf-report { display: grid; gap: 8px; padding: 12px 14px; border: 1px solid var(--tone-info-border); border-radius: 12px; background: var(--tone-info-bg); color: var(--text); }
h3 { margin: 0; font-size: 0.9375rem; color: var(--tone-info-fg); }
p { margin: 0; font-size: 0.875rem; line-height: 1.5; }
ul { display: grid; gap: 4px; margin: 0; padding: 0; list-style: none; }
.hf-exceptions li { display: flex; flex-wrap: wrap; gap: 4px 10px; align-items: baseline; padding: 6px 10px; border: 1px solid var(--line); border-radius: 8px; background: var(--card); font-size: 0.8125rem; }
.hf-exceptions span { color: var(--text-2); }
.link-button { border: 0; background: none; padding: 0; color: var(--accent-fg); font: inherit; font-weight: 700; text-decoration: underline; cursor: pointer; }
.link-button:focus-visible { outline: 3px solid var(--focus-ring); outline-offset: 2px; }
.hf-notes { color: var(--text-2); font-size: 0.8125rem; }
.hf-more { color: var(--muted); font-size: 0.8125rem; }
</style>
