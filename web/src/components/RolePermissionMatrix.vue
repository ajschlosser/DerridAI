...
const visible = computed(() => props.capabilities.filter(item => matchesCapabilityFilter({
  id: item.id,
...
<style scoped>
.permission-matrix{display:grid;gap:12px}
.permission-empty{margin:0;padding:18px 4px;color:var(--muted);font-size:.875rem}
.permission-group{margin:0;padding:0;border:1px solid var(--line);border-radius:var(--radius);background:var(--panel);display:grid;gap:4px}
.permission-legend{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px 6px;font-size:.8125rem;font-weight:800;color:var(--text-2)}
.permission-legend-copy{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap}
.permission-legend-count{font-weight:650;color:var(--muted)}
.permission-row{display:grid;grid-template-columns:22px minmax(0,1fr);gap:10px;align-items:start;min-height:52px;margin:0 8px 8px;padding:10px;border:1px solid transparent;border-radius:var(--radius-sm);cursor:pointer}
.permission-row:hover{background:var(--panel-2);border-color:var(--line)}
.permission-row.locked{cursor:not-allowed;color:var(--muted)}
.permission-row input{margin-top:4px;width:17px;height:17px;accent-color:var(--ui-accent)}
.permission-copy{display:grid;gap:4px;min-width:0}
.permission-title{display:flex;flex-wrap:wrap;align-items:center;gap:8px}
.permission-row b{font-size:.875rem;color:var(--text)}
.permission-row.locked b{color:var(--muted)}
.permission-row small{font-size:.8125rem;line-height:1.45;color:var(--muted)}
.permission-row code{width:fit-content;margin-top:2px;padding:2px 6px;border-radius:var(--radius-sm);background:var(--soft);font-size:.8125rem;color:var(--muted)}
.permission-row:focus-within{outline:3px solid var(--focus-ring);outline-offset:2px}
@media (max-width:640px){
  .permission-legend{flex-direction:column;align-items:flex-start}
}
</style>
