...
<template>
  <div class="pdf-workspace-native">
    ...
    <RuntimeSurface v-else />
  </div>
</template>

<style scoped>
.pdf-mode-tabs{display:flex;align-items:center;gap:5px;padding:9px 20px;border-bottom:1px solid var(--line);background:color-mix(in srgb,var(--card) 97%,transparent);position:sticky;top:0;z-index:20;backdrop-filter:blur(12px)}.pdf-mode-tabs button{border:1px solid transparent;background:transparent;padding:8px 12px;border-radius:9px;font-size:0.8125rem;font-weight:750;color:var(--muted);cursor:pointer}.pdf-mode-tabs button:hover{background:var(--soft);color:var(--text)}.pdf-mode-tabs button.active{background:var(--accent-soft,#eef6f2);border-color:var(--accent-soft-2,#dce9e3);color: var(--accent-fg);box-shadow:inset 0 -2px 0 var(--accent)}.pdf-mode-tabs button:focus-visible{outline:3px solid var(--accent);outline-offset:2px}.pdf-mode-tabs span{margin-inline-start:auto;color:var(--muted);font-size:.8125rem}
