...
const trackedFallbackKeys = computed(() => new Set((current.value?.translation_report?.failed_keys || []).map(String)));
const trackedFallbackCount = computed(() => trackedFallbackKeys.value.size);
const trackedFallbackFailures = computed(() => (current.value?.translation_report?.failures || []).filter(item => item?.key && trackedFallbackKeys.value.has(String(item.key))));
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter(([key, value]) => {
    ...
    if (statusFilter.value === "review") return !isCanonical.value && trackedFallbackKeys.value.has(key);
...
const modelTranslationRiskMessage = computed(() => {
  ...
  if (!risk) return "";
  if (risk.kind === "embedding") return i18n.tf("language.model_translation_risk_embedding", "{model} appears to be an embedding or reranking model rather than a text-generation model. It is unlikely to be able to translate an interface dictionary.", { model: risk.label });
  if (risk.kind === "code") return i18n.tf("language.model_translation_risk_code", "{model} appears to be code-specialized. DerridAI treats code-focused families as higher-risk for natural-language interface translation.", { model: risk.label });
  if (risk.kind === "language") return i18n.tf("language.model_translation_risk_language", "{model} belongs to a smaller or English-centric family that is higher-risk for sustained multilingual interface translation. Consider a multilingual/instruction model if one is available.", { model: risk.label });
  return i18n.tf("language.model_translation_risk_small", "{model} appears to be a small model. Small models are higher-risk for complete dictionary translation, placeholder fidelity, and non-English fluency.", { model: risk.label });
});
...
function translationRiskForModel(model: string) {
  ...
  if (/(?:bge(?:-|_)|nomic[-_.]?embed|mxbai[-_.]?embed|all[-_.]?minilm|e5[-_.]|gte[-_.]|rerank|cross[-_.]?encoder)/i.test(id)) {
    return { kind: "embedding", label: model };
  }
  if (/(?:codegemma|codellama|starcoder|deepseek[^/:]*coder|qwen[^/:]*coder|granite[^/:]*code)/i.test(id)) {
  ...
  try {
    ...
    if (["completed", "failed", "cancelled"].includes(job.status)) {
      if (job.status === "completed") {
        ...
        runtime.notifyToast?.(i18n.t("language.partial_translation_restored", "The incomplete translation was retained and can be resumed."), { tone: "info" });
      }
      return;
    }
...
function openResumeDialog() {
  ...
  if (!code || languages.value.some(item => item.code.replaceAll("_", "-").toLowerCase() === code.replaceAll("_", "-").toLowerCase())) {
    error.value = i18n.t("language.resume_no_longer_needed", "This incomplete translation cannot be resumed because the locale is already installed or its locale code is unavailable.");
    return;
  }
  install.value = {
    code,
  ...
  if (priorProfileId && providerProfiles.value.some(item => item.id === priorProfileId)) selectedProviderId.value = priorProfileId;
  translationRiskAcknowledged.value = false;
  error.value = "";
  installOpen.value = true;
}
...
async function restoreLanguageTranslationJob() {
  try {
    ...
    const code = String(job.result?.code || job.request?.code || "");
    if (code && languages.value.some(item => item.code.replaceAll("_", "-").toLowerCase() === code.replaceAll("_", "-").toLowerCase()) && ["failed", "cancelled"].includes(job.status)) return;
    installJob.value = job;
...
async function installLanguage() {
  ...
  if (modelTranslationRisk.value && !translationRiskAcknowledged.value) {
    error.value = i18n.t("language.model_translation_risk_ack_required", "Review and acknowledge the translation-quality warning before continuing with this model.");
    return;
  }
  ...
  try {
    ...
    runtime.notifyToast?.(i18n.t(wasResume ? "language.translation_resumed" : "language.translation_started_modern", wasResume ? "Translation resumed from the retained partial dictionary." : "Translation started. DerridAI is translating the English interface set before installing the locale."), { tone: "success" });
    resumeJobId.value = "";
...
watch(selectedProviderId, () => { translationRiskAcknowledged.value = false; });
watch(() => selectedProvider.value?.model, () => { translationRiskAcknowledged.value = false; });
  ...
  <main class="vue-native-page languages-page language-studio">
    ...
    <section v-if="resumableInstallJob" class="translation-recovery-card" aria-live="polite">
      ...
      <div class="translation-recovery-copy">
        ...
        <span>{{ i18n.tf("language.translation_incomplete_help", "{done} safe translations were retained. Retry resumes this partial dictionary instead of starting over; {failed} string(s) still need translation or review.", { done:installPartialCount.toLocaleString(i18n.locale), failed:installFailureCount.toLocaleString(i18n.locale) }) }}</span>
      </div>
      <div class="translation-recovery-actions">
        <button type="button" class="btn primary" @click="openResumeDialog">{{ i18n.t("language.resume_translation", "Resume translation") }}</button>
        <button type="button" class="btn tiny" @click="runtime.triggerOperations?.()">{{ i18n.t("language.track_operations", "Track it in Operations") }}</button>
      </div>
    </section>
      ...
      <section class="language-editor-workspace" aria-live="polite">
        ...
        <template v-else-if="current">
          ...
          <section v-if="!isCanonical && trackedFallbackCount" class="language-fallback-report" role="status">
            <div class="language-fallback-report-main"><AppIcon name="warning"/><div><b>{{ i18n.tf("language.tracked_fallbacks_title", "{count} translation fallback(s) need review", { count:trackedFallbackCount.toLocaleString(i18n.locale) }) }}</b><span>{{ i18n.t("language.tracked_fallbacks_help", "These exact keys were not translated safely during installation and are currently using canonical English. They remain tracked until you edit and save a localized value.") }}</span></div></div>
            <div class="language-fallback-report-actions"><button type="button" class="btn small" @click="keyQuery='';statusFilter='review'">{{ i18n.t("language.review_fallbacks", "Review tracked fallbacks") }}</button><details v-if="trackedFallbackFailures.length"><summary>{{ i18n.t("language.failure_details", "Failure details") }}</summary><ul><li v-for="item in trackedFallbackFailures" :key="String(item.key)"><code>{{item.key}}</code><span>{{item.reason}}</span></li></ul></details></div>
          </section>
          ...
          <section class="language-translation-toolbar">
            ...
            <div class="language-filter-tabs" :aria-label="i18n.t('language.translation_filters','Translation filters')">
              ...
              <button v-if="!isCanonical && trackedFallbackCount" type="button" :aria-pressed="statusFilter==='review'" @click="statusFilter='review'">{{ i18n.t("language.needs_review", "Needs review") }} <span>{{ trackedFallbackCount }}</span></button>
      ...
      <div v-if="installOpen" class="workflow-overlay language-modal-overlay" role="presentation" @mousedown.self="installOpen=false" @keydown.esc.stop.prevent="installOpen=false" @keydown="trapFocus($event, installDialog)">
        <section ref="installDialog" class="workflow-dialog language-install-dialog modern-language-dialog" role="dialog" aria-modal="true" aria-labelledby="language-install-title" aria-describedby="language-install-description">
          ...
          <form class="workflow-form language-install-form" @submit.prevent="installLanguage">
            ...
            <section v-if="resumeJobId" class="language-resume-notice" role="status"><AppIcon name="history"/><span><b>{{ i18n.t("language.resuming_partial", "Resuming retained work") }}</b><small>{{ i18n.tf("language.resuming_partial_help", "DerridAI will keep the {count} validated strings from the previous attempt and translate only unfinished or unsafe entries.", { count:installPartialCount.toLocaleString(i18n.locale) }) }}</small></span></section>
            <section class="workflow-section"><div class="workflow-section-copy"><b>{{ i18n.t("language.translation_section", "Translation provider") }}</b><span>{{ i18n.t("language.translation_section_help_modern", "DerridAI checks the provider before starting. Translation runs in bounded batches so smaller local models do not receive the entire dictionary at once.") }}</span></div><div class="workflow-provider-area"><ProviderProfileSelect v-model="selectedProviderId" :profiles="providerProfiles" :default-profile-id="runtime.getDefaultProviderProfileId?.() || ''" :label="i18n.t('language.provider_profile','Provider profile')" :help="i18n.t('language.provider_profile_help','Uses the same provider profiles and model defaults as Research, PDF tools, and LLM review.')" :empty-title="i18n.t('language.no_provider_profiles','No LLM provider profiles are configured')" :empty-help="i18n.t('language.no_provider_profiles_help','Create a provider profile first, then return here to translate a dictionary.')" :manage-label="i18n.t('language.manage_providers','Manage provider profiles')" :model-not-set-label="i18n.t('language.model_not_set','model not set')" :default-label="i18n.t('ui.default','Default')" :concurrent-label="i18n.t('language.concurrent_requests','max concurrent request(s)')" :context-label="i18n.t('providers.context_tokens','context tokens')" @manage="requestManageProviders" />
