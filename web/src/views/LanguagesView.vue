...
const modelTranslationRiskMessage = computed(() => {
  ...
  return i18n.tf("language.model_translation_risk_small", "{model} appears to be a very small model. Small models are higher-risk for complete dictionary translation, placeholder fidelity, and non-English fluency.", { model: risk.label });
});
...
function translationRiskForModel(model: string) {
  ...
  if (size && Number(size[1]) <= 3) return { kind: "small", label: model };
  return null;
}
