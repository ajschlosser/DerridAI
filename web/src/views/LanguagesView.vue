...
const categories = computed(() => {
  ...
  return [{ id: "all", count: allKeys.value.length }, ...[...counts.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([id, count]) => ({ id, count }))];
});
...
function exportDictionary() {
  if (!current.value) return;
  const keys = activeCategory.value === "all"
    ? allKeys.value
    : allKeys.value.filter(key => (key.includes(".") ? key.split(".")[0] : "other") === activeCategory.value);
  const dictionary = Object.fromEntries(keys.map(key => [key, current.value?.dictionary?.[key] || ""]));
  const payload = {
...
function confirmManageProviders() {
  ...
  installOpen.value = false;
  window.dispatchEvent(new CustomEvent("derridai:navigate-native", { detail: { path: "/providers", runtimeView: "providers" } }));
}
      ...
      <section class="language-editor-workspace" aria-live="polite">
        ...
        <template v-else-if="current">
          ...
          <section class="language-policy-card" :aria-label="i18n.t('language.content_policy_title','Researcher text policy')">
            <div class="language-policy-copy">
              ...
              <details v-if="policyReady" class="language-policy-terms">
                <summary>{{ i18n.t("language.content_policy_show_terms", "Show sensitive policy terms") }}</summary>
          ...
          <section class="language-translation-toolbar">
            ...
            <label class="language-category-select"><span class="sr-only">{{ i18n.t("language.category", "Category") }}</span><select v-model="activeCategory" class="control"><option v-for="category in categories" :key="category.id" :value="category.id">{{ category.id === 'all' ? i18n.t("language.all_categories", "All categories") : category.id }} · {{ category.count }}</option></select></label>
            <div class="language-transfer-actions">
              <button type="button" class="btn small" @click="exportDictionary">{{ i18n.t("language.export_dictionary", "Export dictionary") }}</button>
              <button type="button" class="btn small" @click="openImport">{{ i18n.t("language.import_dictionary", "Import dictionary") }}</button>
              <input ref="importInput" class="sr-only" type="file" accept="application/json,.json" @change="importDictionary">
            </div>
