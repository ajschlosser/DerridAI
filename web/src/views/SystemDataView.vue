...
const primaryKeys = computed(() => (table.value?.columns || []).filter((column) => column.primary_key));
...
async function addRow() {
  ...
  if (!values) {
    runtime.notifyToast(t("runtime.system_invalid_json", "Enter a valid JSON object."), { tone: "danger" });
    return;
  }
  await persist(() => systemApi.insertSystemDataRow(selectedDatabase.value, selectedTable.value, values));
  newRow.value = "{}";
}
...
function startEdit(row: DataRow, index: number) {
  ...
  const editable = Object.fromEntries(
    Object.entries(row).filter(([name]) => !table.value?.columns.find((column) => column.name === name)?.sensitive),
  );
...
async function saveRow() {
  ...
  await persist(() => systemApi.updateSystemDataRow(selectedDatabase.value, selectedTable.value, key, values));
  editingIndex.value = -1;
}

async function deleteRow(row: DataRow) {
  const key = Object.fromEntries(primaryKeys.value.map((column) => [column.name, row[column.name]]));
  ...
  if (approved) await persist(() => systemApi.deleteSystemDataRow(selectedDatabase.value, selectedTable.value, key));
}

async function persist(action: () => Promise<unknown>) {
  if (!table.value?.writable) {
    runtime.notifyToast(t("runtime.system_read_only", "This table is read-only."), { tone: "danger" });
    return;
  }
  saving.value = true;
  try {
...
<template>
  <main id="main" class="runtime-surface" aria-live="polite">
    <div v-if="loading" class="info">{{ t("runtime.loading_response_cache", "Loading system data") }}</div>
    ...
    <div v-else class="system-data-page">
      <header class="page-header">
        <div>
          ...
          <p>{{ t("runtime.system_data_help", "Manage the response cache and durable application databases through a safe, backend-neutral interface.") }}</p>
        </div>
        <button class="btn" type="button" @click="load"><AppIcon name="refresh" /> {{ t("runtime.system_refresh", "Refresh data") }}</button>
      </header>
      ...
      <section class="card cache-summary" aria-labelledby="cache-title">
        <div>
          ...
          <p>{{ t("runtime.help.system_cache_only", "System cache only. This collection is excluded from corpus stores and source selection.") }}</p>
        </div>
      ...
      <section class="card" aria-labelledby="cache-records-title">
        <div class="table-header">
          <div>
            <h2 id="cache-records-title">{{ t("runtime.system_cache_records", "Response cache records") }}</h2>
            <p>{{ t("runtime.system_cache_records_help", "Latest response-cache entries.") }}</p>
          </div>
        </div>
        <div class="tablewrap">
          <table>
            <thead><tr><th>{{ t("runtime.system_created", "Created") }}</th><th>{{ t("runtime.system_question", "Question") }}</th><th>{{ t("runtime.system_generation", "Generation") }}</th><th>{{ t("runtime.system_actions", "Actions") }}</th></tr></thead>
            <tbody>
              <tr v-for="record in cachePayload.records || []" :key="String(record.record_id)">
                ...
                <td><button class="btn tiny danger" type="button" @click="deleteCacheRecord(record)">{{ t("runtime.system_delete_row", "Delete row") }}</button></td>
              </tr>
              <tr v-if="!(cachePayload.records || []).length"><td colspan="4" class="note">{{ t("runtime.system_no_rows", "No rows are available.") }}</td></tr>
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section class="data-grid">
        <aside class="card database-list" aria-labelledby="database-title">
          ...
          <button
            ...
            type="button"
            @click="selectedDatabase = item.name; selectedTable = item.tables[0]?.name || ''; loadTable()"
          >
            <b>{{ item.name }}</b>
            <small>{{ item.backend }} · {{ item.tables.length }} {{ t("runtime.system_table", "table") }}</small>
          </button>
        </aside>
        ...
        <section class="card table-panel" aria-labelledby="table-title">
          ...
          <template v-else>
            <header class="table-header">
              ...
              <span v-if="!table.writable" class="readonly">{{ t("runtime.system_read_only", "This table is read-only.") }}</span>
            </header>
            <div class="table-tabs" role="tablist" :aria-label="t('runtime.system_table', 'Table')">
              <button
                ...
                type="button"
                @click="selectedTable = item.name; loadTable()"
              >{{ item.name }}</button>
            </div>
            <div v-if="table.writable" class="editor-grid">
              <label>
                ...
                <button class="btn" type="button" :disabled="saving" @click="addRow">{{ t("runtime.system_add_row", "Add row") }}</button>
              </label>
              <label v-if="editingIndex >= 0">
                ...
                <button class="btn primary" type="button" :disabled="saving" @click="saveRow">{{ t("runtime.system_save_row", "Save row") }}</button>
              </label>
            </div>
            <div class="tablewrap">
              <table>
                <thead><tr><th v-for="column in table.columns" :key="column.name">{{ column.name }}</th><th>{{ t("runtime.system_table", "Actions") }}</th></tr></thead>
                <tbody>
                  <tr v-for="(row, index) in rows" :key="index">
                    <td v-for="column in table.columns" :key="column.name">{{ column.sensitive ? t("runtime.system_redacted", "[redacted]") : String(row[column.name] ?? "") }}</td>
                    <td class="row-actions">
                      <button v-if="table.writable && primaryKeys.length" class="btn tiny" type="button" @click="startEdit(row, index)">{{ t("runtime.system_save_row", "Edit") }}</button>
                      <button v-if="table.writable && primaryKeys.length" class="btn tiny danger" type="button" @click="deleteRow(row)">{{ t("runtime.system_delete_row", "Delete row") }}</button>
                    </td>
                  </tr>
                  <tr v-if="!rows.length"><td :colspan="table.columns.length + 1" class="note">{{ t("runtime.system_no_tables", "No rows are available.") }}</td></tr>
                </tbody>
              </table>
            </div>
          </template>
...
<style scoped>
.system-data-page{display:grid;gap:16px}.page-header,.cache-summary,.table-header{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}.page-header h1{margin:4px 0;font-size:1.4rem}.page-header p,.cache-summary p,.table-header p{margin:4px 0;color:var(--muted);line-height:1.5}.eyebrow{color:var(--muted);font-size:.72rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.cache-summary strong{font-size:1.8rem}.cache-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.data-grid{display:grid;grid-template-columns:minmax(190px,.35fr) minmax(0,1fr);gap:16px}.database-list{display:grid;align-content:start;gap:8px}.database-list h2,.table-panel h2{margin:0 0 8px;font-size:1rem}.database-button{display:grid;gap:3px;text-align:start;padding:10px;border:1px solid var(--line);border-radius:8px;background:var(--soft);color:inherit;cursor:pointer}.database-button.active{border-color:var(--accent);box-shadow:0 0 0 2px color-mix(in srgb,var(--accent) 25%,transparent)}.database-button small{color:var(--muted)}.table-panel{min-width:0}.readonly{color:var(--tone-warn-fg);font-size:.8rem;font-weight:700}.table-tabs{display:flex;gap:6px;flex-wrap:wrap;margin:12px 0}.editor-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:12px 0}.editor-grid label{display:grid;gap:6px;color:var(--muted);font-size:.8rem;font-weight:700}.editor-grid textarea{width:100%;resize:vertical;padding:8px;border:1px solid var(--line);border-radius:8px;background:var(--card);color:inherit;font:inherit;font-weight:400}.row-actions{display:flex;gap:5px;flex-wrap:wrap}@media(max-width:800px){.data-grid,.editor-grid{grid-template-columns:1fr}.page-header,.cache-summary,.table-header{display:grid}}
</style>
