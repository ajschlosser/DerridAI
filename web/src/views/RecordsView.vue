    ...
    <template v-if="snapshot.available">
      ...
      >
        ...
        >
          ...
          >
            ...
            <button v-if="filterCount" type="button" class="btn" @click="records.clearFilters()">
      ...
      >
        <table class="records-table" :class="density">
          ...
          <thead>
            <tr>
              <th v-if="snapshot.capabilities.can_select" class="select-col" scope="col">
                <input
                  type="checkbox"
            ...
            >
              ...
              >
                <span
                  ...
                  :class="['db-status', cell.status_kind]"
                  :title="cell.title"
                  >{{ cellText(cell) }}</span
                >
                <span v-else-if="cell.kind === 'review'" :class="{ review: cell.text === 'yes' }">{{
                  cellText(cell)
                }}</span>
                <HighlightedText
...
.records-more-menu {
  ...
  border: 1px solid var(--line);
  border-radius: 12px;
  background: var(--overlay, var(--panel, #fff));
  box-shadow: 0 16px 40px rgba(20, 30, 24, 0.16);
}
...
.actions-col {
  ...
  right: 0;
  background: inherit;
  box-shadow: -8px 0 12px rgba(20, 30, 24, 0.04);
}
.select-col,
.sticky-status {
  position: sticky;
