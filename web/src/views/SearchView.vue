...
function columnStyle(key:string){const width=columnWidths[key];return width?{width:`${width}px`,minWidth:`${width}px`,maxWidth:key==='text'?`${Math.max(width,260)}px`:undefined}:undefined}
        ...
        <section class="search-results-panel" aria-labelledby="search-results-title">
          <div class="search-results-toolbar">
            ...
            <div class="search-results-controls">
              ...
              <button v-if="snapshot.layout!=='cards'" type="button" class="btn" @click="openColumns"><AppIcon name="list"/>{{i18n.t('records.columns','Columns')}}</button>
          ...
          <div v-else-if="snapshot.layout==='cards'" class="search-result-cards">
            <article v-for="result in snapshot.results" :key="result.key" class="search-result-card" :class="{selected:result.selected}">
              ...
              <footer>
                ...
                <div class="search-record-actions"><button type="button" class="btn" @click="resultAction(result,'open')">{{result.kind==='database'&&!snapshot.is_researcher?i18n.t('ui.edit','Edit'):i18n.t('record.open','Open record')}}</button><button v-if="result.evidence_available" type="button" class="btn" :class="{soft:result.evidence_selected}" @click="resultAction(result,'evidence')"><AppIcon :name="result.evidence_selected?'check':'plus'"/>{{result.evidence_selected?i18n.t('ui.selected','Selected'):i18n.t('ui.add_evidence','Add evidence')}}</button><details class="record-citation-menu"><summary class="btn">{{i18n.t('ui.get_citation','Get Citation')}}</summary><div class="record-citation-popover"><button type="button" class="btn" @click="resultAction(result,'citation-inline')">{{i18n.t('ui.inline','Inline')}}</button><button type="button" class="btn" @click="resultAction(result,'citation-full')">{{i18n.t('ui.full','Full')}}</button></div></details></div>
              </footer>
            </article>
          </div>
