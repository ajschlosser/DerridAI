...
const resultKeys = computed(
  () =>
    snapshot.value?.results
      .filter((result) => result.kind === "workspace")
      .map((result) => result.key) || [],
);
const pageAllSelected = computed(
  () =>
    Boolean(resultKeys.value.length) &&
    snapshot.value?.results
      .filter((result) => result.kind === "workspace")
      .every((result) => result.selected),
);
const activeFacetChips = computed(
  () =>
    snapshot.value?.facets.flatMap((facet) =>
      facet.values
...
const resultSummary = computed(() => {
  ...
  return total === 1
    ? i18n.tf("search.result_count_one", "{count} result", {
        count: total.toLocaleString(i18n.locale),
      })
    : i18n.tf("search.result_count_many", "{count} results", {
        count: total.toLocaleString(i18n.locale),
      });
});
const searchPlaceholder = computed(() =>
  databaseMode.value
    ? i18n.t("search.database_placeholder", "Search the corpus semantically…")
    : i18n.t("search.loaded_placeholder", "Search extracted text across loaded records…"),
);
const methodHelp = computed(() =>
  snapshot.value?.method === "mmr"
    ? i18n.t("search.mmr_help", "Balances semantic relevance with diversity across the result set.")
    : snapshot.value?.method === "filter"
      ? i18n.t(
          "search.filter_only_help",
          "Returns records using metadata filters without embedding a text query.",
        )
      : i18n.t("search.similarity_help", "Ranks records by semantic similarity to your query."),
);
...
const sortOptions = computed(() => {
  const base = [
    { key: "work", label: i18n.t("field.work", "Work") },
    { key: "page_start", label: i18n.t("field.page_start", "Page Start") },
    { key: "record_id", label: i18n.t("field.record_id", "Record ID") },
  ];
  if (databaseMode.value)
    base.unshift({ key: "similarity", label: i18n.t("search.relevance", "Relevance") });
  return base;
});
const activeSortLabel = computed(
  () =>
    sortOptions.value.find((item) => item.key === snapshot.value?.sort.key)?.label ||
    i18n.t("search.sort", "Sort"),
);

function loadColumnWidths() {
  try {
    return JSON.parse(localStorage.getItem("derridai.search.columnWidths.v1") || "{}") || {};
  } catch {
    return {};
  }
}
...
async function load(options: { refresh?: boolean; autoRun?: boolean } = {}) {
  ...
  try {
    ...
    if (
      ...
      !next.filter_fields.some((field) => field.key === newFilterField.value)
    )
      newFilterField.value = next.filter_fields[0]?.key || "work";
    shell.sync();
...
function onFilterFieldChange() {
  const ops = filterOps(newFilterField.value);
  if (!ops.some(([value]) => value === newFilterOp.value)) newFilterOp.value = ops[0]?.[0] || "eq";
  newFilterValue.value = "";
}
...
function openColumns() {
  ...
  void nextTick(() => columnsDialog.value?.querySelector<HTMLElement>("button,input")?.focus());
}
function closeColumns() {
  columnsDialog.value?.close();
}
function toggleDraftColumn(key: string, checked: boolean) {
  if (checked && !draftColumns.value.includes(key)) draftColumns.value.push(key);
  else if (!checked) draftColumns.value = draftColumns.value.filter((item) => item !== key);
}
function moveDraftColumn(key: string, direction: number) {
  const at = draftColumns.value.indexOf(key),
    to = at + direction;
  if (at < 0 || to < 0 || to >= draftColumns.value.length) return;
...
function openSavedViews() {
  loadSavedState();
  viewsDialog.value?.showModal();
  void nextTick(() => viewsDialog.value?.querySelector<HTMLElement>("button")?.focus());
}
...
function openSaveView() {
  ...
  void nextTick(() => saveDialog.value?.querySelector<HTMLInputElement>("input")?.focus());
}
...
function saveCurrentView() {
  ...
  persistSavedViews();
  closeSaveView();
  runtime.notifyToast(i18n.t("search.view_saved", "Search view saved"), { tone: "success" });
}
...
async function copyLink() {
  try {
    ...
    runtime.notifyToast(i18n.t("search.link_copied", "Shareable search link copied"), {
      tone: "success",
    });
  } catch (exc) {
...
function recordRecentSearch() {
  ...
  const entry: RecentSearchEntry = {
    id: crypto.randomUUID(),
    query: q || i18n.t("search.filtered_view", "Filtered view"),
    scope: snapshot.value.scope,
    method: snapshot.value.method,
    href,
    created_at: new Date().toISOString(),
  };
  recentSearches.value = [
    entry,
    ...recentSearches.value.filter((item) => item.href !== href),
  ].slice(0, 12);
  persistRecent();
}
...
function columnValue(result: SearchResult, key: string) {
  if (key === "__file") return result.file_name || "—";
  if (key === "page_start") return result.page_span || "—";
  if (key === "text") return result.text;
  if (key === "__db_status")
    return result.db_status?.label || i18n.t("search.db_in_database", "In DB");
  return result.record[key];
}
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- SA-13: preserve legacy setup binding until its owning workflow is extracted.
function columnLabel(key: string) {
  return snapshot.value?.available_columns.find((column) => column.key === key)?.label || key;
}
function columnStyle(key: string) {
  if (!databaseMode.value) return undefined;
  const width = columnWidths[key];
  return width
    ? {
        width: `${width}px`,
        minWidth: `${width}px`,
        maxWidth: key === "text" ? `${Math.max(width, 260)}px` : undefined,
      }
    : undefined;
}
...
function similarityPercent(result: SearchResult) {
  return result.similarity == null ? null : Math.round(result.similarity * 100);
}
function onDialogCancel(event: Event, dialog: HTMLDialogElement | null) {
  event.preventDefault();
  dialog?.close();
}
...
onMounted(async () => {
  loadSavedState();
  const homeQuery = typeof route.query.q === "string" ? route.query.q : "";
  const homeWork = typeof route.query.work === "string" ? route.query.work : "";
  const homeMode = route.query.mode === "database" ? "database" : "traditional";
  if (homeQuery) {
    query.value = homeQuery;
    runtime.updateSearchQuery(homeQuery, { replace: true });
  }
  if (homeMode === "database") await runtime.setSearchScope("database");
  if (homeWork) runtime.addSearchAdvancedFilter({ field: "work", op: "eq", value: homeWork });
  await load({ refresh: true, autoRun: true });
});
onBeforeUnmount(() => {
  window.clearTimeout(localSearchTimer);
  window.clearTimeout(recentTimer);
...
<template>
  ...
  >
    <div v-if="loading && !snapshot" class="search-page-loading" role="status">
      <span class="spinner"></span>{{ i18n.t("search.loading", "Loading Search workspace…") }}
    </div>
    <section v-else-if="error" class="search-page-error">
      <h1>{{ i18n.t("search.load_failed", "Could not load Search") }}</h1>
      <p>{{ error }}</p>
      <button type="button" class="btn" @click="load()">{{ i18n.t("ui.retry", "Retry") }}</button>
    </section>
    <AccessibleEmptyState
      v-else-if="noDatabase"
      icon="database"
      :title="i18n.t('search.nothing_to_search_title', 'Search needs something to search')"
      :description="
        snapshot?.capabilities.can_manage_database
          ? i18n.t(
              'search.nothing_to_search_help',
              'Open a JSONL workspace or create a corpus collection, then search across your works, metadata, and annotations.',
            )
          : i18n.t(
              'search.empty_state_denied',
              'Ask an administrator to configure a corpus database or grant you access.',
            )
      "
      :action-label="
        snapshot?.capabilities.can_manage_database
          ? i18n.t('search.empty_state_action', 'Create a collection')
          : ''
      "
    ...
    <template v-else-if="snapshot">
      ...
      <section
        ...
        :aria-label="i18n.t('search.search_controls', 'Search controls')"
      >
        <div v-if="databaseMode" class="search-database-context">
          ...
          >
          <span class="search-database-note">{{
            i18n.t(
              "search.database_context_help",
              "Results and evidence stay bound to this corpus database.",
            )
          }}</span>
        </div>
        <form
        ...
        >
          <label class="search-command-input">
            ...
            <button
              ...
              :aria-label="i18n.t('search.clear_query', 'Clear search query')"
              @click="clearQuery"
            >
              ×
            </button>
          ...
          >
            <AppIcon name="search" />{{
              snapshot.loading
                ? i18n.t("search.searching", "Searching…")
                : i18n.t("ui.search", "Search")
            }}
          </button>
        </form>
        <div
          ...
          :aria-label="i18n.t('search.active_filters', 'Active filters')"
        >
          <button
          ...
          >
            ...
            ><span class="sr-only">{{ i18n.t("ui.remove", "Remove") }}</span>
          </button>
          <button
          ...
          <button type="button" class="search-clear-filters" @click="clearAll">
            {{ i18n.t("search.clear_all", "Clear all") }}
          </button>
        </div>
        <details
        ...
        >
          <summary>
            <AppIcon name="gear" />{{ i18n.t("search.search_options", "Search options")
          ...
          <div class="search-options-body">
            <div v-if="databaseMode" class="search-method-grid">
              <fieldset class="search-method-picker">
                <legend>{{ i18n.t("search.ranking_method", "Ranking method") }}</legend>
                <label
                  v-for="item in [
                  ...
                  /><span
                    ...
                    ><small>{{
                      item[0] === "similarity"
                        ? i18n.t("search.method_similarity_short", "Closest semantic matches")
                        : item[0] === "mmr"
                          ? i18n.t("search.method_mmr_short", "Relevant but less repetitive")
                          : i18n.t("search.method_filter_short", "Metadata without embeddings")
                    }}</small></span
                  ></label
                >
              </fieldset>
              ...
              <div v-if="snapshot.method === 'mmr'" class="search-mmr-controls">
                <label
                  ><span>{{ i18n.t("research.fetch_k_label", "MMR candidate pool") }}</span
                  ><input
                    class="control"
                    type="number"
                    min="1"
                    ...
                    " /></label
                ><label
                  ><span>{{ i18n.t("search.mmr_lambda", "Relevance weight (λ)") }}</span
                  ><input
                    class="control"
                    type="number"
                    min="0"
            ...
            <div class="search-advanced-filter-builder">
              <div class="search-options-heading">
                <div>
                  <span class="section-label">{{
                    i18n.t("search.advanced_filters", "Advanced filters")
                  }}</span>
                  <h3>{{ i18n.t("search.precise_metadata_filter", "Precise metadata filter") }}</h3>
                </div>
                <p>
                  {{
                    i18n.t(
              ...
              <div class="search-filter-builder-grid">
                <label
                  ><span>{{ i18n.t("search.field", "Field") }}</span
                  ><select v-model="newFilterField" class="control" @change="onFilterFieldChange">
                    <option
                      v-for="field in snapshot.filter_fields"
                      :key="field.key"
                      :value="field.key"
                    >
                      {{ field.label }}
                ...
                >
                <label
                  ><span>{{ i18n.t("search.condition", "Condition") }}</span
                  ><select v-model="newFilterOp" class="control">
                    <option
                      v-for="[value, key, fallback] in filterOps(newFilterField)"
                      :key="value"
                      :value="value"
                    >
                      {{ i18n.t(key, fallback) }}
                ...
                >
                <label
                  ><span>{{ i18n.t("search.value", "Value") }}</span
                  ><input
                    v-model="newFilterValue"
                    class="control"
                    :list="`search-suggestions-${newFilterField}`"
                    :disabled="['empty', 'notempty'].includes(newFilterOp)"
                    :placeholder="
                      i18n.t('search.filter_value_placeholder', 'Type or choose a value…')
                    "
                    @keydown.enter.prevent="addAdvancedFilter" /><datalist
                    :id="`search-suggestions-${newFilterField}`"
                  >
                    <option
                      v-for="value in suggestionsFor(newFilterField)"
                      :key="value"
                      :value="value"
                    ></option></datalist
                ></label>
                <button
                  type="button"
                  class="btn search-add-filter"
                  :disabled="!['empty', 'notempty'].includes(newFilterOp) && !newFilterValue.trim()"
      ...
      <section class="search-explorer-grid">
        ...
        >
          <AppIcon name="filter" />{{ i18n.t("search.filters", "Filters")
        ...
        <section class="search-results-panel" aria-labelledby="search-results-title">
          <div class="search-results-toolbar">
            <div class="search-results-count">
              <span class="section-label">{{ i18n.t("search.results", "Results") }}</span>
              <h2 id="search-results-title">{{ resultSummary }}</h2>
              <p v-if="databaseMode && snapshot.search_has_run">
                {{
                  i18n.t(
                    "search.database_result_note",
                    "Semantic result facets refine the returned candidate set; relevance remains tied to the selected ranking method.",
                  )
                }}
              </p>
            </div>
            <div class="search-results-controls">
              <details class="search-sort-menu">
                <summary class="btn">
                  {{ i18n.t("search.sort", "Sort") }}: {{ activeSortLabel }}
                  <span aria-hidden="true">⌄</span>
                </summary>
                <div class="search-sort-popover">
                  <button
                    v-for="item in sortOptions"
                    :key="item.key"
                    type="button"
                    :class="{ active: snapshot.sort.key === item.key }"
                    @click="sortBy(item.key)"
                  >
                    {{ item.label
                    }}<span v-if="snapshot.sort.key === item.key" aria-hidden="true">{{
                      snapshot.sort.dir > 0 ? "↑" : "↓"
                    }}</span>
                  </button>
                </div>
              ...
              />
              <button
                v-if="databaseMode && snapshot.layout !== 'cards'"
                type="button"
                class="btn"
                @click="openColumns"
              >
          ...
          <div v-if="snapshot.loading" class="search-results-loading" role="status">
            <span class="spinner"></span>{{ i18n.t("search.searching", "Searching…") }}
          </div>
          <AccessibleEmptyState
            v-else-if="databaseMode && !snapshot.has_database"
            icon="database"
            icon-tone="neutral"
            :title="i18n.t('search.no_database_title', 'No corpus database available')"
            :description="
              i18n.t(
                'search.no_database_help',
                'A corpus database must be created before database search can run.',
              )
            "
          /><AccessibleEmptyState
            ...
            icon="search"
            icon-tone="neutral"
            :title="i18n.t('search.ready_title', 'Search the corpus database')"
            :description="
              i18n.t(
                'search.ready_help',
                'Enter a question, phrase, concept, or passage. Search options control semantic ranking and metadata constraints.',
              )
            "
          />
          <AccessibleEmptyState
            ...
            icon="search"
            icon-tone="neutral"
            :title="i18n.t('search.no_results', 'No matching records')"
            :description="
              i18n.t(
                'search.no_results_help',
                'Try a broader query, remove a filter, or choose another corpus scope.',
              )
            "
          />
            ...
            >
              <header>
                <div>
                  ...
                  <h3>{{ result.work || i18n.t("works.untitled", "Untitled work") }}</h3>
                </div>
                <span
                  ...
                  :title="
                    i18n.t(
                      ...
                      'Similarity is derived from vector distance; higher values indicate a closer semantic match.',
                    )
                  "
                  >{{ i18n.t("search.relevance", "Relevance") }}
                  {{ similarityPercent(result) }}</span
                >
              </header>
              <div class="search-result-meta">
                <span v-if="result.page_span"
                  >{{ i18n.t("record.page", "Page") }} {{ result.page_span }}</span
              ...
              >
                {{
                  expandedText.has(result.key)
                    ? i18n.t("search.show_less", "Show less")
                    : i18n.t("search.show_more", "Show more")
                }}
              </button>
              <div v-if="result.match_reasons.length" class="search-why-result">
                <span>{{ i18n.t("search.why_result", "Why this result") }}</span
                ><span
              ...
              <footer>
                <label
                  ...
                  ><input
                    ...
                    "
                  /><span>{{ i18n.t("search.select_record", "Select") }}</span></label
                >
                  ...
                  <button type="button" class="btn" @click="resultAction(result, 'open')">
                    {{
                      result.kind === "database" && !snapshot.is_researcher
                        ? i18n.t("ui.edit", "Edit")
                        : i18n.t("record.open", "Open record")
                    }}</button
                  ><button
          ...
          <div
            v-else
            class="search-table-scroll"
            tabindex="0"
            :aria-label="
              i18n.t(
                'search.results_table_scroll',
                'Search results table. Scroll horizontally to reveal additional columns.',
              )
            "
          >
            <table
            ...
            >
              ...
              <thead>
                <tr>
                  <th v-if="snapshot.capabilities.can_select" class="search-select-column">
                    <input
                      type="checkbox"
                  ...
                  >
                    ...
                    ><button
                      ...
                      :aria-label="
                        i18n.tf('search.resize_column', 'Resize {column} column', {
                          column: column.label,
                        })
                      "
                  ...
                  <th class="search-actions-column">{{ i18n.t("ui.actions", "Actions") }}</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  ...
                  :class="{ selected: result.selected }"
                >
                  <td v-if="snapshot.capabilities.can_select" class="search-select-column">
                    <input
                      type="checkbox"
                      :checked="result.selected"
                      :aria-label="
                  ...
                  >
                    <template v-if="column.key === '__db_status'"
                      ><span
                        ...
                        ><i></i
                        >{{
                          result.db_status?.label || i18n.t("search.db_in_database", "In DB")
                        }}</span
                      ></template
                  ...
                  <td class="search-actions-column">
                    <div class="search-record-actions">
                      ...
                      >
                        {{ result.evidence_selected ? "✓ " : ""
                        }}{{ i18n.t("ui.add_evidence", "Evidence") }}</button
                      ><CitationMenu
                        compact
                        @inline="resultAction(result, 'citation-inline')"
                        @full="resultAction(result, 'citation-full')"
          ...
          <nav
            ...
            :aria-label="i18n.t('search.pagination', 'Search results pages')"
          >
            <button
              type="button"
              class="btn"
      ...
      <dialog
        ...
        aria-labelledby="search-columns-dialog-title"
        @cancel="onDialogCancel($event, columnsDialog)"
      >
        <div class="dh">
          <div>
            <span class="section-label">{{ i18n.t("search.table_view", "Table view") }}</span>
            <h2 id="search-columns-dialog-title">
              {{ i18n.t("search.configure_columns", "Configure columns") }}
            </h2>
          </div>
          <button
            type="button"
        ...
        <div class="db search-column-dialog-body">
          <p>
            {{
              i18n.t(
                "search.column_help",
          ...
          <div class="search-column-list">
            <div
              v-for="column in snapshot.available_columns"
              :key="column.key"
              class="search-column-choice"
              :class="{ active: draftColumns.includes(column.key) }"
            >
              <label
                ><input
                  type="checkbox"
                  ...
                  @change="
                    toggleDraftColumn(column.key, ($event.target as HTMLInputElement).checked)
                  "
                /><span>{{ column.label }}</span></label
              >
              <div v-if="draftColumns.includes(column.key)" class="search-column-order">
                <button
                  type="button"
                  class="btn tiny"
                  :disabled="draftColumns.indexOf(column.key) <= 0"
                  :aria-label="
                    i18n.tf('search.move_column_up', 'Move {column} up', { column: column.label })
                  "
                  @click="moveDraftColumn(column.key, -1)"
                >
                  ↑</button
                ><button
                  type="button"
                  class="btn tiny"
                  :disabled="draftColumns.indexOf(column.key) >= draftColumns.length - 1"
                  :aria-label="
        ...
        <div class="da">
          <button type="button" class="btn" @click="resetColumns">
            {{ i18n.t("search.reset_defaults", "Reset defaults") }}</button
          ><button type="button" class="btn" @click="closeColumns">
      ...
      >
        <form @submit.prevent="saveCurrentView">
          <div class="dh">
            <div>
              <span class="section-label">{{ i18n.t("search.saved_views", "Saved views") }}</span>
              <h2 id="search-save-dialog-title">{{ i18n.t("search.save_view", "Save view") }}</h2>
            </div>
            <button
              type="button"
              class="btn icon-only"
          ...
          <div class="db">
            <label class="field"
              ><span>{{ i18n.t("search.view_name", "View name") }}</span
              ><input
                ...
                maxlength="80"
                :placeholder="
                  i18n.t('search.view_name_placeholder', 'e.g. Adieu passages needing review')
                "
            /></label>
            <p class="note">
              {{
                i18n.t(
                  "search.save_view_help",
                  "Saved views preserve the current shareable URL, including scope, query, filters, sort, columns, and layout.",
                )
              }}
            </p>
          </div>
          <div class="da">
            ...
            ><button type="submit" class="btn primary" :disabled="!saveViewName.trim()">
              {{ i18n.t("search.save_view", "Save view") }}
            </button>
          </div>
        </form>
      </dialog>
      ...
      >
        <div class="dh">
          <div>
            <span class="section-label">{{
              i18n.t("search.exploration_history", "Exploration history")
            }}</span>
            <h2 id="search-views-dialog-title">
              {{ i18n.t("search.saved_and_recent", "Saved & recent searches") }}
            </h2>
          </div>
          <button
            type="button"
        ...
        <div class="db search-views-body">
          <section>
            <div class="search-options-heading">
              <div>
                <h3>{{ i18n.t("search.saved_views", "Saved views") }}</h3>
                <p>
                  {{
                    i18n.t(
                      "search.saved_views_help",
                      "Reusable search workspaces stored in this browser.",
                    )
                  }}
                </p>
              </div>
              <button
                ...
                "
              >
                {{ i18n.t("search.save_current", "Save current") }}
              </button>
            </div>
            <div v-if="savedViews.length" class="search-view-list">
              <article v-for="view in savedViews" :key="view.id">
                ...
                ><button
                  ...
                  :aria-label="
                    i18n.tf('search.delete_saved_view', 'Delete saved view {name}', {
                      name: view.name,
                    })
                  "
            ...
            <p v-else class="note">{{ i18n.t("search.no_saved_views", "No saved views yet.") }}</p>
          </section>
          <section>
            <div class="search-options-heading">
              <div>
                <h3>{{ i18n.t("search.recent_searches", "Recent searches") }}</h3>
                <p>
                  {{
                    i18n.t(
                      "search.recent_searches_help",
                      "Recent exploration states from this browser.",
                    )
                  }}
                </p>
              </div>
              ...
              <article v-for="item in recentSearches" :key="item.id">
                <button type="button" class="search-view-open" @click="openSavedView(item)">
                  ...
                  ><small
                    >{{
                      item.scope === "database"
                        ? i18n.t("search.corpus_database", "Corpus database")
                        : i18n.t("search.loaded_records", "Loaded records")
                    }}
            ...
            <p v-else class="note">
              {{ i18n.t("search.no_recent_searches", "No recent searches yet.") }}
            </p>
          </section>
        </div>
        <div class="da">
