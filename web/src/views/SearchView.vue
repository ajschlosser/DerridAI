...
const resultSummary = computed(() => {
  ...
  return total === 1
    ...
    : i18n.tf("search.result_count_many", { count: total.toLocaleString(i18n.locale) });
});
...
const associatedSchemaId = computed(
  () =>
    snapshot.value?.stores.find((item) => item.name === snapshot.value?.active_store)?.schema_id ||
    defaultFilterSchemaId(),
);
...
const sortOptions = computed(() => {
  const base = [
    { key: "work", label: i18n.t("field.work") },
    { key: "page_start", label: i18n.t("field.page_start") },
    { key: "record_id", label: i18n.t("field.record_id") },
  ];
...
const sortMenuItems = computed(() =>
  sortOptions.value.map((item) => ({
    id: item.key,
    label: item.label,
    checked: snapshot.value?.sort.key === item.key,
  })),
);
...
async function syncFilterSchema(next: SearchWorkspaceSnapshot) {
  try {
    if (!schemaSummaries.value.length)
      schemaSummaries.value = (await metadataSchemasApi.list()).items || [];
  } catch {
    schemaSummaries.value = [];
  }
  const store = next.stores.find((item) => item.name === next.active_store);
  filterSchemaId.value = chosenFilterSchemaId({
    store: next.active_store || "loaded",
    associatedId: store?.schema_id,
  });
...
async function changeFilterSchema(id: string) {
  ...
  saveFilterSchemaOverride(
    snapshot.value?.active_store || "loaded",
    filterSchemaId.value === defaultFilterSchemaId() ? "" : filterSchemaId.value,
  );
  await loadSchema(filterSchemaId.value);
...
function columnValue(result: SearchResult, key: string) {
  ...
  if (key === "__db_status") return result.db_status?.label || i18n.t("search.db_in_database");
