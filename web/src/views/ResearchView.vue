...
const canConfigureResearch=computed(()=>Boolean(workspace.value?.can_run&&auth.can("rag.run")));
const canGrade=computed(()=>auth.isAdmin);
const runDisabledReason=computed(()=>{
  if(!workspace.value?.can_run||!auth.can("rag.run"))return i18n.t("permissions.rag_denied","Your role cannot run Research pipelines.");
  if(!prompt.value.trim())return i18n.t("research.prompt_required","Enter a research question.");
  if(!profiles.value.length)return i18n.t("research.no_profile_configured","No Research LLM profile is available.");
  if(preset.value==="evidence"&&!selectedEvidence.value.length)return i18n.t("research.evidence_required","Select at least one evidence record for evidence-only research.");
  if(preset.value!=="evidence"&&!config.value?.source_collection&&!stores.value.length)return i18n.t("research.database_required","A corpus database or selected evidence is required.");
  return "";
});
...
function profileModel(profile:ResearchProfile|null){
  if(!profile)return "";
  return profile.type==="openai"&&profile.model_mode==="auto"?"auto":String(profile.model||"");
}
function inferPreset(cfg:ResearchConfig){
  ...
  if(cfg.k===64&&cfg.fetch_k===500&&cfg.rerank_top_n===24&&Math.abs(Number(cfg.lambda_mult)-.7)<.001)return "balanced";
  return "custom";
}
function hydrate(snapshot:ResearchWorkspaceSnapshot,{preserveDraft=false,preserveActive=true}={}){
  ...
  if(snapshot.can_manage_jobs||auth.isAdmin){
    ...
    const merged=[...snapshotJobs,...retained].filter((job,index,all)=>all.findIndex(candidate=>candidate.id===job.id)===index);
    jobs.value=merged;
  }
  ...
  const profile=snapshot.profiles.find(item=>item.id===snapshot.config.provider_profile_id)||snapshot.profiles[0]||null;
  ...
  if(preserveActive&&activeJob.value){
    ...
    if(updated)activeJob.value={...activeJob.value,...updated,result:activeJob.value.result||updated.result};
  }else if(!activeJob.value){
    activeJob.value=jobs.value.find(job=>["queued","running","cancelling"].includes(job.status))||jobs.value.find(job=>job.status==="completed")||null;
  }
}
async function loadWorkspace(refresh=true){
  ...
  try{
    ...
    if(requestedJobId){
      ...
      if(!jobs.value.some(job=>job.id===activeJob.value?.id))jobs.value=[activeJob.value,...jobs.value];
      activeEvidenceIndex.value=0;
      await nextTick();
      document.querySelector(".research-answer-workspace")?.scrollIntoView({behavior:"smooth",block:"start"});
...
function persistDraft(){
  ...
  draftTimer=window.setTimeout(()=>runtime.updateResearchConfig({prompt:prompt.value,instructions:instructions.value}),250);
}
...
function applyPreset(value:string){
  ...
  if(!config.value)return;
  if(value==="evidence"){
    if(!selectedEvidence.value.length){preset.value="balanced";return}
  ...
  try{
    if(canManageRuns.value){
      ...
      if(activeJob.value){
        ...
        if(summary){
          activeJob.value={...activeJob.value,...summary,result:activeJob.value.result||summary.result};
          if(["completed","failed","cancelled"].includes(summary.status))activeJob.value=await runtime.getResearchJob(summary.id) as ResearchJob;
        }
      }
    }else{
      ...
      const activeSessionJobs=jobs.value.filter(job=>sessionJobIds.value.has(job.id)&&["queued","running","cancelling"].includes(job.status));
      if(activeSessionJobs.length){
        const refreshed=await Promise.all(activeSessionJobs.map(job=>runtime.getResearchJob(job.id) as Promise<ResearchJob>));
        const byId=new Map(refreshed.map(job=>[job.id,job]));
        jobs.value=jobs.value.map(job=>byId.get(job.id)||job);
        if(activeJob.value&&byId.has(activeJob.value.id))activeJob.value=byId.get(activeJob.value.id)||activeJob.value;
      }
    }
  }catch(error){console.warn("Research polling failed",error)}
  schedulePoll();
}
function schedulePoll(force=false){
  window.clearTimeout(pollTimer);
  const active=Boolean(activeJob.value&&["queued","running","cancelling"].includes(activeJob.value.status))||jobs.value.some(job=>["queued","running","cancelling"].includes(job.status));
  if((force||active)&&isNativeResearch.value)pollTimer=window.setTimeout(poll,3000);
}
async function openJob(job:ResearchJob){
  try{
    if(route.query.job)await router.replace({path:"/rag"});
    activeJob.value=job.status==="completed"?await runtime.getResearchJob(job.id) as ResearchJob:job;
    activeEvidenceIndex.value=0;
    runsDrawer.value?.close();
    await nextTick();document.querySelector(".research-answer-workspace")?.scrollIntoView({behavior:"smooth",block:"start"});
  }catch(error){runtime.notifyToast(error instanceof Error?error.message:String(error),{tone:"danger"})}
}
async function cancelJob(job:ResearchJob){
  try{const updated=await runtime.cancelResearchJob(job.id) as ResearchJob;if(activeJob.value?.id===job.id)activeJob.value=updated;await refreshRuns()}catch(error){runtime.notifyToast(error instanceof Error?error.message:String(error),{tone:"danger"})}
}
async function removeJob(job:ResearchJob){
  if(!window.confirm(i18n.t("research.remove_run_confirm","Remove this retained Research run and its result?")))return;
  try{await runtime.deleteResearchJob(job.id);if(activeJob.value?.id===job.id)activeJob.value=null;await refreshRuns()}catch(error){runtime.notifyToast(error instanceof Error?error.message:String(error),{tone:"danger"})}
}
async function refreshRuns(){jobs.value=await runtime.refreshResearchJobs() as ResearchJob[];schedulePoll()}
function loadHistory(item:Record<string,unknown>){prompt.value=String(item.prompt||"");instructions.value=String(item.instructions||"");persistDraft();runtime.notifyToast(i18n.t("research.question_restored","Recent question restored"),{tone:"success"})}
function removeEvidence(key:string){
  try{const evidence=runtime.removeResearchEvidence(key);if(workspace.value)workspace.value={...workspace.value,selected_evidence:evidence};if(!evidence.length&&preset.value==="evidence")applyPreset("balanced")}catch(error){runtime.notifyToast(error instanceof Error?error.message:String(error),{tone:"danger"})}
}
function clearEvidence(){
  try{runtime.clearResearchEvidence();if(workspace.value)workspace.value={...workspace.value,selected_evidence:[]};if(config.value)config.value={...config.value,skip_retrieval:false};if(preset.value==="evidence")preset.value="balanced"}catch(error){runtime.notifyToast(error instanceof Error?error.message:String(error),{tone:"danger"})}
}
async function copyAnswer(){try{await navigator.clipboard.writeText(activeResult.value?.answer||"");runtime.notifyToast(i18n.t("research.answer_copied","Answer copied"),{tone:"success"})}catch{runtime.notifyToast(i18n.t("research.clipboard_failed","Could not access the clipboard"),{tone:"danger"})}}
function focusEvidence(index:number){activeEvidenceIndex.value=index;if(window.matchMedia("(max-width: 860px)").matches)void nextTick(()=>document.querySelector("#researchEvidencePanel")?.scrollIntoView({behavior:"smooth",block:"start"}))}
async function gradeAnswer(){if(!activeJob.value)return;try{await runtime.gradeResearchJob(activeJob.value.id)}catch(error){runtime.notifyToast(error instanceof Error?error.message:String(error),{tone:"danger"})}}
function prepareRerun(){if(!activeJob.value)return;if(route.query.job)void router.replace({path:"/rag"});const next=runtime.prepareResearchRerun(activeJob.value) as ResearchConfig;config.value={...next};prompt.value=next.prompt||"";instructions.value=next.instructions||"";preset.value=inferPreset(next);const profile=profiles.value.find(item=>item.id===next.provider_profile_id)||selectedProfile.value;model.value=profileModel(profile||null);generation.value=profileGeneration(profile||null);activeJob.value=null;runtime.notifyToast(i18n.t("research.rerun_loaded","Run parameters loaded into the composer"),{tone:"success"});void nextTick(()=>document.querySelector<HTMLTextAreaElement>("#researchQuestion")?.focus())}
async function discoverModels(){if(!config.value)return;try{discoveredModels.value=await runtime.discoverResearchModels(config.value.provider_profile_id) as string[];runtime.notifyToast(`${discoveredModels.value.length} ${i18n.t('research.models_found','models found')}`,{tone:"success"})}catch(error){runtime.notifyToast(error instanceof Error?error.message:String(error),{tone:"danger"})}}
function applySettings(payload:{config:Partial<ResearchConfig>;generation:Record<string,unknown>;model:string}){updateConfig(payload.config);generation.value={...payload.generation};model.value=payload.model||profileModel(selectedProfile.value);preset.value="custom";runtime.notifyToast(i18n.t("research.settings_applied","Research settings applied"),{tone:"success"})}

watch(prompt,persistDraft);
watch(instructions,persistDraft);
watch(()=>[route.name,route.query.job],([name])=>{window.clearTimeout(pollTimer);if(name==="rag")void loadWorkspace(true)});
onMounted(()=>{if(isNativeResearch.value)void loadWorkspace(true)});
onBeforeUnmount(()=>{window.clearTimeout(pollTimer);window.clearTimeout(draftTimer)});
</script>
