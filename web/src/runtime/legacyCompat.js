...
export function translateDynamicUiValue(state, value) {
  ...
  const noun = (oneKey, manyKey, oneFallback, manyFallback, count) =>
    tr(state, numericCount(count) === 1 ? oneKey : manyKey, numericCount(count) === 1 ? oneFallback : manyFallback);
  let match;
  ...
  if ((match = text.match(/^([0-9][0-9., \u00a0]*) selected evidence$/i))) return `${prefix}${match[1]} ${tr(state, "dynamic.selected_evidence", "selected evidence")}${suffix}`;
  if ((match = text.match(/^([0-9][0-9., \u00a0]*) selected$/i))) return `${prefix}${trf(state, "dynamic.selected_count", "{count} selected", { count: match[1] })}${suffix}`;
  if ((match = text.match(/^([0-9][0-9., \u00a0]*) of ([0-9][0-9., \u00a0]*) records$/i))) return `${prefix}${trf(state, "dynamic.record_range_count", "{shown} of {total} records", { shown: match[1], total: match[2] })}${suffix}`;
  if ((match = text.match(/^Page ([0-9]+) \/ ([0-9]+)$/i))) return `${prefix}${trf(state, "dynamic.page_of_pages", "Page {page} / {pages}", { page: match[1], pages: match[2] })}${suffix}`;
  if ((match = text.match(/^Ready · ([0-9][0-9., \u00a0]*) models$/i))) return `${prefix}${trf(state, "dynamic.ready_models", "Ready · {count} models", { count: match[1] })}${suffix}`;
  if ((match = text.match(/^Showing latest ([0-9]+) of ([0-9]+) changes\. Full history is preserved in the record's updates field\.$/i))) return `${prefix}${trf(state, "dynamic.history_latest", "Showing latest {shown} of {total} changes. Full history is preserved in the record's updates field.", { shown: match[1], total: match[2] })}${suffix}`;
  if ((match = text.match(/^Cleared updates history from ([0-9][0-9., \u00a0]*) records$/i))) return `${prefix}${trf(state, "dynamic.cleared_history_records", "Cleared updates history from {count} records", { count: match[1] })}${suffix}`;
  if ((match = text.match(/^Restored original record state · ([0-9][0-9., \u00a0]*) fields changed$/i))) return `${prefix}${trf(state, "dynamic.restored_fields", "Restored original record state · {count} fields changed", { count: match[1] })}${suffix}`;
  if ((match = text.match(/^Loaded ([0-9][0-9., \u00a0]*) records(?: · ([0-9][0-9., \u00a0]*) parse issues)?$/i))) return `${prefix}${trf(state, match[2] ? "dynamic.loaded_records_issues" : "dynamic.loaded_records", match[2] ? "Loaded {count} records · {issues} parse issues" : "Loaded {count} records", { count: match[1], issues: match[2] || "0" })}${suffix}`;
  if ((match = text.match(/^Merged and replaced ([0-9][0-9., \u00a0]*) tabs · ([0-9][0-9., \u00a0]*) records$/i))) return `${prefix}${trf(state, "dynamic.merged_tabs_records", "Merged and replaced {tabs} tabs · {records} records", { tabs: match[1], records: match[2] })}${suffix}`;
  if ((match = text.match(/^([0-9][0-9., \u00a0]*) records cleaned · ([0-9][0-9., \u00a0]*) tracked changes$/i))) return `${prefix}${trf(state, "dynamic.cleaned_records", "{records} records cleaned · {changes} tracked changes", { records: match[1], changes: match[2] })}${suffix}`;
  if ((match = text.match(/^Exported ([0-9][0-9., \u00a0]*) records from (.+)$/i))) return `${prefix}${trf(state, "dynamic.exported_records", "Exported {count} records from {collection}", { count: match[1], collection: match[2] })}${suffix}`;
  if ((match = text.match(/^Grading ([0-9]+) of ([0-9]+) · (.+)$/i))) return `${prefix}${trf(state, "dynamic.grading_progress", "Grading {current} of {total} · {question}", { current: match[1], total: match[2], question: match[3] })}${suffix}`;
  if ((match = text.match(/^Translating ([0-9][0-9., \u00a0]*) interface strings to (.+)$/i))) return `${prefix}${trf(state, "dynamic.translating_interface", "Translating {count} interface strings to {locale}", { count: match[1], locale: match[2] })}${suffix}`;
  if ((match = text.match(/^Running (.+)$/i))) return `${prefix}${trf(state, "dynamic.running_operation", "Running {label}", { label: match[1] })}${suffix}`;
  if ((match = text.match(/^Completed · ([0-9][0-9., \u00a0]*) graded, ([0-9][0-9., \u00a0]*) failed$/i))) return `${prefix}${trf(state, "dynamic.completed_grading", "Completed · {graded} graded, {failed} failed", { graded: match[1], failed: match[2] })}${suffix}`;
  if ((match = text.match(/^Completed · ([0-9][0-9., \u00a0]*) works, ([0-9][0-9., \u00a0]*) failed$/i))) return `${prefix}${trf(state, "dynamic.completed_works", "Completed · {works} works, {failed} failed", { works: match[1], failed: match[2] })}${suffix}`;
  if ((match = text.match(/^Waiting for provider slot: ([0-9]+) active \/ ([0-9]+) allowed$/i))) return `${prefix}${trf(state, "dynamic.waiting_provider_active", "Waiting for provider slot: {active} active / {allowed} allowed", { active: match[1], allowed: match[2] })}${suffix}`;
  if ((match = text.match(/^Waiting for provider slot \(([0-9]+)\/([0-9]+) active\)$/i))) return `${prefix}${trf(state, "dynamic.waiting_provider_active_paren", "Waiting for provider slot ({active}/{allowed} active)", { active: match[1], allowed: match[2] })}${suffix}`;
  if ((match = text.match(/^Waiting for Ollama slot: ([0-9]+) active \/ ([0-9]+) allowed$/i))) return `${prefix}${trf(state, "dynamic.waiting_ollama_active", "Waiting for Ollama slot: {active} active / {allowed} allowed", { active: match[1], allowed: match[2] })}${suffix}`;
  return raw;
}
