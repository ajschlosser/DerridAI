...
export function createVectorCollectionBridge({state,workIndex,recordStores,tr,trf,esc,icon,api,refreshStores,persistPrefs,upsertRows,toast,openMessageModal,decorateDisabledControls,showAppModal}){
...
function openCollectionCreationWizard({defaultProvider="ollama",defaultModel="bge-m3:latest",installedModels=[],providerProfiles=[]}={}){
  ...
  let step=0;
  const steps=[tr("vector.create_step_source"),tr("vector.create_step_retrieval"),tr("vector.create_step_review")];
  ...
  const runPreflight=async(button)=>{
    persistFields();
    if(form.provider.startsWith("profile:")&&!form.model){toast(tr("vector.embedding_model_required"));return false}
    button.disabled=true;const old=button.innerHTML;button.textContent=tr("vector.running_preflight");
    try{
      form.preflight=await api("/api/stores/preflight/embedding",{method:"POST",body:JSON.stringify({embedding_provider:form.provider,embedding_model:form.provider.startsWith("profile:")?form.model:null,embedding_dimension:form.dimension?Number(form.dimension):null,distance_metric:form.distance})});
      if(form.preflight?.embedding_dimension)form.dimension=String(form.preflight.embedding_dimension);
      return true;
    }catch(error){openMessageModal({title:tr("vector.preflight_failed"),message:error.message||String(error),tone:"danger"});return false}
  ...
  const render=()=>{
    ...
    dialog.querySelector("#wizardNext")?.addEventListener("click",async event=>{persistFields();if(step===0&&!form.name)return toast(tr("vector.collection_name_required"));if(step===0){step=1;render();return}if(step===1){const ok=await runPreflight(event.currentTarget);if(ok){step=2;render()}}});
    dialog.querySelectorAll('input[name="wizardEmbeddingProviderRadio"]').forEach(input=>input.addEventListener("change",event=>{persistFields();form.provider=event.target.value;const profile=embeddingProfiles.find(item=>`profile:${item.id}`===form.provider);if(profile?.model)form.model=String(profile.model);form.preflight=null;render()}));
    ...
    dialog.querySelector("#wizardCreate")?.addEventListener("click",async()=>{
      ...
      const button=dialog.querySelector("#wizardCreate");button.disabled=true;button.textContent=tr("vector.creating_collection");
      try{
        await api("/api/stores",{method:"POST",body:JSON.stringify({name:form.name,description:form.description||null,embedding_provider:form.provider,embedding_model:form.provider.startsWith("profile:")?form.model:null,embedding_dimension:form.dimension?Number(form.dimension):null,distance_metric:form.distance,retrieval_mode:form.retrieval,text_field:"text",language_codes:form.languages.size?[...form.languages]:null,collection_role:form.role||null,protected:form.protected})});
        ...
        const syncStarted=rows.length?await upsertRows(rows,trf("vector.selected_works_label", {count:form.selectedWorks.size}),{largeSyncConfirmed:true}):false;
        toast(rows.length&&syncStarted?trf("vector.collection_created_build", {name:form.name}):trf("vector.collection_created", {name:form.name}),{tone:"success"});
        notifyVectorStoresChanged();
      }catch(error){button.disabled=false;button.textContent=tr("vector.create_collection");openMessageModal({title:tr("vector.create_failed"),message:error.message||String(error),tone:"danger"})}
    });
  ...
  return {notifyVectorStoresChanged,openDatabaseCreationFromResearch,openCollectionCreationWizard};
}
