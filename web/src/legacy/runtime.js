...
const TABLE_DEFAULTS={
  list:["__db_status","record_id","work","page_start","speaker","position_holder","target","discourse_role","needs_review","text"],
  global:["__file","__db_status","record_id","work","page_start","speaker","position_holder","target","discourse_role","needs_review","text"],
...
async function restoreWorkspace(){
  try{
    ...
    if(prefs){
      ...
      for(const key of ["selected","searches","listFilters","pages","sorts","globalSearch","globalFilters","globalSort","globalPage","globalSearchMode","globalSearchAutoRun","searchResultLayouts","dbSearchMethod","dbSearchWhere","dbSearchFetchK","dbSearchLambda","worksSearch","workOverview","researcherRecordId","researcherCompareA","researcherCompareB","dashboardMetricIndex","lastViewedRecord","compareA","compareB","compareMode","comparePasteA","comparePasteB","faqSearch","faqPage","faqExpanded","activeStore","storePage","storePageSize","storeQuery","storeWork","storeSort","storeFilters","storeBrowseMode","storeSearchSort","selectedEvidence","navHistory","navForward","sidebarCollapsed","collectionsCollapsed","operationToastsMinimized","operationStackPosition","collapsedPanels","tableColumns","upsertState","upsertIgnored","jobApplied","upsertJobApplied"]){
...
function sortHead(text,key,sort){const arrow=sort.key===key?(sort.dir===1?"▲":"▼"):"";return `<th><button data-sort="${esc(key)}">${esc(text)} ${arrow}</button></th>`}
...
function dataCellHtml(row,key,query=""){
  ...
  if(key==="text")return `<td class="textcell">${highlightTerms(snippet(record.text,query),query)}</td>`;
...
function dataHeadHtml(key,sort){
  if(["__db_status"].includes(key))return `<th>${esc(label(key))}</th>`;
  return sortHead(label(key),key,sort);
}
...
async function foregroundUpsertRows(rows,items,store,labelText){
  if(state.foregroundUpsertActive)return openMessageModal({title:tr("operations.foreground_sync_active_title","Large sync already running"),message:tr("operations.foreground_sync_active_help","Wait for the current large foreground sync to finish before starting another."),confirmLabel:"OK"});
  state.foregroundUpsertActive=true;state.foregroundUpsertCancelRequested=false;const total=items.length,batchSize=500;
  const dialog=document.createElement("dialog");dialog.className="message-dialog blocking-upsert-dialog";
  dialog.innerHTML=`<div class="dh"><div><h2 class="dialog-title">${esc(trf("operations.foreground_sync_title","Syncing {count} records",{count:total.toLocaleString()}))}</h2><div class="dialog-subtitle">${esc(tr("operations.blocking_sync_help","This large upsert runs in foreground batches of 500. Other DerridAI actions are paused until it finishes or you cancel."))}</div></div></div><div class="db"><div class="blocking-upsert-progress"><progress id="blockingUpsertProgress" max="${total}" value="0"></progress><div><b id="blockingUpsertCount">0 / ${total.toLocaleString()}</b><span id="blockingUpsertStatus">${esc(tr("operations.preparing_sync","Preparing records…"))}</span></div></div><div class="info warn">${esc(tr("operations.keep_tab_open","Keep this DerridAI tab open while the sync is running."))}</div></div><div class="da"><button class="btn danger" id="cancelBlockingUpsert">${esc(tr("ui.cancel","Cancel"))}</button></div>`;
  document.body.appendChild(dialog);showAppModal(dialog);
  const progress=dialog.querySelector("#blockingUpsertProgress"),count=dialog.querySelector("#blockingUpsertCount"),status=dialog.querySelector("#blockingUpsertStatus"),cancel=dialog.querySelector("#cancelBlockingUpsert");
  ...
  try{
    ...
    for(let start=0;start<items.length;start+=batchSize){
      ...
      const transportItems=chunk.map(item=>({
    ...
    closeDialog();toast(trf("operations.foreground_sync_done","Synced {count} records to {store}",{count:total.toLocaleString(),store}),{tone:"success"});if(state.view==="works")renderWorks(document.querySelector("#main"));return true;
  }catch(error){
    closeDialog();if(error?.cancelled||error?.name==="AbortError"||state.foregroundUpsertCancelRequested){toast(tr("operations.sync_cancelled","Sync cancelled. Completed batches remain synced."),{tone:"warn"});return false}
    openMessageModal({title:tr("operations.foreground_sync_failed_title","Large sync failed"),message:error.message||String(error),tone:"danger"});return false;
  }finally{state.foregroundUpsertActive=false;state.foregroundUpsertCancelRequested=false;controller=null}
}
async function upsertRows(rows,labelText="records",{largeSyncConfirmed=false}={}){
  ...
  if(activeUpsert)return openMessageModal({title:"An upsert is already active",message:`${activeUpsert.label||activeUpsert.store_name||"The current sync"} must finish or be cancelled before another Chroma upsert starts.`,confirmLabel:"OK"});
  const store=state.activeStore;
  // Presence is a tiny ID-only request and lets us distinguish a new row (which
  // may need one-time history initialization) from an existing row whose audit
  // history should remain server-side.
  await refreshPresenceForRows(rows,{force:true});
  if(rows.length>500){
    if(!largeSyncConfirmed){
      const approved=await openMessageModal({title:tr("operations.large_sync_foreground_title","Large sync runs in the foreground"),message:trf("operations.large_sync_foreground_help","{count} records exceed the 500-record background limit. DerridAI will sync them in foreground batches of 500. Other actions are blocked until the sync finishes or you cancel.",{count:rows.length.toLocaleString()}),confirmLabel:tr("operations.continue_sync","Continue sync"),cancelLabel:tr("ui.cancel","Cancel")});
      if(!approved)return false;
    }
    const items=await buildUpsertItems(rows,store,{yieldEvery:80});
    return foregroundUpsertRows(rows,items,store,labelText);
  }
  const items=await buildUpsertItems(rows,store);
  try{
    ...
    const job=await api("/api/jobs/upsert",{method:"POST",body:JSON.stringify({store_name:store,items:transportItems,document_field:"text",embedding_field:"embedding",batch_size:500,mirror_languages:true,label:labelText})});
    state.jobs=[job,...state.jobs.filter(existing=>existing.id!==job.id)];
    syncJobProgressToasts();startJobPolling();
    toast(`Queued ${rows.length.toLocaleString()} ${labelText} for ${store}`);
    if(state.view==="home")refreshOperationsPanelOnly();
    return true;
  }catch(error){toast(`Could not start Chroma upsert: ${error.message}`);return false}
}
...
function currentTableUrlState(){
  if(state.view==="list"){
    const f=activeFile();if(!f)return null;
    return {c:state.tableColumns.list||null,s:state.sorts[f.id]||null,f:state.listFilters[f.id]||null,p:state.pages[f.id]||1,z:state.pageSize};
  }
  if(state.view==="global")return {c:state.tableColumns.global||null,s:state.globalSort,f:state.globalFilters,p:state.globalPage,z:state.pageSize,q:state.globalSearch};
  if(state.view==="vector")return {c:state.tableColumns.vector||null,s:state.storeSort,f:state.storeFilters,p:state.storePage,z:state.storePageSize,w:state.storeWork,b:state.storeBrowseMode,ss:state.storeSearchSort,q:state.storeQuery};
  return null;
}
function applyCompressedTableUrlState(value){
  ...
  if(state.view==="list"){
    ...
    if(value.f)state.listFilters[f.id]=value.f;
    ...
    if(value.f)state.storeFilters=value.f;
...
function applyUrlState(){
  ...
  if(file&&state.files.some(item=>item.id===file))state.activeFileId=file;
...
function dashboardPieChart(series,title,{valueLabel="records"}={}){
  ...
  return `<div class="dashboard-pie-layout"><div class="dashboard-pie" style="background:conic-gradient(${stops})" role="img" aria-label="${esc(title)}"></div><div class="dashboard-pie-legend">${series.map((item,index)=>{const pct=Number(item.value||0)/total*100;return `<button type="button" data-dashboard-work="${esc(item.key)}" ${item.key===tr("dashboard.other_works","Other works")?"disabled":""}><i style="background:${colors[index%colors.length]}"></i><span title="${esc(item.key)}">${esc(item.key)}</span><b>${pct.toFixed(pct>=10?0:1)}%</b><small>${Number(item.value||0).toLocaleString()} ${esc(valueLabel)}</small></button>`}).join("")}</div></div>`;
}
function dashboardMetricBody(metric){
  if(metric.type==="pie")return dashboardPieChart(metric.values,metric.title,{valueLabel:metric.valueLabel});
  ...
  return `<div class="dashboard-average-list">${ranking.map(item=>`<button class="dashboard-average-row" data-dashboard-work="${esc(item.key)}"><span>${esc(item.key)}</span><i><em style="width:${Math.max(4,Math.round(Number(item.value)/maxRank*100))}%"></em></i><b>${esc(metric.format(item.value))}</b></button>`).join("")||`<div class="note">${esc(tr("research.no_works","No works loaded yet."))}</div>`}</div>`;
}
...
async function renderFaq(main){
  ...
  let timer=null;
  main.querySelector("#faqSearch").oninput=e=>{state.faqSearch=e.target.value;state.faqPage=1;persistPrefs();clearTimeout(timer);timer=setTimeout(()=>{if(state.view==="faq")renderFaq(main)},280)};
  main.querySelector("#faqClearSearch")?.addEventListener("click",()=>{state.faqSearch="";state.faqPage=1;persistPrefs();renderFaq(main)});
  ...
  main.querySelector("#faqPrev").onclick=()=>{state.faqPage=Math.max(1,(state.faqPage||1)-1);persistPrefs();renderFaq(main)};
  main.querySelector("#faqNext").onclick=()=>{state.faqPage=Math.min(pagesTotal,(state.faqPage||1)+1);persistPrefs();renderFaq(main)};
...
async function renderRag(main){
    ...
    <section class="research-hero"><div><span class="section-label">${esc(tr("nav.rag","Research"))}</span><h1>${esc(tr("research.workspace_title","Evidence-grounded research workspace"))}</h1><p>${esc(tr("research.workspace_help","Build a question, choose retrieval and generation settings, pin evidence, then run the full provenance-aware pipeline."))}</p></div><div class="research-hero-status"><span>${selectedEvidenceEntries().length.toLocaleString()} ${esc(tr("rag.selected_evidence","selected evidence"))}</span><span>${corpusStores.length.toLocaleString()} ${esc(tr("dashboard.databases","databases"))}</span></div></section><div class="rag-runner-page">
      <section class="card rag-runner-main">
        ...
        <section class="rag-options card-inset research-settings-body">
          ...
          <div class="rag-option-section"><b>Retrieval routes</b><div class="language-checks">${[["mmr","MMR"],["similarity","Similarity"]].map(([value,name])=>`<label><input type="checkbox" data-rag-search="${value}" ${cfg.search_types.includes(value)?"checked":""}><span>${name}</span></label>`).join("")}</div></div>
  ...
  document.querySelector("#runRag").onclick=async()=>{
    ...
    if(!cfg.skip_retrieval&&!cfg.search_types.length)return toast("Select MMR and/or similarity retrieval");
...
function renderEmpty(main){
  main.innerHTML=`<section class="empty"><div class="drop"><div class="drop-icon">${icon("upload")}</div><h1>Open a corpus workspace</h1><p>Drop one or more JSONL files anywhere on this page, or choose files manually. Each file stays in its own tab and can be edited, compared, searched, exported, or sent to ChromaDB.</p><button class="btn primary" id="choose">${icon("upload")}Choose JSONL files</button></div></section>`;
...
async function importFiles(fileList){
  ...
  for(const file of [...fileList]){
    ...
    const item={id:uid(),name:file.name,records:parsed.records,errors:parsed.errors,dirty:new Set(),imported_at:new Date().toISOString()};
    state.files.push(item);persistFileNow(item);first ||= item.id;total+=item.records.length;errors+=item.errors.length;
  }
  if(first)state.activeFileId=first;
  shell();renderView();toast(`Loaded ${total} records${errors?` · ${errors} parse issues`:""}`);
}
...
function renderList(main){
  ...
  const available=tableAvailableFields(f.records.map((record,index)=>({file:f,record,index})),["__db_status"]);
  ...
  <section class="card tablewrap"><table class="configurable-table"><thead><tr><th class="select-col"><input id="selectPage" type="checkbox" title="Select visible records" ${pageSelected?"checked":""}></th>${columns.map(key=>dataHeadHtml(key,sort)).join("")}<th class="record-actions-head">Record actions</th></tr><tr class="column-filter-row"><th></th>${columns.map(key=>`<th>${listFilterControl(f.id,key)}</th>`).join("")}<th></th></tr></thead><tbody>${slice.map(x=>`<tr class="clickable ${state.reviewSelection.has(reviewKey(f,x.index))?"row-selected":""}" data-index="${x.index}"><td class="select-col"><input class="row-select" type="checkbox" data-select-index="${x.index}" ${state.reviewSelection.has(reviewKey(f,x.index))?"checked":""}></td>${columns.map(key=>dataCellHtml(x,key,q)).join("")}${workspaceRecordActionsHtml(x)}</tr>`).join("")||`<tr><td colspan="${columns.length+2}" style="padding:30px;text-align:center;color:#777">No matches</td></tr>`}</tbody></table></section>${pager(pg,rows.length,"list")}`;
  ...
  document.querySelectorAll("tr[data-index]").forEach(row=>row.onclick=e=>{if(e.target.closest("input,button"))return;navigateTo("record",{fileId:f.id,index:+row.dataset.index})});
...
function workMetadataControl(field,rows){
  ...
  return `<div class="work-meta-row"><label class="work-meta-apply"><input type="checkbox" data-work-meta-apply="${esc(field)}"><span>Apply</span></label><div class="work-meta-field"><b>${esc(label(field))}</b>${mixed?'<span class="mixed-value">mixed across records</span>':""}</div>${control}</div>`;
}
...
async function renderWorks(main){
  ...
  const metadataValue=(rows,field)=>{const value=commonWorkValue(rows,field);return value.mixed?`<span class="mixed-value">${esc(tr("works.mixed","Mixed"))}</span>`:esc(display(value.value))};
  ...
  search.oninput=e=>{
    ...
    searchTimer=setTimeout(()=>{if(state.view!=="works")return;renderWorks(main);requestAnimationFrame(()=>{const x=main.querySelector("#worksSearch");if(x){x.focus();x.setSelectionRange(pos,pos)}})},180);
  };
  ...
  grid.addEventListener("click",e=>{
    ...
    if(card){const y=window.scrollY;state.workOverview=card.dataset.work||"";persistPrefs();renderWorks(main);requestAnimationFrame(()=>window.scrollTo(0,y))}
  });
...
async function renderResearcherGlobal(main){
  ...
  const run=async()=>{const query=String(main.querySelector("#rGlobalQuery")?.value||"").trim();if(method!=="filter"&&!query)return;state.globalSearch=query;state.storeSearchLoading=true;state.storeSearchResults=[];persistPrefs();renderResearcherGlobal(main);try{const body={query,mode:method,n_results:100,where:Object.keys(dbSearchWhere()).length?dbSearchWhere():null,fetch_k:Number(state.dbSearchFetchK||100),lambda_mult:Number(state.dbSearchLambda??0.7)};const data=await api(`/api/stores/${encodeURIComponent(state.activeStore)}/search`,{method:"POST",body:JSON.stringify(body)});state.storeSearchResults=data.results||[]}catch(error){toast(`${tr("research.search_failed","Search failed")}: ${error.message}`,{tone:"danger"})}finally{state.storeSearchLoading=false;persistPrefs();renderResearcherGlobal(main)}};
  main.querySelectorAll("[data-r-global-mode]").forEach(button=>button.onclick=()=>{state.globalSearchMode=button.dataset.rGlobalMode;state.storeSearchResults=[];persistPrefs();renderResearcherGlobal(main)});
  wireSearchLayoutControls(main,()=>renderResearcherGlobal(main));
  main.querySelector("#rGlobalStore")?.addEventListener("change",event=>{state.activeStore=event.target.value;state.storeSearchResults=[];persistPrefs();renderResearcherGlobal(main)});
  main.querySelector("#rSearchMethod")?.addEventListener("change",event=>{state.dbSearchMethod=event.target.value;state.storeSearchResults=[];persistPrefs();renderResearcherGlobal(main)});
  main.querySelector("#rFetchK")?.addEventListener("change",event=>{state.dbSearchFetchK=Math.max(1,Number(event.target.value)||100);persistPrefs()});main.querySelector("#rLambda")?.addEventListener("change",event=>{state.dbSearchLambda=Math.max(0,Math.min(1,Number(event.target.value)||0.7));persistPrefs()});
  main.querySelector("#runRGlobalSearch")?.addEventListener("click",run);main.querySelector("#rGlobalQuery")?.addEventListener("keydown",event=>{if(event.key==="Enter"){event.preventDefault();void run()}});main.querySelector("#clearRGlobalSearch")?.addEventListener("click",()=>{state.globalSearch="";state.storeSearchResults=[];state.dbSearchWhere={};persistPrefs();renderResearcherGlobal(main)});
  main.querySelector("#addRFilter")?.addEventListener("click",()=>{const field=main.querySelector("#rFilterField")?.value;const value=main.querySelector("#rFilterValue")?.value?.trim();if(field&&value){state.dbSearchWhere={...(state.dbSearchWhere||{}),[field]:value};persistPrefs();renderResearcherGlobal(main)}});main.querySelectorAll("[data-remove-r-filter]").forEach(button=>button.onclick=()=>{const next={...(state.dbSearchWhere||{})};delete next[button.dataset.removeRFilter];state.dbSearchWhere=next;persistPrefs();renderResearcherGlobal(main)});
...
async function renderAdminDatabaseGlobal(main){
  ...
  try{await refreshStores()}catch(error){main.innerHTML=`<div class="info error">${esc(error.message)}</div>`;return}
  const stores=recordStores();if(!state.activeStore&&stores.length)state.activeStore=stores[0].name;
  if(!stores.length){main.innerHTML=`<section class="empty"><div class="drop"><div class="drop-icon">${icon("database")}</div><h1>${esc(tr("research.no_database","No corpus database available"))}</h1><p>${esc(tr("research.no_database_admin_help","Create or restore a corpus database to use semantic search."))}</p></div></section>`;return}
  ...
  const run=async()=>{const query=String(main.querySelector("#adminGlobalQuery")?.value||"").trim();if(method!=="filter"&&!query)return;state.globalSearch=query;state.storeSearchLoading=true;state.storeSearchResults=[];persistPrefs();renderAdminDatabaseGlobal(main);try{const body={query,mode:method,n_results:100,where:Object.keys(dbSearchWhere()).length?dbSearchWhere():null,fetch_k:Number(state.dbSearchFetchK||100),lambda_mult:Number(state.dbSearchLambda??0.7)};const data=await api(`/api/stores/${encodeURIComponent(state.activeStore)}/search`,{method:"POST",body:JSON.stringify(body)});state.storeSearchResults=data.results||[]}catch(error){toast(`${tr("research.search_failed","Search failed")}: ${error.message}`,{tone:"danger"})}finally{state.storeSearchLoading=false;persistPrefs();renderAdminDatabaseGlobal(main)}};
  main.querySelectorAll("[data-global-mode]").forEach(button=>button.onclick=()=>{state.globalSearchMode=button.dataset.globalMode;state.storeSearchResults=[];persistPrefs();renderGlobal(main)});
  wireSearchLayoutControls(main,()=>renderAdminDatabaseGlobal(main));
  main.querySelector("#adminGlobalStore")?.addEventListener("change",event=>{state.activeStore=event.target.value;state.storeSearchResults=[];persistPrefs();renderAdminDatabaseGlobal(main)});
  main.querySelector("#adminSearchMethod")?.addEventListener("change",event=>{state.dbSearchMethod=event.target.value;state.storeSearchResults=[];persistPrefs();renderAdminDatabaseGlobal(main)});
  main.querySelector("#adminFetchK")?.addEventListener("change",event=>{state.dbSearchFetchK=Math.max(1,Number(event.target.value)||100);persistPrefs()});main.querySelector("#adminLambda")?.addEventListener("change",event=>{state.dbSearchLambda=Math.max(0,Math.min(1,Number(event.target.value)||0.7));persistPrefs()});
  main.querySelector("#runAdminGlobalSearch")?.addEventListener("click",run);main.querySelector("#adminGlobalQuery")?.addEventListener("keydown",event=>{if(event.key==="Enter"){event.preventDefault();void run()}});main.querySelector("#clearAdminGlobalSearch")?.addEventListener("click",()=>{state.globalSearch="";state.storeSearchResults=[];state.dbSearchWhere={};persistPrefs();renderAdminDatabaseGlobal(main)});
  main.querySelector("#addAdminFilter")?.addEventListener("click",()=>{const field=main.querySelector("#adminFilterField")?.value;const value=main.querySelector("#adminFilterValue")?.value?.trim();if(field&&value){state.dbSearchWhere={...(state.dbSearchWhere||{}),[field]:value};persistPrefs();renderAdminDatabaseGlobal(main)}});main.querySelectorAll("[data-remove-admin-filter]").forEach(button=>button.onclick=()=>{const next={...(state.dbSearchWhere||{})};delete next[button.dataset.removeAdminFilter];state.dbSearchWhere=next;persistPrefs();renderAdminDatabaseGlobal(main)});
...
function renderTraditionalGlobal(main){
  ...
  document.querySelectorAll("tr[data-f]").forEach(row=>row.onclick=e=>{if(e.target.closest("input,button"))return;navigateTo("record",{fileId:row.dataset.f,index:+row.dataset.i})});
...
function openCollectionCreationWizard({defaultProvider="ollama",defaultModel="bge-m3:latest",installedModels=[]}={}){
  const dialog=document.createElement("dialog");
  dialog.className="collection-wizard-dialog workflow-dialog";
  const works=collectionSyncableWorks();
  const form={name:recordStores().length?"":"chroma_primary",role:"primary",languages:new Set(),provider:defaultProvider,model:defaultModel,selectedWorks:new Set()};
  let step=0;
  const steps=[tr("vector.create_step_basics","Collection"),tr("vector.create_step_embedding","Embeddings"),tr("vector.create_step_sync","Sync works")];
  ...
  const close=()=>{dialog.close();dialog.remove()};
  const stepContent=()=>{
    if(step===0)return `<section class="workflow-step-body"><div class="workflow-step-copy"><span class="section-label">${esc(tr("vector.create_collection","Create collection"))}</span><h3>${esc(tr("vector.collection_basics","Collection basics"))}</h3><p>${esc(tr("vector.collection_basics_help","Name the collection and describe how DerridAI should treat it."))}</p></div><div class="workflow-fields"><label class="field"><span>${esc(tr("vector.collection_name","Collection name"))}</span><input class="control" id="wizardCollectionName" value="${esc(form.name)}" autocomplete="off" placeholder="chroma_primary"><small>${esc(tr("vector.collection_name_help","Use a short stable name. You can browse and search it from Vector Stores."))}</small></label><label class="field"><span>${esc(tr("vector.collection_role","Collection role"))}</span><select class="control" id="wizardCollectionRole"><option value="primary" ${form.role==="primary"?"selected":""}>${esc(tr("vector.role_primary","Primary"))}</option><option value="general" ${form.role==="general"?"selected":""}>${esc(tr("vector.role_general","General"))}</option><option value="language" ${form.role==="language"?"selected":""}>${esc(tr("vector.role_language","Language-specific"))}</option></select><small>${esc(tr("vector.collection_role_help","Primary collections are the main corpus database; language-specific collections are derived or scoped by language."))}</small></label><fieldset class="vector-language-choice"><legend>${esc(tr("vector.language_tags","Language tags"))}</legend><div class="vector-language-options">${[["en","English"],["fr","Français"]].map(([code,name])=>`<label><input type="checkbox" data-wizard-language="${code}" ${form.languages.has(code)?"checked":""}><span>${name}</span></label>`).join("")}</div><small>${esc(tr("vector.language_tags_help","Optional descriptive tags. Leave both unchecked for a mixed-language collection."))}</small></fieldset></div></section>`;
    if(step===1)return `<section class="workflow-step-body"><div class="workflow-step-copy"><span class="section-label">${esc(tr("vector.create_collection","Create collection"))}</span><h3>${esc(tr("vector.embedding_setup","Embedding setup"))}</h3><p>${esc(tr("vector.embedding_setup_help","Choose how new records in this collection will receive embeddings."))}</p></div><div class="workflow-fields"><fieldset class="vector-provider-choice"><legend>${esc(tr("vector.embedding_provider","Embedding provider"))}</legend><div class="vector-provider-options">${[["ollama",tr("vector.provider_ollama","Ollama"),tr("vector.provider_ollama_help","Generate embeddings with a configured local Ollama model.")],["chroma",tr("vector.provider_chroma","Chroma default"),tr("vector.provider_chroma_help","Let Chroma use its default embedding function.")],["precomputed",tr("vector.provider_precomputed","Precomputed"),tr("vector.provider_precomputed_help","Records must already contain embedding vectors; semantic query embedding is unavailable.")]].map(([value,name,help])=>`<label class="${form.provider===value?"selected":""}"><input type="radio" name="wizardEmbeddingProviderRadio" value="${value}" ${form.provider===value?"checked":""}><span><b>${esc(name)}</b><small>${esc(help)}</small></span></label>`).join("")}</div></fieldset><label class="field ${form.provider==="ollama"?"":"muted-field"}"><span>${esc(tr("vector.embedding_model","Embedding model"))}</span><input class="control" id="wizardEmbeddingModel" list="wizardEmbeddingModels" value="${esc(form.model)}" ${form.provider==="ollama"?"":"disabled"} placeholder="bge-m3:latest"><datalist id="wizardEmbeddingModels">${installedModels.map(model=>`<option value="${esc(model.name)}"></option>`).join("")}</datalist><small>${esc(tr("vector.embedding_model_help","This setting is locked once the collection contains records."))}</small></label></div></section>`;
    const total=selectedCount();
    return `<section class="workflow-step-body"><div class="workflow-step-copy"><span class="section-label">${esc(tr("vector.create_collection","Create collection"))}</span><h3>${esc(tr("vector.sync_available_works","Sync available works"))}</h3><p>${esc(tr("vector.sync_available_works_help","Optionally add loaded corpus works immediately after the collection is created. You can also leave the collection empty."))}</p></div><div class="workflow-fields"><div class="wizard-work-toolbar"><div><b>${esc(tr("vector.available_loaded_works","Available loaded works"))}</b><small>${works.length?trf("vector.available_works_count","{count} works from the current browser workspace",{count:works.length.toLocaleString()}):tr("vector.no_loaded_works","No loaded JSONL works are available to sync.")}</small></div>${works.length?`<div class="tools"><button class="btn small" type="button" id="wizardSelectWorks">${esc(tr("ui.select_all","Select all"))}</button><button class="btn small" type="button" id="wizardClearWorks">${esc(tr("ui.clear","Clear"))}</button></div>`:""}</div><div class="wizard-sync-summary"><span><b>${form.selectedWorks.size.toLocaleString()}</b>${esc(tr("dynamic.works","works"))}</span><span><b>${total.toLocaleString()}</b>${esc(tr("dynamic.records","records"))}</span>${total>500?`<div class="info warn"><b>${esc(tr("operations.large_sync_foreground_title","Large sync runs in the foreground"))}</b><span>${esc(trf("vector.create_large_sync_help","Because this selection contains {count} records, DerridAI will request confirmation and sync it in foreground batches of 500 after creating the collection. Other actions are blocked until it finishes or you cancel.",{count:total.toLocaleString()}))}</span></div>`:""}</div><div class="wizard-work-list">${works.map(item=>`<label class="wizard-work-row"><input type="checkbox" data-wizard-work="${esc(item.work)}" ${form.selectedWorks.has(item.work)?"checked":""}><span><b>${esc(item.work)}</b><small>${item.count.toLocaleString()} ${esc(tr("dynamic.records","records"))}</small></span></label>`).join("")||`<div class="empty-inline">${esc(tr("vector.no_loaded_works","No loaded JSONL works are available to sync."))}</div>`}</div></div></section>`;
  };
  const render=()=>{
    dialog.innerHTML=`<div class="workflow-dialog-head"><div><span class="section-label">${esc(tr("vector.new_collection","New vector collection"))}</span><h2>${esc(tr("vector.create_collection","Create collection"))}</h2><p>${esc(tr("vector.create_collection_help","Configure the collection first, then optionally sync loaded works."))}</p></div><button class="btn icon-only" type="button" data-close aria-label="${esc(tr("ui.close","Close"))}">${icon("close")}</button></div><ol class="workflow-steps" aria-label="${esc(tr("vector.creation_steps","Collection creation steps"))}">${steps.map((label,index)=>`<li class="${index===step?"active":index<step?"done":""}"><span>${index+1}</span><b>${esc(label)}</b></li>`).join("")}</ol>${stepContent()}<div class="workflow-dialog-actions"><button class="btn" type="button" data-close>${esc(tr("ui.cancel","Cancel"))}</button><div class="workflow-action-spacer"></div>${step?`<button class="btn" type="button" id="wizardBack">← ${esc(tr("ui.back","Back"))}</button>`:""}${step<2?`<button class="btn primary" type="button" id="wizardNext">${esc(tr("ui.next","Next"))} →</button>`:`<button class="btn primary" type="button" id="wizardCreate">${icon("plus")}${esc(form.selectedWorks.size?tr("vector.create_and_sync","Create & sync selected works"):tr("vector.create_empty","Create empty collection"))}</button>`}</div>`;
    dialog.querySelectorAll("[data-close]").forEach(button=>button.onclick=close);
    dialog.querySelector("#wizardBack")?.addEventListener("click",()=>{persistFields();step=Math.max(0,step-1);render()});
    dialog.querySelector("#wizardNext")?.addEventListener("click",()=>{persistFields();if(step===0&&!form.name)return toast(tr("vector.collection_name_required","Enter a collection name"));if(step===1&&form.provider==="ollama"&&!form.model)return toast(tr("vector.embedding_model_required","Choose an Ollama embedding model"));step=Math.min(2,step+1);render()});
    dialog.querySelectorAll('input[name="wizardEmbeddingProviderRadio"]').forEach(input=>input.addEventListener("change",event=>{persistFields();form.provider=event.target.value;render()}));
    ...
    dialog.querySelector("#wizardCreate")?.addEventListener("click",async()=>{
      ...
      if(form.provider==="ollama"&&!form.model)return toast(tr("vector.embedding_model_required","Choose an Ollama embedding model"));
      ...
      try{
        await api("/api/stores",{method:"POST",body:JSON.stringify({name:form.name,embedding_provider:form.provider,embedding_model:form.provider==="ollama"?form.model:null,language_codes:form.languages.size?[...form.languages]:null,collection_role:form.role||null})});
        ...
        const syncStarted=rows.length?await upsertRows(rows,trf("vector.selected_works_label","{count} selected works",{count:form.selectedWorks.size}),{}):false;
        toast(rows.length&&syncStarted?trf("vector.collection_created_sync","Created {name}; selected works are syncing.",{name:form.name}):trf("vector.collection_created","Created {name}",{name:form.name}),{tone:"success"});
...
async function renderVector(main){
  ...
  <section class="card vector-collection-toolbar"><div><span class="section-label">${esc(tr("vector.workspace_actions","Collection workspace"))}</span><b>${esc(tr("vector.manage_collections","Manage collections"))}</b><small>${esc(tr("vector.manage_collections_help","Create a new collection or refresh the collection list after changes made elsewhere."))}</small></div><div class="tools"><button class="btn" id="refreshStores">${icon("refresh")}${esc(tr("ui.refresh","Refresh"))}</button><button class="btn primary" id="createCollectionWizard">${icon("plus")}${esc(tr("vector.create_collection","Create collection"))}</button></div></section>
  <section class="vector-summary-grid" aria-label="${esc(tr("vector.summary","Vector store summary"))}"><div class="card vector-summary-card"><span>${esc(tr("vector.collections","Collections"))}</span><b>${stores.length.toLocaleString()}</b></div><div class="card vector-summary-card"><span>${esc(tr("dynamic.records","Records"))}</span><b>${stores.reduce((sum,store)=>sum+Number(store.count||0),0).toLocaleString()}</b></div><div class="card vector-summary-card"><span>${esc(tr("vector.selected_collection","Selected collection"))}</span><b>${esc(state.activeStore||tr("research.none_selected","None selected"))}</b></div></section>
  <section class="storegrid vector-store-layout">
    <aside class="card vector-collection-sidebar"><div class="vector-collection-head"><div><b>${esc(tr("vector.collections","Collections"))}</b><span>${esc(tr("vector.collection_switch_help","Choose a collection to manage or browse it."))}</span></div><span class="badge">${stores.length}</span></div><div class="vector-collection-list">${stores.map(store=>`<button class="storeitem ${store.name===state.activeStore?"active":""}" data-store="${esc(store.name)}"><span class="vector-store-icon">${icon("database")}</span><span><b>${esc(store.name)}</b><small>${store.count.toLocaleString()} ${esc(tr("dynamic.records","records"))} · ${esc(store.collection_role||"general")}</small></span><span aria-hidden="true">›</span></button>`).join("")}</div></aside>

    <article class="vector-store-main">${state.activeStore?`
      <section class="card vector-collection-hero"><div><span class="section-label">${esc(tr("vector.collection","Collection"))}</span><h2>${esc(state.activeStore)}</h2><div class="vector-collection-meta"><span>${(current?.count||0).toLocaleString()} ${esc(tr("dynamic.records","records"))}</span><span>${esc(currentRole)}</span><span>${esc(providerLabel)}</span>${currentCodes.length?`<span>${esc(currentCodes.join(", "))}</span>`:""}</div></div><div class="tools"><button class="btn soft" id="vectorUpsertQueue" ${pending.length?"":`disabled data-disabled-reason="${esc(tr("vector.no_unsynced_changes_help","DerridAI has not found any loaded workspace records that changed since their last sync or are confirmed missing from this collection."))}"`}>${icon("database")}${esc(pending.length?pendingTitle:tr("vector.unsynced_changes","Unsynced local changes"))}</button><button class="btn danger" id="deleteStore">${esc(tr("vector.delete_collection","Delete collection"))}</button></div></section>
      ...
      <div class="vector-settings-grid">
        ...
        <section class="card vector-settings-card"><div class="cardhead"><div><b>${esc(tr("vector.embedding_configuration","Embedding configuration"))}</b><div class="note">${esc(current?.count?tr("vector.embedding_locked_help","Embedding settings are locked after records have been added. Create a new empty collection to change them."):tr("vector.embedding_edit_help","Choose how DerridAI creates vectors for this collection."))}</div></div></div><div class="vector-settings-body"><label class="field"><span>${esc(tr("vector.embedding_provider","Provider"))}</span><select class="control" id="collectionEmbeddingProvider" ${current?.count?`disabled data-disabled-reason="${esc(tr("vector.embedding_locked_help","Embedding settings are locked after records have been added. Create a new empty collection to change them."))}"`:""}><option value="ollama" ${currentProvider==="ollama"?"selected":""}>Ollama</option><option value="chroma" ${currentProvider==="chroma"?"selected":""}>${esc(tr("vector.provider_chroma","Chroma default"))}</option><option value="precomputed" ${currentProvider==="precomputed"?"selected":""}>${esc(tr("vector.provider_precomputed","Precomputed"))}</option></select></label><label class="field"><span>${esc(tr("vector.embedding_model","Model"))}</span><input class="control" id="collectionEmbeddingModel" list="ollamaModelList" value="${esc(currentModel||"")}" placeholder="bge-m3:latest" ${currentProvider==="ollama"&&!current?.count?"":`disabled data-disabled-reason="${esc(current?.count?tr("vector.embedding_locked_help","Embedding settings are locked after records have been added. Create a new empty collection to change them."):tr("vector.model_not_used","This provider does not use an Ollama model name."))}"`}></label><button class="btn" id="saveEmbeddingSettings" ${current?.count?`disabled data-disabled-reason="${esc(tr("vector.embedding_locked_help","Embedding settings are locked after records have been added. Create a new empty collection to change them."))}"`:""}>${esc(tr("ui.save","Save"))}</button></div></section>
      </div>

      <section class="card vector-transfer-card"><div class="cardhead"><div><b>${esc(tr("vector.import_export","Import & export records"))}</b><div class="note">${esc(tr("vector.import_export_help","Sync loaded JSONL records into this collection, or export collection records back to clean JSONL. Exporting does not modify the collection."))}</div></div></div><div class="vector-transfer-groups"><div><span class="section-label">${esc(tr("vector.sync_into_collection","Sync into collection"))}</span><div class="store-actions"><button class="btn" id="importActive" ${activeFile()?"":`disabled data-disabled-reason="${esc(tr("vector.load_jsonl_first","Load and select a JSONL tab first."))}"`}>${icon("database")}${esc(tr("vector.sync_active_jsonl","Sync active JSONL"))}</button><button class="btn" id="importAll" ${state.files.length?"":`disabled data-disabled-reason="${esc(tr("vector.load_jsonl_any_first","Load at least one JSONL tab first."))}"`}>${icon("database")}${esc(tr("vector.sync_all_loaded","Sync all loaded JSONL"))}</button></div><p class="note">${esc(tr("vector.sync_behavior_help","Sync updates matching records and inserts new ones. Any sync above 500 records requires confirmation and runs in foreground batches of 500 and requires this tab to remain open."))}</p></div><div><span class="section-label">${esc(tr("vector.export_from_collection","Export from collection"))}</span><div class="store-actions"><button class="btn" id="loadEntireStore">${icon("download")}${esc(tr("vector.open_db_jsonl","Open full collection as JSONL tab"))}</button><button class="btn" id="downloadEntireStore">${icon("download")}${esc(tr("vector.download_db_jsonl","Download full collection JSONL"))}</button>${state.storeWork?`<button class="btn" id="loadCurrentWork">${icon("download")}${esc(tr("vector.open_current_work","Open current work as JSONL"))}</button><button class="btn" id="downloadCurrentWork">${icon("download")}${esc(tr("vector.download_current_work","Download current work"))}</button>`:""}</div><details class="vector-secondary-action"><summary>${esc(tr("vector.page_export_title","Current table page"))}</summary><p>${esc(tr("vector.page_export_help","Open only the records currently visible on this paginated table as a temporary JSONL tab. This is not a full collection export."))}</p><button class="btn small" id="loadStorePage">${icon("download")}${esc(tr("vector.open_current_page","Open current table page as JSONL tab"))}</button></details></div></div></section>

      <section class="card vector-search-card"><div class="cardhead"><div><b>${esc(tr("vector.semantic_search","Semantic search"))}</b><div class="note">${esc(currentProvider==="precomputed"?tr("vector.semantic_precomputed_help","Semantic search is unavailable because this collection stores precomputed vectors without a query embedding function."):tr("vector.semantic_search_help","Find records by meaning using this collection's configured embedding provider."))}</div></div></div><div class="vector-search-row"><input class="control" id="storeQuery" value="${esc(state.storeQuery)}" placeholder="${esc(tr("vector.semantic_search_placeholder","Search this collection by meaning…"))}"><button class="btn primary" id="runSearch" ${currentProvider==="precomputed"?`disabled data-disabled-reason="${esc(tr("vector.semantic_precomputed_help","Semantic search is unavailable because this collection stores precomputed vectors without a query embedding function."))}"`:""}>${esc(tr("ui.search","Search"))}</button><button class="btn" id="clearStoreSearch">${esc(tr("ui.clear","Clear"))}</button></div><div id="storeResults" class="vector-search-results">${state.storeSearchResults.length?state.storeSearchResults.map(result=>{const resultId=String(result.id||result.record?._chroma_id||result.record?.record_id||"");const eKey=dbEvidenceKey(state.activeStore,resultId);return `<div class="result store-result"><div class="result-main"><div class="note">${similarityHtml(result.distance)} · ${highlightTerms(result.record.record_id||resultId,state.storeQuery)}</div><b>${highlightTerms(result.record.work||"",state.storeQuery)}</b><div class="textcell">${highlightTerms(snippet(result.record.text,state.storeQuery,320),state.storeQuery)}</div></div><div class="tools">${evidenceButtonHtml(eKey,evidenceIsSelected(eKey)?"Selected":"Evidence")}<button class="btn small" data-search-cite="inline" data-search-id="${esc(resultId)}">${esc(tr("record.inline_citation","Inline cite"))}</button><button class="btn small" data-search-cite="full" data-search-id="${esc(resultId)}">${esc(tr("record.full_citation","Full cite"))}</button><button class="btn small" data-edit-search="${esc(resultId)}">${esc(tr("ui.edit","Edit"))}</button><button class="btn small danger" data-delete-search="${esc(resultId)}">${esc(tr("ui.delete","Delete"))}</button></div></div>`}).join(""):`<div class="note">${esc(tr("vector.search_results_empty","Search results will appear here."))}</div>`}</div></section>

      ${canDerive?`<section class="card vector-language-databases"><div class="cardhead"><div><b>${esc(tr("vector.language_collections","Language databases"))}</b><div class="note">${esc(tr("vector.language_derive_help","Create separate English and French collections by routing records according to document_language metadata."))}</div></div><span class="badge">${esc(tr("vector.advanced","Advanced"))}</span></div><div class="language-database-grid"><label class="field"><span>${esc(tr("vector.english_collection","English collection"))}</span><input class="control" id="deriveEnName" value="${esc(derived.en)}"><small>${esc(tr("vector.english_collection_help","Receives records routed as English."))}</small></label><label class="field"><span>${esc(tr("vector.french_collection","French collection"))}</span><input class="control" id="deriveFrName" value="${esc(derived.fr)}"><small>${esc(tr("vector.french_collection_help","Receives records routed as French."))}</small></label><div class="language-database-action"><button class="btn primary" id="deriveLanguages">${icon("database")}${esc(tr("vector.generate_language_collections","Create language databases"))}</button><small>${esc(tr("vector.language_source_help","Source collection stays unchanged. Existing target collections are updated according to the current routing rules."))}</small></div></div></section>`:""}

      <section class="card vector-browser-card"><div class="db-browser-tabs" role="tablist"><button class="db-browser-tab ${state.storeBrowseMode==="works"?"active":""}" role="tab" aria-selected="${state.storeBrowseMode==="works"}" id="browseWorks">${esc(tr("dashboard.works","Works"))} <span>${state.storeWorkStats.length}</span></button><button class="db-browser-tab ${state.storeBrowseMode==="records"?"active":""}" role="tab" aria-selected="${state.storeBrowseMode==="records"}" id="browseRecords">${esc(tr("dashboard.records","Records"))} <span>${(current?.count||0).toLocaleString()}</span></button></div>${workBrowser}${recordsBrowser}</section>
  ...
  <section class="card vector-storage-settings modern-storage-settings">
    <div class="cardhead vector-storage-head"><div><b>${esc(tr("vector.storage_settings","Storage settings"))}</b><div class="note">${esc(tr("vector.storage_settings_help","Review where Chroma data is stored. Most installations should leave these settings unchanged."))}</div></div><span class="vector-storage-status">${icon("database")}${esc(tr("vector.persistent_storage","Persistent storage"))}</span></div>
    <div class="vector-storage-overview">
      <div class="vector-storage-primary"><span>${esc(tr("vector.host_path","Host folder"))}</span><code>${esc(hostPath)}</code><small>${esc(tr("vector.host_path_help","This is the persistent folder on the machine running DerridAI. Back up this folder to preserve collections."))}</small></div>
      <div class="vector-storage-secondary"><span>${esc(tr("vector.container_path","Container path"))}</span><code>${esc(currentPath)}</code><small>${esc(tr("vector.container_path_help","This is the corresponding path inside the DerridAI service. It is mainly useful for deployment troubleshooting."))}</small></div>
    </div>
    <div class="vector-storage-action-row"><div><b>${esc(tr("vector.change_storage_location","Change data location"))}</b><small>${esc(tr("vector.change_storage_location_help","Advanced deployment setting. Existing collections are not moved automatically."))}</small></div><button class="btn" id="openChromaPathDialog">${icon("gear")}${esc(tr("vector.change_location","Change location"))}</button></div>
  </section>
  ...
  document.querySelectorAll("[data-store]").forEach(button=>button.onclick=()=>{
    state.activeStore=button.dataset.store;state.storePage=1;state.storeSearchResults=[];state.storeQuery="";state.storeWork="";state.storeSort={key:"",dir:1};state.storeFilters={};state.storeWorksStore="";state.storeBrowseMode="works";persistPrefs();renderVector(main);
  });
  ...
  const syncCollectionModel=()=>{if(modelControl&&providerControl)modelControl.disabled=Boolean(current?.count)||providerControl.value!=="ollama"};
  ...
  document.querySelector("#deleteStore").onclick=async()=>{
  ...
  document.querySelector("#browseWorks")?.addEventListener("click",()=>{state.storeBrowseMode="works";persistPrefs();renderVector(main)});
  document.querySelector("#browseRecords")?.addEventListener("click",()=>{state.storeBrowseMode="records";state.storeWork="";state.storePage=1;persistVectorLocation();renderVector(main)});
  document.querySelectorAll("[data-db-work]").forEach(button=>button.onclick=()=>{state.storeWork=button.dataset.dbWork;state.storeBrowseMode="records";state.storePage=1;persistVectorLocation();renderVector(main)});
  ...
  document.querySelector("#runSearch").onclick=async()=>{
    const q=document.querySelector("#storeQuery").value.trim();if(!q)return;
    state.storeQuery=q;persistPrefs();
    try{
      const data=await api(`/api/stores/${encodeURIComponent(state.activeStore)}/search`,{method:"POST",body:JSON.stringify({query:q,n_results:30})});
...
async function openTouchup(inputItems=null,initialMode="foreground"){
  ...
  function captureConfig(){
    ...
    const base_url=provider==="openai"
      ? (dialog.querySelector("#openaiBaseUrl")?.value?.trim()||profile?.base_url||"")
      : (dialog.querySelector("#ollamaBaseUrl")?.value?.trim()||profile?.base_url||"");
    const api_key=provider==="openai"
      ? (dialog.querySelector("#openaiApiKey")?.value||profile?.api_key||"")
      : null;
    ...
    if(profile){
      Object.assign(profile,{
        base_url:base_url||profile.base_url,
        model,
        model_mode:modelMode,
        model_kind:modelKind,
        api_key:provider==="openai"?api_key:"",
  ...
  function renderSetup(){
    ...
    <div class="db llm-body"><div class="llm-layout">
      <aside class="llm-config">
        ...
        <div class="llm-section"><div class="llm-section-title">Provider profile</div><select class="control" id="llmProvider">${providerProfiles().map(p=>`<option value="${esc(p.id)}" ${p.id===profileId?"selected":""}>${esc(providerDisplayName(p))} · ${p.type==="ollama"?"Ollama":"OpenAI-compatible"}</option>`).join("")}</select>
          ${provider==="ollama"
            ? `<div class="field" style="margin-top:8px"><label>Ollama endpoint</label><input class="control" id="ollamaBaseUrl" value="${esc(profile?.base_url||"http://host.docker.internal:11434")}"></div>`
            : `<div class="field" style="margin-top:8px"><label>OpenAI-compatible endpoint</label><input class="control" id="openaiBaseUrl" value="${esc(profile?.base_url||"http://host.docker.internal:3001/v1")}"></div><div class="field"><label>API key</label><input class="control" id="openaiApiKey" type="password" value="${esc(profile?.api_key||"")}" autocomplete="off"></div>`
          }
        </div>
  ...
  <div class="config-page">
    ...
    <section class="card config-card">
      ...
      <div class="config-grid">
        ...
        <div class="field field-full"><label>Retrieval routes</label><div class="language-checks">${[["mmr","MMR"],["similarity","Similarity"]].map(([value,name])=>`<label><input type="checkbox" data-cfg-rag-search="${value}" ${rag.search_types.includes(value)?"checked":""}><span>${name}</span></label>`).join("")}</div></div>
...
function setRecordWorkspaceFind(value){state.recordFind=String(value||"");persistPrefs();return state.recordFind}
