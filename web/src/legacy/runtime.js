...
async function renderVector(main){
  ...
  <details class="card vector-storage-settings modern-storage-settings vector-system-storage"><summary><span>${icon("database")}<b>${esc(tr("vector.system_storage","System storage"))}</b><small>${esc(hostPath)} · ${esc(tr("vector.persistent_storage","Persistent storage"))}</small></span><span>${esc(tr("vector.advanced","Advanced"))}</span></summary><div class="vector-system-storage-body"><div class="cardhead vector-storage-head"><div><b>${esc(tr("vector.storage_settings","Storage settings"))}</b><div class="note">${esc(tr("vector.storage_settings_help","Infrastructure setting. Collection manifests and retrieval configuration are managed above; most installations should leave the Chroma location unchanged."))}</div></div><span class="vector-storage-status">${icon("database")}${esc(tr("vector.persistent_storage","Persistent storage"))}</span></div><div class="vector-storage-overview"><div class="vector-storage-primary"><span>${esc(tr("vector.host_path","Host folder"))}</span><code>${esc(hostPath)}</code><small>${esc(tr("vector.host_path_help","This is the persistent folder on the machine running DerridAI. Back up this folder to preserve collections."))}</small></div><div class="vector-storage-secondary"><span>${esc(tr("vector.container_path","Container path"))}</span><code>${esc(currentPath)}</code><small>${esc(tr("vector.container_path_help","This is the corresponding path inside the DerridAI service. It is mainly useful for deployment troubleshooting."))}</small></div></div><div class="vector-storage-action-row"><div><b>${esc(tr("vector.change_storage_location","Change data location"))}</b><small>${esc(tr("vector.change_storage_location_help","Advanced deployment setting. Existing collections are not moved automatically."))}</small></div><button class="btn" id="openChromaPathDialog">${icon("gear")}${esc(tr("vector.change_location","Change location"))}</button></div></div></details>
  ...
  document.querySelectorAll("[data-store]").forEach(button=>button.onclick=()=>{
    state.activeStore=button.dataset.store;state.storePage=1;state.storeSearchResults=[];state.storeQuery="";state.storeSearchMode="";state.storeWork="";state.storeSort={key:"",dir:1};state.storeFilters={};state.storeWorksStore="";state.storeBrowseMode="works";persistPrefs();renderVector(main);
  });
  ...
  document.querySelector("#vectorTestRetrieval")?.addEventListener("click",()=>document.querySelector("#vectorRetrievalTest")?.scrollIntoView({behavior:"smooth",block:"start"}));
