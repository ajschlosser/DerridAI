...
function currentTableUrlState(view=state.view){
  ...
  if(view==="global")return {c:state.tableColumns.global||null,s:state.globalSort,f:state.globalFilters,p:state.globalPage,z:state.pageSize,q:state.globalSearch,m:state.globalSearchMode,dm:state.dbSearchMethod,dw:state.dbSearchWhere,dk:state.dbSearchFetchK,dl:state.dbSearchLambda,l:state.searchResultLayouts};
...
function workInsightMetrics(rows,work){
  return [
    {id:"persons",field:"persons",title:tr("dashboard.top_persons_work","Top 5 persons mentioned in the work")},
    {id:"concepts",field:"concepts",title:tr("dashboard.top_concepts_work","Top 5 concepts mentioned in the work")},
    {id:"topics",field:"topics",title:tr("dashboard.top_topics_work","Top 5 topics in the work")},
    {id:"speakers",field:"speaker",title:tr("dashboard.top_speakers_work","Top 5 speakers in the work")},
    {id:"position-holders",field:"position_holder",title:tr("dashboard.top_position_holders_work","Top 5 position holders in the work")},
  ].map(metric=>({...metric,type:"bars",work,values:topRecordFieldValues(rows,metric.field),format:value=>Number(value).toLocaleString()}));
}
function workInsightsPanelHtml(rows,work){
  const metrics=workInsightMetrics(rows,work);
  return `<section class="work-insights-panel" aria-label="${esc(tr("works.work_insights","Work insights"))}"><div class="work-insights-heading"><div><span class="section-label">${esc(tr("works.work_insights","Work insights"))}</span><h2>${esc(tr("works.indexed_patterns","Indexed patterns in this work"))}</h2></div><p>${esc(tr("works.work_insights_help","Counts are derived from the currently loaded records and use the corpus metadata fields directly."))}</p></div><div class="work-insights-grid">${metrics.map(metric=>`<article class="work-insight-card"><h3>${esc(metric.title.replace(" in the work","").replace(" mentioned in the work",""))}</h3><ol>${metric.values.map(item=>`<li><button type="button" data-work-insight-field="${esc(metric.field)}" data-work-insight-value="${esc(item.key)}"><span>${esc(item.key)}</span><b>${Number(item.value).toLocaleString()}</b></button></li>`).join("")||`<li class="note">${esc(tr("works.no_indexed_values","No indexed values in the loaded records."))}</li>`}</ol></article>`).join("")}</div></section>`;
}
