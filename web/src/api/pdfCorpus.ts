...
export const pdfCorpusApi = {
  ...
  sliceRecord: (buildId:string, recordId:string, direction:"previous"|"next"|"keep"|"new", offset:number, expectedRevision?:number, keepEnd?:number) => apiRequest<{record:CorpusRecord;neighbor:CorpusRecord;left_neighbor?:CorpusRecord;right_neighbor?:CorpusRecord;direction:string;transaction_id:string}>(`/api/pdf/corpus-builds/${encodeURIComponent(buildId)}/records/${encodeURIComponent(recordId)}/slice`, {method:"POST",body:JSON.stringify({direction,offset,keep_end:keepEnd,expected_revision:expectedRevision})}),
  requeueMetadata: (buildId:string, recordId:string, payload:Record<string,unknown>) => apiRequest<CorpusBuild>(`/api/pdf/corpus-builds/${encodeURIComponent(buildId)}/records/${encodeURIComponent(recordId)}/rerun-metadata`, {method:"POST",body:JSON.stringify(payload)}),
