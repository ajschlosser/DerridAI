...
export interface SourceBlock {block_id:string;page:number;bbox:number[];type:string;text:string;extraction_method:string;confidence:number;speaker?:string}

export interface GutenbergHit {etext_id:number;title:string;author:string;language:string}
...
export const pdfCorpusApi = {
  ...
  importUrl: (url: string, sourceIllegibility=0) => apiRequest<PdfAsset>("/api/pdf/assets/url", {method:"POST", body: JSON.stringify({url, source_illegibility: sourceIllegibility})}),
  searchGutenberg: (query: string) => apiRequest<{items: GutenbergHit[]}>(`/api/pdf/gutenberg/search?q=${encodeURIComponent(query)}&limit=12`),
  importGutenberg: (etextId: number, sourceIllegibility=0) => apiRequest<PdfAsset>("/api/pdf/gutenberg/import", {method:"POST", body: JSON.stringify({etext_id: etextId, source_illegibility: sourceIllegibility})}),
  ...
  metadataDecision: (buildId:string, recordId:string, field:string, value:unknown, expectedRevision?:number, confirmNoSupportedValue=false) => apiRequest<{applied:boolean;record:CorpusRecord;build:CorpusBuild;queue_counts?:CorpusBuild["review_queue_counts"];remaining_fields?:string[];ready_for_acceptance?:boolean;review_state?:string}>(`/api/pdf/corpus-builds/${encodeURIComponent(buildId)}/records/${encodeURIComponent(recordId)}/metadata-decision`, {method:"POST",body:JSON.stringify({field,value,expected_revision:expectedRevision,confirm_no_supported_value:confirmNoSupportedValue})}),
  ...
  previewRecord: (buildId:string, recordId:string) => apiRequest<{record:Record<string,unknown>;jsonl:string;validation_errors:string[];unresolved_fields:string[];would_publish:boolean}>(`/api/pdf/corpus-builds/${encodeURIComponent(buildId)}/records/${encodeURIComponent(recordId)}/preview`),
  touchupText: (buildId:string, recordId:string, payload:Record<string,unknown>) => apiRequest<{record_id:string;source_text:string;proposed_text:string;changes:string[];warnings:string[];provider:string;model:string}>(`/api/pdf/corpus-builds/${encodeURIComponent(buildId)}/records/${encodeURIComponent(recordId)}/text-touchup`, {method:"POST",body:JSON.stringify(payload)}),
