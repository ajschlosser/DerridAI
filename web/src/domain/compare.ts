...
const PRIORITY = [
  ...
  "speaker", "quoted_speaker", "position_holder", "stance", "target", "discourse_role",
  ...
  try {
    ...
    for (const line of lines) {
      try { parsed.push(JSON.parse(line)); }
        ...
        return {record: null, errorKey: "compare.error.invalid_json", errorFallback: "This side is not valid JSON or JSONL."};
      }
    }
    if (parsed.length !== 1) {
      return {record: null, errorKey: "compare.error.one_record", errorFallback: "Paste exactly one JSON/JSONL record on each side."};
    }
...
function coerceRecord(parsed: unknown): CompareParseResult {
  if (Array.isArray(parsed)) {
    if (parsed.length !== 1 || !parsed[0] || typeof parsed[0] !== "object" || Array.isArray(parsed[0])) {
      return {record: null, errorKey: "compare.error.one_record", errorFallback: "Paste exactly one JSON record, not an array of multiple records."};
    }
    return {record: parsed[0] as Record<string, unknown>, errorKey: "", errorFallback: ""};
  }
  if (!parsed || typeof parsed !== "object") {
    return {record: null, errorKey: "compare.error.not_object", errorFallback: "Pasted value is not a JSON object."};
  }
...
export function buildCompareRows(a: Record<string, unknown> | null, b: Record<string, unknown> | null, filter: CompareFilter): CompareFieldRow[] {
  if (!a || !b) return [];
  const rows = compareFieldKeys(a, b).map(key => {
    const left = a[key];
    const right = b[key];
    const changed = !sameCompareValue(left, right);
    return {key, changed, left, right, parts: changed ? diffParts(left, right) : [{kind: "equal" as const, value: displayCompareValue(left)}]};
  });
  return filter === "changed" ? rows.filter(row => row.changed) : rows;
}

export function filterLibraryOptions(options: CompareLibraryOption[], query: string, limit = 18): CompareLibraryOption[] {
