...
const NOTHING = new Set([
  'null','none','nil','undefined','nan','n/a','na','n.a.','n.a','unknown','unspecified','unnamed','untitled','not specified','not stated',
  'not applicable','not available','not provided','not given','not mentioned','not identified','not identifiable','no value','no author',
  'no speaker','tbd','todo','empty','blank','-','--','—','–','?','??',
]);
const ROLES='author|speaker|writer|narrator|person|record|text|passage|document|book|work|source|party|individual|figure|reader|interlocutor|voice|subject|object|entity|thing|topic|concept|idea|position|claim|argument|excerpt|section|chapter|quotation|quote';
...
export function isPlaceholderValue(value:unknown):boolean{
  ...
  const plain=value.trim().replace(/^["'“”‘’`.,;:()[\]{}<>]+|["'“”‘’`.,;:()[\]{}<>]+$/g,'').trim().replace(/\s+/g,' ').toLowerCase();
...
export function usableOptions(values:Iterable<unknown>):string[]{
  ...
  for(const value of values)if(typeof value==='string'&&!isPlaceholderValue(value))out.add(value.trim());
  return [...out];
}
