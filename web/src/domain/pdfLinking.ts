...
export function createPdfLinking(deps: Deps) {
  ...
  function openLoadedPdfPage(page: Any) {
    if (!state.pdf.doc && !state.pdf.file)
      return toast("Open the linked PDF in PDF Explorer first");
  ...
  async function linkPdfPage(file: Any, index: Any, page: Any) {
    ...
    if (
      ...
      return toast(`Record is already linked to page ${page}`);
    if (links.length && links.some((link: Any) => link.pdf_file !== target.pdf_file)) {
      if (
        !(await openMessageModal({
          title: "Replace PDF links?",
          message: `This record is linked to ${links[0].pdf_file}. Replace those PDF links with ${target.pdf_file}?`,
          tone: "danger",
