...
const OCR_BOILERPLATE_RE =
  /(?:downloaded\s+from|all\s+use\s+subject\s+to|digitized\s+by\s+the\s+internet\s+archive|created\s+from\s+.+ebooks|ebook\s+central|jstor\.org|proquest\s+ebook)/iu;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const shouldJoin = Boolean(
        current.trim() &&
          ...
          !LIST_OR_QUOTE_RE.test(current) &&
          !LIST_OR_QUOTE_RE.test(next) &&
          !HEADINGISH_RE.test(current) &&
          !HEADINGISH_RE.test(next) &&
          (LOWERCASE_START_RE.test(next) || current.length >= 45),
      );
      if (shouldJoin) {
        out.push(
          current.endsWith("-") && LOWERCASE_START_RE.test(next)
...
function looksLikeOcrArtifact(line: string): boolean {
  ...
  for (const ch of value) {
    if (/[\p{L}\p{N}\s.,;:!?()[\]{}'"–—-]/u.test(ch)) printable += 1;
  }
...
export function cleanupText(
  ...
  const recurring = new Set(
    recurringLines.map((v) => v.trim().toLocaleLowerCase()).filter(Boolean),
  );
  const terms = new Set(
    documentTerms.map((v) => v.trim().toLocaleLowerCase()).filter((v) => v.length >= 3),
  );
  ...
  if (
    rules.has("page_numbers") ||
    rules.has("repeated_short_lines") ||
    rules.has("ocr_artifacts")
  ) {
    ...
    const boundaryIndexes = new Set([
      ...meaningfulIndexes.slice(0, 3),
      ...meaningfulIndexes.slice(-3),
    ]);
    for (const [index, line] of lines.entries()) {
      const normalized = line.trim().toLocaleLowerCase();
      const pageNumber =
        rules.has("page_numbers") && boundaryIndexes.has(index) && PAGE_NUMBER_RE.test(line);
      const recurringLine =
        rules.has("repeated_short_lines") &&
        boundaryIndexes.has(index) &&
        normalized.length > 0 &&
        normalized.length <= 120 &&
        (recurring.has(normalized) || terms.has(normalized));
      const ocrArtifact = rules.has("ocr_artifacts") && looksLikeOcrArtifact(line);
      if (pageNumber || recurringLine || ocrArtifact) {
...
export function recurringShortLines(texts: string[], minimumOccurrences = 2): string[] {
  ...
  return Array.from(counts.values())
    .filter((item) => item.count >= minimumOccurrences)
