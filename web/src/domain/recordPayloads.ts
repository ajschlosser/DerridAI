...
const NON_METADATA_TRANSPORT_FIELDS = new Set([
  ...
  "review_reason", "updates", "activity", "metadata_field_status",
  "metadata_evidence", "metadata_decisions", "metadata_stage_status",
  "metadata_execution_ledger", "metadata_incomplete_fields", "metadata_review_fields",
  "metadata_attention_reasons", "metadata_needs_attention", "metadata_complete",
  "metadata_enrichment_state", "metadata_enrichment_history", "metadata_disputes",
  "human_touched_fields", "human_touched_at", "field_assertions",
  "current_field_assertions", "_chroma_id", "_researcher_text_policy",
]);
...
function assertionFieldNames(source: Loose): string[] {
  ...
  for (const values of Object.values(assertions as Record<string, unknown>)) {
    ...
    for (const assertion of values) {
      if (assertion && typeof assertion === "object" && typeof (assertion as Loose).field_name === "string")
...
function customMetadataFields(source: Loose, baseFields: string[]): string[] {
  ...
  return [
    ...
    ...Object.keys(source).filter((key) =>
      ...
      !key.startsWith("_") &&
      !["record_id", "text", "source_spans", "source_units", "source_unit_ids", "source_block_ids"].includes(key) &&
      (typeof source[key] === "string" || typeof source[key] === "number" || typeof source[key] === "boolean" || Array.isArray(source[key])),
    ),
  ];
}
...
export function touchupRecordPayload(record: unknown, fields: string[] = []): Loose {
  ...
  return recordPayload(record, { fields: [...fields, ...TOUCHUP_TRANSPORT_CONTEXT_FIELDS, ...customMetadataFields(source, TOUCHUP_TRANSPORT_CONTEXT_FIELDS)] });
}

export function ragEvidenceRecordPayload(record: unknown): Loose {
  const source = record && typeof record === "object" ? (record as Loose) : {};
  return recordPayload(record, { fields: [...RAG_EVIDENCE_TRANSPORT_FIELDS, ...customMetadataFields(source, RAG_EVIDENCE_TRANSPORT_FIELDS)] });
}
