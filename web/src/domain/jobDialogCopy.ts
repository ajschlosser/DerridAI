...
export function createJobDialogCopy(translate: Tr, interpolate: Trf) {
  ...
  return {
    ...
    loadDetailsFailed: (error: string) =>
      trf("jobs.toast.load_details_failed", { error }),
    ...
    refreshFailed: (error: string) =>
      trf("jobs.toast.refresh_failed", { error }),
    ...
    startedBackground: (title: string) =>
      trf("jobs.toast.started_background", { title }),
    failed: (title: string, error: string) =>
      trf("jobs.toast.failed", { title, error }),
    invalidDraft: (error: string) =>
      trf("jobs.toast.invalid_draft", { error }),
    chooseDestination: tr("jobs.toast.choose_destination"),
    jsonlGone: tr("jobs.toast.jsonl_gone"),
    chromaUpsertFailed: (error: string) =>
      trf("jobs.toast.chroma_upsert_failed",
        { error },
      ),
    draftAddedBoth: (id: string) =>
      trf("jobs.toast.draft_added_both", { id }),
    draftAddedJsonl: (id: string) =>
      trf("jobs.toast.draft_added_jsonl", { id }),
    draftAddedChroma: (id: string) =>
      trf("jobs.toast.draft_added_chroma", { id }),
  };
}
