...
export function createDashboardRenderer(deps: Deps) {
  ...
  async function renderDashboard(main: Any) {
      ...
      : [
          {
            id: "average",
            type: "bars",
            title: tr("dashboard.top_avg_record_length"),
            values: [...workItems]
          ...
          },
          {
            id: "words",
            type: "bars",
            title: tr("dashboard.top_total_words"),
            values: [...workItems]
          ...
          },
          {
            id: "records",
            type: "bars",
            title: tr("dashboard.top_works_records"),
            values: [...workItems]
          ...
          },
          {
            id: "record-share",
            type: "pie",
            title: tr("dashboard.work_record_share"),
            values: pieShareSeries(workItems, "count"),
            valueLabel: tr("dynamic.records"),
          },
          {
            id: "word-share",
            type: "pie",
            title: tr("dashboard.work_word_share"),
            values: pieShareSeries(workItems, "totalWords"),
            valueLabel: tr("dashboard.words"),
          },
        ];
    ...
    <section class="dashboard-page-top"><article class="card dashboard-hero"><img src="/brand/derridai-mark.png" alt="" class="dashboard-hero-mark"><div class="dashboard-hero-copy"><h1>${esc(tr("dashboard.welcome"))}</h1><p class="dashboard-hero-tagline">${esc(tr("dashboard.tagline"))}</p><blockquote>${esc(tr("dashboard.quote"))}</blockquote><small>— Jacques Derrida</small><div class="dashboard-hero-actions"><button class="btn dark" id="dashStartSearch">${icon("search")}${esc(tr("dashboard.start_searching"))}</button><button class="btn" id="dashBrowseWorks">${icon("books")}${esc(tr("dashboard.browse_works"))}</button></div></div></article>
    <article class="card dashboard-search-card"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("search")}</span><b>${esc(tr("dashboard.global_search"))}</b></div><div class="dashboard-search-tabs"><button class="${state.globalSearchMode === "traditional" ? "active" : ""}" data-dash-search-mode="traditional">${esc(tr(isResearcher() ? "research.traditional_search" : "runtime.traditional_search"))}</button><button class="${state.globalSearchMode !== "traditional" ? "active" : ""}" data-dash-search-mode="database">${esc(tr("research.semantic_db_search"))}</button></div><div class="dashboard-search-line"><div class="dashboard-search-input">${icon("search")}<input id="dashSearchQuery" value="${esc(state.globalSearch || "")}" placeholder="${esc(tr("dashboard.search_corpus_placeholder"))}"></div><select id="dashSearchWork" class="control" aria-label="${esc(tr("field.work"))}"><option value="">${esc(tr("dashboard.all_works"))}</option>${works.map((item: Any) => `<option value="${esc(item.work)}">${esc(item.work)}</option>`).join("")}</select><button class="btn dark" id="dashRunSearch">${icon("search")}${esc(tr("ui.search"))}</button></div><div class="dashboard-search-footer"><button class="dashboard-advanced-link" id="dashAdvancedSearch">${esc(tr("dashboard.advanced_filters"))} →</button><p class="dashboard-search-help">${esc(tr("dashboard.search_help"))}</p></div></article></section>
    <section class="dashboard-page-middle"><article class="card dashboard-overview-card"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("books")}</span><b>${esc(tr("dashboard.corpus_overview"))}</b></div><div class="dashboard-overview-grid"><button data-dashboard-nav="works"><span class="dashboard-overview-icon">${icon("books")}</span><strong>${works.length.toLocaleString()}</strong><small>${esc(tr("dashboard.works"))}</small></button><button data-dashboard-nav="${isResearcher() ? "vector" : "list"}"><span class="dashboard-overview-icon">${icon("record")}</span><strong>${totals.records.toLocaleString()}</strong><small>${esc(tr("dashboard.records"))}</small></button><button ${isResearcher() ? `disabled data-disabled-reason="${esc(tr("dashboard.words_hidden_researcher"))}"` : ""}}><span class="dashboard-overview-icon">${icon("list")}</span><strong>${isResearcher() ? "—" : compactNumber(words)}</strong><small>${esc(tr("dashboard.total_words"))}</small></button><button data-dashboard-nav="vector"><span class="dashboard-overview-icon">${icon("database")}</span><strong>${totals.dbs.toLocaleString()}</strong><small>${esc(tr("dashboard.databases"))}</small></button></div></article>
    <article class="card dashboard-average-card dashboard-metric-carousel" aria-roledescription="carousel"><div class="dashboard-metric-head"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("chart")}</span><b>${esc(activeMetric.title)}</b></div><div class="dashboard-metric-controls"><button class="dashboard-metric-arrow" id="dashMetricPrev" type="button" aria-label="${esc(tr("dashboard.previous_chart"))}">←</button><span>${state.dashboardMetricIndex + 1} / ${metricSets.length}</span><button class="dashboard-metric-arrow" id="dashMetricNext" type="button" aria-label="${esc(tr("dashboard.next_chart"))}">→</button></div></div><div class="dashboard-metric-body">${dashboardMetricBody(activeMetric)}</div><div class="dashboard-metric-dots" role="tablist" aria-label="${esc(tr("dashboard.work_charts"))}">${metricSets.map((metric: Any, index: Any) => `<button type="button" role="tab" data-dashboard-metric="${index}" class="${index === state.dashboardMetricIndex ? "active" : ""}" aria-label="${esc(metric.title)}" aria-selected="${index === state.dashboardMetricIndex}" tabindex="${index === state.dashboardMetricIndex ? 0 : -1}"></button>`).join("")}</div></article>
    <article class="card dashboard-activity-card"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("history")}</span><b>${esc(tr("dashboard.recent_activity"))}</b></div><div class="dashboard-activity-list">${recent.map((item: Any) => (item.kind === "annotation" ? `<button class="dashboard-activity-row" data-recent-server-annotation-record="${esc(item.annotation.record_id || "")}" data-recent-server-annotation-store="${esc(item.annotation.store || "")}"><span class="dashboard-activity-clock">${icon("record")}</span><time>${esc(relativeTime(item.timestamp))}</time><span>${esc(tr("annotations.record_note"))} · ${esc(item.annotation.work || item.annotation.record_id || tr("nav.record"))}</span></button>` : item.kind === "rag" ? `<button class="dashboard-activity-row" ${item.job.status === "completed" ? `data-recent-rag-result="${esc(item.job.id)}"` : ""}><span class="dashboard-activity-clock">${icon("spark")}</span><time>${esc(relativeTime(item.timestamp))}</time><span>${esc(tr("nav.rag"))} · ${esc(String(item.job.prompt || item.job.label || "RAG").slice(0, 90))}</span></button>` : `<button class="dashboard-activity-row" data-recent-file="${item.file.id}" data-recent-index="${item.index}"><span class="dashboard-activity-clock">${icon("history")}</span><time>${esc(relativeTime(item.update.timestamp))}</time><span>${esc(label(item.update.field_name || tr("dashboard.updated_record")))} · ${esc(item.record.work || item.record.record_id || item.file.name)}</span></button>`)).join("") || `<div class="dashboard-activity-empty">${esc(tr("dashboard.no_recent_activity"))}</div>`}</div></article></section>
    <section class="card dashboard-works-card"><div class="dashboard-section-heading"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("books")}</span><b>${esc(tr("dashboard.works"))}</b></div><button class="dashboard-text-link" id="dashViewAllWorks">${esc(tr("dashboard.view_all_works"))} →</button></div><div class="dashboard-work-carousel-shell"><button class="carousel-arrow" id="dashWorksPrev" type="button" title="${esc(tr("ui.previous"))}" aria-label="${esc(tr("ui.previous"))}">‹</button><div class="dashboard-work-strip" id="dashWorksCarousel">${works.map((item: Any, index: Any) => `<button class="dashboard-work-card" data-dashboard-work="${esc(item.work)}">${item.cover ? `<img class="dashboard-book-cover image" src="${esc(item.cover)}" alt="${esc(trf("works.cover_alt", { work: item.work }))}" loading="lazy">` : `<span class="dashboard-book-cover placeholder">${String(index + 1).padStart(2, "0")}</span>`}<span><b>${esc(item.work)}</b><small>${esc(item.year || tr("dashboard.year_not_recorded"))}</small><small>${item.count.toLocaleString()} ${esc(tr("dynamic.records"))}</small></span></button>`).join("") || `<div class="note">${esc(tr("research.no_works"))}</div>`}</div><button class="carousel-arrow" id="dashWorksNext" type="button" title="${esc(tr("ui.next"))}" aria-label="${esc(tr("ui.next"))}">›</button></div></section>
    <section class="dashboard-page-lower">${
      isResearcher()
        ? hasCapability("appearance.manage")
          ? `<article class="card dashboard-quick-card dashboard-appearance-card"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("gear")}</span><b>${esc(tr("dashboard.appearance"))}</b></div><p>${esc(tr("dashboard.appearance_help"))}</p><fieldset class="dashboard-theme-options"><legend>${esc(tr("dashboard.interface_theme"))}</legend>${[
              ["green", tr("theme.green")],
              ["blue", tr("theme.blue")],
              ["slate", tr("theme.slate")],
            ]
              .map(
                ([value, name]) =>
              ...
              .join(
                "",
              )}</fieldset><button class="btn" id="dashAppearanceSettings">${esc(tr("dashboard.more_appearance_settings"))}</button></article>`
          : `<article class="card dashboard-quick-card"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("gear")}</span><b>${esc(tr("dashboard.appearance"))}</b></div><p>${esc(tr("permissions.appearance_denied"))}</p></article>`
        : `<article class="card dashboard-quick-card dashboard-language-card"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("gear")}</span><b>${esc(tr("dashboard.language_settings"))}</b></div><p>${esc(tr("dashboard.language_settings_help"))}</p><div class="dashboard-quick-field dashboard-locale-field"><span>${esc(tr("dashboard.interface_language"))}</span><b><i class="dashboard-locale-symbol">${currentLanguageFlag}</i>${esc(currentLanguage)}</b></div><button class="btn primary" id="dashLanguages">${esc(tr("dashboard.manage_languages"))}</button></article>`
    }
    <article class="card dashboard-quick-card dashboard-provider-card"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("spark")}</span><b>${esc(tr("dashboard.llm_provider_settings"))}</b></div><p>${esc(tr("dashboard.llm_provider_help"))}</p><div class="dashboard-provider-fields"><div class="dashboard-quick-field"><span>${esc(tr("dashboard.default_provider"))}</span><b>${currentProvider ? esc(providerDisplayName(currentProvider)) : esc(tr("dashboard.not_configured"))}</b></div><div class="dashboard-quick-field"><span>${esc(tr("dashboard.model"))}</span><b>${currentProvider ? esc(currentProvider.model || "auto") : "—"}</b></div></div><button class="btn" id="dashProviders">${esc(isResearcher() ? tr("nav.rag") : tr("dashboard.manage_provider"))}</button></article>
    <article class="card dashboard-quick-card dashboard-record-preview"><div class="dashboard-section-heading"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("record")}</span><b>${esc(tr("dashboard.record_view"))}</b></div><button class="dashboard-text-link" id="dashRecordView" ${previewTarget ? "" : `disabled data-disabled-reason="${esc(tr("dashboard.no_record_available"))}"`}>${esc(tr("research.open"))} →</button></div>${
      previewRecord
        ? `<div class="dashboard-record-state">${esc(preview.lastViewed ? tr("dashboard.last_viewed_record") : tr("dashboard.random_record"))}</div><div class="dashboard-record-meta"><b>${esc(previewRecord.work || previewRecord.record_id || tr("dashboard.record"))}</b><span class="dashboard-record-pages">${esc(mlaPageSpan(previewRecord) || "")}</span></div><div class="dashboard-record-text">${esc(
            String(previewRecord.text || "")
              .replace(/\s+/g, " ")
              .slice(0, 220),
          )}${String(previewRecord.text || "").length > 220 ? "…" : ""}</div>`
        : `<div class="dashboard-record-empty">${esc(tr("dashboard.no_record_selected"))}</div>`
    }</article>
    ${latestAnnotation ? `<article class="card dashboard-quick-card dashboard-annotations-card"><div class="dashboard-section-heading"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("record")}</span><b>${esc(tr("dashboard.latest_annotation"))}</b></div><button class="dashboard-text-link" id="dashAnnotations">${esc(tr("annotations.view_all"))} →</button></div><button class="dashboard-annotation-preview" ${latestAnnotation.server ? `data-recent-server-annotation-record="${esc(latestAnnotation.annotation.record_id || "")}" data-recent-server-annotation-store="${esc(latestAnnotation.annotation.store || "")}"` : `data-recent-annotation-file="${esc(latestAnnotation.file.id)}" data-recent-annotation-index="${latestAnnotation.index}"`}><div class="dashboard-annotation-meta"><span class="dashboard-annotation-work">${esc(latestAnnotation.work)}</span><span class="dashboard-annotation-pages">${esc(mlaPageSpan(latestAnnotation.record) || tr("record.page_not_recorded"))}</span><span class="dashboard-annotation-author">${esc(latestAnnotation.annotation.initiated_by || latestAnnotation.annotation.author || tr("annotations.unknown_author"))}</span><time>${esc(formatTimestamp(latestAnnotation.annotation.created_at))}</time><small>${esc(latestAnnotation.record.record_id || tr("nav.record"))}</small></div>${latestAnnotation.annotation.note ? `<p>${esc(latestAnnotation.annotation.note)}</p>` : latestAnnotation.annotation.quote ? `<blockquote>${esc(latestAnnotation.annotation.quote)}</blockquote>` : `<p>${esc(tr("annotations.record_note"))}</p>`}</button></article>` : `<article class="card dashboard-quick-card dashboard-annotations-card"><div class="dashboard-section-heading"><div class="dashboard-card-title"><span class="dashboard-title-icon">${icon("record")}</span><b>${esc(tr("dashboard.annotations"))}</b></div><button class="dashboard-text-link" id="dashAnnotations">${esc(tr("research.open"))} →</button></div><p>${esc(isResearcher() ? tr("annotations.researcher_help") : tr("dashboard.annotations_help"))}</p></article>`}
    ...
    const goSearch = async () => {
      ...
      if (semantic) {
        ...
        try {
          ...
          toast(`${tr("research.search_failed")}: ${error.message}`, {
            tone: "danger",
          });
        } finally {
    ...
    main.querySelectorAll("[data-dashboard-theme]").forEach((input: Any) =>
      input.addEventListener("change", () => {
        ...
        persistPrefs();
        toast(tr("dashboard.appearance_saved"), { tone: "success" });
      }),
    );
    main
