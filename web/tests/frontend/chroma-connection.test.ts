...
describe("chroma connection URL parsing", () => {
  it("strips API version suffixes and rejects credentials in the URL", () => {
    expect(parseChromaHttpUrl("https://chroma.example:8000/api/v2").display).toBe("https://chroma.example:8000");
  ...
  it("never puts a token into the identity label", () => {
    expect(chromaIdentity("http", undefined, "http://chroma:8000")).toBe("Chroma server · http://chroma:8000");
