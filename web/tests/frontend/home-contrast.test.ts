...
const rule = (selector: string): string => {
  const match = css.match(new RegExp(`${selector.replace(/[.[\]()]/g, "\\$&")}\\s*\\{([^}]*)\\}`, "g"));
  if (!match) throw new Error(`rule ${selector} not found`);
  return match.join(" ");
};
const declared = (selector: string, property: string): string => {
  const found = [...rule(selector).matchAll(new RegExp(`(?:^|[;{\\s])${property}:\\s*([^;}]+)`, "g"))].map((m) => m[1].trim());
...
const ratio = (a: string, b: string) => { const [hi, lo] = [luminance(rgb(a)), luminance(rgb(b))].sort((x, y) => y - x); return (hi + 0.05) / (lo + 0.05); };

describe("Home page and sign-in contrast", () => {
  it("the sign-in subtitle is readable text", () => {
    expect(ratio(declared(".auth-brand-lockup span", "color"), "#ffffff")).toBeGreaterThanOrEqual(4.5);
  });

  it("dashboard helper text and the empty record hint are readable", () => {
    for (const selector of [".dashboard-metric-controls", ".dashboard-quick-field span", ".dashboard-record-empty"]) {
