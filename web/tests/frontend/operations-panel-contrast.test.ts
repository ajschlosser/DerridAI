...
const luminance = (c: [number, number, number]) => 0.2126 * channel(c[0]) + 0.7152 * channel(c[1]) + 0.0722 * channel(c[2]);
...
describe("Operations panel contrast", () => {
  it.each(["text", "muted"])("--ops-%s is readable on the surface and on the sunken background", (name) => {
    expect(ratio(rgb(token(`ops-${name}`)), rgb(token("ops-surface")))).toBeGreaterThanOrEqual(4.5);
    expect(ratio(rgb(token(`ops-${name}`)), rgb(token("ops-sunken")))).toBeGreaterThanOrEqual(4.5);
  });

  it.each(["info", "success", "warning", "danger", "neutral"])("%s text on its tint is at least 4.5:1", (tone) => {
    expect(ratio(rgb(token(`ops-${tone}-fg`)), rgb(token(`ops-${tone}-bg`)))).toBeGreaterThanOrEqual(4.5);
  });

  it.each(["info", "success", "warning", "danger", "neutral"])("%s text is also readable on plain white (badges over a card)", (tone) => {
    expect(ratio(rgb(token(`ops-${tone}-fg`)), WHITE)).toBeGreaterThanOrEqual(4.5);
  });

  it.each(Object.entries(ACCENTS))("on the %s theme, white on the accent (pressed chip, primary button) is at least 4.5:1", (_name, accent) => {
  ...
  it("the count inside a pressed chip is solid white with accent text, not a translucent overlay", () => {
    ...
    expect(source).toMatch(/\.ops-chip\[aria-pressed="true"\] \.ops-chip-count\{background:#fff;color:var\(--ops-accent\)\}/);
  });

  it.each(Object.entries(ACCENTS))("on the %s theme, accent text and the focus ring are readable on white", (_name, accent) => {
    expect(ratio(rgb(accent), WHITE)).toBeGreaterThanOrEqual(4.5);
  });

  it.each(Object.entries(ACCENTS))("on the %s theme, the progress fill is distinguishable from its track (1.4.11)", (_name, accent) => {
    const track = rgb(source.match(/\.ops-progress\{[^}]*background:(#[0-9a-fA-F]{6})/)![1]);
  ...
  it("the danger button text is readable on white and on its hover tint", () => {
    ...
    expect(ratio(rgb(token("ops-danger-fg")), rgb(token("ops-danger-bg")))).toBeGreaterThanOrEqual(4.5);
  });
});
