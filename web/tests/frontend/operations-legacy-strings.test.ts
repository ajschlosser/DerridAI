...
import { operationDetailPairs, operationViewModel, setTranslationDictionary, state } from "../../src/runtime/runtime.js";
...
const base = { status: "completed", total: 10, completed: 4, owner: "admin", created_at: "2026-09-20T10:00:00Z", started_at: "2026-09-20T10:00:05Z", finished_at: "2026-09-20T10:01:00Z" };
const JOBS = [
  { id: "auto", type: "llm", mode: "auto", model: "m", provider: "p", fields: ["speaker"], current_record_id: "r-1", resolution_state: "partially_accepted", accepted_results: 1, accepted_fields: 2, rejected_results: 0, rejected_fields: 0, failed: 2 },
  { id: "review", type: "llm", mode: "manual", model: "m", pending_result_count: 2 },
  { id: "rag", type: "rag", stage: "retrieval", source_collection: "c", model: "m", request: { auto_grade: true, search_types: ["dense"], locales: ["fr-CA"] } },
  { id: "upsert", type: "upsert", store_name: "c", mirrored: { fr: 3 }, label: "" },
  { id: "pdf", type: "pdf_corpus", source_filename: "book.pdf", stage: "enriching", stage_detail: "backend text", record_count: 5, review_count: 1, unresolved_regions: 2 },
  ...["pdf_clean_text", "pdf_draft_record", "pdf_link_record", "rag_grade", "rag_grade_batch"].map((tool) => ({ id: tool, type: "llm_tool", tool, provider: "p", model: "m", request: { pdf_file: "x.pdf", response_record_id: "rr" } })),
  { id: "generic-tool", type: "llm_tool", provider: "p", model: "m" },
].map((job) => ({ ...base, ...job }));

describe("Operations legacy strings are translatable", () => {
  beforeEach(() => { setTranslationDictionary("fr-CA", pseudo, {}); });
  ...
  it("every label, fact name and sentence fragment comes from a translation key", () => {
    for (const job of JOBS) {
      ...
      if (["llm", "upsert"].includes(job.type) && view.subtitle) expect(view.subtitle, `${job.id} subtitle`).toContain("⟦");
    }
  });
  ...
  it("the decision and result counts are translated, not raw enum text", () => {
    ...
    const byName = (name: string) => pairs.find(([n]: [string]) => n === `⟦operations.fact.${name}⟧`)?.[1];
  ...
  it("progress text follows the language", () => {
    const view = operationViewModel({ ...base, id: "p", type: "llm", status: "running", finished_at: null });
...
describe("real French dictionary", () => {
  it("renders the same fields in French", () => {
    ...
    for (const m of fr.matchAll(/^\s*'(operations\.[^']+)':\s*'((?:[^'\\]|\\.)*)'/gm)) dictionary[m[1]] = m[2].replaceAll("\\'", "'");
    setTranslationDictionary("fr-CA", dictionary, {});
    state.jobs = [];
    const auto = operationViewModel(JOBS.find((j) => j.id === "auto")!);
    expect(auto.label).toBe("Amélioration automatique");
    expect(auto.facts.map((f) => f.name)).toEqual(expect.arrayContaining(["Modèle", "Champs", "Fiche en cours"]));
    expect(auto.subtitle).toMatch(/fiches/);
    expect(operationViewModel({ ...base, id: "rag", type: "rag", stage: "s" }).label).toBe("Pipeline RAG");
  });
