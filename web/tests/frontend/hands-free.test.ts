...
const policy = (over = {}) => ({
  ...
  passes: 1,
  min_confidence: 0.8,
  unresolved: "best_guess" as const,
  accept_records: true,
  publish: false,
  ...over,
});
...
describe("hands-free settings", () => {
  ...
  it("emits a changed copy of the policy and clamps the pass count", async () => {
    const wrapper = mount(CorpusHandsFreeSettings, {
      props: { modelValue: policy({ enabled: true }) },
    });
    ...
    const emitted = wrapper
      ...
      .map((e) => e[0] as Record<string, unknown>);
  ...
  it("without the enable switch (running on an existing build) the rules are always live", () => {
    const wrapper = mount(CorpusHandsFreeSettings, {
      props: { modelValue: policy(), showEnable: false },
    });
...
describe("hands-free report", () => {
  ...
  const report = {
    ...
    notes: ["Not published: some records still need a person."],
  ...
  it("summarizes the run and lets a person open each exception", async () => {
    ...
    expect(wrapper.text()).toContain(
      "52 of 60 records accepted, 130 values taken from the model, 8 left for you.",
    );
  ...
  it("shows nothing when there has been no run", () => {
    expect(
      mount(CorpusHandsFreeReport, { props: { report: null } })
        .find("section")
        .exists(),
    ).toBe(false);
  });
