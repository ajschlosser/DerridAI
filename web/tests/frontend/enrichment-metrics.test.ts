...
const model = (over = {}) => ({
  ...
  precision_at_threshold: [{ threshold: 0.9, reviews: 10, precision: 0.93, coverage: 0.6 }], learning_curve: [], ...over,
  acceptance_ci: { rate: 0.85, low: 0.64, high: 0.95, n: 20 }, substantive_error_rate: { rate: 0.05, low: 0.01, high: 0.24, n: 20 },
  autofill_precision_ci: { rate: null, low: null, high: null, n: 0 }, spot_checks_still_needed: 3, correction_severity: {}, supported_rate: 0.8,
...
describe("Enrichment metrics", () => {
  ...
  it("loads only when opened, shows a row per model, and says when data is missing", async () => {
    const spy = vi.spyOn(pdfCorpusApi, "enrichmentMetrics").mockResolvedValue({
      models: { qwen3: model() }, inter_model_agreement: { compared: 0, agreement: null }, unresolved_remaining: 7,
      runs: ["r1"], concurrency: { limit: 1, working: 0 },
    });
    const wrapper = mount(CorpusEnrichmentMetrics, { props: { buildId: "b1" }, attachTo: document.body });
