...
describe("Corpus Builder setup and launch controls", () => {
  ...
  it("keeps record sizing exception limits consistent when the preferred size grows", async () => {
    const wrapper=mount(CorpusRecordSizingSettings,{props:{modelValue:{preferred_record_chars:1750,record_length_tolerance:200,long_record_chars:3500,absolute_record_chars:6000}}});
  ...
  it("clamps execution concurrency and deadlines and exposes unsafe context", async () => {
    const wrapper=mount(CorpusExecutionSettings,{props:{
  ...
  it("disables generation overrides when provider defaults are selected", () => {
    const wrapper=mount(CorpusExecutionSettings,{props:{generation:{num_ctx:32768,temperature:0},useProfileDefaults:true,maxConcurrentRequests:2}});
  ...
  it("tracks document-structure edits as dirty and saves the reviewer-owned plan", async () => {
    const asset:any={
      ...
      document_layout:{page_layout:"single",reading_order:"left_to_right",thread_mode:"continuous",main_text_pdf_start:8,main_text_printed_start:1},
      pages:Array.from({length:12},(_,index)=>({pdf_page:index+1,width:612,height:792})),
    };
