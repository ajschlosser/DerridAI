...
const base = { field: "discourse_role", value: null, control: "enum" as const, options: ["assertion", "critique"] };

describe("Blind review in the field editor", () => {
  beforeEach(() => setActivePinia(createPinia()));

  it("says the field is blind and shows no model value or prefill", () => {
    const wrapper = mount(CorpusMetadataFieldEditor, { props: { ...base, status: { status: "unresolved", method: "llm", blind: true }, open: true } });
  ...
  it("reveals the model's suggestion once the reviewer has decided", () => {
    const wrapper = mount(CorpusMetadataFieldEditor, { props: { ...base, value: "critique", status: { status: "human_confirmed" }, revealed: "assertion" } });
