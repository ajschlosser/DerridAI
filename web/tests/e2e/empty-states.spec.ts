...
for (const story of ["database-required", "database-required-without-access"]) {
  test(`database-required empty state (${story}) is announced and WCAG 2.0 AA clean`, async ({ page }) => {
    ...
    const results = await runAxe(page, (builder) => builder.include(".accessible-empty-state").withTags(["wcag2a", "wcag2aa"]));
