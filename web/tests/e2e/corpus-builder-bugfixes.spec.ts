...
const LONG = Array.from(
  { length: 60 },
  (_, i) => `Sentence ${i + 1} of a very long record that needs the whole pane to scroll.`,
).join(" ");
...
test("the end of a long record can be scrolled fully into view above the decision bar", async ({
  page,
}) => {
...
test("bulk edit reads like Edit document metadata and applies a value it was given", async ({
  page,
}) => {
  await open(page);
  await page
...
for (const scheme of ["light", "dark"] as const) {
  test(`the logo has a transparent background in the ${scheme} theme`, async ({ page }) => {
    ...
    await page.screenshot({
      path: `test-results/logo-${scheme}.png`,
      clip: { x: 0, y: 0, width: 420, height: 140 },
    });
  });
}

test("the providers page lists installed models with their sizes, and the list is accessible", async ({
  page,
}) => {
  await mockBackend(page, {
    fixtures: {
      "POST /api/llm/status": {
        ...
        models: [
          ...
          {
            name: "hf.co/tvall43/Qwen3.6-14B-A3B:Q6_K",
            parameter_size: "14B",
...
for (const scheme of ["light", "dark"] as const) {
  test(`the enrichment changes list is readable and accessible in ${scheme} mode`, async ({
    page,
  }) => {
    const source = CORPUS_RECORDS.find((r) => r.review_state === "metadata")!;
    const record = {
      ...source,
      review_reason:
        "Metadata enrichment added, replaced, or disputed metadata; review the highlighted changes.",
...
for (const scheme of ["light", "dark"] as const) {
  test(`the provider model list is readable and accessible in ${scheme} mode`, async ({ page }) => {
      ...
      fixtures: {
        "POST /api/llm/status": {
          ...
          models: [
            { name: "qwen3.5:4b", parameter_size: "4.7B", quantization_level: "Q4_K_M" },
            {
              name: "hf.co/tvall43/Qwen3.6-14B-A3B-FableVibes-GGUF:Q6_K",
              parameter_size: "14B",
              quantization_level: "Q6_K",
            },
            { name: "gemma4:e2b" },
    ...
    await dialog
      .getByRole("button", { name: /use model/i })
      .first()
      .focus();
...
const SCHEMA = {
  ...
  description: "The fields DerridAI has always produced.",
  groups: [
    {
      key: "discourse",
      label: "Discourse and attribution",
      intro: "Infer ONLY discourse metadata.",
      fields_heading: "Fields:",
      notes: ["Prefer short names."],
      trailer: "",
      footer: "Also return field_assessments for {assessed_fields}.\n",
    },
  ],
  fields: [
    {
    ...
    },
    {
      ...
      group: "discourse",
      values: [
        { value: "affirm", definition: "Agrees." },
        { value: "reject", definition: "" },
      ],
      strict: false,
      instruction: "is the orientation. Prefer one of: {values}",
      definitions_heading: "",
      evidence: true,
      assess: true,
      review: true,
          ...
          items: [
            {
              ...
              builtin: true,
              field_count: 5,
              groups: ["Discourse and attribution"],
              hash: "h",
            },
          ],
        },
